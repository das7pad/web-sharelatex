...
module.exports = {
  ...
  i18n: {
    ...
    defaultLng: 'en',
    loadLocalesFromDisk:
      process.env.LOAD_LOCALES_FROM_DISK !== undefined
        ? process.env.LOAD_LOCALES_FROM_DISK === 'true'
  ...
  sentry: {
    frontend: {
      // overleaf
      allowedOriginRegex: `^(${siteUrl}${cdnUrl && '|' + cdnUrl})/`,
      // official
  ...
  useDevAssets:
    process.env.USE_DEV_ASSETS !== undefined
  ...
  prometheusScrapeInterval:
    parseInt(process.env.PROMETHEUS_SCRAPE_INTERVAL, 10) || '60000',

  precompileViews:
    process.env.PRECOMPILE_VIEWS !== undefined
      ? process.env.PRECOMPILE_VIEWS === 'true'
      : process.env.NODE_ENV === 'production',
