...
class MockSpellingApi extends AbstractMockApi {
  ...
  applyRoutes() {
    ...
    // /v20200714
    this.app.delete('/v20200714/user/:userId', (req, res) => {
      const { userId } = req.params
      this.words.delete(userId)
      res.sendStatus(200)
    })

    this.app.get('/v20200714/user/:userId', (req, res) => {
      const { userId } = req.params
      const words = this.words[userId] || []
      res.json(words)
    })

    this.app.post('/v20200714/user/:userId/learn', (req, res) => {
      const word = req.body.word
      const { userId } = req.params
      if (word) {
        this.words[userId] = this.words[userId] || []
    ...
    this.app.post('/v20200714/user/:userId/unlearn', (req, res) => {
      const word = req.body.word
      const { userId } = req.params
      if (word && this.words[userId]) {
        const wordIndex = this.words[userId].indexOf(word)
