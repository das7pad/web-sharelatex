...
describe("LinkedFiles", function() {
	...
	describe("creating a project linked file", function() {
		before(function(done) {
			...
			return async.series([
				cb => {
					return this.owner.createProject('plf-test-one', {template: 'blank'}, (error, project_id) => {
						this.project_one_id = project_id;
						return cb(error);
					});
				},
				cb => {
					return this.owner.getProject(this.project_one_id, (error, project) => {
				...
				cb => {
					return this.owner.createProject('plf-test-two', {template: 'blank'}, (error, project_id) => {
						this.project_two_id = project_id;
						return cb(error);
					});
				},
				cb => {
					return this.owner.getProject(this.project_two_id, (error, project) => {
				...
				cb => {
					return this.owner.createDocInProject(this.project_two_id,
		...
		it('should produce a list of the users projects', function(done) {
			return this.owner.request.get({
				...
				expect(body).to.deep.equal({
					projects: [
						{ _id: this.project_one_id, name: 'plf-test-one', accessLevel: 'owner' },
						{ _id: this.project_two_id, name: 'plf-test-two', accessLevel: 'owner' }
					]
				});
				return done();
			});
		...
		it('should refresh the file', function(done) {
			return this.owner.request.post({
				url: `/project/${this.project_one_id}/linked_file/${this.existing_file_id}/refresh`,
				json: true
			}, (error, response, body) => {
				expect(response.statusCode).to.equal(200);
				const {
	...
	describe("with a linked project_file from a v1 project that has not been imported", function() {
		before(function(done) {
			return async.series([
				cb => {
					return this.owner.createProject('plf-v1-test-one', {template: 'blank'}, (error, project_id) => {
						this.project_one_id = project_id;
						return cb(error);
					});
				},
	...
	describe("creating a URL based linked file", function() {
		before(function(done) {
			return this.owner.createProject("url-linked-files-project", {template: "blank"}, (error, project_id) => {
		...
		it("should download the URL and create a file with the contents and linkedFileData", function(done) {
			return this.owner.request.post({
				...
				return this.owner.getProject(this.project_id, (error, project) => {
					...
					return this.owner.request.get(`/project/${this.project_id}/file/${file._id}`, function(error, response, body) {
						if (error != null) { throw error; }
						expect(response.statusCode).to.equal(200);
		...
		return it("should accept a URL withuot a leading http://, and add it", function(done) {
			return this.owner.request.post({
				...
				return this.owner.getProject(this.project_id, (error, project) => {
					if (error != null) { throw error; }
					const file = _.find(project.rootFolder[0].fileRefs, file => file.name === 'url-test-file-6');
					expect(file.linkedFileData).to.deep.equal({
						provider: 'url',
						url: 'http://example.com/foo'
					});
	...
	describe("creating a linked output file", function() {
		before(function(done) {
			return async.series([
				cb => {
					return this.owner.createProject('output-test-one', {template: 'blank'}, (error, project_id) => {
						this.project_one_id = project_id;
						return cb(error);
					});
				},
				...
				cb => {
					return this.owner.createProject('output-test-two', {template: 'blank'}, (error, project_id) => {
						this.project_two_id = project_id;
						return cb(error);
					});
				},
	...
	return describe("with a linked project_output_file from a v1 project that has not been imported", function() {
		before(function(done) {
			return async.series([
				cb => {
					return this.owner.createProject('output-v1-test-one', {template: 'blank'}, (error, project_id) => {
						this.project_one_id = project_id;
						return cb(error);
					});
				},
