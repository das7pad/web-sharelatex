...
describe('History', function() {
	...
	return describe('zip download of version', function() {
		it('should stream the zip file of a version', function(done) {
			return this.owner.createProject('example-project', (error, project_id) => {
				...
				return db.projects.update({
					...
					if (error != null) { return done(error); }
					return this.owner.request(`/project/${this.project_id}/version/42/zip`, (error, response, body) => {
						if (error != null) { return done(error); }
						...
						expect(body).to.equal(`Mock zip for ${this.v1_history_id} at version 42`);
						return done();
					});
				});
			});
