...
module.exports = (MockDocUpdaterApi = {
	...
	addProjectStructureUpdates(project_id, userId, docUpdates, fileUpdates, version) {
		let update;
		if (!this.updates[project_id]) { this.updates[project_id] = { docUpdates: [], fileUpdates: [] }; }
	...
	run() {
		...
		app.post("/project/:project_id", jsonParser, (req, res, next) => {
			...
			this.addProjectStructureUpdates(project_id, userId, docUpdates, fileUpdates, version);
