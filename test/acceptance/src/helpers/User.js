...
class User {
	...
	setOverleafId(overleaf_id, callback) {
		if (callback == null) { callback = function(error) {}; }
		return UserModel.update({ _id: this.id }, { 'overleaf.id': overleaf_id }, callback);
	}
	...
	ensure_admin(callback) {
		if (callback == null) { callback = function(error) {}; }
		return db.users.update({_id: ObjectId(this.id)}, { $set: { isAdmin: true }}, callback);
	}
	...
	upgradeFeatures(callback ) {
		...
		return db.users.update({_id: ObjectId(this.id)}, { $set: { features }}, callback);
	}
	...
	getProject(project_id, callback) {
		...
		return db.projects.findOne({_id: ObjectId(project_id.toString())}, callback);
	}
	...
	deleteProjects(callback) {
		if (callback == null) { callback = error; }
		return db.projects.remove({owner_ref:ObjectId(this.id)}, {multi:true}, err => callback(err));
	}
	...
	isLoggedIn(callback) {
		...
		return this.request.get("/user/personal_info", function(error, response, body) {
			if (error != null) { return callback(error); }
			if (response.statusCode === 200) {
