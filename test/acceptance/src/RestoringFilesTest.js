...
describe("RestoringFiles", function() {
	...
	describe("restoring a deleted doc", function() {
		beforeEach(function(done) {
			return this.owner.getProject(this.project_id, (error, project) => {
				if (error != null) { throw error; }
				this.doc = _.find(project.rootFolder[0].docs, doc => doc.name === 'main.tex');
		...
		return it('should have restored the doc', function(done) {
			return this.owner.getProject(this.project_id, (error, project) => {
				if (error != null) { throw error; }
				const restored_doc = _.find(project.rootFolder[0].docs, doc => doc.name === 'main.tex');
				...
				expect(MockDocstoreApi.docs[this.project_id][this.restored_doc_id].lines).to.deep.equal(
	...
	return describe("restoring from v2 history", function() {
		describe("restoring a text file", function() {
			beforeEach(function(done) {
				MockProjectHistoryApi.addOldFile(this.project_id, 42, "foo.tex", "hello world, this is foo.tex!");
				return this.owner.request({
					method: "POST",
			...
			return it("should have created a doc", function(done) {
				return this.owner.getProject(this.project_id, (error, project) => {
					if (error != null) { throw error; }
					let doc = _.find(project.rootFolder[0].docs, doc => doc.name === 'foo.tex');
		...
		describe("restoring a binary file", function() {
			beforeEach(function(done) {
				this.pngData = fs.readFileSync(Path.resolve(__dirname, '../files/1pixel.png'), 'binary');
				MockProjectHistoryApi.addOldFile(this.project_id, 42, "image.png", this.pngData);
				return this.owner.request({
					method: "POST",
					url: `/project/${this.project_id}/restore_file`,
					json: {
			...
			return it("should have created a file", function(done) {
				return this.owner.getProject(this.project_id, (error, project) => {
					if (error != null) { throw error; }
					let file = _.find(project.rootFolder[0].fileRefs, file => file.name === 'image.png');
		...
		describe("restoring to a directory that exists", function() {
			beforeEach(function(done) {
				MockProjectHistoryApi.addOldFile(this.project_id, 42, "foldername/foo2.tex", "hello world, this is foo-2.tex!");
			...
			return it("should have created the doc in the named folder", function(done) {
				return this.owner.getProject(this.project_id, (error, project) => {
					if (error != null) { throw error; }
					const folder = _.find(project.rootFolder[0].folders, folder => folder.name === 'foldername');
		...
		describe("restoring to a directory that no longer exists", function() {
			beforeEach(function(done) {
				MockProjectHistoryApi.addOldFile(this.project_id, 42, "nothere/foo3.tex", "hello world, this is foo-3.tex!");
				return this.owner.request({
					method: "POST",
					url: `/project/${this.project_id}/restore_file`,
					json: {
			...
			return it("should have created the folder and restored the doc to it", function(done) {
				return this.owner.getProject(this.project_id, (error, project) => {
					if (error != null) { throw error; }
					const folder = _.find(project.rootFolder[0].folders, folder => folder.name === 'nothere');
		...
		return describe("restoring to a filename that already exists", function() {
			beforeEach(function(done) {
				MockProjectHistoryApi.addOldFile(this.project_id, 42, "main.tex", "hello world, this is main.tex!");
				return this.owner.request({
					method: "POST",
					url: `/project/${this.project_id}/restore_file`,
					json: {
			...
			return it("should have created the doc in the root folder", function(done) {
				return this.owner.getProject(this.project_id, (error, project) => {
					if (error != null) { throw error; }
					let doc = _.find(project.rootFolder[0].docs, doc => doc.name.match(/main \(Restored on/));
					expect(doc).to.exist;
					doc = MockDocstoreApi.docs[this.project_id][doc._id];
					expect(doc.lines).to.deep.equal([
