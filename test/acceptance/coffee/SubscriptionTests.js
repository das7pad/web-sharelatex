		...
		describe('when the user is a member of an affiliation', function() {
			before(function(done) {
				...
				return async.series([
					...
					cb => {
						return this.user.addEmail('unconfirmed-affiliation-email@harvard.example.edu', cb);
					},
					cb => {
						return this.user.addEmail('confirmed-affiliation-email@stanford.example.edu', cb);
					},
					cb => {
						return this.user.confirmEmail('confirmed-affiliation-email@stanford.example.edu', cb);
					},
					cb => {
						return this.user.addEmail('confirmed-affiliation-email@mit.example.edu', cb);
					},
					cb => {
						return this.user.confirmEmail('confirmed-affiliation-email@mit.example.edu', cb);
					}
				], error => {
					if (error != null) { return done(error); }
					return SubscriptionViewModelBuilder.buildUsersSubscriptionViewModel(this.user, (error, data) => {
