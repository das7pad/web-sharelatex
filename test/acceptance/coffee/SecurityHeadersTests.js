...
describe("SecurityHeaders", function() {
	...
	it('should not have x-powered-by header', done => request.get('/', (err, res, body) => {
        assert.isUndefined(res.headers['x-powered-by']);
        return done();
    }));

	it('should have all common headers', done => request.get('/', (err, res, body) => {
        assert_has_common_headers(res);
        return done();
    }));

	it('should not have cache headers on public pages', done => request.get('/', (err, res, body) => {
	...
	return it('should have cache headers on project page', function(done) {
		...
		return async.series([
			cb => this.user.login(cb),
			cb => {
				return this.user.createProject("public-project", (error, project_id) => {
