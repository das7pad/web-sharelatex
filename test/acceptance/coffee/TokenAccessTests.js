...
const try_read_only_token_access = (user, token, test, callback) => async.series([
    cb => user.request.get(`/read/${token}`, function(error, response, body) {
        if (error != null) { return cb(error); }
        test(response, body);
        return cb();
    })
...
const try_read_and_write_token_access = (user, token, test, callback) => async.series([
    cb => user.request.get(`/${token}`, function(error, response, body) {
        if (error != null) { return cb(error); }
        test(response, body);
        return cb();
    })
...
const try_anon_content_access = function(user, project_id, token, test, callback) {
	// The real-time service calls this end point to determine the user's 
	// permissions.
	let user_id;
	if (user.id != null) {
...
describe('TokenAccess', function() {
	...
	describe('no token-access', function() {
		before(function(done) {
			return this.owner.createProject(`token-ro-test${Math.random()}`, (err, project_id) => {
		...
		it('should deny access ', function(done) {
			return try_read_access(this.other1, this.project_id, (response, body) => {
		...
		return it('should not allow the user to join the project', function(done) {
			return try_content_access(this.other1, this.project_id, (response, body) => {
	...
	describe('read-only token', function() {
		...
		it('should allow the user to access project via read-only token url', function(done) {
			return try_read_only_token_access(this.other1, this.tokens.readOnly, (response, body) => {
		...
		return describe('made private again', function() {
			before(function(done) {
				return this.owner.makePrivate(this.project_id, () => setTimeout(done, 1000));
			});
	...
	describe('anonymous read-only token', function() {
		before(function(done) {
			return this.owner.createProject(`token-anon-ro-test${Math.random()}`, (err, project_id) => {
				if (err != null) { return done(err); }
				this.project_id = project_id;
				return this.owner.makeTokenBased(this.project_id, err => {
					if (err != null) { return done(err); }
		...
		it('should deny access before the token is used', function(done) {
			return try_read_access(this.anon, this.project_id, (response, body) => {
				expect(response.statusCode).to.equal(302);
				return expect(body).to.match(/.*\/restricted.*/);
			}
			, done);
		...
		it('should allow the user to access project via read-only token url', function(done) {
			return try_read_only_token_access(this.anon, this.tokens.readOnly, (response, body) => {
				return expect(response.statusCode).to.equal(200);
			}
			, done);
		});
		...
		it('should allow the user to anonymously join the project with read-only access', function(done) {
			return try_anon_content_access(this.anon, this.project_id, this.tokens.readOnly, (response, body) => {
				return expect(body.privilegeLevel).to.equal('readOnly');
			}
			, done);
		});
	...
	describe('read-and-write token', function() {
		before(function(done) {
			return this.owner.createProject(`token-rw-test${Math.random()}`, (err, project_id) => {
				if (err != null) { return done(err); }
				this.project_id = project_id;
				return this.owner.makeTokenBased(this.project_id, err => {
					if (err != null) { return done(err); }
		...
		it('should allow the user to access project via read-and-write token url', function(done) {
			return try_read_and_write_token_access(this.other1, this.tokens.readAndWrite, (response, body) => {
				return expect(response.statusCode).to.equal(200);
			}
			, done);
		});
	...
	if (!settings.allowAnonymousReadAndWriteSharing) {
		describe('anonymous read-and-write token, disabled', function() {
			before(function(done) {
				return this.owner.createProject(`token-anon-rw-test${Math.random()}`, (err, project_id) => {
					if (err != null) { return done(err); }
					this.project_id = project_id;
					return this.owner.makeTokenBased(this.project_id, err => {
						if (err != null) { return done(err); }
			...
			it('should not allow the user to access read-and-write token', function(done) {
				return try_read_and_write_token_access(this.anon, this.tokens.readAndWrite, (response, body) => {
					expect(response.statusCode).to.equal(302);
					return expect(body).to.match(/.*\/restricted.*/);
				}
				, done);
			...
			return it('should not allow the user to join the project', function(done) {
				return try_anon_content_access(this.anon, this.project_id, this.tokens.readAndWrite, (response, body) => {
					return expect(body.privilegeLevel).to.equal(false);
				}
				, done);
			});
	...
	describe('private overleaf project', function() {
				...
				return this.owner.makeTokenBased(this.project_id, err => {
					return this.owner.getProject(this.project_id, (err, project) => {
						...
						return this.owner.makePrivate(this.project_id, () => {
							return db.projects.update({_id: project._id}, {
								$set: {
									overleaf: {id: 1234}
								}
							}, err => {
		...
		it('should redirect to canonical path, when owner uses read-write token', function(done) {
			return try_read_and_write_token_access(this.owner, this.tokens.readAndWrite, (response, body) => {
				expect(response.statusCode).to.equal(302);
				return expect(response.headers.location).to.equal(`/project/${this.project_id}`);
			}
			, done);
		});

		it('should allow the owner access to the project', function(done) {
			return try_read_access(this.owner, this.project_id, (response, body) => {
				return expect(response.statusCode).to.equal(200);
			}
			, done);
		});
		...
		it('should allow owner to join the project', function(done) {
			return try_content_access(this.owner, this.project_id, (response, body) => {
		...
		return it('should not allow other user to join the project', function(done) {
			return try_content_access(this.other2, this.project_id, (response, body) => {
				return expect(body.privilegeLevel).to.equal(false);
			}
			, done);
		});
	...
	describe('private project, with higher access', function() {
		before(function(done) {
			return this.owner.createProject(`higher-access-test-${Math.random()}`, (err, project_id) => {
				this.project_id = project_id;
				return this.owner.addUserToProject(this.project_id, this.other1, 'readAndWrite', err => {
					return this.owner.makeTokenBased(this.project_id, err => {
						return this.owner.getProject(this.project_id, (err, project) => {
							this.tokens = project.tokens;
							return this.owner.makePrivate(this.project_id, () => {
	...
	describe('unimported v1 project', function() {
		...
		it('should redirect read and write token to v1', function(done) {
			...
			return try_read_and_write_token_access(this.owner, unimportedV1Token, (response, body) => {
				expect(response.statusCode).to.equal(302);
		...
		return it('should redirect read only token to v1', function(done) {
			...
			return try_read_only_token_access(this.owner, unimportedV1Token, (response, body) => {
				expect(response.statusCode).to.equal(302);
				return expect(response.headers.location).to.equal(
	...
	return describe('importing v1 project', function() {
		before(function(done) {
			...
			return this.owner.createProject(`token-rw-test${Math.random()}`, (err, project_id) => {
				...
				return this.owner.makeTokenBased(this.project_id, err => {
					if (err != null) { return done(err); }
					return db.projects.update({_id: ObjectId(project_id)}, {$set: {overleaf: {id: 1234}}}, err => {
						if (err != null) { return done(err); }
						return this.owner.getProject(this.project_id, (err, project) => {
							if (err != null) { return done(err); }
							this.tokens = project.tokens;
							MockV1Api.setDocExported(this.tokens.readAndWrite, {exporting: true});
							MockV1Api.setDocExported(this.tokens.readOnly, {exporting: true});
							return done();
						});
					});
				});
		...
		it('should show importing page for read only token', function(done) {
			return try_read_only_token_access(this.owner, this.tokens.readOnly, (response, body) => {
				expect(response.statusCode).to.equal(200);
				return expect(body).to.include('ImportingController');
			}
			, done);
