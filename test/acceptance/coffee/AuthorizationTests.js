...
const try_read_access = (user, project_id, test, callback) => async.series([
    cb => user.request.get(`/project/${project_id}`, function(error, response, body) {
        if (error != null) { return cb(error); }
        test(response, body);
        return cb();
    }),
    cb => user.request.get(`/project/${project_id}/download/zip`, function(error, response, body) {
        if (error != null) { return cb(error); }
        test(response, body);
        return cb();
    })
...
const try_settings_write_access = (user, project_id, test, callback) => async.series([
    cb => user.request.post({
...
const try_admin_access = (user, project_id, test, callback) => async.series([
    cb => user.request.post({
...
const expect_read_access = (user, project_id, callback) => async.series([
    cb => try_read_access(user, project_id, (response, body) => expect(response.statusCode).to.be.oneOf([200, 204])
    , cb),
    cb => try_content_access(user, project_id, (response, body) => expect(body.privilegeLevel).to.be.oneOf(["owner", "readAndWrite", "readOnly"])
    , cb)
], callback);

const expect_content_write_access = (user, project_id, callback) => try_content_access(user, project_id, (response, body) => expect(body.privilegeLevel).to.be.oneOf(["owner", "readAndWrite"])
, callback);

const expect_settings_write_access = (user, project_id, callback) => try_settings_write_access(user, project_id, (response, body) => expect(response.statusCode).to.be.oneOf([200, 204])
, callback);

const expect_admin_access = (user, project_id, callback) => try_admin_access(user, project_id, (response, body) => expect(response.statusCode).to.be.oneOf([200, 204])
, callback);

const expect_no_read_access = (user, project_id, options, callback) => async.series([
    cb => try_read_access(user, project_id, function(response, body) {
        expect(response.statusCode).to.equal(302);
        return expect(response.headers.location).to.match(new RegExp(options.redirect_to));
    }
    , cb),
    cb => try_content_access(user, project_id, (response, body) => expect(body.privilegeLevel).to.be.equal(false)
    , cb)
], callback);

const expect_no_content_write_access = (user, project_id, callback) => try_content_access(user, project_id, (response, body) => expect(body.privilegeLevel).to.be.oneOf([false, "readOnly"])
, callback);

const expect_no_settings_write_access = (user, project_id, options, callback) => try_settings_write_access(user, project_id, function(response, body) {
    expect(response.statusCode).to.equal(302);
    return expect(response.headers.location).to.match(new RegExp(options.redirect_to));
}
, callback);

const expect_no_admin_access = (user, project_id, options, callback) => try_admin_access(user, project_id, function(response, body) {
    expect(response.statusCode).to.equal(302);
    return expect(response.headers.location).to.match(new RegExp(options.redirect_to));
}
, callback);
...
describe("Authorization", function() {
		...
		return async.parallel([
			...
			cb => {
				return this.site_admin.login(err => {
					if (typeof error !== 'undefined' && error !== null) { return cb(err); }
	...
	describe("private project", function() {
		...
		it("should not allow another user read access to the project", function(done) {
			return expect_no_read_access(this.other1, this.project_id, {redirect_to: "/restricted"}, done);
		});
		...
		it("should not allow another user write access to its settings", function(done) {
			return expect_no_settings_write_access(this.other1, this.project_id, {redirect_to: "/restricted"}, done);
		});
			
		it("should not allow another user admin access to it", function(done) {
			return expect_no_admin_access(this.other1, this.project_id, {redirect_to: "/restricted"}, done);
		});
			
		it("should not allow anonymous user read access to it", function(done) {
			return expect_no_read_access(this.anon, this.project_id, {redirect_to: "/restricted"}, done);
		});
		...
		it("should not allow anonymous user write access to its settings", function(done) {
			return expect_no_settings_write_access(this.anon, this.project_id, {redirect_to: "/restricted"}, done);
		});
			
		it("should not allow anonymous user admin access to it", function(done) {
			return expect_no_admin_access(this.anon, this.project_id, {redirect_to: "/restricted"}, done);
		});
		...
		it("should allow site admin users write access to its settings", function(done) {
			return expect_settings_write_access(this.site_admin, this.project_id, done);
		});
	...
	describe("shared project", function() {
		before(function(done) {
			...
			return this.owner.createProject("private-project", (error, project_id) => {
				if (error != null) { return done(error); }
				this.project_id = project_id;
				return this.owner.addUserToProject(this.project_id, this.ro_user, "readOnly", error => {
					if (error != null) { return done(error); }
					return this.owner.addUserToProject(this.project_id, this.rw_user, "readAndWrite", error => {
						if (error != null) { return done(error); }
						return done();
					});
				});
		...
		it("should not allow the read-only user write access to its settings", function(done) {
			return expect_no_settings_write_access(this.ro_user, this.project_id, {redirect_to: "/restricted"}, done);
		});
		
		it("should not allow the read-only user admin access to it", function(done) {
			return expect_no_admin_access(this.ro_user, this.project_id, {redirect_to: "/restricted"}, done);
		});
		...
		return it("should not allow the read-write user admin access to it", function(done) {
			return expect_no_admin_access(this.rw_user, this.project_id, {redirect_to: "/restricted"}, done);
		});
	});

	describe("public read-write project", function() {
		before(function(done) {
			return this.owner.createProject("public-rw-project", (error, project_id) => {
				if (error != null) { return done(error); }
				this.project_id = project_id;
	...
	return describe("public read-only project", function() {
		before(function(done) {
			return this.owner.createProject("public-ro-project", (error, project_id) => {
				if (error != null) { return done(error); }
				this.project_id = project_id;
