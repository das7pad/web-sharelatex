	...
	run() {
		app.post("/project/:project_id/file/:file_id", (req, res, next) => {
			...
			return req.on('end', () => {
				...
				if (this.files[project_id] == null) { this.files[project_id] = {}; }
		...
		app.put("/project/:project_id/file/:file_id", bodyParser.json(), (req, res, next) => {
			...
			const {content} = this.files[source.project_id] != null ? this.files[source.project_id][source.file_id] : undefined;
			if ((content == null)) {
		...
		return app.listen(3009, function(error) {
