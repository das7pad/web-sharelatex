...
module.exports = (MockV1Api = {
	...
	setAffiliations(affiliations) { return this.affiliations = affiliations; },

	doc_exported: {},

	setDocExported(token, info) { return this.doc_exported[token] = info; },
	...
	run() {
		...
		app.get('/universities/list/:id', (req, res, next) => res.json({
		...
		app.get("/api/v2/partners/:partner/conversions/:id", (req, res, next) => {
			...
			const conversion = __guard__(partner != null ? partner.conversions : undefined, x => x[req.params.id]);
			if (conversion != null) {
				return res.status(200).json({input_file_uri: conversion, brand_variation_id: partner.brand_variation_id});
			} else {
		...
		app.get('/api/v1/sharelatex/docs/:token/exported_to_v2', (req, res, next) => {
			if (this.doc_exported[req.params.token] != null) { return res.json(this.doc_exported[req.params.token]); }
