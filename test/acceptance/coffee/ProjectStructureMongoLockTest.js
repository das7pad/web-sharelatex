...
const ProjectGetter = require(`${APP_PATH}/Features/Project/ProjectGetter`);
const ProjectEntityMongoUpdateHandler = require(`${APP_PATH}/Features/Project/ProjectEntityMongoUpdateHandler`);
const UserCreator = require(`${APP_PATH}/Features/User/UserCreator`);
...
describe("ProjectStructureMongoLock", () => describe("whilst a project lock is taken", function() {
    before(function(done) {
        ...
        UserCreator.createNewUser(userDetails, (err, user) => {
            ...
            return ProjectCreationHandler.createBlankProject(user._id, 'locked-project', (err, project) => {
                if (err != null) { throw err; }
                ...
                return LockManager._getLock(this.lock_key, namespace, (err, lockValue) => {
    ...
    describe('interacting with the locked project', function() {
        const LOCKING_UPDATE_METHODS = ['addDoc', 'addFile', 'mkdirp', 'moveEntity', 'renameEntity', 'addFolder'];
        ...
        it("cannot get the project if rootFolder is in the projection", function(done) {
            return ProjectGetter.getProject(this.locked_project._id, {rootFolder: true}, function(err) {
                expect(err).to.deep.equal(new Error("Timeout"));
                return done();
            });
        });
        ...
        return it("can get the project if rootFolder is not in the projection", function(done) {
            return ProjectGetter.getProject(this.locked_project._id, {_id: true}, (err, project) => {
                expect(err).to.equal(null);
                expect(project._id.toString()).to.equal(this.locked_project._id.toString());
                return done();
            });
        });
    });
    ...
    return describe('interacting with other projects', function() {
        before(function(done) {
            return ProjectCreationHandler.createBlankProject(this.user._id, 'unlocked-project', (err, project) => {
                if (err != null) { throw err; }
        ...
        it("can add folders to other projects", function(done) {
            return ProjectEntityMongoUpdateHandler.addFolder(this.unlocked_project._id, this.unlocked_project.rootFolder[0]._id, 'new folder', function(err, folder) {
                expect(err).to.equal(null);
        ...
        return it("can get other projects without a projection", function(done) {
            return ProjectGetter.getProject(this.unlocked_project._id, (err, project) => {
                expect(err).to.equal(null);
                expect(project._id.toString()).to.equal(this.unlocked_project._id.toString());
                return done();
            });
        });
    });
