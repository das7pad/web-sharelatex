...
const ownPort = __guard__(Settings.internal != null ? Settings.internal.web : undefined, x => x.port) || Settings.port || 3000;
const port = (Settings.web != null ? Settings.web.web_router_port : undefined) || ownPort; // send requests to web router if this is the api process
const cookeFilePath = `/tmp/smoke-test-cookie-${ownPort}-to-${port}.txt`;
const buildUrl = path => ` -b ${cookeFilePath} --resolve 'smoke${Settings.cookieDomain}:${port}:127.0.0.1' http://smoke${Settings.cookieDomain}:${port}/${path}?setLng=en`;
const logger = require("logger-sharelatex");
const LoginRateLimiter = require("../../../app/js/Features/Security/LoginRateLimiter");
const RateLimiter = require("../../../app/js/infrastructure/RateLimiter");
...
describe("Opening", function() {
	...
	before(function(done) {
		...
		LoginRateLimiter.recordSuccessfulLogin(Settings.smokeTest.user, function(err){
			if (err != null) {
				logger.err({err}, "smoke test: error recoring successful login");
				return done(err);
			}
			return RateLimiter.clearRateLimit("open-project", `${Settings.smokeTest.projectId}:${Settings.smokeTest.userId}`, function(err){
				if (err != null) {
					logger.err({err}, "smoke test: error clearing open-project rate limit");
					return done(err);
				}
				return RateLimiter.clearRateLimit("overleaf-login", Settings.smokeTest.rateLimitSubject, function(err){
					if (err != null) {
						logger.err({err}, "smoke test: error clearing overleaf-login rate limit");
						return done(err);
					}
					return done();
				});
...
`;
		return child.exec(command, function(error, stdout, stderr){
			...
			const match = !!stdout.match(`window.project_id = \"${Settings.smokeTest.projectId}\"`);
