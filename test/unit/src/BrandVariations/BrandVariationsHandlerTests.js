...
const modulePath = path.join(__dirname, "../../../../app/js/Features/BrandVariations/BrandVariationsHandler");
...
describe("BrandVariationsHandler", function() {
	...
	beforeEach(function() {
		...
		this.V1Api = 
			{request: sinon.stub()};
		this.BrandVariationsHandler = SandboxedModule.require(modulePath, { requires: {
	...
	return describe("getBrandVariationById", function() {
		it("should call the callback with an error when the branding variation id is not provided", function(done) {
			return this.BrandVariationsHandler.getBrandVariationById(null, (err, brandVariationDetails) => {
		...
		it("should call the callback with an error when the request errors", function(done) {
			...
			return this.BrandVariationsHandler.getBrandVariationById("12", (err, brandVariationDetails) => {
				expect(err).to.be.instanceof(Error);
				return done();
			});
		});
		...
		it("should call the callback with branding details when request succeeds", function(done) {
			this.V1Api.request.callsArgWith(1, null, { statusCode: 200 }, this.mockedBrandVariationDetails);
			return this.BrandVariationsHandler.getBrandVariationById("12", (err, brandVariationDetails) => {
				expect(err).to.not.exist;
				expect(brandVariationDetails).to.deep.equal(this.mockedBrandVariationDetails);
				return done();
			});
		});
		...
		return it("should transform relative URLs in v1 absolute ones", function(done) {
			...
			this.V1Api.request.callsArgWith(1, null, { statusCode: 200 }, this.mockedBrandVariationDetails);
			return this.BrandVariationsHandler.getBrandVariationById("12", (err, brandVariationDetails) => {
				expect(brandVariationDetails.logo_url.startsWith(this.settings.apis.v1.url)).to.be.true;
				return done();
			});
		});
	});
