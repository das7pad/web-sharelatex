...
const modulePath = require('path').join(__dirname, '../../../../app/js/Features/Referal/ReferalFeatures.js');

describe('ReferalFeatures', function() {

	beforeEach(function() {
		this.ReferalFeatures = SandboxedModule.require(modulePath, { requires: {
	...
	describe("getBonusFeatures", function() {
			...
			return it("should call the callback with the features", function() {
				return this.callback.calledWith(null, this.Settings.bonus_features[3]).should.equal(true);
			});
	});
	...
	return describe("when the user is not at a bonus level", function() {
			beforeEach(function() {
				...
				this.User.findOne = sinon.stub().callsArgWith(1, null, { refered_user_count: this.refered_user_count });
				return this.ReferalFeatures.getBonusFeatures(this.user_id, this.callback);
			});
