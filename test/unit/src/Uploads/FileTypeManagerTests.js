...
describe("FileTypeManager", function() {
	beforeEach(function() {
		...
		return this.FileTypeManager = SandboxedModule.require(modulePath, { requires: {
			"fs": this.fs,
			"is-utf8": this.isUtf8
		}
	}
	...
	describe("getType", function() {
		beforeEach(function() {
			...
			this.fs.readFile = sinon.stub().callsArgWith(1, null, Buffer.from(this.contents, "utf-8"));
			this.fs.readFile.withArgs("/path/on/disk/utf16.tex").callsArgWith(1, null, Buffer.from("\uFEFF" + this.contents, "utf-16le"));
			this.fs.readFile.withArgs("/path/on/disk/latin1.tex").callsArgWith(1, null, Buffer.from(this.contents, "latin1"));
		...
		describe("when the file extension is text", function() {
			it("should return .tex files as not binary", function() {
				return this.FileTypeManager.getType("file.tex", "/path/on/disk", (error, binary) => binary.should.equal(false));
			});

			it("should return .bib files as not binary", function() {
				return this.FileTypeManager.getType("file.bib", "/path/on/disk", (error, binary) => binary.should.equal(false));
			});

			it("should return .bibtex files as not binary", function() {
				return this.FileTypeManager.getType("file.bibtex", "/path/on/disk", (error, binary) => binary.should.equal(false));
			});

			it("should return .cls files as not binary", function() {
				return this.FileTypeManager.getType("file.cls", "/path/on/disk", (error, binary) => binary.should.equal(false));
			});

			it("should return .sty files as not binary", function() {
				return this.FileTypeManager.getType("file.sty", "/path/on/disk", (error, binary) => binary.should.equal(false));
			});

			it("should return .bst files as not binary", function() {
				return this.FileTypeManager.getType("file.bst", "/path/on/disk", (error, binary) => binary.should.equal(false));
			});

			it("should return .latexmkrc file as not binary", function() {
				return this.FileTypeManager.getType(".latexmkrc", "/path/on/disk", (error, binary) => binary.should.equal(false));
			});
			...
			it("should return lbx file as not binary", function() {
				return this.FileTypeManager.getType("file.lbx", "/path/on/disk", (error, binary) => binary.should.equal(false));
			});

			it("should return bbx file as not binary", function() {
				return this.FileTypeManager.getType("file.bbx", "/path/on/disk", (error, binary) => binary.should.equal(false));
			});

			it("should return cbx file as not binary", function() {
				return this.FileTypeManager.getType("file.cbx", "/path/on/disk", (error, binary) => binary.should.equal(false));
			});

			it("should return m file as not binary", function() {
				return this.FileTypeManager.getType("file.m", "/path/on/disk", (error, binary) => binary.should.equal(false));
			});
			...
			it("should return large text files as binary", function() {
				...
				return this.FileTypeManager.getType("file.tex", "/path/on/disk", (error, binary) => binary.should.equal(true));
			});
			...
			it("should detect the file as utf8", function() {
				return this.FileTypeManager.getType("file.tex", "/path/on/disk", (error, binary, encoding) => {
			...
			it("should return 'latin1' for non-unicode encodings", function() {
				return this.FileTypeManager.getType("file.tex", "/path/on/disk/latin1.tex", (error, binary, encoding) => {
					sinon.assert.calledOnce(this.isUtf8);
			...
			return it("should detect utf16 with BOM as utf-16", function() {
				return this.FileTypeManager.getType("file.tex", "/path/on/disk/utf16.tex", (error, binary, encoding) => {
					sinon.assert.calledOnce(this.isUtf8);
					this.isUtf8.returned(false).should.equal(true);
		...
		return describe("when the file extension is non-text", function() {
			it("should return .eps files as binary", function() {
				return this.FileTypeManager.getType("file.eps", "/path/on/disk", (error, binary) => binary.should.equal(true));
			});

			it("should return .dvi files as binary", function() {
				return this.FileTypeManager.getType("file.dvi", "/path/on/disk", (error, binary) => binary.should.equal(true));
			});

			it("should return .png files as binary", function() {
				return this.FileTypeManager.getType("file.png", "/path/on/disk", (error, binary) => binary.should.equal(true));
			});

			it("should return files without extensions as binary", function() {
				return this.FileTypeManager.getType("tex", "/path/on/disk", (error, binary) => binary.should.equal(true));
			});
	...
	return describe("shouldIgnore", function() {
		it("should ignore tex auxiliary files", function() {
			return this.FileTypeManager.shouldIgnore("file.aux", (error, ignore) => ignore.should.equal(true));
		});

		it("should ignore dotfiles", function() {
			return this.FileTypeManager.shouldIgnore("path/.git", (error, ignore) => ignore.should.equal(true));
		});

		it("should not ignore .latexmkrc dotfile", function() {
			return this.FileTypeManager.shouldIgnore("path/.latexmkrc", (error, ignore) => ignore.should.equal(false));
		});

		it("should ignore __MACOSX", function() {
			return this.FileTypeManager.shouldIgnore("path/__MACOSX", (error, ignore) => ignore.should.equal(true));
		});

		it("should not ignore .tex files", function() {
			return this.FileTypeManager.shouldIgnore("file.tex", (error, ignore) => ignore.should.equal(false));
		});
