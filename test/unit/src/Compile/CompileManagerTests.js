...
describe("CompileManager", function() {
	beforeEach(function() {
		...
		this.CompileManager = SandboxedModule.require(modulePath, { requires: {
			...
			"../../infrastructure/RedisWrapper": {
				client: () => { return this.rclient = { auth() {} }; }
			},
	...
	describe("compile", function() {
		beforeEach(function() {
			this.CompileManager._checkIfRecentlyCompiled = sinon.stub().callsArgWith(2, null, false);
			this.ProjectRootDocManager.ensureRootDocumentIsSet = sinon.stub().callsArgWith(1, null);
			this.CompileManager.getProjectCompileLimits = sinon.stub().callsArgWith(1, null, this.limits);
			return this.ClsiManager.sendRequest = sinon.stub().callsArgWith(3, null, (this.status = "mock-status"), (this.outputFiles = "mock output files"), (this.output = "mock output"));
		});

		describe("succesfully", function() {
			beforeEach(function() {
				this.CompileManager._checkIfAutoCompileLimitHasBeenHit = (isAutoCompile, compileGroup, cb) => cb(null, true);
				return this.CompileManager.compile(this.project_id, this.user_id, {}, this.callback);
			});
			...
			return it("should log out the compile", function() {
				return this.logger.log
					.calledWith({project_id: this.project_id, user_id: this.user_id}, "compiling project")
					.should.equal(true);
			});
		});

		describe("when the project has been recently compiled", () => it("should return", function(done){
            this.CompileManager._checkIfAutoCompileLimitHasBeenHit = (isAutoCompile, compileGroup, cb) => cb(null, true);
            this.CompileManager._checkIfRecentlyCompiled = sinon.stub().callsArgWith(2, null, true);
            return this.CompileManager.compile(this.project_id, this.user_id, {}, function(err, status){
                status.should.equal("too-recently-compiled");
                return done();
            });
        }));

		return describe("should check the rate limit", () => it("should return", function(done){
            this.CompileManager._checkIfAutoCompileLimitHasBeenHit = sinon.stub().callsArgWith(2, null, false);
            return this.CompileManager.compile(this.project_id, this.user_id, {}, function(err, status){
	...
	describe("getProjectCompileLimits", function() {
		beforeEach(function() {
			...
			this.ProjectGetter.getProject = sinon.stub().callsArgWith(2, null, (this.project = { owner_ref: (this.owner_id = "owner-id-123") }));
			this.UserGetter.getUser = sinon.stub().callsArgWith(2, null, (this.user = { features: this.features }));
			return this.CompileManager.getProjectCompileLimits(this.project_id, this.callback);
		});
	...
	describe("deleteAuxFiles", function() {
		beforeEach(function() {
			this.CompileManager.getProjectCompileLimits = sinon.stub().callsArgWith(1, null, (this.limits = { compileGroup: "mock-compile-group" }));
			this.ClsiManager.deleteAuxFiles = sinon.stub().callsArg(3);
			return this.CompileManager.deleteAuxFiles(this.project_id, this.user_id, this.callback);
		});
	...
	describe("_checkIfRecentlyCompiled", function() {
		describe("when the key exists in redis", function() {
			beforeEach(function() {
				...
				return this.CompileManager._checkIfRecentlyCompiled(this.project_id, this.user_id, this.callback);
			});

			it("should try to set the key", function() {
				return this.rclient.set
					.calledWith(`compile:${this.project_id}:${this.user_id}`, true, "EX", this.CompileManager.COMPILE_DELAY, "NX")
					.should.equal(true);
			});
	...
	describe("_checkIfAutoCompileLimitHasBeenHit", function() {
		...
		it("should be able to compile if it is not an autocompile", function(done){
			...
			return this.CompileManager._checkIfAutoCompileLimitHasBeenHit(false, "everyone", (err, canCompile)=> {
		...
		it("should be able to compile if rate limit has remianing", function(done){
			...
			return this.CompileManager._checkIfAutoCompileLimitHasBeenHit(true, "everyone", (err, canCompile)=> {
	...
	return describe("wordCount", function() {
		beforeEach(function() {
			...
			return this.CompileManager.wordCount(this.project_id, this.user_id, false, this.callback);
		});
