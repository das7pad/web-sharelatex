...
describe("ClsiCookieManager", function() {
	beforeEach(function() {
		...
		return this.ClsiCookieManager = SandboxedModule.require(modulePath, {requires:this.requires})();
	});
	...
	describe("getServerId", function() {
		...
		it("should call get for the key", function(done){
			...
			return this.ClsiCookieManager._getServerId(this.project_id, (err, serverId)=> {
				this.redis.get.calledWith(`clsiserver:${this.project_id}`).should.equal(true);
		...
		it("should _populateServerIdViaRequest if no key is found", function(done){
			this.ClsiCookieManager._populateServerIdViaRequest = sinon.stub().callsArgWith(1);
			this.redis.get.callsArgWith(1, null);
			return this.ClsiCookieManager._getServerId(this.project_id, (err, serverId)=> {
				this.ClsiCookieManager._populateServerIdViaRequest.calledWith(this.project_id).should.equal(true);
				return done();
			});
		});
	...
	describe("_populateServerIdViaRequest", function() {
		...
		it("should make a request to the clsi", function(done){
			return this.ClsiCookieManager._populateServerIdViaRequest(this.project_id, (err, serverId)=> {
	...
	describe("setServerId", function() {
		...
		beforeEach(function() {
			...
			this.ClsiCookieManager._parseServerIdFromResponse = sinon.stub().returns("clsi-8");
			return this.redisMulti.exec.callsArgWith(0);
		});

		it("should set the server id with a ttl", function(done){
			return this.ClsiCookieManager.setServerId(this.project_id, this.response, err=> {
				this.redisMulti.set.calledWith(`clsiserver:${this.project_id}`, "clsi-8").should.equal(true);
				this.redisMulti.expire.calledWith(`clsiserver:${this.project_id}`, this.settings.clsiCookie.ttl).should.equal(true);
				return done();
			});
		});

		it("should return the server id", function(done){
			return this.ClsiCookieManager.setServerId(this.project_id, this.response, (err, serverId)=> {
		...
		it("should not set the server id if clsiCookies are not enabled", function(done){
			...
			this.ClsiCookieManager = SandboxedModule.require(modulePath, {requires:this.requires})();
			return this.ClsiCookieManager.setServerId(this.project_id, this.response, (err, serverId)=> {
		...
		it("should not set the server id there is no server id in the response", function(done){
			this.ClsiCookieManager._parseServerIdFromResponse = sinon.stub().returns(null);
			return this.ClsiCookieManager.setServerId(this.project_id, this.response, (err, serverId)=> {
				this.redisMulti.exec.called.should.equal(false);
				return done();
			});
		...
		return it("should also set in the secondary if secondary redis is enabled", function(done) {
			...
			this.redis_secondary =
				{multi: () => this.redisSecondaryMulti};
			...
			this.RedisWrapper.client.withArgs("clsi_cookie_secondary").returns(this.redis_secondary);
			this.ClsiCookieManager = SandboxedModule.require(modulePath, {requires:this.requires})();
			this.ClsiCookieManager._parseServerIdFromResponse = sinon.stub().returns("clsi-8");
			return this.ClsiCookieManager.setServerId(this.project_id, this.response, (err, serverId)=> {
				this.redisSecondaryMulti.set.calledWith(`clsiserver:${this.project_id}`, "clsi-8").should.equal(true);
				this.redisSecondaryMulti.expire.calledWith(`clsiserver:${this.project_id}`, this.settings.clsiCookie.ttl).should.equal(true);
				return done();
			});
		});
	});
	...
	return describe("getCookieJar", function() {
		...
		beforeEach(function() {
			return this.ClsiCookieManager._getServerId = sinon.stub().callsArgWith(1, null, "clsi-11");
		});

		it("should return a jar with the cookie set populated from redis", function(done){
			return this.ClsiCookieManager.getCookieJar(this.project_id, (err, jar)=> {
				jar._jar.store.idx["clsi.example.com"]["/"][this.settings.clsiCookie.key].key.should.equal; 
				jar._jar.store.idx["clsi.example.com"]["/"][this.settings.clsiCookie.key].value.should.equal("clsi-11");
				return done();
			});
		});
		...
		return it("should return empty cookie jar if clsiCookies are not enabled", function(done){
			delete this.settings.clsiCookie.key; 
			this.ClsiCookieManager = SandboxedModule.require(modulePath, {requires:this.requires})();
			return this.ClsiCookieManager.getCookieJar(this.project_id, function(err, jar){
