...
describe("ClsiFormatChecker", function() {
	beforeEach(function() {
		this.ClsiFormatChecker = SandboxedModule.require(modulePath, { requires: {
	...
	return describe("checkRecoursesForProblems", function() {
		...
		it("should call _checkForDuplicatePaths and _checkForConflictingPaths", function(done){
			...
			this.ClsiFormatChecker._checkForConflictingPaths = sinon.stub().callsArgWith(1, null);
			this.ClsiFormatChecker._checkDocsAreUnderSizeLimit = sinon.stub().callsArgWith(1);
			return this.ClsiFormatChecker.checkRecoursesForProblems(this.resources, (err, problems)=> {
				this.ClsiFormatChecker._checkForConflictingPaths.called.should.equal(true);
				this.ClsiFormatChecker._checkDocsAreUnderSizeLimit.called.should.equal(true);
				return done();
			});
		});
		...
		it("should remove undefined errors", function(done){
			this.ClsiFormatChecker._checkForConflictingPaths = sinon.stub().callsArgWith(1, null, []);
			this.ClsiFormatChecker._checkDocsAreUnderSizeLimit = sinon.stub().callsArgWith(1, null, {});
			return this.ClsiFormatChecker.checkRecoursesForProblems(this.resources, (err, problems)=> {
		...
		it("should keep populated arrays", function(done){
			this.ClsiFormatChecker._checkForConflictingPaths = sinon.stub().callsArgWith(1, null, [{path:"somewhere/main.tex"}]);
			this.ClsiFormatChecker._checkDocsAreUnderSizeLimit = sinon.stub().callsArgWith(1, null, {});
			return this.ClsiFormatChecker.checkRecoursesForProblems(this.resources, (err, problems)=> {
		...
		it("should keep populated object", function(done){
			this.ClsiFormatChecker._checkForConflictingPaths = sinon.stub().callsArgWith(1, null, []);
			this.ClsiFormatChecker._checkDocsAreUnderSizeLimit = sinon.stub().callsArgWith(1, null, {resources:[{"a.tex": "a.tex"},{"b.tex": "b.tex"}], totalSize:1000000});
			return this.ClsiFormatChecker.checkRecoursesForProblems(this.resources, (err, problems)=> {
		...
		describe("_checkForConflictingPaths", function() {
			...
			it("should flag up when a nested file has folder with same subpath as file elsewhere", function(done){
				...
				return this.ClsiFormatChecker._checkForConflictingPaths(this.resources, function(err, conflictPathErrors){
		...
		return describe("_checkDocsAreUnderSizeLimit", function() {
			...
			it("should error when there is more than 5mb of data", function(done){
				...
				while (this.resources.length < 20) {
					this.resources.push({path:"chapters/chapter1.tex",url: "http://somwhere.com"});
				}

				return this.ClsiFormatChecker._checkDocsAreUnderSizeLimit(this.resources, function(err, sizeError){
