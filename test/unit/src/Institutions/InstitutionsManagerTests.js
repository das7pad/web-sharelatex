...
const modulePath = path.join(__dirname, "../../../../app/js/Features/Institutions/InstitutionsManager");
const {
    expect
} = require('chai');
...
describe("InstitutionsManager", function() {
	beforeEach(function() {
		...
		this.institutionWithV1Data =
			{name: 'Wombat University'};
		this.institution =
			{fetchV1Data: sinon.stub().callsArgWith(0, null, this.institutionWithV1Data)};
		...
		return this.InstitutionsManager = SandboxedModule.require(modulePath, { requires: {
	...
	describe('upgradeInstitutionUsers', function() {
		beforeEach(function() {
			...
			this.UserGetter.getUser.withArgs(this.user1Id).callsArgWith(1, null, this.user1);
			this.UserGetter.getUser.withArgs(this.user2Id).callsArgWith(1, null, this.user2);
			this.SubscriptionLocator.getUsersSubscription.withArgs(this.user2).callsArgWith(1, null, this.subscription);
			this.refreshFeatures.withArgs(this.user1Id).callsArgWith(2, null, {}, true);
			return this.getInstitutionAffiliations.yields(null, this.affiliations);
		});

		it('refresh all users Features', function(done) {
			return this.InstitutionsManager.upgradeInstitutionUsers(this.institutionId, error => {
		...
		it("notifies users if their features have been upgraded", function(done) {
			return this.InstitutionsManager.upgradeInstitutionUsers(this.institutionId, error => {
				should.not.exist(error);
				sinon.assert.calledOnce(this.NotificationsBuilder.featuresUpgradedByAffiliation);
				sinon.assert.calledWith(this.NotificationsBuilder.featuresUpgradedByAffiliation, this.affiliations[0], this.user1);
				return done();
			});
		});
		...
		return it("notifies users if they have a subscription that should be cancelled", function(done) {
			return this.InstitutionsManager.upgradeInstitutionUsers(this.institutionId, error => {
				should.not.exist(error);
				sinon.assert.calledOnce(this.NotificationsBuilder.redundantPersonalSubscription);
				sinon.assert.calledWith(this.NotificationsBuilder.redundantPersonalSubscription, this.affiliations[1], this.user2);
				return done();
			});
		});
	});
	...
	describe('checkInstitutionUsers', () => it('check all users Features', function(done) {
        const affiliations = [
            { email: 'foo@bar.com' },
            { email: 'baz@boo.edu' }
        ];
        ...
        return this.InstitutionsManager.checkInstitutionUsers(this.institutionId, (error, usersSummary) => {
            should.not.exist(error);
	...
	return describe('getInstitutionUsersSubscriptions', () => it('returns all institution users subscriptions', function(done) {
        const stubbedUsers = [
        ...
        return this.InstitutionsManager.getInstitutionUsersSubscriptions(this.institutionId, (error, subscriptions) => {
            should.not.exist(error);
