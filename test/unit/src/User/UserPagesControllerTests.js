...
const modulePath = path.join(__dirname, "../../../../app/js/Features/User/UserPagesController");
const {
    expect
} = require("chai");
...
describe("UserPagesController", function() {
	...
	beforeEach(function() {
		...
		this.dropboxStatus = {};
		this.DropboxHandler =
			{getUserRegistrationStatus : sinon.stub().callsArgWith(1, null, this.dropboxStatus)};
		...
		this.UserPagesController = SandboxedModule.require(modulePath, { requires: {
		...
		return describe('when an explicit redirect is set via query string', function() {
			...
			return it('should set a redirect', function(done) {
				this.res.render = page => {
					...
					expect(this.AuthenticationController.setRedirectInSession.lastCall.args[1]).to.equal(this.req.query.redir);
					return done();
				};
				return this.UserPagesController.loginPage(this.req, this.res);
			});
	...
	describe('sessionsPage', function() {
		...
		beforeEach(function() {
			return this.UserSessionsManager.getAllUserSessions.callsArgWith(2, null, []);
		});
		...
		return describe('when getAllUserSessions produces an error', function() {
			...
			beforeEach(function() {
				return this.UserSessionsManager.getAllUserSessions.callsArgWith(2, new Error('woops'));
			});
			...
			return it('should call next with an error', function(done) {
				...
				return this.UserPagesController.sessionsPage(this.req, this.res, this.next);
			});
		});
	});
	...
	describe("settingsPage", function() {
		beforeEach(function() {
			this.request.get = sinon.stub().callsArgWith(1, null, {statusCode: 200}, {has_password: true});
			return this.UserGetter.getUser = sinon.stub().callsArgWith(1, null, this.user);
		});
