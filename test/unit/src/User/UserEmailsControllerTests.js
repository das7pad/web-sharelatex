...
describe("UserEmailsController", function() {
	beforeEach(function() {
		...
		return this.UserEmailsController = SandboxedModule.require(modulePath, { requires: {
			...
			"../Institutions/InstitutionsAPI": { endorseAffiliation: this.endorseAffiliation
		},
	...
	describe('Add', function() {
		beforeEach(function() {
			...
			this.UserEmailsConfirmationHandler.sendConfirmationEmail = sinon.stub().yields();
		...
		it('adds new email', function(done) {
			return this.UserEmailsController.add(this.req, { 
				sendStatus: code => {
					...
					assertCalledWith(this.UserUpdater.addEmailAddress, this.user._id, this.newEmail);

					const affiliationOptions = this.UserUpdater.addEmailAddress.lastCall.args[2];
		...
		it('sends an email confirmation', function(done) {
			return this.UserEmailsController.add(this.req, { 
				sendStatus: code => {
					code.should.equal(204);
					assertCalledWith(this.UserEmailsConfirmationHandler.sendConfirmationEmail, this.user._id, this.newEmail);
					return done();
				}
			}
			);
		...
		it('removes email', function(done) {
			...
			return this.UserEmailsController.remove(this.req, { 
				sendStatus: code => {
					...
					assertCalledWith(this.UserUpdater.removeEmailAddress, this.user._id, this.email);
					return done();
				}
			}
			);
		...
		it('sets default email', function(done) {
			...
			return this.UserEmailsController.setDefault(this.req, { 
				sendStatus: code => {
					code.should.equal(200);
					assertCalledWith(this.EmailHelper.parseEmail, this.email);
					assertCalledWith(this.UserUpdater.updateV1AndSetDefaultEmailAddress, this.user._id, this.email);
					return done();
				}
			}
			);
	...
	describe('endorse', function() {
		...
		return it('endorses affiliation', function(done) {
			...
			return this.UserEmailsController.endorse(this.req, {
				sendStatus: code => {
					code.should.equal(204);
					assertCalledWith(this.endorseAffiliation, this.user._id, this.email, 'Role', 'Department');
					return done();
				}
			}
			);
	...
	return describe('confirm', function() {
		beforeEach(function() {
			this.UserEmailsConfirmationHandler.confirmEmailFromToken = sinon.stub().yields();
			this.res = {
