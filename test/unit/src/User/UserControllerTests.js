...
describe("UserController", function() {
	beforeEach(function() {
		...
		this.ReferalAllocator =
			{allocate:sinon.stub()};
		this.SubscriptionDomainHandler =
		...
		this.UserController = SandboxedModule.require(modulePath, { requires: {
			...
			"../Subscription/SubscriptionDomainHandler":this.SubscriptionDomainHandler,
	...
	describe('tryDeleteUser', function() {
		...
		it('should try to authenticate user', function(done) {
			this.res.sendStatus = code => {
				...
				this.AuthenticationManager.authenticate.calledWith({_id: this.user._id}, this.req.body.password).should.equal(true);
				return done();
			};
			return this.UserController.tryDeleteUser(this.req, this.res, this.next);
		});
		...
		describe('when authenticate produces an error', function() {
			...
			beforeEach(function() {
				return this.AuthenticationManager.authenticate = sinon.stub().callsArgWith(2, new Error('woops'));
			});
		...
		describe('when authenticate does not produce a user', function() {
			...
			beforeEach(function() {
				return this.AuthenticationManager.authenticate = sinon.stub().callsArgWith(2, null, null);
			});
		...
		describe('when deleteUser produces an error', function() {
			...
			beforeEach(function() {
				return this.UserDeleter.deleteUser = sinon.stub().callsArgWith(1, new Error('woops'));
			});
		...
		describe('when deleteUser produces a known error', function() {
			...
			beforeEach(function() {
				return this.UserDeleter.deleteUser = sinon.stub().yields(
					new Errors.SubscriptionAdminDeletionError()
				);
			});

			return it('should return a json error', function(done) {
				return this.UserController.tryDeleteUser(this.req, { status(status) {
		...
		return describe('when session.destroy produces an error', function() {
			...
			beforeEach(function() {
				return this.req.session.destroy = sinon.stub().callsArgWith(0, new Error('woops'));
			});
	...
	describe("unsubscribe", () => it("should send the user to unsubscribe", function(done){
        this.res.send = code=> {
	...
	describe("updateUserSettings", function() {
		beforeEach(function() {
			...
			return this.req.externalAuthenticationSystemUsed = sinon.stub().returns(false);
		});
		...
		it("should call the user updater with the new email and user _id", function(done){
			...
			this.res.sendStatus = code=> {
				code.should.equal(200);
				this.UserUpdater.changeEmailAddress.calledWith(this.user_id, this.newEmail).should.equal(true);
				return done();
			};
			return this.UserController.updateUserSettings(this.req, this.res);
		});
		...
		it("should update the email on the session", function(done){
			...
			this.res.sendStatus = code=> {
				code.should.equal(200);
				this.AuthenticationController.setInSessionUser.calledWith(
					this.req, {email: this.newEmail, first_name: undefined, last_name: undefined}
				).should.equal(true);
				return done();
			};
			return this.UserController.updateUserSettings(this.req, this.res);
		...
		return describe('when using an external auth source', function() {
			...
			beforeEach(function() {
				...
				return this.req.externalAuthenticationSystemUsed = sinon.stub().returns(true);
			});
			...
			return it('should not set a new email', function(done) {
				...
				this.res.sendStatus = code=> {
					code.should.equal(200);
					this.UserUpdater.changeEmailAddress.calledWith(this.user_id, this.newEmail).should.equal(false);
					return done();
				};
				return this.UserController.updateUserSettings(this.req, this.res);
			});
	...
	describe("logout", function() {
		...
		return it('should clear sudo-mode', function(done) {
			...
			this.res.redirect = url=> {
				...
				this.SudoModeHandler.clearSudoMode.calledWith(this.user._id).should.equal(true);
				return done();
			};
	...
	describe("register", function() {
		beforeEach(function() {
			this.UserRegistrationHandler.registerNewUserAndSendActivationEmail = sinon.stub().callsArgWith(1, null, this.user, (this.url = "mock/url"));
	...
	describe('clearSessions', function() {
		...
		return describe('when revokeAllUserSessions produces an error', () => it('should call next with an error', function(done) {
            this.UserSessionsManager.revokeAllUserSessions.callsArgWith(2, new Error('woops'));
            const next = err => {
                expect(err).to.not.equal(null);
                expect(err).to.be.instanceof(Error);
                return done();
	...
	return describe("changePassword", function() {
		...
		it("should check the old password is the current one at the moment", function(done){
			...
			this.res.send = () => {
				this.AuthenticationManager.authenticate.calledWith({_id:this.user._id}, "oldpasshere").should.equal(true);
		...
		it("should set the new password if they do match", function(done){
			...
			this.res.send = () => {
				this.AuthenticationManager.setUserPassword.calledWith(this.user._id, "newpass").should.equal(true);
				return done();
			};
			return this.UserController.changePassword(this.req, this.res);
		});
		...
		return it("it should not set the new password if it is invalid", function(done){
			this.AuthenticationManager.validatePassword = sinon.stub().returns({ message: 'password contains invalid characters' });
			this.AuthenticationManager.authenticate.callsArgWith(2, null, {});
			this.req.body = {
