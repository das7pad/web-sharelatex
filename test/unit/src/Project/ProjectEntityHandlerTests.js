...
describe('ProjectEntityHandler', function() {
	...
	beforeEach(function() {
		...
		return this.ProjectEntityHandler = SandboxedModule.require(modulePath, { requires: {
	...
	describe("getting folders, docs and files", function() {
		beforeEach(function() {
			...
			return this.ProjectGetter.getProjectWithoutDocLines = sinon.stub().yields(null, this.project);
		});
		...
		describe("getAllDocs", function() {
			beforeEach(function() {
				...
				this.DocstoreManager.getAllDocs = sinon.stub().callsArgWith(1, null, this.docs);
		...
		describe("getAllDocPathsFromProject", function() {
			beforeEach(function() {
				...
				this.callback = sinon.stub();
				return this.ProjectEntityHandler.getAllDocPathsFromProject(this.project, this.callback);
			});
		...
		describe("_getAllFolders", function() {
			beforeEach(function() {
				this.callback = sinon.stub();
				return this.ProjectEntityHandler._getAllFolders(project_id, this.callback);
			});
		...
		return describe("_getAllFoldersFromProject", function() {
			beforeEach(function() {
				this.callback = sinon.stub();
				return this.ProjectEntityHandler._getAllFoldersFromProject(this.project, this.callback);
			});
	...
	describe("flushProjectToThirdPartyDataStore", function() {
		beforeEach(function(done) {
			...
			this.ProjectEntityHandler.getAllDocs = sinon.stub().yields(null, this.docs);
			this.ProjectEntityHandler.getAllFiles = sinon.stub().yields(null, this.files);

			this.ProjectGetter.getProject = sinon.stub().yields(null, this.project);

			return this.ProjectEntityHandler.flushProjectToThirdPartyDataStore(project_id, () => done());
		});

		it("should flush the project from the doc updater", function() {
			return this.DocumentUpdaterHandler.flushProjectToMongo.calledWith(project_id).should.equal(true);
		});

		it("should look up the project in mongo", function() {
			return this.ProjectGetter.getProject.calledWith(project_id).should.equal(true);
		});

		it("should get all the docs in the project", function() {
			return this.ProjectEntityHandler.getAllDocs.calledWith(project_id).should.equal(true);
		});

		it("should get all the files in the project", function() {
			return this.ProjectEntityHandler.getAllFiles.calledWith(project_id).should.equal(true);
		});
		...
		it("should flush each doc to the TPDS", function() {
			return (() => {
				...
				for (let path in this.docs) {
					...
					result.push(this.TpdsUpdateSender.addDoc
						.calledWith({
							project_id,
							doc_id: doc._id,
		...
		return it("should flush each file to the TPDS", function() {
			return (() => {
				...
				for (let path in this.files) {
					...
					result.push(this.TpdsUpdateSender.addFile
						.calledWith({
							project_id,
							file_id: file._id,
							project_name: this.project.name,
	...
	return describe('getDoc', function() {
		beforeEach(function() {
			...
			this.DocstoreManager.getDoc = sinon.stub().callsArgWith(3, null, this.lines, this.rev, this.version, this.ranges);
