...
const secondSubFolder = {name:"secondSubFolder", _id:"dsa3e23", docs:[subSubDoc], fileRefs:[subSubFile], folders:[]};
const subFolder = {name:"subFolder", _id:"dsadsa93", folders:[secondSubFolder, null], docs:[], fileRefs:[]};
...
describe('ProjectLocator', function() {
	...
	beforeEach(function() {
		...
		return this.locator = SandboxedModule.require(modulePath, { requires: {
	...
	describe('finding a doc', function() {
		it('finds one at the root level', function(done){
			return this.locator.findElement({project_id:project._id, element_id:doc2._id, type:"docs"}, function(err, foundElement, path, parentFolder){
				assert((err == null));
		...
		it('when it is nested', function(done){
			return this.locator.findElement({project_id:project._id, element_id:subSubDoc._id, type:"doc"}, function(err, foundElement, path, parentFolder){
				assert((err == null));
		...
		return it('should give error if element could not be found', function(done){
			return this.locator.findElement({project_id:project._id, element_id:"ddsd432nj42", type:"docs"}, function(err, foundElement, path, parentFolder){
	...
	describe('finding a folder', function() {
		it('should return root folder when looking for root folder', function(done){
			return this.locator.findElement({project_id:project._id, element_id:rootFolder._id, type:"folder"}, function(err, foundElement, path, parentFolder){
				assert(!err);
		...
		it('when at root', function(done){
			return this.locator.findElement({project_id:project._id, element_id:subFolder._id, type:"folder"}, function(err, foundElement, path, parentFolder){
				assert(!err);
		...
		return it('when deeply nested', function(done){
			return this.locator.findElement({project_id:project._id, element_id:secondSubFolder._id, type:"folder"}, function(err, foundElement, path, parentFolder){
				assert(!err);
	...
	describe('finding a file', function() {
		it('when at root', function(done){
			return this.locator.findElement({project_id:project._id, element_id:file1._id, type:"fileRefs"}, function(err, foundElement, path, parentFolder){
				assert(!err);
		...
		return it('when deeply nested', function(done){
			return this.locator.findElement({project_id:project._id, element_id:subSubFile._id, type:"fileRefs"}, function(err, foundElement, path, parentFolder){
				assert(!err);
	...
	describe('should be able to take actual project as well as id', function() {
		...
		return it('should find doc in project', function(done){
			return this.locator.findElement({project:project2, element_id:doc3._id, type:"docs"}, function(err, foundElement, path, parentFolder){
				assert((err == null));
	...
	describe('findElementByPath', function() {
		...
		it('should take a doc path and return the element for a root level document', function(done){
			...
			return this.locator.findElementByPath({project, path}, function(err, element, type){
		...
		describe("where duplicate folder exists", function() {
			beforeEach(function() {
				this.duplicateFolder = {name:"duplicate1", _id:"1234", folders:[{
					name: "1",
					docs:[{name:"main.tex", _id:"456"}],
					folders: [],
					fileRefs: []
				}], docs:[(this.doc = {name:"main.tex", _id:"456"})], fileRefs:[]};
				return this.project = {
					rootFolder:[{
		...
		describe("with a null doc", function() {
			...
			return it("should not crash with a null", function(done){
				...
				return this.locator.findElementByPath({project: this.project, path}, function(err, element){
		...
		describe("with a null project", function() {
			...
			return it("should not crash with a null", function(done){
				const path = "/other.tex";
				return this.locator.findElementByPath({project_id: project._id, path}, function(err, element){
					expect(err).to.exist;
					return done();
				});
			});
		...
		return describe("with a project_id", () => it('should take a doc path and return the element for a root level document', function(done){
            const path = `${doc1.name}`;
            return this.locator.findElementByPath({project_id: project._id, path}, (err, element, type)=> {
	...
	return describe('findUsersProjectByName finding a project by user_id and project name', function(){
		it('should return the project from an array case insenstive', function(done){
			...
			this.ProjectGetter.findAllUsersProjects = sinon.stub().callsArgWith(2, null, projects);
			return this.locator.findUsersProjectByName(user_id, stubbedProject.name.toLowerCase(), function(err, project){
