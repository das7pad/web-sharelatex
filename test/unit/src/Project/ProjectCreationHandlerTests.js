...
const modulePath = "../../../../app/js/Features/Project/ProjectCreationHandler.js";
...
describe('ProjectCreationHandler', function() {
	...
	describe('Creating a Blank project', function() {
		beforeEach(function() {
			...
			this.HistoryManager.initializeProject = sinon.stub().callsArgWith(0, null, { overleaf_id: this.overleaf_id });
		...
		describe("successfully", function() {
			...
			it("should return the project in the callback", function(done){
				return this.handler.createBlankProject(ownerId, projectName, function(err, project){
			...
			it("should initialize the project overleaf if history id not provided", function(done){
				...
				return this.HistoryManager.initializeProject.calledWith().should.equal(true);
			});

			it("should set the overleaf id if overleaf id not provided", function(done){
				return this.handler.createBlankProject(ownerId, projectName, (err, project)=> {
			...
			it("should set the overleaf id if overleaf id provided", function(done){
				...
				return this.handler.createBlankProject(ownerId, projectName, attributes, function(err, project){
			...
			it("should set the imageName to the attribute value if set and not overwrite it with the currentImageName", function(done) {
				...
				return this.handler.createBlankProject(ownerId, projectName, attributes, (err, project)=> {
			...
			it("should send a project-created event to analytics", function(done) {
				return this.handler.createBlankProject(ownerId, projectName, (err, project) => {
					...
					expect(
						this.AnalyticsManager.recordEvent.calledWith(ownerId, 'project-created')
					).to.equal(true);
					return done();
				});
			});
			...
			return it("should send a project-imported event when importing a project", function(done) {
				return this.handler.createBlankProject(ownerId, projectName, 1234, (err, project) => {
					expect(this.AnalyticsManager.recordEvent.callCount).to.equal(1);
					expect(
						this.AnalyticsManager.recordEvent.calledWith(ownerId, 'project-imported')
					).to.equal(true);
					return done();
				});
			});
		...
		describe("with an error", function() {
			beforeEach(function() {
				this.ProjectModel.prototype.save = sinon.stub().callsArgWith(0, new Error("something went wrong"));
				return this.handler.createBlankProject(ownerId, projectName, this.callback);
			});
		...
		return describe("with an invalid name", function() {
			beforeEach(function() {
				this.ProjectDetailsHandler.validateProjectName = sinon.stub().yields(new Error("bad name"));
				return this.handler.createBlankProject(ownerId, projectName, this.callback);
			});
	...
	describe('Creating a basic project', function() {
		beforeEach(function() {
			...
			this.handler._buildTemplate = function(template_name, user, project_name, callback) {
			...
			this.handler.createBlankProject = sinon.stub().callsArgWith(2, null, this.project);
			this.handler._createRootDoc = sinon.stub().callsArgWith(3, null, this.project);
			return this.handler.createBasicProject(ownerId, projectName, this.callback);
		});

		it("should create a blank project first", function() {
			return this.handler.createBlankProject.calledWith(ownerId, projectName)
				.should.equal(true);
		});
	...
	describe('Creating a project from a snippet', function() {
		beforeEach(function() {
			...
			this.handler.createBlankProject = sinon.stub().callsArgWith(2, null, this.project);
			this.handler._createRootDoc = sinon.stub().callsArgWith(3, null, this.project);
			return this.handler.createProjectFromSnippet(ownerId, projectName, ["snippet line 1", "snippet line 2"], this.callback);
		});
	...
	describe('Creating an example project', function() {
		beforeEach(function() {
			...
			this.handler.createBlankProject = sinon.stub().callsArgWith(2, null, this.project);
			this.handler._createRootDoc = sinon.stub().callsArgWith(3, null, this.project);
			return this.handler.createExampleProject(ownerId, projectName, this.callback);
		});
	...
	describe("_buildTemplate", function() {
		...
		beforeEach(function(done){
			return this.handler._buildTemplate("main.tex", this.user_id, projectName, (err, templateLines)=> {
				this.template = templateLines.reduce((singleLine, line) => `${singleLine}\n${line}`);
				return done();
			});
		});
	...
	return describe("_createRootDoc", function() {
		beforeEach(function(done){
			...
			return this.handler._createRootDoc(this.project, ownerId, ["line 1", "line 2"], done);
		});
		...
		return it('should set the main doc id', function() {
			return this.ProjectEntityUpdateHandler.setRootDoc.calledWith(project_id, docId).should.equal(true);
		});
	});
});