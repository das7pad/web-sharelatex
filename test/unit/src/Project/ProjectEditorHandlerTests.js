...
describe("ProjectEditorHandler", function() {
	...
	describe("buildProjectModelView", function() {
		describe("with owner and members included", function() {
			beforeEach(function() {
				return this.result = this.handler.buildProjectModelView(this.project, this.members, this.invites);
			});
			...
			it("should gather readOnly_refs and collaberators_refs into a list of members", function() {
				...
				return findMember("read-write-id").email.should.equal("read-write@sharelatex.com");
			});
			...
			it("should include files in the project", function() {
				this.result.rootFolder[0].folders[0].fileRefs[0]._id.should.equal("file-id");
				this.result.rootFolder[0].folders[0].fileRefs[0].name.should.equal("image.png");
				this.result.rootFolder[0].folders[0].fileRefs[0].created.should.equal(this.created);
				return should.not.exist(this.result.rootFolder[0].folders[0].fileRefs[0].size);
			});
			...
			it("should include docs in the project but not the lines", function() {
				...
				this.result.rootFolder[0].folders[0].docs[0].name.should.equal("main.tex");
				return should.not.exist(this.result.rootFolder[0].folders[0].docs[0].lines);
			});
		...
		describe("deletedByExternalDataSource", function() {
			...
			it("should set the deletedByExternalDataSource flag to false when it is not there", function() {
				...
				const result = this.handler.buildProjectModelView(this.project, this.members, [], []);
		...
		return describe("features", function() {
			beforeEach(function() {
				...
				return this.result = this.handler.buildProjectModelView(this.project, this.members, [], []);});

			return it("should copy the owner features to the project", function() {
				this.result.features.versioning.should.equal(this.owner.features.versioning);
				this.result.features.collaborators.should.equal(this.owner.features.collaborators);
				this.result.features.compileGroup.should.equal(this.owner.features.compileGroup);
				return this.result.features.compileTimeout.should.equal(this.owner.features.compileTimeout);
			});
		});
	});
	...
	return describe('buildOwnerAndMembersViews', function() {
		beforeEach(function() {
			...
			return this.result = this.handler.buildOwnerAndMembersViews(this.members);
		});

		it('should produce an object with the right keys', function() {
			return expect(this.result).to.have.all.keys(['owner', 'ownerFeatures', 'members']);
	});
		...
		it('should separate the owner from the members', function() {
			...
			return expect(this.result.members.filter(m => m._id === this.owner._id).length).to.equal(0);
		});

		it('should extract the ownerFeatures from the owner object', function() {
			return expect(this.result.ownerFeatures).to.deep.equal(this.owner.features);
		});

		return describe('when there is no owner', function() {
			beforeEach(function() {
				// remove the owner from members list
				this.membersWithoutOwner = this.members.filter(m => m.user._id !== this.owner._id);
				return this.result = this.handler.buildOwnerAndMembersViews(this.membersWithoutOwner);
			});
