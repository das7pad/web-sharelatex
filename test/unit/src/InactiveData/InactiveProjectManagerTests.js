...
const modulePath = path.join(__dirname, "../../../../app/js/Features/InactiveData/InactiveProjectManager");
const {
    expect
} = require("chai");
...
describe("InactiveProjectManager", function() {
	...
	beforeEach(function() {
		...
		this.TrackChangesManager =
			{archiveProject:sinon.stub()};
		this.InactiveProjectManager = SandboxedModule.require(modulePath, { requires: {
	...
	describe("reactivateProjectIfRequired", function() {
		...
		it("should call unarchiveProject", function(done){
			...
			return this.InactiveProjectManager.reactivateProjectIfRequired(this.project_id, err=> {
				this.DocstoreManager.unarchiveProject.calledWith(this.project_id).should.equal(true);
				this.ProjectUpdateHandler.markAsActive.calledWith(this.project_id).should.equal(true);
				return done();
			});
		});
		...
		it("should not mark project as active if error with unarchinging", function(done){
			...
			return this.InactiveProjectManager.reactivateProjectIfRequired(this.project_id, err=> {
				...
				this.DocstoreManager.unarchiveProject.calledWith(this.project_id).should.equal(true);
				this.ProjectUpdateHandler.markAsActive.calledWith(this.project_id).should.equal(false);
				return done();
			});
		});
		...
		return it("should not call unarchiveProject if it is active", function(done){
			...
			this.DocstoreManager.unarchiveProject.callsArgWith(1);
			return this.InactiveProjectManager.reactivateProjectIfRequired(this.project_id, err=> {
				this.DocstoreManager.unarchiveProject.calledWith(this.project_id).should.equal(false);
				this.ProjectUpdateHandler.markAsActive.calledWith(this.project_id).should.equal(false);
				return done();
			});
		});
	...
	return describe("deactivateProject", function() {
		...
		it("should call unarchiveProject and markAsInactive", function(done){
			...
			return this.InactiveProjectManager.deactivateProject(this.project_id, err=> {
				this.DocstoreManager.archiveProject.calledWith(this.project_id).should.equal(true);
				// @TrackChangesManager.archiveProject.calledWith(@project_id).should.equal true
				this.ProjectUpdateHandler.markAsInactive.calledWith(this.project_id).should.equal(true);
				return done();
			});
		});
		...
		return it("should not call markAsInactive if there was a problem archiving in docstore", function(done){
			...
			return this.InactiveProjectManager.deactivateProject(this.project_id, err=> {
				...
				this.DocstoreManager.archiveProject.calledWith(this.project_id).should.equal(true);
				this.ProjectUpdateHandler.markAsInactive.calledWith(this.project_id).should.equal(false);
				return done();
			});
		});
	});
