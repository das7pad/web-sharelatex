...
describe('SudoModeController', function() {
	beforeEach(function() {
		...
		this.UserGetter =
			{getUser: sinon.stub()};
		return this.SudoModeController = SandboxedModule.require(modulePath, { requires: {
			'logger-sharelatex': {log: sinon.stub(), err: sinon.stub()},
			'./SudoModeHandler': this.SudoModeHandler,
			'../Authentication/AuthenticationController': this.AuthenticationController,
			'../../infrastructure/Mongoose': {mongo: {ObjectId() { return 'some_object_id'; }}},
	...
	describe('sudoModePrompt', function() {
		beforeEach(function() {
			this.SudoModeHandler.isSudoModeActive = sinon.stub().callsArgWith(1, null, false);
		...
		it('should get the logged in user id', function() {
			...
			return this.AuthenticationController.getLoggedInUserId.calledWith(this.req).should.equal(true);
		});

		it('should check if sudo-mode is active', function() {
			this.SudoModeController.sudoModePrompt(this.req, this.res, this.next);
			this.SudoModeHandler.isSudoModeActive.callCount.should.equal(1);
			return this.SudoModeHandler.isSudoModeActive.calledWith(this.user._id).should.equal(true);
		});

		it('should redirect when sudo-mode is active', function() {
			this.SudoModeHandler.isSudoModeActive = sinon.stub().callsArgWith(1, null, true);
			this.SudoModeController.sudoModePrompt(this.req, this.res, this.next);
		...
		it('should render the sudo_mode_prompt page when sudo mode is not active', function() {
			...
			return this.res.render.calledWith('sudo_mode/sudo_mode_prompt').should.equal(true);
		});

		describe('when isSudoModeActive produces an error', function() {
			beforeEach(function() {
				this.SudoModeHandler.isSudoModeActive = sinon.stub().callsArgWith(1, new Error('woops'));
				return this.next = sinon.stub();
			});
	...
	return describe('submitPassword', function() {
		beforeEach(function() {
			...
			this.SudoModeHandler.authenticate = sinon.stub().callsArgWith(2, null, this.user);
		...
		return describe('when all goes well', function() {
			...
			it('should get redirect from session', function() {
				...
				return this.AuthenticationController._getRedirectFromSession.calledWith(this.req).should.equal(true);
			});

			it('should get the user from storage', function() {
				this.SudoModeController.submitPassword(this.req, this.res, this.next);
				this.UserGetter.getUser.callCount.should.equal(1);
				return this.UserGetter.getUser.calledWith('some_object_id', {email: 1}).should.equal(true);
			});
			...
			it('should try to authenticate the user with the password', function() {
				...
				return this.SudoModeHandler.authenticate.calledWith(this.user.email, this.password).should.equal(true);
			});

			it('should activate sudo mode', function() {
				this.SudoModeController.submitPassword(this.req, this.res, this.next);
				this.SudoModeHandler.activateSudoMode.callCount.should.equal(1);
				return this.SudoModeHandler.activateSudoMode.calledWith(this.user._id).should.equal(true);
			});

			it('should send back a json response', function() {
				this.SudoModeController.submitPassword(this.req, this.res, this.next);
				this.res.json.callCount.should.equal(1);
				return this.res.json.calledWith({redir: '/somewhere'}).should.equal(true);
			});
			...
			describe('when getUser produces an error', function() {
				beforeEach(function() {
					this.UserGetter.getUser = sinon.stub().callsArgWith(2, new Error('woops'));
					return this.next = sinon.stub();
				});
			...
			describe('when authentication fails', function() {
				beforeEach(function() {
					this.SudoModeHandler.authenticate = sinon.stub().callsArgWith(2, null, null);
					this.res.json = sinon.stub();
				...
				it('should send back a failure message', function() {
					...
					expect(this.res.json.lastCall.args[0].message).to.have.keys(['text', 'type']);
			...
			describe('when authentication produces an error', function() {
				beforeEach(function() {
					this.SudoModeHandler.authenticate = sinon.stub().callsArgWith(2, new Error('woops'));
					return this.next = sinon.stub();
				});
			...
			return describe('when sudo mode activation produces an error', function() {
				beforeEach(function() {
					this.SudoModeHandler.activateSudoMode = sinon.stub().callsArgWith(1, new Error('woops'));
					return this.next = sinon.stub();
				});
