...
const modulePath = path.join(__dirname, '../../../../../app/js/infrastructure/LockManager.js');
...
describe('LockManager - getting the lock', function() {
	...
	return describe("when there are multiple requests for the same lock", function() {
		beforeEach(function(done) {
			...
			this.LockManager._tryLock = function(key, namespace, callback) {
				if (callback == null) { callback = function(error, gotLock, lockValue) {}; }
				if (locked) {
					return callback(null, false);
				} else {
			...
			return Array.from(randomDelays).map((randomDelay, i) =>
				((randomDelay, i) => {
					...
					return setTimeout(() => {
						...
						return this.LockManager._getLock(this.key, this.namespace, (...args) => {
							setTimeout(() => locked = false  // release the lock after a random amount of time
							, randomDelay);
							this.results.push(i);
