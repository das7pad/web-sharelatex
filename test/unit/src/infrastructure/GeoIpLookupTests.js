...
const modulePath = path.join(__dirname, "../../../../app/js/infrastructure/GeoIpLookup");
const {
    expect
} = require("chai");
...
describe("GeoIpLookup", function() {
	...
	beforeEach(function() {
		...
		this.request = 
			{get: sinon.stub()};
		this.GeoIpLookup = SandboxedModule.require(modulePath, { requires: {
	...
	describe("getDetails", function() {
		...
		it("should request the details using the ip", function(done){
			return this.GeoIpLookup.getDetails(this.ipAddress, err=> {
				this.request.get.calledWith({url:this.settings.apis.geoIpLookup.url+"/"+this.ipAddress, timeout:1000, json:true}).should.equal(true);
				return done();
			});
		});

		it("should return the ip details", function(done){
			return this.GeoIpLookup.getDetails(this.ipAddress, (err, returnedDetails)=> {
		...
		return it("should take the first ip in the string", function(done){
			return this.GeoIpLookup.getDetails(` ${this.ipAddress} 456.312.452.102 432.433.888.234`, err=> {
				this.request.get.calledWith({url:this.settings.apis.geoIpLookup.url+"/"+this.ipAddress, timeout:1000, json:true}).should.equal(true);
				return done();
			});
		});
	...
	return describe("getCurrencyCode", function() {
		...
		it("should return GBP for GB country", function(done){
			this.GeoIpLookup.getDetails = sinon.stub().callsArgWith(1, null, this.stubbedResponse);
			return this.GeoIpLookup.getCurrencyCode(this.ipAddress, function(err, currencyCode){
