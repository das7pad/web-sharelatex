...
const modulePath = '../../../../app/js/Features/Subscription/SubscriptionController';
...
describe("SubscriptionController", function() {
	beforeEach(function() {
		this.user = {email:"tom@yahoo.com", _id: 'one', signUpDate: new Date('2000-10-01')};
		this.activeRecurlySubscription = mockSubscriptions["subscription-123-active"];
		...
		this.SubscriptionController = SandboxedModule.require(modulePath, { requires: {
	...
	describe("plansPage", function() {
		beforeEach(function() {
			...
			return this.GeoIpLookup.getCurrencyCode.callsArgWith(1, null, this.stubbedCurrencyCode);
		});
		...
		describe('when user is logged in', function(done) {
			...
			return describe('not dependant on logged in state', function(done) {
				...
				it("should set the recommended currency from the geoiplookup", function(done){
					this.res.renderedVariables.recomendedCurrency.should.equal(this.stubbedCurrencyCode);
					this.GeoIpLookup.getCurrencyCode.calledWith(this.req.ip).should.equal(true);
					return done();
				});
	...
	describe("paymentPage", function() {
		beforeEach(function() {
			this.req.headers = {};
			this.SubscriptionHandler.validateNoSubscriptionInRecurly = sinon.stub().yields(null, true);
			return this.GeoIpLookup.getCurrencyCode.callsArgWith(1, null, this.stubbedCurrencyCode);
		});

		describe("with a user without a subscription", function() {
			beforeEach(function() {
				this.LimitationsManager.userHasV1OrV2Subscription.callsArgWith(1, null, false);
				return this.PlansLocator.findLocalPlanInSettings.returns({});
			});

			return describe("with a valid plan code", () => it("should render the new subscription page", function(done){
		...
		describe("with a user with subscription", () => it("should redirect to the subscription dashboard", function(done){
            this.LimitationsManager.userHasV1OrV2Subscription.callsArgWith(1, null, true);
		...
		describe("with an invalid plan code", () => it("should redirect to the subscription dashboard", function(done){
            this.LimitationsManager.userHasV1OrV2Subscription.callsArgWith(1, null, false);
		...
		return describe("with a recurly subscription already", () => it("should redirect to the subscription dashboard", function(done){
            this.LimitationsManager.userHasV1OrV2Subscription.callsArgWith(1, null, false);
            this.SubscriptionHandler.validateNoSubscriptionInRecurly = sinon.stub().yields(null, false);
            this.res.redirect = url=> {
                url.should.equal("/user/subscription?hasSubscription=true");
                return done();
            };
	...
	describe("successful_subscription", () => beforeEach(function(done) {
        this.SubscriptionViewModelBuilder.buildUsersSubscriptionViewModel.callsArgWith(1, null, {});
        this.res.callback = done;
        return this.SubscriptionController.successful_subscription(this.req, this.res);
    }));

	describe("userSubscriptionPage", function() {
		beforeEach(function(done) {
			this.SubscriptionViewModelBuilder.buildUsersSubscriptionViewModel.callsArgWith(1, null, {
				personalSubscription: (this.personalSubscription = { 'personal-subscription': 'mock' }),
				memberGroupSubscriptions: (this.memberGroupSubscriptions = { 'group-subscriptions': 'mock' })
			});
			this.SubscriptionViewModelBuilder.buildViewModel.returns(this.plans = {'plans': 'mock'});
			this.LimitationsManager.userHasV1OrV2Subscription.callsArgWith(1, null, false);
			...
			return this.SubscriptionController.userSubscriptionPage(this.req, this.res);
		});

		it("should load the personal, groups and v1 subscriptions", function() {
			expect(this.data.personalSubscription).to.deep.equal(this.personalSubscription);
			return expect(this.data.memberGroupSubscriptions).to.deep.equal(this.memberGroupSubscriptions);
		});
	...
	describe("createSubscription", function() {
		...
		it("should send the user and subscriptionId to the handler", function(done){
			this.SubscriptionHandler.createSubscription.calledWith(this.user, this.subscriptionDetails, this.req.body.recurly_token_id).should.equal(true);
			return done();
		});
	...
	describe("updateSubscription via post", function() {
		...
		it("should send the user and subscriptionId to the handler", function(done){
			this.SubscriptionHandler.updateSubscription.calledWith(this.user, this.plan_code).should.equal(true);
			return done();
		});
	...
	describe("reactivateSubscription", function() {
		beforeEach(function(done){
			...
			sinon.spy(this.res, "redirect");
			return this.SubscriptionController.reactivateSubscription(this.req, this.res);
		});

		it("should tell the handler to reactivate this user", function(done){
			this.SubscriptionHandler.reactivateSubscription.calledWith(this.user).should.equal(true);
			return done();
		});
	...
	describe("cancelSubscription", function() {
		...
		return it("should redurect to the subscription page", function(done){
			this.res.redirect.calledWith("/user/subscription/canceled").should.equal(true);
			return done();
		});
	});
	...
	describe("recurly callback", function() {
		...
		return describe("with a non-actionable request", function() {
			...
			it("should not call the subscriptionshandler", function() {
				return this.SubscriptionHandler.recurlyCallback.called.should.equal(false);
			});
	...
	describe("renderUpgradeToAnnualPlanPage", function() {
		...
		it("should redirect to the plans page if the user does not have a subscription", function(done){
			...
			return this.SubscriptionController.renderUpgradeToAnnualPlanPage(this.req, this.res);
		});


		it("should pass the plan code to the view - student", function(done){

			this.LimitationsManager.userHasV2Subscription.callsArgWith(1, null, true, {planCode:"Student free trial 14 days"});
		...
		it("should pass the plan code to the view - collaborator", function(done){
			...
			this.LimitationsManager.userHasV2Subscription.callsArgWith(1, null, true, {planCode:"free trial for Collaborator free trial 14 days"});
			this.res.render = function(view, opts){
		...
		return it("should pass annual as the plan name if the user is already on an annual plan", function(done){
			...
			this.LimitationsManager.userHasV2Subscription.callsArgWith(1, null, true, {planCode:"student annual with free trial"});
			this.res.render = function(view, opts){
	...
	return describe("processUpgradeToAnnualPlan", function() {
		...
		it("should tell the subscription handler to update the subscription with the annual plan and apply a coupon code", function(done){
			...
			this.res.sendStatus = ()=> {
				this.SubscriptionHandler.updateSubscription.calledWith(this.user, "student-annual", "STUDENTCODEHERE").should.equal(true);
				return done();
			};

			return this.SubscriptionController.processUpgradeToAnnualPlan(this.req, this.res);
		});
		...
		return it("should get the collaborator coupon code", function(done){
			...
			this.res.sendStatus = url=> {
				this.SubscriptionHandler.updateSubscription.calledWith(this.user, "collaborator-annual", "COLLABORATORCODEHERE").should.equal(true);
				return done();
			};
