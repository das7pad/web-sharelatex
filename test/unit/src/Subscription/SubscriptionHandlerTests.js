...
const modulePath = "../../../../app/js/Features/Subscription/SubscriptionHandler";
...
describe("SubscriptionHandler", function() {
	...
	beforeEach(function() {
		...
		this.activeRecurlySubscription = mockRecurlySubscriptions["subscription-123-active"];
		this.User = {};
		this.user =
			{_id: (this.user_id = "user_id_here_")};
		this.subscription = 
			{recurlySubscription_id: this.activeRecurlySubscription.uuid};
		this.RecurlyWrapper = { 
			getSubscription: sinon.stub().callsArgWith(2, null, this.activeRecurlySubscription),
			updateSubscription: sinon.stub().callsArgWith(2, null, this.activeRecurlySubscription),
			cancelSubscription: sinon.stub().callsArgWith(1),
			reactivateSubscription: sinon.stub().callsArgWith(1),
			redeemCoupon:sinon.stub().callsArgWith(2),
			createSubscription: sinon.stub().callsArgWith(3, null, this.activeRecurlySubscription)
		};
		...
		this.SubscriptionHandler = SandboxedModule.require(modulePath, { requires: {
		...
		return this.SubscriptionHandler.syncSubscriptionToUser = sinon.stub().callsArgWith(2);
	});
	...
	describe("createSubscription", function() {
		beforeEach(function() {
			...
			return this.SubscriptionHandler.validateNoSubscriptionInRecurly = sinon.stub().yields(null, true);
		});

		describe("successfully", function() {
			beforeEach(function() {
				return this.SubscriptionHandler.createSubscription(this.user, this.subscriptionDetails, this.recurly_token_id, this.callback);
			});

			it("should create the subscription with the wrapper", function() {
				return this.RecurlyWrapper.createSubscription.calledWith(this.user, this.subscriptionDetails, this.recurly_token_id).should.equal(true);
			});
			...
			return it("should sync the subscription to the user", function() {
				...
				this.SubscriptionUpdater.syncSubscription.args[0][0].should.deep.equal(this.activeRecurlySubscription);
				return this.SubscriptionUpdater.syncSubscription.args[0][1].should.deep.equal(this.user._id);
			});
		});
		...
		return describe("when there is already a subscription in Recurly", function() {
			...
			return it("should return an error", function() {
				return this.callback.calledWith(new Error("user already has subscription in recurly"));
			});
		});
	});

	describe("updateSubscription", function() {
		describe("with a user with a subscription", () => describe("with a valid plan code", function() {
            beforeEach(function(done) {
                this.plan_code = "collaborator";
                this.LimitationsManager.userHasV2Subscription.callsArgWith(1, null, true, this.subscription);
                return this.SubscriptionHandler.updateSubscription(this.user, this.plan_code, null, done);
            });

            it("should update the subscription", function() {
                this.RecurlyWrapper.updateSubscription.calledWith(this.subscription.recurlySubscription_id).should.equal(true);
		...
		describe("with a user without a subscription", function() {
			...
			return it("should redirect to the subscription dashboard", function() {
				...
				return this.SubscriptionHandler.syncSubscriptionToUser.called.should.equal(false);
			});
		});
		...
		return describe("with a coupon code", function() {
			beforeEach(function(done) {
				...
				this.LimitationsManager.userHasV2Subscription.callsArgWith(1, null, true, this.subscription);
				return this.SubscriptionHandler.updateSubscription(this.user, this.plan_code, this.coupon_code, done);
			});

			it("should get the users account", function() {
				return this.RecurlyWrapper.getSubscription.calledWith(this.activeRecurlySubscription.uuid).should.equal(true);
			});

			it("should redeme the coupon", function(done){
				this.RecurlyWrapper.redeemCoupon.calledWith(this.activeRecurlySubscription.account.account_code, this.coupon_code).should.equal(true);
				return done();
			});
	...
	describe("cancelSubscription", function() {
		describe("with a user without a subscription", function() {
			beforeEach(function(done) {
				this.LimitationsManager.userHasV2Subscription.callsArgWith(1, null, false, this.subscription);
		...
		return describe("with a user with a subscription", function() {
			...
			it("should cancel the subscription", function() {
				...
				return this.RecurlyWrapper.cancelSubscription.calledWith(this.subscription.recurlySubscription_id).should.equal(true);
			});

			return it("should trigger the cancel subscription event", function() {
				return this.Events.emit.calledWith("cancelSubscription", this.user._id).should.equal(true);
			});
		});
	});
	...
	describe("reactiveRecurlySubscription", function() {
		describe("with a user without a subscription", function() {
			...
			it("should redirect to the subscription dashboard", function() {
				return this.RecurlyWrapper.reactivateSubscription.called.should.equal(false);
			});
		...
		return describe("with a user with a subscription", function() {
			...
			it("should reactivate the subscription", function() {
				...
				return this.RecurlyWrapper.reactivateSubscription.calledWith(this.subscription.recurlySubscription_id).should.equal(true);
			});

			return it("should send a notification email", function() {
				return sinon.assert.calledWith(this.EmailHandler.sendEmail, 'reactivatedSubscription');
			});
		});
	});

	describe("recurlyCallback", () => describe("with an actionable request", function() {
        beforeEach(function(done) {
            ...
            return this.SubscriptionHandler.recurlyCallback(this.activeRecurlySubscription, done);
        });
	...
	return describe("validateNoSubscriptionInRecurly", function() {
		beforeEach(function() {
			this.subscriptions = [];
			this.RecurlyWrapper.listAccountActiveSubscriptions = sinon.stub().yields(null, this.subscriptions);
		...
		describe("with no subscription in recurly", function() {
			beforeEach(function() {
				...
				return this.SubscriptionHandler.validateNoSubscriptionInRecurly(this.user_id, this.callback);
			});
