...
describe("TeamInvitesHandler", function() {
	beforeEach(function() {
		...
		this.UserGetter.getUserByAnyEmail.withArgs(this.manager.email).yields(null, this.manager);

		this.SubscriptionLocator.getUsersSubscription.yields(null, this.subscription);
		this.Subscription.findOne.yields(null, this.subscription);

		return this.TeamInvitesHandler = SandboxedModule.require(modulePath, { requires: {
	...
	describe("getInvite", function() {
		it("returns the invite if there's one", function(done) {
			return this.TeamInvitesHandler.getInvite(this.token, (err, invite, subscription) => {
		...
		return it("returns teamNotFound if there's none", function(done) {
			...
			return this.TeamInvitesHandler.getInvite(this.token, function(err, invite, subscription) {
	...
	describe("createInvite", function() {
		it("adds the team invite to the subscription", function(done) {
			return this.TeamInvitesHandler.createInvite(this.manager.id, this.subscription, "John.Snow@example.com", (err, invite) => {
				expect(err).to.eq(null);
		...
		it("sends an email", function(done) {
			return this.TeamInvitesHandler.createInvite(this.manager.id, this.subscription, "John.Snow@example.com", (err, invite) => {
				this.EmailHandler.sendEmail.calledWith("verifyEmailToJoinTeam",
					sinon.match({
		...
		it("refreshes the existing invite if the email has already been invited", function(done) {
			...
			return this.TeamInvitesHandler.createInvite(this.manager.id, this.subscription, originalInvite.email, (err, invite) => {
				expect(err).to.eq(null);
	...
	describe("importInvite", function() {
		...
		return it("can imports an invite from v1", function() {
			return this.TeamInvitesHandler.importInvite(this.subscription, "A-Team", "hannibal@a-team.org",
				"secret", this.sentAt, error => {
					expect(error).not.to.exist;

					this.subscription.save.calledOnce.should.eq(true);

					const invite = this.subscription.teamInvites.find(i => i.email === "hannibal@a-team.org");
	...
	describe("acceptInvite", function() {
		beforeEach(function() {
			...
			this.UserGetter.getUserByAnyEmail.withArgs(this.user.email).yields(null, this.user);
		...
		it("adds the user to the team", function(done) {
			return this.TeamInvitesHandler.acceptInvite("dddddddd", this.user.id, () => {
				this.SubscriptionUpdater.addUserToGroup.calledWith(this.subscription._id, this.user.id).should.eq(true);
				return done();
			});
		});
	...
	describe("revokeInvite", () => it("removes the team invite from the subscription", function(done) {
        return this.TeamInvitesHandler.revokeInvite(this.manager.id, this.subscription, "jorah@example.com", () => {
            this.Subscription.update.calledWith(
                { _id: new ObjectId("55153a8014829a865bbf700d") },
	...
	describe("createTeamInvitesForLegacyInvitedEmail", function(done) {
		beforeEach(function() {
			this.subscription.invited_emails = ["eddard@example.com", "robert@example.com"];
			this.TeamInvitesHandler.createInvite = sinon.stub().yields(null);
			return this.SubscriptionLocator.getGroupsWithEmailInvite = sinon.stub().yields(null, [this.subscription]);
		});

		return it("sends an invitation email to addresses in the legacy invited_emails field", function(done) {
			return this.TeamInvitesHandler.createTeamInvitesForLegacyInvitedEmail("eddard@example.com", (err, invite) => {
				expect(err).not.to.exist;

				this.TeamInvitesHandler.createInvite.calledWith(
	...
	return describe("validation", function() {
		it("doesn't create an invite if the team limit has been reached", function(done) {
			this.LimitationsManager.teamHasReachedMemberLimit = sinon.stub().returns(true);
			return this.TeamInvitesHandler.createInvite(this.manager.id, this.subscription, "John.Snow@example.com", (err, invite) => {
		...
		return it("doesn't create an invite if the user is already part of the team", function(done) {
			...
			this.UserGetter.getUserByAnyEmail.withArgs(member.email).yields(null, member);

			return this.TeamInvitesHandler.createInvite(this.manager.id, this.subscription, "tyrion@example.com", (err, invite) => {
