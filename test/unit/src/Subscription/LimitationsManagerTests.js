...
require('chai').should();
const modulePath = require('path').join(__dirname, '../../../../app/js/Features/Subscription/LimitationsManager');
...
describe("LimitationsManager", function() {
	beforeEach(function() {
		...
		return this.LimitationsManager = SandboxedModule.require(modulePath, { requires: {
	...
	describe("allowedNumberOfCollaboratorsInProject", function() {
		describe("when the project is owned by a user without a subscription", function() {
			beforeEach(function() {
				...
				return this.LimitationsManager.allowedNumberOfCollaboratorsInProject(this.project_id, this.callback);
			});

			return it("should return the default number", function() {
				return this.callback.calledWith(null, this.Settings.defaultFeatures.collaborators).should.equal(true);
			});
		});
		...
		return describe("when the project is owned by a user with a subscription", function() {
			...
			return it("should return the number of collaborators the user is allowed", function() {
				return this.callback.calledWith(null, this.user.features.collaborators).should.equal(true);
			});
		});
	});
	...
	describe("allowedNumberOfCollaboratorsForUser", function() {
		describe("when the user has no features", function() {
			beforeEach(function() {
				...
				delete this.user.features;
				this.callback = sinon.stub();
				return this.LimitationsManager.allowedNumberOfCollaboratorsForUser(this.user_id, this.callback);
			});
	...
	describe("canAddXCollaborators", function() {
		describe("when the project has fewer collaborators than allowed", function() {
			beforeEach(function() {
				...
				this.invite_count = 0;
				this.CollaboratorsHandler.getInvitedCollaboratorCount = (project_id, callback) => callback(null, this.current_number);
				this.CollaboratorsInviteHandler.getInviteCount = (project_id, callback) => callback(null, this.invite_count);
				sinon.stub(this.LimitationsManager, "allowedNumberOfCollaboratorsInProject", (project_id, callback) => {
					return callback(null, this.allowed_number);
				});
				this.callback = sinon.stub();
				return this.LimitationsManager.canAddXCollaborators(this.project_id, 1, this.callback);
			});
		...
		describe("when the project has fewer collaborators than allowed but I want to add more than allowed", function() {
			beforeEach(function() {
				...
				this.callback = sinon.stub();
				return this.LimitationsManager.canAddXCollaborators(this.project_id, 2, this.callback);
			});
	...
	describe("userHasV2Subscription", function() {
		...
		it("should return true if the recurly token is set", function(done){
			this.SubscriptionLocator.getUsersSubscription.callsArgWith(1, null, {recurlySubscription_id : "1234"});
			return this.LimitationsManager.userHasV2Subscription(this.user, function(err, hasSubscription){
		...
		it("should return the subscription", function(done){
			...
			this.SubscriptionLocator.getUsersSubscription.callsArgWith(1, null, stubbedSubscription);
			return this.LimitationsManager.userHasV2Subscription(this.user, function(err, hasSubOrIsGroupMember, subscription){
		...
		return describe("when user has a custom account", function() {
			...
			beforeEach(function() {
				...
				return this.SubscriptionLocator.getUsersSubscription.callsArgWith(1, null, this.fakeSubscription);
			});

			it('should return true', function(done) {
				return this.LimitationsManager.userHasV2Subscription(this.user, function(err, hasSubscription, subscription){
					hasSubscription.should.equal(true);
					return done();
				});
			});
			...
			return it('should return the subscription', function(done) {
				return this.LimitationsManager.userHasV2Subscription(this.user, (err, hasSubscription, subscription)=> {
	...
	describe("userIsMemberOfGroupSubscription", function() {
		...
		it("should return false if there are no groups subcriptions", function(done){
			...
			return this.LimitationsManager.userIsMemberOfGroupSubscription(this.user, function(err, isMember){
		...
		return it("should return true if there are no groups subcriptions", function(done){
			let subscriptions;
			this.SubscriptionLocator.getMemberSubscriptions.callsArgWith(1, null, (subscriptions = ["mock-subscription"]));
			return this.LimitationsManager.userIsMemberOfGroupSubscription(this.user, function(err, isMember, retSubscriptions){
	...
	describe("hasPaidSubscription", function() {
		beforeEach(function() {
			this.LimitationsManager.userIsMemberOfGroupSubscription = sinon.stub().yields(null, false);
			this.LimitationsManager.userHasV2Subscription = sinon.stub().yields(null, false);
			return this.LimitationsManager.userHasV1Subscription = sinon.stub().yields(null, false);
		});

		it("should return true if userIsMemberOfGroupSubscription", function(done){
			this.LimitationsManager.userIsMemberOfGroupSubscription = sinon.stub().yields(null, true);
			return this.LimitationsManager.hasPaidSubscription(this.user, function(err, hasSubOrIsGroupMember){
		...
		it("should return true if userHasV2Subscription", function(done){
			this.LimitationsManager.userHasV2Subscription = sinon.stub().yields(null, true);
			return this.LimitationsManager.hasPaidSubscription(this.user, function(err, hasSubOrIsGroupMember){
				hasSubOrIsGroupMember.should.equal(true);
				return done();
			});
		...
		it("should return true if userHasV1Subscription", function(done){
			this.LimitationsManager.userHasV1Subscription= sinon.stub().yields(null, true);
			return this.LimitationsManager.hasPaidSubscription(this.user, function(err, hasSubOrIsGroupMember){
				hasSubOrIsGroupMember.should.equal(true);
				return done();
			});
		...
		return it("should have userHasSubscriptionOrIsGroupMember alias", function(done){
			return this.LimitationsManager.userHasSubscriptionOrIsGroupMember(this.user, function(err, hasSubOrIsGroupMember){
				hasSubOrIsGroupMember.should.equal(false);
				return done();
			});
		});
	...
	describe("userHasV1OrV2Subscription", function() {
		...
		it("should return true if userHasV2Subscription", function(done){
			this.LimitationsManager.userHasV2Subscription = sinon.stub().yields(null, true);
			return this.LimitationsManager.userHasV1OrV2Subscription(this.user, function(err, hasSub){
	...
	describe("hasGroupMembersLimitReached", function() {
		...
		it("should return true if the limit is hit (including members and invites)", function(done){
			this.SubscriptionLocator.getSubscription.callsArgWith(1, null, this.subscription);
			return this.LimitationsManager.hasGroupMembersLimitReached(this.subscriptionId, function(err, limitReached){
	...
	return describe('userHasV1Subscription', function() {
		it('should return true if v1 returns has_subscription = true', function(done) {
			this.V1SubscriptionManager.getSubscriptionsFromV1 = sinon.stub().yields(null, { has_subscription: true });
			return this.LimitationsManager.userHasV1Subscription(this.user, (error, result) => {
				this.V1SubscriptionManager.getSubscriptionsFromV1.calledWith(this.user_id).should.equal(true);
		...
		it('should return false if v1 returns has_subscription = false', function(done) {
			this.V1SubscriptionManager.getSubscriptionsFromV1 = sinon.stub().yields(null, { has_subscription: false });
			return this.LimitationsManager.userHasV1Subscription(this.user, (error, result) => {
				this.V1SubscriptionManager.getSubscriptionsFromV1.calledWith(this.user_id).should.equal(true);
		...
		return it('should return false if v1 returns nothing', function(done) {
			this.V1SubscriptionManager.getSubscriptionsFromV1 = sinon.stub().yields(null, null);
			return this.LimitationsManager.userHasV1Subscription(this.user, (error, result) => {
				this.V1SubscriptionManager.getSubscriptionsFromV1.calledWith(this.user_id).should.equal(true);
				result.should.equal(false);
				return done();
