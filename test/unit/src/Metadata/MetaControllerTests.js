...
const modulePath = "../../../../app/js/Features/Metadata/MetaController";
...
describe('MetaController', function() {
	beforeEach(function() {
		...
		return this.MetadataController = SandboxedModule.require(modulePath, { requires: {
	...
	describe('getMetadata', function() {
		beforeEach(function() {
			...
			this.MetaHandler.getAllMetaForProject = sinon.stub().callsArgWith(1, null, this.fakeLabels);
		...
		it('should call MetaHandler.getAllMetaForProject', function() {
			...
			return this.MetaHandler.getAllMetaForProject.calledWith(this.projectId).should.equal(true);
		});
		...
		it('should send a json response', function() {
			...
			return expect(this.res.json.lastCall.args[0]).to.have.all.keys(['projectId', 'projectMeta']);
	});

		return describe('when MetaHandler.getAllMetaForProject produces an error', function() {
			beforeEach(function() {
				this.MetaHandler.getAllMetaForProject = sinon.stub().callsArgWith(1, new Error('woops'));
				this.req = {params: {project_id: this.projectId}};
				this.res = {json: sinon.stub()};
				return this.next = sinon.stub();
			});
	...
	return describe('broadcastMetadataForDoc', function() {
		beforeEach(function() {
			this.MetaHandler.getMetaForDoc = sinon.stub().callsArgWith(2, null, this.fakeLabels);
		...
		it('should call MetaHandler.getMetaForDoc', function() {
			this.MetadataController.broadcastMetadataForDoc(this.req, this.res, this.next);
			this.MetaHandler.getMetaForDoc.callCount.should.equal(1);
			return this.MetaHandler.getMetaForDoc.calledWith(this.projectId).should.equal(true);
		});
		...
		return describe('when MetaHandler.getMetaForDoc produces an error', function() {
			beforeEach(function() {
				this.MetaHandler.getMetaForDoc = sinon.stub().callsArgWith(2, new Error('woops'));
				this.EditorRealTimeController.emitToRoom = sinon.stub();
				this.docId = 'somedoc';
				this.req = {params: {project_id: this.projectId, doc_id: this.docId}};
				this.res = {json: sinon.stub()};
