...
describe('ExportsHandler', function() {
	...
	beforeEach(function() {
		this.stubRequest = {};
		this.request = {defaults: () => { return this.stubRequest; }};
		this.ExportsHandler = SandboxedModule.require(modulePath, { requires: {
			'logger-sharelatex': {
				log() {},
				err() {}
			},
	...
	describe('exportProject', function() {
		beforeEach(function() {
			...
			this.ExportsHandler._buildExport = sinon.stub().yields(null, this.export_data);
			return this.ExportsHandler._requestExport = sinon.stub().yields(null, this.response_body);
		});

		describe("when all goes well", function() {
			beforeEach(function(done) {
				return this.ExportsHandler.exportProject(this.export_params, (error, export_data) => {
		...
		describe("when request can't be built", function() {
			beforeEach(function(done) {
				this.ExportsHandler._buildExport = sinon.stub().yields(new Error("cannot export project without root doc"));
				return this.ExportsHandler.exportProject(this.export_params, (error, export_data) => {
					this.callback(error, export_data);
					return done();
				});
			...
			return it("should return an error", function() {
				return (this.callback.args[0][0] instanceof Error)
				.should.equal(true);
			});
		});
		...
		return describe("when export request returns an error to forward to the user", function() {
			beforeEach(function(done) {
				...
				this.ExportsHandler._requestExport = sinon.stub().yields(null, {forwardResponse: this.error_json});
				return this.ExportsHandler.exportProject(this.export_params, (error, export_data) => {
					this.callback(error, export_data);
					return done();
				});
			...
			return it("should return success and the response to forward", function() {
				(this.callback.args[0][0] instanceof Error)
				.should.equal(false);
				return this.callback.calledWith(null, {forwardResponse: this.error_json});
			});
		});
	});
	...
	describe('_buildExport', function() {
		beforeEach(function(done) {
			...
			this.ProjectHistoryHandler.ensureHistoryExistsForProject = sinon.stub().yields(null);
			this.ProjectLocator.findRootDoc = sinon.stub().yields(null, [null, {fileSystem: 'main.tex'}]);
			this.ProjectRootDocManager.ensureRootDocumentIsValid = sinon.stub().callsArgWith(1, null);
			this.UserGetter.getUser = sinon.stub().yields(null, this.user);
			this.ExportsHandler._requestVersion = sinon.stub().yields(null, this.historyVersion);
			return done();
		});

		describe("when all goes well", function() {
			beforeEach(function(done) {
				return this.ExportsHandler._buildExport(this.export_params, (error, export_data) => {
					this.callback(error, export_data);
					return done();
				});
			});
			...
			it("should ensure the project has history", function() {
				return this.ProjectHistoryHandler.ensureHistoryExistsForProject.called
				.should.equal(true);
			});

			it("should request the project history version", function() {
				return this.ExportsHandler._requestVersion.called
				.should.equal(true);
			});
			...
			return it("should return export data", function() {
				...
				return this.callback.calledWith(null, expected_export_data)
				.should.equal(true);
			});
		});
		...
		describe("when project is not found", function() {
			beforeEach(function(done) {
				this.ProjectGetter.getProject = sinon.stub().yields(new Error("project not found"));
				return this.ExportsHandler._buildExport(this.export_params, (error, export_data) => {
					this.callback(error, export_data);
					return done();
				});
		...
		describe("when project has no root doc", () => describe("when a root doc can be set automatically", function() {
            beforeEach(function(done) {
                this.project.rootDoc_id = null;
                this.ProjectLocator.findRootDoc = sinon.stub().yields(null, [null, {fileSystem: 'other.tex'}]);
                return this.ExportsHandler._buildExport(this.export_params, (error, export_data) => {
                    this.callback(error, export_data);
                    return done();
                });
            ...
            it("should set a root doc", function() {
                return this.ProjectRootDocManager.ensureRootDocumentIsValid.called
                .should.equal(true);
            });
		...
		describe("when project has an invalid root doc", function() {
			...
			return describe("when no root doc can be identified", function() {
				beforeEach(function(done) {
					this.ProjectLocator.findRootDoc = sinon.stub().yields(null, [null, null]);
					return this.ExportsHandler._buildExport(this.export_params, (error, export_data) => {
						this.callback(error, export_data);
						return done();
					});
		...
		describe("when user is not found", function() {
			beforeEach(function(done) {
				this.UserGetter.getUser = sinon.stub().yields(new Error("user not found"));
				return this.ExportsHandler._buildExport(this.export_params, (error, export_data) => {
					this.callback(error, export_data);
					return done();
				});
		...
		return describe("when project history request fails", function() {
			beforeEach(function(done) {
				this.ExportsHandler._requestVersion = sinon.stub().yields(new Error("project history call failed"));
				return this.ExportsHandler._buildExport(this.export_params, (error, export_data) => {
					this.callback(error, export_data);
					return done();
				});
	...
	describe('_requestExport', function() {
		beforeEach(function(done) {
			...
			this.export_data = {iAmAnExport: true};
			this.export_id = 4096;
			this.stubPost = sinon.stub().yields(null, {statusCode: 200}, { exportId: this.export_id });
			return done();
		});
		...
		describe("when all goes well", function() {
			beforeEach(function(done) {
				...
				return this.ExportsHandler._requestExport(this.export_data, (error, export_v1_id) => {
			...
			return it('should return the v1 export id', function() {
				return this.callback.calledWith(null, this.export_id)
				.should.equal(true);
			});
		});
		...
		describe("when the request fails", function() {
			beforeEach(function(done) {
				this.stubRequest.post = sinon.stub().yields(new Error("export request failed"));
				return this.ExportsHandler._requestExport(this.export_data, (error, export_v1_id) => {
					this.callback(error, export_v1_id);
					return done();
				});
		...
		return describe("when the request returns an error response to forward", function() {
			beforeEach(function(done) {
				...
				this.stubRequest.post = sinon.stub().yields(null, {statusCode: this.error_code}, this.error_json);
				return this.ExportsHandler._requestExport(this.export_data, (error, export_v1_id) => {
					this.callback(error, export_v1_id);
					return done();
				});
	...
	describe('fetchExport', function() {
		beforeEach(function(done) {
			...
			this.stubGet = sinon.stub().yields(null, {statusCode: 200}, { body: this.body });
			return done();
		});
		...
		return describe("when all goes well", function() {
			beforeEach(function(done) {
				...
				return this.ExportsHandler.fetchExport(this.export_id, (error, body) => {
			...
			it('should issue the request', function() {
				return expect(this.stubGet.getCall(0).args[0]).to.deep.equal({
					url: this.settings.apis.v1.url + '/api/v1/sharelatex/exports/' + this.export_id,
					auth: {
						user: this.settings.apis.v1.user,
						pass: this.settings.apis.v1.pass
					}
			...
			return it('should return the v1 export id', function() {
				return this.callback.calledWith(null, { body: this.body })
				.should.equal(true);
			});
		});
	});
	...
	return describe('fetchDownload', function() {
		beforeEach(function(done) {
			...
			this.export_id = 897;
			this.body = "https://writelatex-conversions-dev.s3.amazonaws.com/exports/ieee_latexqc/tnb/2912/xggmprcrpfwbsnqzqqmvktddnrbqkqkr.zip?X-Amz-Expires=14400&X-Amz-Date=20180730T181003Z&X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=AKIAJDGDIJFGLNVGZH6A/20180730/us-east-1/s3/aws4_request&X-Amz-SignedHeaders=host&X-Amz-Signature=dec990336913cef9933f0e269afe99722d7ab2830ebf2c618a75673ee7159fee";
			this.stubGet = sinon.stub().yields(null, {statusCode: 200}, { body: this.body });
			return done();
		});
		...
		return describe("when all goes well", function() {
			beforeEach(function(done) {
				this.stubRequest.get = this.stubGet;
				return this.ExportsHandler.fetchDownload(this.export_id, 'zip', (error, body) => {
					this.callback(error, body);
					return done();
				});
			});
			...
			it('should issue the request', function() {
				return expect(this.stubGet.getCall(0).args[0]).to.deep.equal({
					url: this.settings.apis.v1.url + '/api/v1/sharelatex/exports/' + this.export_id + '/zip_url',
					auth: {
						user: this.settings.apis.v1.user,
						pass: this.settings.apis.v1.pass
					}
