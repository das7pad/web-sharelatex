...
const modulePath = require('path').join(__dirname, '../../../../app/js/Features/Exports/ExportsController.js');
...
describe('ExportsController', function() {
	...
	describe("without gallery fields",() => it('should ask the handler to perform the export', function(done) {
        this.handler.exportProject = sinon.stub().yields(null, {iAmAnExport: true, v1_id: 897});
        const expected = {
            project_id,
            user_id,
            brand_variation_id,
        ...
        return this.controller.exportProject(this.req, { json:body => {
	...
	describe("with an error return from v1 to forward to the publish modal",() => it('should forward the response onward', function(done) {
        this.error_json = { status: 422, message: 'nope' };
        this.handler.exportProject = sinon.stub().yields({forwardResponse: this.error_json});
...
\"token\":\"token\" \
		...
		return this.controller.exportStatus(this.req, { json:body => {
