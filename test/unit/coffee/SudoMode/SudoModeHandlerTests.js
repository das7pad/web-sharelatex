...
const modulePath = require('path').join(__dirname, '../../../../app/js/Features/SudoMode/SudoModeHandler');
...
describe('SudoModeHandler', function() {
	beforeEach(function() {
		...
		this.RedisWrapper =
			{client: () => this.rclient};
		return this.SudoModeHandler = SandboxedModule.require(modulePath, { requires: {
	...
	describe('_buildKey', () => it('should build a properly formed key', function() {
        return expect(this.SudoModeHandler._buildKey('123')).to.equal('SudoMode:{123}');
    }));
		...
		describe('when all goes well', function() {
			...
			return it('should set a value in redis', function(done) {
				return this.call(err => {
					...
					expect(this.rclient.set.calledWith(
	...
	describe('clearSudoMode', function() {
		...
		it('should delete key from redis', function(done) {
			return this.call(err => {
				...
				expect(this.rclient.del.calledWith(
	...
	describe('authenticate', function() {
		beforeEach(function() {
			return this.AuthenticationManager.authenticate = sinon.stub().callsArgWith(2, null, this.user);
		});

		return it('should call AuthenticationManager.authenticate', function(done) {
			return this.SudoModeHandler.authenticate(this.email, 'password', (err, user) => {
		...
		describe('when sudo-mode is active for that user', function() {
			...
			it('should get the value from redis', function(done) {
				return this.call((err, isActive) => {
					...
					expect(this.rclient.get.calledWith('SudoMode:{some_user_id}')).to.equal(true);
					return done();
				});
			});
