...
const modulePath = "../../../../app/js/Features/Project/ProjectRootDocManager.js";
...
describe('ProjectRootDocManager', function() {
	beforeEach(function() {
		...
		this.globby = sinon.stub().returns(new Promise(resolve => {
		...
		return this.ProjectRootDocManager = SandboxedModule.require(modulePath, { requires: {
	...
	describe("setRootDocAutomatically", function() {
		describe("when there is a suitable root doc", function() {
			beforeEach(function(done){
				...
				return this.ProjectRootDocManager.setRootDocAutomatically(this.project_id, done);
			});

			it("should check the docs of the project", function() {
				return this.ProjectEntityHandler.getAllDocs.calledWith(this.project_id)
					.should.equal(true);
			});

			return it("should set the root doc to the doc containing a documentclass", function() {
				return this.ProjectEntityUpdateHandler.setRootDoc.calledWith(this.project_id, "doc-id-2")
					.should.equal(true);
			});
		});
		...
		describe("when the root doc is an Rtex file", function() {
			beforeEach(function() {
				...
				this.ProjectEntityHandler.getAllDocs = sinon.stub().callsArgWith(1, null, this.docs);
				this.ProjectEntityUpdateHandler.setRootDoc = sinon.stub().callsArgWith(2);
				return this.ProjectRootDocManager.setRootDocAutomatically(this.project_id, this.callback);
			});
		...
		return describe("when there is no suitable root doc", function() {
			...
			return it("should not set the root doc to the doc containing a documentclass", function() {
				return this.ProjectEntityUpdateHandler.setRootDoc.called.should.equal(false);
			});
		});
	});
	...
	describe("findRootDocFileFromDirectory", function() {
		beforeEach(function() {
			this.fs.readFile.withArgs('/foo/a.tex').callsArgWith(2, null, 'Hello World!');
			this.fs.readFile.withArgs('/foo/b.tex').callsArgWith(2, null, "I'm a little teapot, get me out of here.");
			this.fs.readFile.withArgs('/foo/main.tex').callsArgWith(2, null, "Help, I'm trapped in a unit testing factory");
			this.fs.readFile.withArgs('/foo/c.tex').callsArgWith(2, null, 'Tomato, tomahto.');
			this.fs.readFile.withArgs('/foo/a/a.tex').callsArgWith(2, null, 'Potato? Potahto. Potootee!');
		...
		describe("when there is a file in a subfolder", function() {
			beforeEach(function() {
				...
				return this.globbyFiles.splice(0, this.globbyFiles.length, 'c.tex', 'a.tex', 'a/a.tex', 'b.tex');
			});

			it("processes the root folder files first, and then the subfolder, in alphabetical order", function(done) {
				return this.ProjectRootDocManager.findRootDocFileFromDirectory('/foo', (error, path) => {
		...
		describe("when main.tex contains a documentclass", function() {
			beforeEach(function() {
				return this.fs.readFile.withArgs('/foo/main.tex').callsArgWith(2, null, this.documentclassContent);
			});

			it("returns main.tex", function(done) {
				return this.ProjectRootDocManager.findRootDocFileFromDirectory('/foo', (error, path, content) => {
					expect(error).not.to.exist;
			...
			return it("processes main.text first and stops processing when it finds the content", function(done) {
				return this.ProjectRootDocManager.findRootDocFileFromDirectory('/foo', () => {
		...
		describe("when a.tex contains a documentclass", function() {
			beforeEach(function() {
				return this.fs.readFile.withArgs('/foo/a.tex').callsArgWith(2, null, this.documentclassContent);
			});
		...
		return describe("when there is an error reading a file", function() {
			beforeEach(function() {
				return this.fs.readFile.withArgs('/foo/a.tex').callsArgWith(2, new Error('something went wrong'));
			});
	...
	describe("setRootDocFromName", function() {
		describe("when there is a suitable root doc", function() {
			beforeEach(function(done){
				...
				this.ProjectEntityHandler.getAllDocPathsFromProjectById = sinon.stub().callsArgWith(1, null, this.docPaths);
				this.ProjectEntityUpdateHandler.setRootDoc = sinon.stub().callsArgWith(2);
				return this.ProjectRootDocManager.setRootDocFromName(this.project_id, '/main.tex', done);
			});

			it("should check the docs of the project", function() {
				return this.ProjectEntityHandler.getAllDocPathsFromProjectById.calledWith(this.project_id)
					.should.equal(true);
			});
		...
		describe("when there is a suitable root doc with a basename match", function() {
			beforeEach(function(done){
				this.ProjectEntityHandler.getAllDocPathsFromProjectById = sinon.stub().callsArgWith(1, null, this.docPaths);
				this.ProjectEntityUpdateHandler.setRootDoc = sinon.stub().callsArgWith(2);
				return this.ProjectRootDocManager.setRootDocFromName(this.project_id, 'chapter1a.tex', done);
			});
			...
			return it("should set the root doc using the basename", function() {
				return this.ProjectEntityUpdateHandler.setRootDoc.calledWith(this.project_id, "doc-id-3")
					.should.equal(true);
			});
		});
		...
		return describe("when there is no suitable root doc", function() {
			beforeEach(function(done){
				this.ProjectEntityHandler.getAllDocPathsFromProjectById = sinon.stub().callsArgWith(1, null, this.docPaths);
				this.ProjectEntityUpdateHandler.setRootDoc = sinon.stub().callsArgWith(2);
				return this.ProjectRootDocManager.setRootDocFromName(this.project_id, "other.tex", done);
			});
	...
	describe("ensureRootDocumentIsSet", function() {
		beforeEach(function() {
			...
			return this.ProjectRootDocManager.setRootDocAutomatically = sinon.stub().callsArgWith(1, null);
		});

		describe("when the root doc is set", function() {
			beforeEach(function() {
				this.project.rootDoc_id = "root-doc-id";
				return this.ProjectRootDocManager.ensureRootDocumentIsSet(this.project_id, this.callback);
			});
		...
		return describe("when the project does not exist", function() {
			...
			return it("should call the callback with an error", function() {
				return this.callback.calledWith(new Error("project not found")).should.equal(true);
			});
		});
	});
		...
		describe("when the root doc is set", function() {
			describe("when the root doc is valid", function() {
				beforeEach(function() {
					...
					return this.ProjectRootDocManager.ensureRootDocumentIsValid(this.project_id, this.callback);
				});
