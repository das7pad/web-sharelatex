...
const modulePath = path.join(__dirname, "../../../../app/js/Features/Project/ProjectController");
const {
    expect
} = require("chai");
...
describe("ProjectController", function() {
	...
	beforeEach(function() {
		...
		this.ProjectCreationHandler = {
			createExampleProject: sinon.stub().callsArgWith(2, null, {_id:this.project_id}),
			createBasicProject: sinon.stub().callsArgWith(2, null, {_id:this.project_id})
		};
		...
		this.BrandVariationsHandler =
			{getBrandVariationById: sinon.stub().callsArgWith(1, null, this.brandVariationDetails)};
		this.getUserAffiliations = sinon.stub().callsArgWith(1, null, []);

		this.ProjectController = SandboxedModule.require(modulePath, { requires: {
			...
			"../ReferencesSearch/ReferencesSearchHandler": this.ReferencesSearchHandler,
			...
			"../BrandVariations/BrandVariationsHandler": this.BrandVariationsHandler,
	...
	describe("updateProjectSettings", function() {
		...
		it("should update the imageName", function(done) {
			...
			this.req.body =
				{imageName: (this.imageName = "texlive-1234.5")};
			this.res.sendStatus = code => {
		...
		it("should update the spell check language", function(done) {
			...
			this.req.body =
				{spellCheckLanguage: (this.languageCode = "fr")};
			this.res.sendStatus = code => {
		...
		return it("should update the root doc", function(done) {
			...
			this.req.body =
				{rootDocId: (this.rootDocId = "root-doc-id")};
			this.res.sendStatus = code => {
	...
	describe("updateProjectAdminSettings", () => it("should update the public access level", function(done) {
        ...
        return this.ProjectController.updateProjectAdminSettings(this.req, this.res);
    }));

	describe("deleteProject", function() {
		it("should tell the project deleter to archive when forever=false", function(done){
			this.res.sendStatus = code=> {
				this.ProjectDeleter.archiveProject.calledWith(this.project_id).should.equal(true);
		...
		return it("should tell the project deleter to delete when forever=true", function(done){
			...
			this.res.sendStatus = code=> {
				this.ProjectDeleter.deleteProject.calledWith(this.project_id, {deleterUser: this.user, ipAddress:this.req.ip}).should.equal(true);
				code.should.equal(200);
				return done();
			};
			return this.ProjectController.deleteProject(this.req, this.res);
	...
	describe("restoreProject", () => it("should tell the project deleter", function(done){
        this.res.sendStatus = code=> {
            this.ProjectDeleter.restoreProject.calledWith(this.project_id).should.equal(true);
            code.should.equal(200);
            return done();
        };
	...
	describe("cloneProject", () => it("should call the project duplicator", function(done){
        this.res.send = json=> {
            this.ProjectDuplicator.duplicate.calledWith(this.user, this.project_id, this.projectName).should.equal(true);
	...
	describe("newProject", function() {
		...
		it("should call the projectCreationHandler with createExampleProject", function(done){
			...
			this.res.send = json=> {
				this.ProjectCreationHandler.createExampleProject.calledWith(this.user._id, this.projectName).should.equal(true);
				this.ProjectCreationHandler.createBasicProject.called.should.equal(false);
				return done();
			};
		...
		return it("should call the projectCreationHandler with createBasicProject", function(done){
			...
			this.res.send = json=> {
				this.ProjectCreationHandler.createExampleProject.called.should.equal(false);
				this.ProjectCreationHandler.createBasicProject.calledWith(this.user._id, this.projectName).should.equal(true);
				return done();
			};
			return this.ProjectController.newProject(this.req, this.res);
		});
	...
	describe("projectListPage", function() {
		...
		beforeEach(function() {
			this.tags = [{name:1, project_ids:["1","2","3"]}, {name:2, project_ids:["a","1"]}, {name:3, project_ids:["a", "b", "c", "d"]}];
			this.notifications = [{_id:'1',user_id:'2',templateKey:'3',messageOpts:'4',key:'5'}];
			this.projects = [
			...
			this.NotificationsHandler.getUserNotifications = sinon.stub().callsArgWith(1, null, this.notifications, {});
			this.ProjectGetter.findAllUsersProjects.callsArgWith(2, null, this.allProjects);
			return this.Modules.hooks.fire.withArgs('findAllV1Projects', this.user._id).yields(undefined);
		...
		it("should send the projects", function(done){
			this.res.render = (pageName, opts)=> {
				opts.projects.length.should.equal((this.projects.length + this.collabertions.length + this.readOnly.length + this.tokenReadAndWrite.length + this.tokenReadOnly.length));
				return done();
			};
			return this.ProjectController.projectListPage(this.req, this.res);
		});
		...
		it("should inject the users", function(done) {
			this.res.render = (pageName, opts)=> {
				opts.projects[0].owner.should.equal((this.users[this.projects[0].owner_ref]));
				opts.projects[1].owner.should.equal((this.users[this.projects[1].owner_ref]));
				opts.projects[1].lastUpdatedBy.should.equal((this.users[this.projects[1].lastUpdatedBy]));
				return done();
			};
			return this.ProjectController.projectListPage(this.req, this.res);
		});
		...
		it('should send hasSubscription == true when there is a subscription', function(done) {
			this.LimitationsManager.hasPaidSubscription = sinon.stub().callsArgWith(1, null, true);
			this.res.render = (pageName, opts)=> {
		...
		describe('front widget', function(done) {
			...
			it('should show for paid users', function(done) {
				...
				this.res.render = (pageName, opts)=> {
					opts.frontChatWidgetRoomId.should.equal(this.settings.overleaf.front_chat_widget_room_id);
					return done();
				};
				return this.ProjectController.projectListPage(this.req, this.res);
			});
		...
		return describe('with overleaf-integration-web-module hook', function() {
			beforeEach(function() {
				...
				return this.Modules.hooks.fire.withArgs('findAllV1Projects', this.user._id).yields(null, [this.V1Response]);
			...
			it('should include V1 tags', function(done) {
				this.res.render = (pageName, opts) => {
					opts.tags.length.should.equal((this.tags.length + this.V1Response.tags.length));
	...
	describe("projectListPage with duplicate projects", function() {
		...
		return it("should omit one of the projects", function(done){
			this.res.render = (pageName, opts)=> {
				opts.projects.length.should.equal((
					(this.projects.length + this.collabertions.length + this.readOnly.length + this.tokenReadAndWrite.length + this.tokenReadOnly.length) - 1
				));
				return done();
			};
			return this.ProjectController.projectListPage(this.req, this.res);
	...
	describe("renameProject", function() {
		...
		it("should call the editor controller", function(done){
			...
			this.res.sendStatus = code=> {
				code.should.equal(200);
				this.EditorController.renameProject.calledWith(this.project_id, this.newProjectName).should.equal(true);
				return done();
			};
		...
		return it("should send an error to next() if there is a problem", function(done){
			let error;
			this.EditorController.renameProject.callsArgWith(2, (error = new Error("problem")));
			const next = e=> {
	...
	describe("loadEditor", function() {
		beforeEach(function() {
			...
			this.AuthorizationManager.getPrivilegeLevelForProject.callsArgWith(3, null, "owner");
		...
		it("should not render the page if the project can not be accessed", function(done){
			this.AuthorizationManager.getPrivilegeLevelForProject = sinon.stub().callsArgWith(3, null, null);
		...
		it("should reactivateProjectIfRequired", function(done){
			this.res.render = (pageName, opts)=> {
				this.InactiveProjectManager.reactivateProjectIfRequired.calledWith(this.project_id).should.equal(true);
				return done();
			};
			return this.ProjectController.loadEditor(this.req, this.res);
		});
		...
		it("should mark project as opened", function(done){
			this.res.render = (pageName, opts)=> {
				this.ProjectUpdateHandler.markAsOpened.calledWith(this.project_id).should.equal(true);
				return done();
			};
			return this.ProjectController.loadEditor(this.req, this.res);
		});
		...
		it("should call the brand variations handler for branded projects", function(done){
			...
			this.res.render = (pageName, opts)=> {
				this.BrandVariationsHandler.getBrandVariationById.calledWith().should.equal(true);
				return done();
			};
			return this.ProjectController.loadEditor(this.req, this.res);
		});
		...
		it("should not call the brand variations handler for unbranded projects", function(done){
			this.res.render = (pageName, opts)=> {
				this.BrandVariationsHandler.getBrandVariationById.called.should.equal(false);
				return done();
			};
			return this.ProjectController.loadEditor(this.req, this.res);
		});
	...
	describe('userProjectsJson', function() {
		beforeEach(function(done) {
			...
			this.ProjectGetter.findAllUsersProjects = sinon.stub().callsArgWith(2, null, []);
			this.ProjectController._buildProjectList = sinon.stub().returns(projects);
			this.AuthenticationController.getLoggedInUserId = sinon.stub().returns('abc');
			return done();
		});
		...
		return it('should produce a list of projects', function(done) {
			...
			return this.ProjectController.userProjectsJson(this.req, this.res, this.next);
		});
	});
	...
	describe('projectEntitiesJson', function() {
		beforeEach(function() {
			...
			this.ProjectGetter.getProject = sinon.stub().callsArgWith(1, null, this.project);
			return this.ProjectEntityHandler.getAllEntitiesFromProject = sinon.stub().callsArgWith(1, null, this.docs, this.files);
		});
		...
		return it('should produce a list of entities', function(done) {
			this.res.json = data => {
				...
				expect(this.ProjectEntityHandler.getAllEntitiesFromProject.callCount).to.equal(1);
				return done();
			};
			return this.ProjectController.projectEntitiesJson(this.req, this.res, this.next);
		});
	});
