...
describe('ProjectDeleter', function() {
	...
	beforeEach(function() {
		...
		this.editorController = {notifyUsersProjectHasBeenDeletedOrRenamed : sinon.stub().callsArgWith(1)};
		this.TagsHandler = 
			{removeProjectFromAllTags: sinon.stub().callsArgWith(2)};
		this.CollaboratorsHandler = {
			removeUserFromAllProjets: sinon.stub().yields(),
			getMemberIds: sinon.stub().withArgs(this.project_id).yields(null, ["member-id-1", "member-id-2"])
		};
		return this.deleter = SandboxedModule.require(modulePath, { requires: {
	...
	describe("mark as deleted by external source", function() {
		...
		return it('should tell the editor controler so users are notified', function(done){
			return this.deleter.markAsDeletedByExternalSource(project_id, () => {
				this.editorController.notifyUsersProjectHasBeenDeletedOrRenamed.calledWith(project_id).should.equal(true);
				return done();
			});
		});
	});
	...
	describe("unmarkAsDeletedByExternalSource", function() {
		beforeEach(function() {
			...
			return this.deleter.unmarkAsDeletedByExternalSource(this.project_id, this.callback);
		});
		
		return it("should remove the flag from the project", function() {
			return this.Project.update
				.calledWith({_id: this.project_id}, {deletedByExternalDataSource:false})
				.should.equal(true);
		});
	});
	...
	describe("deleteUsersProjects", function() {
		...
		return it("should remove all the projects the user is a collaborator of", function(done){
			return this.deleter.deleteUsersProjects(this.user._id, () => {
				this.CollaboratorsHandler.removeUserFromAllProjets.calledWith(this.user._id).should.equal(true);
				return done();
			});
		});
	});
	...
	describe("deleteProject", function() {
		...
		it("should save a DeletedProject with additional deleterData", function(done) {
			return this.deleter.deleteProject(this.project_id, {deleterUser: this.user, ipAddress: this.ip}, (err, deletedProject) => {
		...
		it("should flushProjectToMongoAndDelete in doc updater", function(done){
			return this.deleter.deleteProject(this.project_id, {deleterUser: this.user, ipAddress: this.ip}, () => {
				this.documentUpdaterHandler.flushProjectToMongoAndDelete.calledWith(this.project_id).should.equal(true);
				return done();
			});
		});
		...
		it("should removeProjectFromAllTags", function(done){
			return this.deleter.deleteProject(this.project_id, () => {
				this.TagsHandler.removeProjectFromAllTags.calledWith("member-id-1", this.project_id).should.equal(true);
				this.TagsHandler.removeProjectFromAllTags.calledWith("member-id-2", this.project_id).should.equal(true);
				return done();
			});
		});
		...
		return it("should remove the project from Mongo", function(done) {
			return this.deleter.deleteProject(this.project_id, () => {
				this.Project.remove.calledWith({
					_id: this.project_id
				}).should.equal(true);
				return done();
			});
