...
const modulePath = "../../../../app/js/Features/Project/ProjectEntityMongoUpdateHandler";
...
describe('ProjectEntityMongoUpdateHandler', function() {
	...
	beforeEach(function() {
		...
		tk.freeze(Date.now());
		return this.subject = SandboxedModule.require(modulePath, { requires: {
	...
	describe('addFile', function() {
		beforeEach(function() {
			...
			return this.subject.addFile(project_id, folder_id, this.file, this.callback);
		});
	...
	describe('replaceFileWithNew', function() {
		beforeEach(function() {
			...
			this.ProjectLocator.findElement = sinon.stub().yields(null, this.file, this.path);
			this.ProjectModel.findOneAndUpdate = sinon.stub().yields(null, this.newProject);
			this.ProjectModel.update = sinon.stub().yields();

			return this.subject.replaceFileWithNew(project_id, file_id, this.newFile, this.callback);
		});
		...
		it('finds the existing element', function() {
			return this.ProjectLocator.findElement
				.calledWith({ project: this.project, element_id: file_id, type: 'file' })
				.should.equal(true);
		});
			...
			return this.ProjectModel.findOneAndUpdate
				.calledWith(
					{ _id: project_id },
					{
						...
						'$set': { 'file.png._id': this.newFile._id, 'file.png.created': new Date(), 'file.png.linkedFileData': this.linkedFileData, 'file.png.hash': this.hash }
					},
					{new: true}
				)
				.should.equal(true);
		...
		return it('calls the callback', function() {
			return this.callback.calledWith(null, this.file, this.project, this.path, this.newProject).should.equal(true);
		});
	});
	...
	describe('mkdirp', function() {
		beforeEach(function() {
			...
			this.ProjectGetter.getProjectWithOnlyFolders = sinon.stub().yields(null, this.project);
		...
		it('should return the root folder if the path is just a slash', function(done){
			const path = "/";
			return this.subject.mkdirp(project_id, path, {}, (err, folders, lastFolder)=> {
		...
		it('should use a case-insensitive match by default', function(done){
			const path = "/differentFolder/";
			return this.subject.mkdirp(project_id, path, {}, (err, folders, lastFolder)=> {
				this.ProjectLocator.findElementByPath.calledWithMatch({exactCaseMatch:undefined})
				.should.equal(true);
				return done();
			});
		});
		...
		return it('should use a case-sensitive match if exactCaseMatch option is set', function(done){
			const path = "/differentFolder/";
			return this.subject.mkdirp(project_id, path, {exactCaseMatch:true}, (err, folders, lastFolder)=> {
				this.ProjectLocator.findElementByPath.calledWithMatch({exactCaseMatch:true})
				.should.equal(true);
				return done();
			});
		});
	...
	describe('moveEntity', function() {
		beforeEach(function() {
			...
			this.ProjectEntityHandler.getAllEntitiesFromProject
				.onFirstCall()
				.yields(null, (this.oldDocs = ['old-doc']), (this.oldFiles = ['old-file']));
			this.ProjectEntityHandler.getAllEntitiesFromProject
				.onSecondCall()
				.yields(null, (this.newDocs = ['new-doc']), (this.newFiles = ['new-file']));

			this.doc = {lines:["1234","312343d"], rev: "1234"};
			this.path = { mongo:"folders[0]", fileSystem:"/old_folder/somewhere.txt" };
			this.newProject = "new-project";
			this.ProjectLocator.findElement = sinon.stub()
				.withArgs({project: this.project, element_id: this.docId, type: 'docs'})
				.yields(null, this.doc, this.path);

			this.subject._checkValidMove = sinon.stub().yields();

			this.subject._removeElementFromMongoArray = sinon.stub().yields(null, this.newProject);
			this.subject._putElement = sinon.stub().yields(null, {path: this.pathAfterMove}, this.newProject);

			return this.subject.moveEntity(project_id, doc_id, folder_id, "docs", this.callback);
		});
		...
		return it("calls the callback", function() {
			const changes = { oldDocs: this.oldDocs, newDocs: this.newDocs, oldFiles: this.oldFiles, newFiles: this.newFiles, newProject: this.newProject };
			return this.callback.calledWith(
				null, this.project, this.path.fileSystem, this.pathAfterMove.fileSystem, this.doc.rev, changes
			).should.equal(true);
		});
	});
	...
	describe('moveEntity must refuse to move the folder to a subfolder of itself', function() {
		beforeEach(function() {
			...
			this.subject._removeElementFromMongoArray = sinon.stub().yields(null, this.project);
			this.subject._putElement = sinon.stub().yields(null, {path: this.pathAfterMove}, this.newProject);
		...
		it("should not put the element in the new folder", function() {
			return this.subject._putElement.called
				.should.equal(false);
		});

		it('should not remove the element from its current position', function() {
			return this.subject._removeElementFromMongoArray.called
				.should.equal(false);
		});
	...
	describe('deleteEntity', function() {
		beforeEach(function() {
			...
			this.doc = {_id: doc_id};
			this.ProjectLocator.findElement = sinon.stub().callsArgWith(1, null, this.doc, this.path);
		...
		it("should find the element", function() {
			return this.ProjectLocator.findElement
				.calledWith({project: this.project, element_id: this.doc._id, type: 'doc'})
				.should.equal(true);
		});

		it("should remove the element from the database", function() {
			return this.subject._removeElementFromMongoArray
				.calledWith(this.ProjectModel, project_id, this.path.mongo, this.doc._id)
				.should.equal(true);
		});

		return it("calls the callbck", function() {
			return this.callback.calledWith(null, this.doc, this.path, this.project).should.equal(true);
		});
	});
	...
	describe("renameEntity", function() {
		beforeEach(function() {
			...
			this.ProjectGetter.getProjectWithoutLock = sinon.stub().yields(null, this.project);
			...
			this.ProjectLocator.findElement = sinon.stub().yields(null, this.doc, this.path, this.folder);
			this.subject._checkValidElementName = sinon.stub().yields();
			this.ProjectModel.findOneAndUpdate = sinon.stub().callsArgWith(3, null, this.newProject);

			return this.subject.renameEntity(project_id, doc_id, 'doc', this.newName, this.callback);
		});
		...
		return it('calls the callback', function() {
			const changes = { oldDocs: this.oldDocs, newDocs: this.newDocs, oldFiles: this.oldFiles, newFiles: this.newFiles, newProject: this.newProject };
			return this.callback.calledWith(
				null, this.project, '/old.tex', '/new.tex', this.doc.rev, changes
			).should.equal(true);
		});
	});
	...
	describe('addFolder', function() {
		beforeEach(function() {
			...
			this.ProjectGetter.getProjectWithOnlyFolders = sinon.stub().callsArgWith(1, null, this.project);
			this.subject._confirmFolder = sinon.stub().yields(folder_id);
			this.subject._putElement = sinon.stub().yields();

			return this.subject.addFolder(project_id, folder_id, this.folderName, this.callback);
		});
		...
		return it('calls the callback', function() {
			...
			return this.callback.calledWithMatch(null, folderMatcher, folder_id).should.equal(true);
		});
	});
	...
	describe('_removeElementFromMongoArray ', function() {
		beforeEach(function() {
			...
			this.entityId = "5678";
			this.ProjectModel.update = sinon.stub().yields();
			this.ProjectModel.findOneAndUpdate = sinon.stub().yields(null, this.project);
			return this.subject._removeElementFromMongoArray(this.ProjectModel, this.id, this.mongoPath, this.entityId, this.callback);
		});

		it('should pull', function() {
			return this.ProjectModel.findOneAndUpdate
				.calledWith({ _id: this.id }, { '$pull': { 'folders[0]': {_id: this.entityId } }, '$inc': {'version': 1} }, {'new': true})
				.should.equal(true);
		});
	...
	describe("_putElement", function() {
		beforeEach(function() {
			...
			this.ProjectLocator.findElement = sinon.stub().yields(null, this.folder, this.path);
			return this.ProjectModel.findOneAndUpdate = sinon.stub().yields(null, this.project);
		});

		return describe("updating the project", function() {
			it("should use the correct mongo path", function(done){
				return this.subject._putElement(this.project, this.folder._id, this.doc, "docs", err=> {
					this.ProjectModel.findOneAndUpdate.args[0][0]._id.should.equal(this.project._id);
					assert.deepEqual(this.ProjectModel.findOneAndUpdate.args[0][1].$push[this.path.mongo+".docs"], this.doc);
					return done();
				});
			});

			it("should return the project in the callback", function(done){
				return this.subject._putElement(this.project, this.folder._id, this.doc, "docs", (err, path, project)=> {
			...
			it("should add an s onto the type if not included", function(done){
				return this.subject._putElement(this.project, this.folder._id, this.doc, "doc", err=> {
					assert.deepEqual(this.ProjectModel.findOneAndUpdate.args[0][1].$push[this.path.mongo+".docs"], this.doc);
					return done();
				});
			});
			...
			it("should not call update if element is null", function(done){
				return this.subject._putElement(this.project, this.folder._id, null, "doc", err=> {
			...
			it("should default to root folder insert", function(done){
				return this.subject._putElement(this.project, null, this.doc, "doc", err=> {
					this.ProjectLocator.findElement.args[0][0].element_id.should.equal(this.project.rootFolder[0]._id);
					return done();
				});
			});
			...
			it("should error if the element has no _id", function(done){
				const doc =
					{name:"something"};
				return this.subject._putElement(this.project, this.folder._id, doc, "doc", err=> {
					this.ProjectModel.findOneAndUpdate.called.should.equal(false);
					return done();
				});
			});
			...
			it("should error if the folder name is too long", function(done){
				...
				this.ProjectLocator.findElement.callsArgWith(1, null, this.folder, this.path);
				const doc = {
					_id: ObjectId(),
					name: "something"
				};
			...
			it("should error if a document already exists with the same name", function(done){
				...
				return this.subject._putElement(this.project, this.folder, doc, "doc", err=> {
					this.ProjectModel.findOneAndUpdate.called.should.equal(false);
	...
	describe('_checkValidElementName', function() {
		...
		it('returns an error if name matches any doc name', function() {
			return this.subject._checkValidElementName(this.folder, 'doc_name', err => expect(err).to.deep.equal(new Errors.InvalidNameError("file already exists")));
		});

		it('returns an error if name matches any file name', function() {
			return this.subject._checkValidElementName(this.folder, 'file_name', err => expect(err).to.deep.equal(new Errors.InvalidNameError("file already exists")));
		});

		it('returns an error if name matches any folder name', function() {
			return this.subject._checkValidElementName(this.folder, 'folder_name', err => expect(err).to.deep.equal(new Errors.InvalidNameError("file already exists")));
		});

		return it('returns nothing if name is valid', function() {
			return this.subject._checkValidElementName(this.folder, 'unique_name', err => expect(err).to.be.undefined);
		});
	});
	...
	describe('_checkValidMove', function() {
		beforeEach(function() {
			...
			this.ProjectLocator.findElement = sinon.stub().yields(null, this.destFolder, this.destFolderPath);
		...
		it('checks the element name is valid', function() {
			...
			return this.subject._checkValidMove(this.project, 'doc', this.doc, {fileSystem: '/main.tex'}, this.destFolder._id, err => {
		...
		return it('returns an error if trying to move a folder inside itself', function() {
			...
			return this.subject._checkValidMove(this.project, 'folder', folder, {fileSystem: '/foo'}, this.destFolder._id, err => {
				return expect(err).to.deep.equal(new Errors.InvalidNameError("destination folder is a child folder of me"));
			});
		});
	});
	...
	return describe("_insertDeletedDocReference", function() {
		beforeEach(function() {
			...
			this.callback = sinon.stub();
			this.ProjectModel.update = sinon.stub().yields();
			return this.subject._insertDeletedDocReference(project_id, this.doc, this.callback);
		});
