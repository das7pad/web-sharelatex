...
const modulePath = "../../../../app/js/Features/Project/ProjectOptionsHandler.js";
...
describe('ProjectOptionsHandler', function() {
	...
	describe("setting the spellCheckLanguage", function() {
		...
		it('should not perform and update on mongo if it is not a reconised compiler', function(done){
			return this.handler.setSpellCheckLanguage(project_id, "no a lang", err=> {
				this.projectModel.update.called.should.equal(false);
				return done();
			});
		});
	...
	return describe("unsetting the brandVariationId", () => it('should perform and update on mongo', function(done){
