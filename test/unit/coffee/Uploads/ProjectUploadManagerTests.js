...
describe("ProjectUploadManager", function() {
	beforeEach(function() {
		...
		this.ProjectUploadManager = SandboxedModule.require(modulePath, { requires: {
		...
		this.ProjectRootDocManager.findRootDocFileFromDirectory = sinon.stub().callsArgWith(1, null, 'main.tex', this.othername);
		this.ProjectRootDocManager.setRootDocFromName = sinon.stub().callsArg(2);
		this.DocumentHelper.getTitleFromTexContent = sinon.stub().returns(this.othername);
		return this.ProjectDetailsHandler.fixProjectName = sinon.stub().returnsArg(0);
	});

	describe("createProjectFromZipArchive", function() {
		describe("when the title can be read from the root document", function() {
			beforeEach(function(done) {
				this.ProjectUploadManager._getDestinationDirectory = sinon.stub().returns(this.destination);
				this.ProjectDetailsHandler.generateUniqueName = sinon.stub().callsArgWith(2, null, this.othername);
				return this.ProjectUploadManager.createProjectFromZipArchive(this.owner_id, this.name, this.source, (err, project) => {
			...
			it("should set up the directory to extract the archive to", function() {
				return this.ProjectUploadManager._getDestinationDirectory.calledWith(this.source).should.equal(true);
			});

			it("should extract the archive", function() {
				return this.ArchiveManager.extractZipArchive.calledWith(this.source, this.destination).should.equal(true);
			});

			it("should find the top level directory", function() {
				return this.ArchiveManager.findTopLevelDirectory.calledWith(this.destination).should.equal(true);
			});

			it("should insert the extracted archive into the folder", function() {
				return this.FileSystemImportManager.addFolderContents.calledWith(this.owner_id, this.project_id, this.folder_id, this.topLevelDestination, false)
					.should.equal(true);
			});
			...
			it("should read the title from the tex contents", function() {
				return this.DocumentHelper.getTitleFromTexContent.called.should.equal(true);
			});

			it("should set the root document", function() {
				return this.ProjectRootDocManager.setRootDocFromName.calledWith(this.project_id, 'main.tex').should.equal(true);
			});

			it("should call the callback", function() {
				return this.callback.calledWith(sinon.match.falsy, this.project).should.equal(true);
			});

			return it("should ensure the name is valid", function() {
				return this.ProjectDetailsHandler.fixProjectName.called.should.equal(true);
			});
		});

		return describe("when the root document can't be determined", function() {
			beforeEach(function(done) {
				this.ProjectRootDocManager.findRootDocFileFromDirectory = sinon.stub().callsArg(1);
				this.ProjectUploadManager._getDestinationDirectory = sinon.stub().returns(this.destination);
				this.ProjectDetailsHandler.generateUniqueName = sinon.stub().callsArgWith(2, null, this.name);
				return this.ProjectUploadManager.createProjectFromZipArchive(this.owner_id, this.name, this.source, (err, project) => {
					this.callback(err, project);
					return done();
				});
			...
			return it("should not try to set the root doc", function() {
				return this.ProjectRootDocManager.setRootDocFromName.called.should.equal(false);
			});
		});
	});
	...
	describe("createProjectFromZipArchiveWithName", function() {
		beforeEach(function(done) {
			this.ProjectDetailsHandler.generateUniqueName = sinon.stub().callsArgWith(2, null, this.name);
			this.ProjectUploadManager.insertZipArchiveIntoFolder = sinon.stub().callsArg(4);
			return this.ProjectUploadManager.createProjectFromZipArchiveWithName(this.owner_id, this.name, this.source, (err, project) => {
				this.callback(err, project);
				return done();
			});
		});
		...
		it("should insert the zip file contents into the root folder", function() {
			return this.ProjectUploadManager
				.insertZipArchiveIntoFolder
				.calledWith(this.owner_id, this.project_id, this.root_folder_id, this.source)
				.should.equal(true);
		});
	...
	describe("insertZipArchiveIntoFolder", function() {
		beforeEach(function(done) {
			this.ProjectUploadManager._getDestinationDirectory = sinon.stub().returns(this.destination);
			return this.ProjectUploadManager.insertZipArchiveIntoFolder(this.owner_id, this.project_id, this.folder_id, this.source, err => {
				this.callback(err);
				return done();
			});
		});
	...
	return describe("_getDestinationDirectory", () => it("should return the path with the time appended", function() {
        const date = Date.now();
