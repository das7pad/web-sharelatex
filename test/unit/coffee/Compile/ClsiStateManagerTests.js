...
describe("ClsiStateManager", function() {
	beforeEach(function() {
		this.ClsiStateManager = SandboxedModule.require(modulePath, { requires: {
	...
	return describe("computeHash", function() {
		beforeEach(function(done) {
			...
			this.ProjectEntityHandler.getAllEntitiesFromProject = sinon.stub().callsArgWith(1, null, this.docs, this.files);
			return this.ClsiStateManager.computeHash(this.project, this.options, (err, hash) => {
				this.hash0 = hash;
				return done();
			});
		});
		...
		describe("with a sample project", function() {
			beforeEach(function() {
				return this.ClsiStateManager.computeHash(this.project, this.options, this.callback);
			});
		...
		describe("when the files and docs are in a different order", function() {
			beforeEach(function() {
				...
				[this.files[0], this.files[1]] = Array.from([this.files[1], this.files[0]]);
				return this.ClsiStateManager.computeHash(this.project, this.options, this.callback);
			});

			return it("should call the callback with the same hash value", function() {
				return this.callback
					.calledWith(null, this.hash0)
					.should.equal(true);
			});
		});
		...
		describe("when a file is added", function() {
			beforeEach(function(done) {
				this.files.push({ path: "/newfile.tex", file: {_id: "newfile-id", rev: 123}});
				return this.ClsiStateManager.computeHash(this.project, this.options, (err, hash) => {
					this.hash1 = hash;
					return done();
				});
