...
const modulePath = "../../../../app/js/Features/Compile/ClsiManager.js";
...
describe("ClsiManager", function() {
	beforeEach(function() {
		...
		this.ClsiManager = SandboxedModule.require(modulePath, { requires: {
	...
	describe("sendRequest", function() {
		beforeEach(function() {
			this.ClsiManager._buildRequest = sinon.stub().callsArgWith(2, null, (this.request = "mock-request"));
		...
		describe("with a successful compile", function() {
			beforeEach(function() {
				...
				return this.ClsiManager.sendRequest(this.project_id, this.user_id, {compileGroup:"standard"}, this.callback);
			});
			...
			return it("should call the callback with the status and output files", function() {
				...
				return this.callback.calledWith(null, this.status, outputFiles).should.equal(true);
			});
		});
		...
		describe("with a failed compile", function() {
			beforeEach(function() {
				...
				return this.ClsiManager.sendRequest(this.project_id, this.user_id, {}, this.callback);
			});
		...
		describe("with a sync conflict", function() {
			beforeEach(function() {
				...
				this.ClsiManager.sendRequestOnce.withArgs(this.project_id, this.user_id, {syncType:"full"}).callsArgWith(3, null,	(this.status = "success"));
				this.ClsiManager.sendRequestOnce.withArgs(this.project_id, this.user_id, {}).callsArgWith(3, null, "conflict");
				return this.ClsiManager.sendRequest(this.project_id, this.user_id, {}, this.callback);
			});
			...
			it("should call the sendRequestOnce method with syncType:full", function() {
				return this.ClsiManager.sendRequestOnce.calledWith(this.project_id, this.user_id, {syncType:"full"}).should.equal(true);
			});

			it("should call the sendRequestOnce method without syncType:full", function() {
				return this.ClsiManager.sendRequestOnce.calledWith(this.project_id, this.user_id, {}).should.equal(true);
			});
		...
		return describe("when the resources fail the precompile check", function() {
			beforeEach(function() {
				this.ClsiFormatChecker.checkRecoursesForProblems = sinon.stub().callsArgWith(1, new Error("failed"));
				this.ClsiManager._postToClsi = sinon.stub().callsArgWith(4, null, {
					compile: {
						status: (this.status = "failure")
					}
			...
			return it("should call the callback with an error", function() {
				return this.callback.calledWithExactly(new Error("failed")).should.equal(true);
			});
		});
	});
	...
	describe("sendExternalRequest", function() {
		...
		describe("with a successful compile", function() {
			beforeEach(function() {
				...
				return this.ClsiManager.sendExternalRequest(this.submission_id, this.clsi_request, {compileGroup:"standard"}, this.callback);
			});
		...
		describe("with a failed compile", function() {
			beforeEach(function() {
				...
				return this.ClsiManager.sendExternalRequest(this.submission_id, this.clsi_request, {}, this.callback);
			});
	...
	describe("deleteAuxFiles", function() {
		beforeEach(function() {
			...
			return this.DocumentUpdaterHandler.clearProjectState = sinon.stub().callsArg(1);
		});

		return describe("with the standard compileGroup", function() {
			beforeEach(function() {
				return this.ClsiManager.deleteAuxFiles(this.project_id, this.user_id, {compileGroup: "standard"}, this.callback);
			});

			it("should call the delete method in the standard CLSI", function() {
				return this.ClsiManager._makeRequest
					.calledWith(this.project_id, { method:"DELETE", url:`${this.settings.apis.clsi.url}/project/${this.project_id}/user/${this.user_id}`})
					.should.equal(true);
			});
	...
	describe("_buildRequest", function() {
		beforeEach(function() {
			...
			return this.DocumentUpdaterHandler.flushProjectToMongo = sinon.stub().callsArgWith(1, null);
		});

		describe("with a valid project", function() {
			beforeEach(function(done) {
				return this.ClsiManager._buildRequest(this.project_id, {timeout:100}, (error, request) => {
			...
			it("should get the project with the required fields", function() {
				return this.ProjectGetter.getProject
					.calledWith(this.project_id, {compiler:1, rootDoc_id: 1, imageName: 1, rootFolder: 1})
					.should.equal(true);
			});
		...
		describe("with the incremental compile option", function() {
			beforeEach(function(done) {
				this.ClsiStateManager.computeHash = sinon.stub().callsArgWith(2, null, (this.project_state_hash = "01234567890abcdef"));
				this.DocumentUpdaterHandler.getProjectDocsIfMatch = sinon.stub().callsArgWith(2, null, [{_id:this.doc_1._id, lines:  this.doc_1.lines, v: 123}]);
				this.ProjectEntityHandler.getAllDocPathsFromProject = sinon.stub().callsArgWith(1, null, {"mock-doc-id-1":"main.tex"});
				return this.ClsiManager._buildRequest(this.project_id, {timeout:100, incrementalCompilesEnabled:true}, (error, request) => {
					this.request = request;
					return done();
				});
			});
			...
			return describe("when the root doc is set and not in the docupdater", function() {
				beforeEach(function(done) {
					this.ClsiStateManager.computeHash = sinon.stub().callsArgWith(2, null, (this.project_state_hash = "01234567890abcdef"));
					this.DocumentUpdaterHandler.getProjectDocsIfMatch = sinon.stub().callsArgWith(2, null, [{_id:this.doc_1._id, lines:  this.doc_1.lines, v: 123}]);
					this.ProjectEntityHandler.getAllDocPathsFromProject = sinon.stub().callsArgWith(1, null, {"mock-doc-id-1":"main.tex", "mock-doc-id-2":"/chapters/chapter1.tex"});
					return this.ClsiManager._buildRequest(this.project_id, {timeout:100, incrementalCompilesEnabled:true, rootDoc_id:"mock-doc-id-2"}, (error, request) => {
						this.request = request;
						return done();
					});
				});
				...
				return it("should still change the root path", function() {
					return this.request.compile.rootResourcePath.should.equal("chapters/chapter1.tex");
				});
			});
		});
		...
		describe("when root doc override is valid", function() {
			beforeEach(function(done) {
				return this.ClsiManager._buildRequest(this.project_id, {rootDoc_id:"mock-doc-id-2"}, (error, request) => {
					this.request = request;
					return done();
				});
			});
		...
		describe("when root doc override is invalid", function() {
			beforeEach(function(done) {
				return this.ClsiManager._buildRequest(this.project_id, {rootDoc_id:"invalid-id"}, (error, request) => {
					this.request = request;
					return done();
				});
			});
		...
		describe("when the project has an invalid compiler", function() {
			beforeEach(function(done) {
				...
				return this.ClsiManager._buildRequest(this.project, null, (error, request) => {
					this.request = request;
					return done();
				});
			});
		...
		describe("when there is no valid root document and no main.tex document", function() {
			...
			return it("should report an error", function() {
				return this.callback.calledWith(new Error("no main file specified")).should.equal(true);
			});
		});
		...
		return describe("with the draft option", () => it("should add the draft option into the request", function(done) {
            return this.ClsiManager._buildRequest(this.project_id, {timeout:100, draft: true}, (error, request) => {
	...
	describe('_postToClsi', function() {
		...
		describe("successfully", function() {
			beforeEach(function() {
				this.ClsiManager._makeRequest = sinon.stub().callsArgWith(2, null, {statusCode: 204}, (this.body = { mock: "foo" }));
				return this.ClsiManager._postToClsi(this.project_id, this.user_id, this.req, "standard", this.callback);
			});

			it('should send the request to the CLSI', function() {
				const url = `${this.settings.apis.clsi.url}/project/${this.project_id}/user/${this.user_id}/compile`;
				return this.ClsiManager._makeRequest.calledWith(this.project_id, {
					method: "POST",
					url,
					json: this.req
				}).should.equal(true);
		...
		return describe("when the CLSI returns an error", function() {
			beforeEach(function() {
				this.ClsiManager._makeRequest = sinon.stub().callsArgWith(2, null, {statusCode: 500}, (this.body = { mock: "foo" }));
				return this.ClsiManager._postToClsi(this.project_id, this.user_id, this.req, "standard", this.callback);
			});

			return it("should call the callback with the body and the error", function() {
				return this.callback.calledWith(new Error("CLSI returned non-success code: 500"), this.body).should.equal(true);
			});
		});
	});
	...
	describe("wordCount", function() {
		beforeEach(function() {
			this.ClsiManager._makeRequest = sinon.stub().callsArgWith(2, null, {statusCode: 200}, (this.body = { mock: "foo" }));
			return this.ClsiManager._buildRequest = sinon.stub().callsArgWith(2, null, (this.req = { compile: { rootResourcePath: "rootfile.text", options: {} } }));
		});

		describe("with root file", function() {
			beforeEach(function() {
				return this.ClsiManager.wordCount(this.project_id, this.user_id, false, {}, this.callback);
			});

			it("should call wordCount with root file", function() {
				return this.ClsiManager._makeRequest
				.calledWith(this.project_id, {method: "GET", url: `http://clsi.example.com/project/${this.project_id}/user/${this.user_id}/wordcount`, qs: {file: "rootfile.text",image:undefined}})
				.should.equal(true);
			});
		...
		describe("with param file", function() {
			beforeEach(function() {
				return this.ClsiManager.wordCount(this.project_id, this.user_id, "main.tex", {}, this.callback);
			});

			return it("should call wordCount with param file", function() {
				return this.ClsiManager._makeRequest
					.calledWith(this.project_id, { method: "GET", url: `http://clsi.example.com/project/${this.project_id}/user/${this.user_id}/wordcount`, qs:{file:"main.tex",image:undefined}})
					.should.equal(true);
			});
		});
		...
		return describe("with image", function() {
			beforeEach(function() {
				this.req.compile.options.imageName = (this.image = "example.com/mock/image");
				return this.ClsiManager.wordCount(this.project_id, this.user_id, "main.tex", {}, this.callback);
			});

			return it("should call wordCount with file and image", function() {
				return this.ClsiManager._makeRequest
					.calledWith(this.project_id, { method: "GET", url: `http://clsi.example.com/project/${this.project_id}/user/${this.user_id}/wordcount`, qs:{file:"main.tex",image:this.image}})
					.should.equal(true);
			});
		});
	});
	...
	describe("_makeRequest", function() {
		...
		return it("should set the cookie again on response as it might have changed", function(done){
			return this.ClsiManager._makeRequest(this.project_id, this.opts, () => {
				this.ClsiCookieManager.setServerId.calledWith(this.project_id, this.response).should.equal(true);
				return done();
			});
		});
	});
	...
	return describe("_makeGoogleCloudRequest", function() {
		...
		it("should change the domain on the url", function(done){
			return this.ClsiManager._makeNewBackendRequest(this.project_id, this.opts, () => {
				const args = this.request.args[0];
				args[0].url.should.equal(`https://compiles.somewhere.test/project/${this.project_id}`);
				return done();
			});
		});
		...
		return it("should not make a request if there is not clsi_new url", function(done){
			...
			return this.ClsiManager._makeNewBackendRequest(this.project_id, this.opts, err=> {
