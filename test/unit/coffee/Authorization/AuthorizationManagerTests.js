...
const modulePath = "../../../../app/js/Features/Authorization/AuthorizationManager.js";
const SandboxedModule = require('sandboxed-module');
const Errors = require("../../../../app/js/Features/Errors/Errors");

describe("AuthorizationManager", function() {
	beforeEach(function() {
		this.AuthorizationManager = SandboxedModule.require(modulePath, { requires: {
	...
	describe("getPrivilegeLevelForProject", function() {
		...
		describe('with a token-based project', function() {
			...
			describe("with a user_id with a privilege level", function() {
				beforeEach(function() {
					this.AuthorizationManager.isUserSiteAdmin.withArgs(this.user_id).yields(null, false);
					this.CollaboratorsHandler.getMemberIdPrivilegeLevel
						.withArgs(this.user_id, this.project_id)
						.yields(null, "readOnly");
					return this.AuthorizationManager.getPrivilegeLevelForProject(this.user_id, this.project_id, this.token, this.callback);
				});

				return it("should return the user's privilege level", function() {
					return this.callback.calledWith(null, "readOnly", false, false).should.equal(true);
				});
			});
			...
			describe("with a user_id with no privilege level", function() {
				...
				return it("should return false", function() {
					return this.callback.calledWith(null, false, false, false).should.equal(true);
				});
			});

			describe("with a user_id who is an admin", function() {
				beforeEach(function() {
					this.AuthorizationManager.isUserSiteAdmin.withArgs(this.user_id).yields(null, true);
					this.CollaboratorsHandler.getMemberIdPrivilegeLevel
						.withArgs(this.user_id, this.project_id)
						.yields(null, false);
					return this.AuthorizationManager.getPrivilegeLevelForProject(this.user_id, this.project_id, this.token, this.callback);
				...
				return it("should return the user as an owner", function() {
					return this.callback.calledWith(null, "owner", false, true).should.equal(true);
				});
			});
			...
			return describe("with no user (anonymous)", function() {
				...
				describe('when the token is not valid', function() {
					...
					beforeEach(function() {
						this.TokenAccessHandler.isValidToken = sinon.stub()
							.withArgs(this.project_id, this.token)
							.yields(null, false, false);
						return this.AuthorizationManager.getPrivilegeLevelForProject(null, this.project_id, this.token, this.callback);
					});

					it("should not call CollaboratorsHandler.getMemberIdPrivilegeLevel", function() {
						return this.CollaboratorsHandler.getMemberIdPrivilegeLevel.called.should.equal(false);
					});

					it("should not call AuthorizationManager.isUserSiteAdmin", function() {
						return this.AuthorizationManager.isUserSiteAdmin.called.should.equal(false);
					});

					it('should check if the token is valid', function() {
						return this.TokenAccessHandler.isValidToken.calledWith(this.project_id, this.token).should.equal(true);
					});
				...
				describe('when the token is valid for read-and-write', function() {
					...
					return describe('when read-write-sharing is enabled', function() {
						...
						return it("should give read-write access", function() {
							return this.callback.calledWith(null, "readAndWrite", false).should.equal(true);
						});
					});
				});
				...
				return describe('when the token is valid for read-only', function() {
					...
					return it("should give read-only access", function() {
						return this.callback.calledWith(null, "readOnly", false).should.equal(true);
					});
				});
			});
		});
		...
		describe("with a public project", function() {
			...
			describe("with a user_id with no privilege level", function() {
				...
				return it("should return the public privilege level", function() {
					return this.callback.calledWith(null, "readAndWrite", true).should.equal(true);
				});
			});
			...
			describe("with a user_id who is an admin", function() {
				...
				return it("should return the user as an owner", function() {
					return this.callback.calledWith(null, "owner", false).should.equal(true);
				});
			});
		...
		describe("when the project doesn't exist", function() {
			...
			return it("should return a NotFoundError", function() {
				return this.AuthorizationManager.getPrivilegeLevelForProject(this.user_id, this.project_id, this.token, error => error.should.be.instanceof(Errors.NotFoundError));
			});
		});
		...
		return describe("when the project id is not valid", function() {
			...
			return it("should return a error", function(done){
				return this.AuthorizationManager.getPrivilegeLevelForProject(undefined, "not project id", this.token, err => {
	...
	describe("canUserReadProject", function() {
		...
		describe("when user is owner", function() {
			...
			return it("should return true", function(done) {
				return this.AuthorizationManager.canUserReadProject(this.user_id, this.project_id, this.token, function(error, canRead) {
	...
	describe("canUserWriteProjectContent", function() {
		...
		describe("when user is owner", function() {
			...
			return it("should return true", function(done) {
				return this.AuthorizationManager.canUserWriteProjectContent(this.user_id, this.project_id, this.token, function(error, canWrite) {
	...
	describe("canUserWriteProjectSettings", function() {
		...
		describe("when user is owner", function() {
			...
			return it("should return true", function(done) {
				return this.AuthorizationManager.canUserWriteProjectSettings(this.user_id, this.project_id, this.token, function(error, canWrite) {
					expect(canWrite).to.equal(true);
					return done();
				});
			});
	...
	describe("canUserAdminProject", function() {
		...
		describe("when user is owner", function() {
			...
			return it("should return true", function(done) {
				return this.AuthorizationManager.canUserAdminProject(this.user_id, this.project_id, this.token, function(error, canAdmin) {
	...
	return describe("isUserSiteAdmin", function() {
		...
		describe("when user is admin", function() {
			...
			return it("should return true", function(done) {
				return this.AuthorizationManager.isUserSiteAdmin(this.user_id, function(error, isAdmin) {
		...
		return describe("when no user is passed", () => it("should return false", function(done) {
            return this.AuthorizationManager.isUserSiteAdmin(null, (error, isAdmin) => {
