...
require('chai').should();
const modulePath = require('path').join(__dirname, '../../../../app/js/Features/ThirdPartyDataStore/TpdsController.js');
...
describe('TpdsController', function() {
	beforeEach(function() {
		...
		this.TpdsController = SandboxedModule.require(modulePath, { requires: {
	...
	describe('getting an update', () => it('should process the update with the update reciver', function(done){
        ...
        const res = {  sendStatus: () => { 
            this.TpdsUpdateHandler.newUpdate.calledWith(this.user_id, "projectName","/here.txt", req, this.source).should.equal(true);
            return done();
        }
    };
	...
	describe('getting a delete update', () => it('should process the delete with the update reciver', function(done){
        const path = "/projectName/here.txt";
        const req = { 
            params:{0:path, "user_id":this.user_id},
            session: {
        ...
        const res = { sendStatus: () => { 
            this.TpdsUpdateHandler.deleteUpdate.calledWith(this.user_id, "projectName", "/here.txt", this.source).should.equal(true);
            return done();
        }
    };
	...
	describe('updateProjectContents', function() {
		...
		it("should merge the update", function() {
			return this.UpdateMerger.mergeUpdate
				.calledWith(null, this.project_id, "/" + this.path, this.req, this.source)
				.should.equal(true);
		});
