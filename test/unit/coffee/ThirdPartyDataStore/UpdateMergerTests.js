...
require('chai').should();
const modulePath = require('path').join(__dirname, '../../../../app/js/Features/ThirdPartyDataStore/UpdateMerger.js');
const BufferedStream = require('bufferedstream');

describe('UpdateMerger :', function() {
	beforeEach(function() {
		this.updateMerger = SandboxedModule.require(modulePath, { requires: {
			'fs': (this.fs =
		...
		this.ProjectEntityHandler.getAllEntities = sinon.stub().callsArgWith(1, null, this.existingDocs, this.existingFiles);
	...
	describe('mergeUpdate', function() {
		...
		describe("doc updates for a new doc", function() {
			...
			beforeEach(function() {
				...
				return this.updateMerger.mergeUpdate(this.user_id, this.project_id, this.docPath, this.updateRequest, this.source, this.callback);
			});
			...
			it('should process update as doc', function() {
				return this.updateMerger.p.processDoc
					.calledWith(this.project_id, this.user_id, this.fsPath, this.docPath, this.source)
					.should.equal(true);
			});
		...
		describe("file updates for a new file ", function() {
			beforeEach(function() {
				...
				return this.updateMerger.mergeUpdate(this.user_id, this.project_id, this.filePath, this.updateRequest, this.source, this.callback);
			});
			...
			it('should process update as file', function() {
				return this.updateMerger.p.processFile
					.calledWith(this.project_id, this.fsPath, this.filePath, this.source, this.user_id)
					.should.equal(true);
			});
		...
		describe("doc updates for an existing doc", function() {
			beforeEach(function() {
				...
				this.updateMerger.p.processDoc = sinon.stub().yields();
				return this.updateMerger.mergeUpdate(this.user_id, this.project_id, this.existingDocPath, this.updateRequest, this.source, this.callback);
			});
			...
			it('should process update as doc', function() {
				return this.updateMerger.p.processDoc
					.calledWith(this.project_id, this.user_id, this.fsPath, this.existingDocPath, this.source)
					.should.equal(true);
			});
		...
		return describe("file updates for an existing file", function() {
			beforeEach(function() {
				this.FileTypeManager.getType = sinon.stub();
				this.updateMerger.p.processFile = sinon.stub().yields();
				return this.updateMerger.mergeUpdate(this.user_id, this.project_id, this.existingFilePath, this.updateRequest, this.source, this.callback);
			});
			...
			it('should process update as file', function() {
				return this.updateMerger.p.processFile
					.calledWith(this.project_id, this.fsPath, this.existingFilePath, this.source, this.user_id)
					.should.equal(true);
			});
	...
	describe('deleteUpdate', function() {
		beforeEach(function() {
			...
			return this.updateMerger.deleteUpdate(this.user_id, this.project_id, this.docPath, this.source, this.callback);
		});
	...
	return describe('private methods', function() {
		describe('processDoc', function() {
			beforeEach(function() {
				this.docLines = "\\documentclass{article}\n\\usepackage[utf8]{inputenc}\n\n\\title{42}\n\\author{Jane Doe}\n\\date{June 2011}";
				this.updateMerger.p.readFileIntoTextArray = sinon.stub().yields(null, this.docLines);
				this.EditorController.upsertDocWithPath = sinon.stub().yields();

				return this.updateMerger.p.processDoc(this.project_id, this.user_id, this.fsPath, this.docPath, this.source, this.callback);
			});
			...
			return it('should upsert the doc in the editor controller', function() {
				return this.EditorController.upsertDocWithPath
					.calledWith(this.project_id, this.docPath, this.docLines, this.source, this.user_id)
					.should.equal(true);
			});
		});
		...
		return describe('processFile', function() {
			beforeEach(function() {
				...
				return this.updateMerger.p.processFile(this.project_id, this.fsPath, this.filePath, this.source, this.user_id, this.callback);
			});

			return it('should upsert the file in the editor controller', function() {
				return this.EditorController.upsertFileWithPath
					.calledWith(this.project_id, this.filePath, this.fsPath, null, this.source, this.user_id)
					.should.equal(true);
			});
		});
	});
