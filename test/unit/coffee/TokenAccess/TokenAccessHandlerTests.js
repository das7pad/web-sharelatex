...
const modulePath = path.join(__dirname, "../../../../app/js/Features/TokenAccess/TokenAccessHandler");
const {
    expect
} = require("chai");
const {
...
describe("TokenAccessHandler", function() {
	...
	beforeEach(function() {
		...
		this.req = {};
		return this.TokenAccessHandler = SandboxedModule.require(modulePath, { requires: {
	...
	describe('findProjectWithReadOnlyToken', function() {
		...
		it('should call Project.findOne', function(done) {
			return this.TokenAccessHandler.findProjectWithReadOnlyToken(this.token, (err, project) => {
				expect(this.Project.findOne.callCount).to.equal(1);
				expect(this.Project.findOne.calledWith({
		...
		it('should return projectExists flag as true', function(done) {
			return this.TokenAccessHandler.findProjectWithReadOnlyToken(this.token, function(err, project, projectExists) {
		...
		describe('when Project.findOne produces an error', function() {
			beforeEach(function() {
				return this.Project.findOne = sinon.stub().callsArgWith(2, new Error('woops'));
			});
		...
		describe('when project does not have tokenBased access level', function() {
			beforeEach(function() {
				...
				return this.Project.findOne = sinon.stub().callsArgWith(2, null, this.project, true);
			});

			it('should not return a project', function(done) {
				return this.TokenAccessHandler.findProjectWithReadOnlyToken(this.token, function(err, project) {
					expect(err).to.not.exist;
					expect(project).to.not.exist;
					return done();
				});
	...
	describe('findProjectWithReadAndWriteToken', function() {
		...
		it('should call Project.findOne', function(done) {
			return this.TokenAccessHandler.findProjectWithReadAndWriteToken(this.token, (err, project) => {
				expect(this.Project.findOne.callCount).to.equal(1);
				expect(this.Project.findOne.calledWith({
		...
		it('should return projectExists flag as true', function(done) {
			return this.TokenAccessHandler.findProjectWithReadAndWriteToken(this.token, function(err, project, projectExists) {
				expect(projectExists).to.equal(true);
				return done();
			});
		});
		...
		describe('when project does not have tokenBased access level', function() {
			...
			it('should not return a project', function(done) {
				return this.TokenAccessHandler.findProjectWithReadAndWriteToken(this.token, function(err, project) {
					expect(err).to.not.exist;
					expect(project).to.not.exist;
					return done();
				});
	...
	describe('findProjectWithHigherAccess', function() {
		describe('when user does have higher access', function() {
			beforeEach(function() {
				...
				return this.CollaboratorsHandler.isUserInvitedMemberOfProject = sinon.stub()
					.callsArgWith(2, null, true);
			});

			it('should call Project.findOne', function(done) {
				return this.TokenAccessHandler.findProjectWithHigherAccess(this.token, this.userId, (err, project) => {
					expect(this.Project.findOne.callCount).to.equal(1);
					expect(this.Project.findOne.calledWith({
						'tokens.readOnly': this.token
					})).to.equal(true);
			...
			it('should call isUserInvitedMemberOfProject', function(done) {
				return this.TokenAccessHandler.findProjectWithHigherAccess(this.token, this.userId, (err, project) => {
					expect(this.CollaboratorsHandler.isUserInvitedMemberOfProject.callCount)
						.to.equal(1);
					expect(this.CollaboratorsHandler.isUserInvitedMemberOfProject.calledWith(
						this.userId, this.project._id
					)).to.equal(true);
					return done();
				});
			});
	...
	describe('addReadOnlyUserToProject', function() {
		...
		it('should call Project.update', function(done) {
			return this.TokenAccessHandler.addReadOnlyUserToProject(this.userId, this.projectId, err => {
				expect(this.Project.update.callCount).to.equal(1);
				expect(this.Project.update.calledWith({
					_id: this.projectId
				})).to.equal(true);
				expect(this.Project.update.lastCall.args[1]['$addToSet'])
		...
		return describe('when Project.update produces an error', function() {
			beforeEach(function() {
				return this.Project.update = sinon.stub().callsArgWith(2, new Error('woops'));
			});
	...
	describe('addReadAndWriteUserToProject', function() {
		...
		it('should call Project.update', function(done) {
			return this.TokenAccessHandler.addReadAndWriteUserToProject(this.userId, this.projectId, err => {
				expect(this.Project.update.callCount).to.equal(1);
				expect(this.Project.update.calledWith({
					_id: this.projectId
				})).to.equal(true);
	...
	describe('grantSessionTokenAccess', function() {
		...
		return it('should add the token to the session', function(done) {
			this.TokenAccessHandler.grantSessionTokenAccess(this.req, this.projectId, this.token);
			expect(this.req.session.anonTokenAccess[this.projectId.toString()])
				.to.equal(this.token);
			return done();
		});
	});
	...
	describe('isValidToken', function() {
		...
		describe('when a read-only project is found', function() {
			...
			it('should try to find projects with both kinds of token', function(done) {
				return this.TokenAccessHandler.isValidToken(this.projectId, this.token, (err, allowed) => {
			...
			return it('should allow read-only access', function(done) {
				return this.TokenAccessHandler.isValidToken(this.projectId, this.token, (err, rw, ro) => {
					expect(err).to.not.exist;
		...
		return describe('with nothing', function() {
			...
			it('should not call findProjectWithReadOnlyToken', function(done) {
				return this.TokenAccessHandler.isValidToken(this.projectId, null, (err, allowed) => {
					expect(this.TokenAccessHandler.findProjectWithReadOnlyToken.callCount)
						.to.equal(0);
					return done();
				});
			...
			return it('should not allow any access', function(done) {
				return this.TokenAccessHandler.isValidToken(this.projectId, null, (err, rw, ro) => {
					expect(err).to.not.exist;
					expect(rw).to.equal(false);
					expect(ro).to.equal(false);
					return done();
	...
	describe('protectTokens', function() {
		beforeEach(function() {
			return this.project = {tokens: {readAndWrite: 'rw', readOnly: 'ro', readAndWritePrefix: 'pre'}};});
	...
	describe('getDocPublishedInfo', function() {
		...
		describe('when v1 api not set', function() {
			beforeEach(function() {
				return this.TokenAccessHandler.getV1DocPublishedInfo(this.token, this.callback);
			});

			return it('should not check access and return default info', function() {
				expect(this.V1Api.request.called).to.equal(false);
				return expect(this.callback.calledWith(null, {
					allow: true
				})).to.equal(true);
			});
		});
		...
		return describe('when v1 api is set', function() {
			...
			describe('on V1Api.request success', function() {
				beforeEach(function() {
					this.V1Api.request = sinon.stub().callsArgWith(1, null, null, 'mock-data');
					return this.TokenAccessHandler.getV1DocPublishedInfo(this.token, this.callback);
				});

				return it('should return response body', function() {
					expect(this.V1Api.request.calledWith({ url: `/api/v1/sharelatex/docs/${this.token}/is_published` })).to.equal(true);
					return expect(this.callback.calledWith(null, 'mock-data')).to.equal(true);
				});
			});
	...
	return describe('getV1DocInfo', function() {
		...
		describe('when v1 api not set', function() {
			beforeEach(function() {
				return this.TokenAccessHandler.getV1DocInfo(this.token, this.v2UserId, this.callback);
			});
		...
		return describe('when v1 api is set', function() {
			...
			describe('on UserGetter.getUser success', function() {
				...
				return it('should get user', function() {
					return expect(this.UserGetter.getUser.calledWith(this.v2UserId)).to.equal(true);
				});
			});
			...
			describe('on V1Api.request success', function() {
				...
				return it('should return response body', function() {
					expect(this.V1Api.request.calledWith({ url: `/api/v1/sharelatex/users/${this.v1UserId}/docs/${this.token}/info` })).to.equal(true);
					return expect(this.callback.calledWith(null, 'mock-data')).to.equal(true);
				});
			});
