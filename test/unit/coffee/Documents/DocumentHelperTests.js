...
describe("DocumentHelper", function() {
	...
	describe("getTitleFromTexContent", function() {
		...
		it("should return the title", function() {
			...
			return expect(this.DocumentHelper.getTitleFromTexContent(document)).to.equal("foo");
		});
		...
		it("should return null if there is no title", function() {
			const document = "\\begin{document}\n\\end{document}";
			return expect(this.DocumentHelper.getTitleFromTexContent(document)).to.eql(null);
		});
		...
		it("should parse out formatting elements from the title", function() {
			...
			return expect(this.DocumentHelper.getTitleFromTexContent(document)).to.equal("Second Year LaTeX Exercise");
		});

		it("should ignore junk after the title", function() {
			const document = "\\title{wombat} potato";
			return expect(this.DocumentHelper.getTitleFromTexContent(document)).to.equal("wombat");
		});

		it("should ignore junk before the title", function() {
			const document = "% this is something that v1 relied on, even though it seems odd \\title{wombat}";
			return expect(this.DocumentHelper.getTitleFromTexContent(document)).to.equal("wombat");
		});
	...
	describe("detex", function() {
		...
		it('leaves basic math', function() {
			return expect(this.DocumentHelper.detex('$\\cal{O}(n^2)$')).to.equal('O(n^2)');
		});
	
		return it('removes line spacing commands', function() {
			return expect(this.DocumentHelper.detex('a \\\\[1.50cm] b')).to.equal('a b');
		});
	});
	...
	return describe("contentHasDocumentclass", function() {
		it("should return true if the content has a documentclass", function() {
			...
			return expect(this.DocumentHelper.contentHasDocumentclass(document)).to.equal(true);
		});
		...
		it("should not allow non-whitespace before the documentclass", function() {
			const document = ["% line", "% line", "% line", "    asdf \\documentclass"];
			return expect(this.DocumentHelper.contentHasDocumentclass(document)).to.equal(false);
		});
