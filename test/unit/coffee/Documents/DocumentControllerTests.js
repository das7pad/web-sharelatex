...
describe("DocumentController", function() {
	beforeEach(function() {
		this.DocumentController = SandboxedModule.require(modulePath, { requires: {
			"logger-sharelatex": {
				log() {},
				err() {}
			},
	...
	describe("getDocument", function() {
		...
		describe("when the project exists without project history enabled", function() {
			...
			describe("when the document exists", function() {
				beforeEach(function() {
					...
					this.ProjectLocator.findElement = sinon.stub().callsArgWith(1, null, this.doc, {fileSystem: this.pathname});
					this.ProjectEntityHandler.getDoc = sinon.stub().callsArgWith(2, null, this.doc_lines, this.rev, this.version, this.ranges);
					return this.DocumentController.getDocument(this.req, this.res, this.next);
				});
				...
				it("should get the pathname of the document", function() {
					return this.ProjectLocator.findElement
						.calledWith({project: this.project, element_id: this.doc_id, type: 'doc'})
						.should.equal(true);
				});
			...
			return describe("when the document doesn't exist", function() {
				beforeEach(function() {
					this.ProjectLocator.findElement = sinon.stub().callsArgWith(1, new Errors.NotFoundError("not found"));
					return this.DocumentController.getDocument(this.req, this.res, this.next);
				});

				return it("should call next with the NotFoundError", function() {
					return this.next.calledWith(new Errors.NotFoundError("not found"))
						.should.equal(true);
				});
			});
		});
		...
		describe("when project exists with project history enabled", function() {
			beforeEach(function() {
				...
				this.project = {_id: this.project_id, overleaf: {history: {id: this.projectHistoryId}}};
	...
	return describe("setDocument", function() {
		...
		describe("when the document exists", function() {
			beforeEach(function() {
				...
				return this.DocumentController.setDocument(this.req, this.res, this.next);
			});
		...
		return describe("when the document doesn't exist", function() {
			beforeEach(function() {
				this.ProjectEntityUpdateHandler.updateDocLines = sinon.stub().yields(new Errors.NotFoundError("document does not exist"));
				this.req.body =
					{lines: this.doc_lines};
				return this.DocumentController.setDocument(this.req, this.res, this.next);
			});
