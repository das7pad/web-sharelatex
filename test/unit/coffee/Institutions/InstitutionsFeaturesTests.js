...
const modulePath = require('path').join(__dirname, '../../../../app/js/Features/Institutions/InstitutionsFeatures.js');
...
describe('InstitutionsFeatures', function() {
	...
	beforeEach(function() {
		...
		this.InstitutionsFeatures = SandboxedModule.require(modulePath, { requires: {
			'./InstitutionsGetter': this.InstitutionsGetter,
			'../Subscription/PlansLocator': this.PlansLocator,
			'settings-sharelatex': { institutionPlanCode: this.institutionPlanCode
		},
			'logger-sharelatex': {
				log() {},
				err() {}
	...
	describe("hasLicence", function() {
			it('should handle error', function(done){
				...
				return this.InstitutionsFeatures.hasLicence(this.userId, function(error, hasLicence) {
			...
			it('should return false if user has no confirmed affiliations', function(done) {
				const institutions = [];
				this.InstitutionsGetter.getConfirmedInstitutions.yields(null, institutions);
				return this.InstitutionsFeatures.hasLicence(this.userId, function(error, hasLicence) {
	...
	describe("getInstitutionsFeatures", function() {
			beforeEach(function() {
				...
				return this.PlansLocator.findLocalPlanInSettings.withArgs(this.institutionPlanCode).returns(this.testFeatures);
			});

			it('should handle error', function(done){
				this.InstitutionsFeatures.getInstitutionsPlan.yields(new Error('Nope'));
				return this.InstitutionsFeatures.getInstitutionsFeatures(this.userId, function(error, features) {
					expect(error).to.exist;
					return done();
				});
			});
			...
			return it('should return feaures if user has affiliations plan code', function(done) {
				this.InstitutionsFeatures.getInstitutionsPlan.yields(null, this.institutionPlanCode);
				return this.InstitutionsFeatures.getInstitutionsFeatures(this.userId, (error, features) => {
					expect(error).to.not.exist;
	...
	return describe("getInstitutionsPlan", function() {
			...
			it('should handle error', function(done){
				...
				return this.InstitutionsFeatures.getInstitutionsPlan(this.userId, function(error) {
					expect(error).to.exist;
					return done();
				});
			});
			...
			it('should return no plan if user has no licence', function(done) {
				...
				return this.InstitutionsFeatures.getInstitutionsPlan(this.userId, function(error, plan) {
					expect(error).to.not.exist;
			...
			return it('should return plan if user has licence', function(done) {
				...
				return this.InstitutionsFeatures.getInstitutionsPlan(this.userId, (error, plan) => {
					expect(error).to.not.exist;
