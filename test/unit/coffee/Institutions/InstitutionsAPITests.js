...
const modulePath = path.join(__dirname, "../../../../app/js/Features/Institutions/InstitutionsAPI");
({
    expect
} = require("chai"));
...
describe("InstitutionsAPI", function() {
	...
	beforeEach(function() {
		...
		this.ipMatcherNotification = {read: (this.markAsReadIpMatcher = sinon.stub().callsArgWith(1, null))};
		this.InstitutionsAPI = SandboxedModule.require(modulePath, { requires: {
	...
	describe('getInstitutionAffiliations', function() {
		it('get affiliations', function(done){
			...
			return this.InstitutionsAPI.getInstitutionAffiliations(this.institutionId, (err, body) => {
	...
	describe('getInstitutionLicences', () => it('get licences', function(done){
        this.institutionId = 123;
        const responseBody = {"lag":"monthly","data":[{"key":"users","values":[{"x":"2018-01-01","y":1}]}]};
        this.request.yields(null, { statusCode: 200 }, responseBody);
        const startDate = '1417392000';
        const endDate = '1420848000';
        return this.InstitutionsAPI.getInstitutionLicences(this.institutionId, startDate, endDate, 'monthly', (err, body) => {
            should.not.exist(err);
            this.request.calledOnce.should.equal(true);
            const requestOptions = this.request.lastCall.args[0];
	...
	describe('getUserAffiliations', function() {
		it('get affiliations', function(done){
			...
			return this.InstitutionsAPI.getUserAffiliations(this.stubbedUser._id, (err, body) => {
				should.not.exist(err);
				this.request.calledOnce.should.equal(true);
				const requestOptions = this.request.lastCall.args[0];
		...
		it('handle error', function(done){
			...
			return this.InstitutionsAPI.getUserAffiliations(this.stubbedUser._id, err => {
				should.exist(err);
	...
	describe('addAffiliation', function() {
		...
		it('add affiliation', function(done){
			...
			return this.InstitutionsAPI.addAffiliation(this.stubbedUser._id, this.newEmail, affiliationOptions, err=> {
				should.not.exist(err);
				this.request.calledOnce.should.equal(true);
				const requestOptions = this.request.lastCall.args[0];
				const expectedUrl = `v1.url/api/v2/users/${this.stubbedUser._id}/affiliations`;
		...
		return it('handle error', function(done){
			...
			return this.InstitutionsAPI.addAffiliation(this.stubbedUser._id, this.newEmail, {}, err=> {
				should.exist(err);
	...
	describe('removeAffiliation', function() {
		...
		it('remove affiliation', function(done){
			return this.InstitutionsAPI.removeAffiliation(this.stubbedUser._id, this.newEmail, err=> {
				should.not.exist(err);
				this.request.calledOnce.should.equal(true);
				const requestOptions = this.request.lastCall.args[0];
	...
	describe('deleteAffiliations', function() {
		it('delete affiliations', function(done){
			...
			return this.InstitutionsAPI.deleteAffiliations(this.stubbedUser._id, err => {
				should.not.exist(err);
				this.request.calledOnce.should.equal(true);
				const requestOptions = this.request.lastCall.args[0];
				const expectedUrl = `v1.url/api/v2/users/${this.stubbedUser._id}/affiliations`;
	...
	return describe('endorseAffiliation', function() {
		...
		return it('endorse affiliation', function(done){
			return this.InstitutionsAPI.endorseAffiliation(this.stubbedUser._id, this.newEmail, 'Student','Physics', err=> {
				should.not.exist(err);
				this.request.calledOnce.should.equal(true);
				const requestOptions = this.request.lastCall.args[0];
