...
describe("UserCreator", function() {
	...
	beforeEach(function() {
		...
		this.UserCreator = SandboxedModule.require(modulePath, { requires: {
			"../../models/User": { User:this.UserModel
		},
			"logger-sharelatex":{ log: sinon.stub(), err: sinon.stub() },
			'metrics-sharelatex': {timeAsyncMethod(){}},
			"../Institutions/InstitutionsAPI": { addAffiliation: this.addAffiliation
		}
		}
	}
		);
	...
	return describe("createNewUser", function() {
		...
		it("should set emails attribute", function(done){
			return this.UserCreator.createNewUser({email: this.email}, (err, user)=> {
		...
		return it("should not add affiliation if skipping", function(done){
			...
			return this.UserCreator.createNewUser(attributes, options, (err, user) => {
				return process.nextTick(() => {
					sinon.assert.notCalled(this.addAffiliation);
					return done();
				});
