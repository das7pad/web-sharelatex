...
const modulePath = path.join(__dirname, "../../../../app/js/Features/User/UserUpdater");
const {
    expect
} = require("chai");
...
describe("UserUpdater", function() {
	...
	beforeEach(function() {
		...
		this.UserUpdater = SandboxedModule.require(modulePath, { requires: {
			...
			'../Subscription/FeaturesUpdater': { refreshFeatures: this.refreshFeatures
		},
	...
	describe('changeEmailAddress', function() {
		beforeEach(function() {
			...
			return this.UserUpdater.removeEmailAddress = sinon.stub().callsArgWith(2);
		});

		it('change email', function(done){
			return this.UserUpdater.changeEmailAddress(this.stubbedUser._id, this.newEmail, err=> {
				should.not.exist(err);
				this.UserUpdater.addEmailAddress.calledWith(
					this.stubbedUser._id, this.newEmail
				).should.equal(true);
				this.UserUpdater.setDefaultEmailAddress.calledWith(
					this.stubbedUser._id, this.newEmail
				).should.equal(true);
				this.UserUpdater.removeEmailAddress.calledWith(
					this.stubbedUser._id, this.stubbedUser.email
				).should.equal(true);
				return done();
			});
		});
		...
		it('validates email', function(done){
			return this.UserUpdater.changeEmailAddress(this.stubbedUser._id, 'foo', err=> {
				should.exist(err);
				return done();
			});
		});
	...
	describe('addEmailAddress', function() {
		...
		it('add email', function(done){
			return this.UserUpdater.addEmailAddress(this.stubbedUser._id, this.newEmail, err=> {
				this.UserGetter.ensureUniqueEmailAddress.called.should.equal(true);
				should.not.exist(err);
				const reversedHostname = this.newEmail.split('@')[1].split('').reverse().join('');
				this.UserUpdater.updateUser.calledWith(
					this.stubbedUser._id,
					{$push: { emails: { email: this.newEmail, createdAt: sinon.match.date, reversedHostname } }}
				).should.equal(true);
				return done();
			});
		});
		...
		it('add affiliation', function(done){
			...
			return this.UserUpdater.addEmailAddress(this.stubbedUser._id, this.newEmail, affiliationOptions, err=> {
				should.not.exist(err);
				this.addAffiliation.calledOnce.should.equal(true);
				const {
                    args
                } = this.addAffiliation.lastCall;
		...
		it('handle error', function(done){
			this.UserUpdater.updateUser = sinon.stub().callsArgWith(2, new Error('nope'));
		...
		return it('validates email', function(done){
			return this.UserUpdater.addEmailAddress(this.stubbedUser._id, 'bar', err=> {
				should.exist(err);
				return done();
			});
		});
	...
	describe('removeEmailAddress', function() {
		beforeEach(function() {
			return this.UserUpdater.updateUser = sinon.stub().callsArgWith(2, null, {nMatched: 1});
		});

		it('remove email', function(done){
			return this.UserUpdater.removeEmailAddress(this.stubbedUser._id, this.newEmail, err=> {
				should.not.exist(err);
				this.UserUpdater.updateUser.calledWith(
		...
		it('remove affiliation', function(done){
			return this.UserUpdater.removeEmailAddress(this.stubbedUser._id, this.newEmail, err=> {
				...
				const {
                    args
                } = this.removeAffiliation.lastCall;
				args[0].should.equal(this.stubbedUser._id);
				args[1].should.equal(this.newEmail);
				return done();
			});
		...
		return it('validates email', function(done){
			return this.UserUpdater.removeEmailAddress(this.stubbedUser._id, 'baz', err=> {
				should.exist(err);
				return done();
			});
		});
	...
	describe('setDefaultEmailAddress', function() {
		...
		it('set default', function(done){
			...
			return this.UserUpdater.setDefaultEmailAddress(this.stubbedUser._id, this.newEmail, err=> {
				should.not.exist(err);
				this.UserUpdater.updateUser.calledWith(
		...
		it('set changed the email in newsletter', function(done){
			...
			return this.UserUpdater.setDefaultEmailAddress(this.stubbedUser._id, this.newEmail, err=> {
				should.not.exist(err);
				this.NewsletterManager.changeEmail.calledWith(
					this.stubbedUser.email, this.newEmail
				).should.equal(true);
				return done();
			});
		});
		...
		return it('validates email', function(done){
			return this.UserUpdater.setDefaultEmailAddress(this.stubbedUser._id, '.edu', err=> {
				should.exist(err);
				return done();
			});
		});
	...
	return describe('confirmEmail', function() {
		beforeEach(function() {
			return this.UserUpdater.updateUser = sinon.stub().callsArgWith(2, null, {n: 1});
		});

		it('should update the email record', function(done){
			return this.UserUpdater.confirmEmail(this.stubbedUser._id, this.newEmail, err=> {
				should.not.exist(err);
				this.UserUpdater.updateUser.calledWith(
					{ _id: this.stubbedUser._id, 'emails.email': this.newEmail },
		...
		it('add affiliation', function(done){
			return this.UserUpdater.confirmEmail(this.stubbedUser._id, this.newEmail, err=> {
				should.not.exist(err);
				this.addAffiliation.calledOnce.should.equal(true);
				sinon.assert.calledWith(this.addAffiliation, this.stubbedUser._id, this.newEmail, { confirmedAt: new Date() } );
				return done();
			});
		});
		...
		return it('refresh features', function(done){
			return this.UserUpdater.confirmEmail(this.stubbedUser._id, this.newEmail, err=> {
				should.not.exist(err);
				sinon.assert.calledWith(this.refreshFeatures, this.stubbedUser._id, true);
				return done();
			});
		});
	});
