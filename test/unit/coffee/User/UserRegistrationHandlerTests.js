...
const modulePath = path.join(__dirname, '../../../../app/js/Features/User/UserRegistrationHandler');
...
describe("UserRegistrationHandler", function() {
	...
	beforeEach(function() {
		...
		this.handler = SandboxedModule.require(modulePath, { requires: {
			...
			"settings-sharelatex": (this.settings = {siteUrl: "http://sl.example.com"}),
			"../Helpers/EmailHelper": EmailHelper
		}
	}
		);

		return this.passingRequest = {email:"something@email.com", password:"123"};});


	describe('validate Register Request', function() {
		it('allows passing validation through', function() {
			const result = this.handler._registrationRequestIsValid(this.passingRequest);
			return result.should.equal(true);
		});

		describe('failing email validation', function() {
			beforeEach(function() {
				return this.AuthenticationManager.validateEmail.returns({ message: 'email not set' });
			});

			return it('does not allow through', function() {
				const result = this.handler._registrationRequestIsValid(this.passingRequest);
				return result.should.equal(false);
			});
		});

		return describe('failing password validation', function() {
			beforeEach(function() {
				return this.AuthenticationManager.validatePassword.returns({ message: 'password is too short' });
			});
	...
	describe("registerNewUser", function() {
		...
		describe("holdingAccount", function(done){
			...
			beforeEach(function() {
				...
				return this.UserGetter.getUserByAnyEmail.callsArgWith(1, null, this.user);
			});
			...
			return it("should set holding account to false", function(done){
				return this.handler.registerNewUser(this.passingRequest, err=> {
		...
		describe("invalidRequest", function() {
			...
			return it("should return email registered in the error if there is a non holdingAccount there", function(done){
				this.UserGetter.getUserByAnyEmail.callsArgWith(1, null, (this.user = {holdingAccount:false}));
				return this.handler.registerNewUser(this.passingRequest, (err, user)=> {
		...
		describe("validRequest", function() {
			...
			it("should create a new user", function(done){
				return this.handler.registerNewUser(this.passingRequest, err=> {
					this.UserCreator.createNewUser.calledWith({email:this.passingRequest.email, holdingAccount:false, first_name:this.passingRequest.first_name, last_name:this.passingRequest.last_name}).should.equal(true);
					return done();
				});
			});
			...
			it('lower case email', function(done){
				...
				return this.handler.registerNewUser(this.passingRequest, err=> {
					this.UserCreator.createNewUser.args[0][0].email.should.equal("some@email.com");
					return done();
				});
			});
			...
			it("should set the password", function(done){
				return this.handler.registerNewUser(this.passingRequest, err=> {
					this.AuthenticationManager.setUserPassword.calledWith(this.user._id, this.passingRequest.password).should.equal(true);
					return done();
				});
			});			
			...
			it("should add the user to the newsletter if accepted terms", function(done){
				...
				return this.handler.registerNewUser(this.passingRequest, err=> {
					this.NewsLetterManager.subscribe.calledWith(this.user).should.equal(true);
					return done();
				});
			});
			...
			it("should not add the user to the newsletter if not accepted terms", function(done){
				return this.handler.registerNewUser(this.passingRequest, err=> {
					this.NewsLetterManager.subscribe.calledWith(this.user).should.equal(false);
					return done();
				});
			});

			return it("should track the registration event", function(done){
				return this.handler.registerNewUser(this.passingRequest, err=> {
		...
		return it("should call the ReferalAllocator", done => done());
	});

	return describe("registerNewUserAndSendActivationEmail", function() {
		beforeEach(function() {
			this.email = "email@example.com";
			this.crypto.randomBytes = sinon.stub().returns({toString: () => { return this.password = "mock-password"; }});
			this.OneTimeTokenHandler.getNewToken.yields(null, (this.token = "mock-token"));
		...
		describe("with a new user", function() {
			beforeEach(function() {
				...
				return this.handler.registerNewUserAndSendActivationEmail(this.email, this.callback);
			});
			...
			return it("should return the user", function() {
				return this.callback
					.calledWith(null, this.user, `${this.settings.siteUrl}/user/activate?token=${this.token}&user_id=${this.user_id}`)
					.should.equal(true);
			});
		});

		return describe("with a user that already exists", function() {
			beforeEach(function() {
				this.handler.registerNewUser.callsArgWith(1, new Error("EmailAlreadyRegistered"), this.user);
				return this.handler.registerNewUserAndSendActivationEmail(this.email, this.callback);
			});
				
			return it("should still generate a new password token and email", function() {
