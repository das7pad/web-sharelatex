...
const modulePath = "../../../../app/js/Features/Collaborators/CollaboratorsInviteController.js";
...
describe("CollaboratorsInviteController", function() {
	beforeEach(function() {
		...
		this.CollaboratorsInviteController = SandboxedModule.require(modulePath, { requires: {
			...
			'logger-sharelatex': (this.logger = {err: sinon.stub(), error: sinon.stub(), log: sinon.stub()}),
			"../Editor/EditorRealTimeController": (this.EditorRealTimeController = {emitToRoom: sinon.stub()}),
	...
	describe('getAllInvites', function() {
		...
		describe('when all goes well', function() {
			...
			beforeEach(function() {
				this.CollaboratorsInviteHandler.getAllInvites = sinon.stub().callsArgWith(1, null, this.fakeInvites);
				return this.CollaboratorsInviteController.getAllInvites(this.req, this.res, this.next);
			});
			...
			it('should produce a list of invite objects', function() {
				...
				return this.res.json.calledWith({invites: this.fakeInvites}).should.equal(true);
			});
			...
			return it('should have called CollaboratorsInviteHandler.getAllInvites', function() {
				...
				return this.CollaboratorsInviteHandler.getAllInvites.calledWith(this.project_id).should.equal(true);
			});
		});
		...
		return describe('when CollaboratorsInviteHandler.getAllInvites produces an error', function() {
			...
			beforeEach(function() {
				this.CollaboratorsInviteHandler.getAllInvites = sinon.stub().callsArgWith(1, new Error('woops'));
				return this.CollaboratorsInviteController.getAllInvites(this.req, this.res, this.next);
			});
	...
	describe('inviteToProject', function() {
		...
		beforeEach(function() {
			...
			this.req.params =
				{Project_id: this.project_id};
			this.current_user =
				{_id: (this.current_user_id = "current-user-id")};
			this.req.session =
			...
			this.LimitationsManager.canAddXCollaborators = sinon.stub().callsArgWith(2, null, true);
			this.CollaboratorsInviteHandler.inviteToProject = sinon.stub().callsArgWith(4, null, this.invite);
		...
		describe('when all goes well', function() {
			...
			beforeEach(function() {
				this.CollaboratorsInviteController._checkShouldInviteEmail = sinon.stub().callsArgWith(1, null, true);
				this.CollaboratorsInviteController._checkRateLimit = sinon.stub().yields(null, true);
				this.LimitationsManager.canAddXCollaborators = sinon.stub().callsArgWith(2, null, true);
				return this.CollaboratorsInviteController.inviteToProject(this.req, this.res, this.next);
			});

			it('should produce json response', function() {
				this.res.json.callCount.should.equal(1);
				return ({invite: this.invite}).should.deep.equal(this.res.json.firstCall.args[0]);
			});

			it('should have called canAddXCollaborators', function() {
				this.LimitationsManager.canAddXCollaborators.callCount.should.equal(1);
				return this.LimitationsManager.canAddXCollaborators.calledWith(this.project_id).should.equal(true);
			});

			it('should have called _checkShouldInviteEmail', function() {
				this.CollaboratorsInviteController._checkShouldInviteEmail.callCount.should.equal(1);
				return this.CollaboratorsInviteController._checkShouldInviteEmail.calledWith(this.targetEmail).should.equal(true);
			});

			it('should have called inviteToProject', function() {
				this.CollaboratorsInviteHandler.inviteToProject.callCount.should.equal(1);
				return this.CollaboratorsInviteHandler.inviteToProject.calledWith(this.project_id,this.current_user,this.targetEmail,this.privileges).should.equal(true);
			});
		...
		describe('when the user is not allowed to add more collaborators', function() {
			...
			beforeEach(function() {
				this.CollaboratorsInviteController._checkShouldInviteEmail = sinon.stub().callsArgWith(1, null, true);
				this.CollaboratorsInviteController._checkRateLimit = sinon.stub().yields(null, true);
				this.LimitationsManager.canAddXCollaborators = sinon.stub().callsArgWith(2, null, false);
				return this.CollaboratorsInviteController.inviteToProject(this.req, this.res, this.next);
			});

			it('should produce json response without an invite', function() {
				this.res.json.callCount.should.equal(1);
				return ({invite: null}).should.deep.equal(this.res.json.firstCall.args[0]);
			});

			it('should not have called _checkShouldInviteEmail', function() {
				this.CollaboratorsInviteController._checkShouldInviteEmail.callCount.should.equal(0);
				return this.CollaboratorsInviteController._checkShouldInviteEmail.calledWith(this.sendingUser, this.targetEmail).should.equal(false);
			});

			return it('should not have called inviteToProject', function() {
				return this.CollaboratorsInviteHandler.inviteToProject.callCount.should.equal(0);
			});
		});
		...
		describe('when canAddXCollaborators produces an error', function() {
			...
			beforeEach(function() {
				this.CollaboratorsInviteController._checkShouldInviteEmail = sinon.stub().callsArgWith(1, null, true);
				this.CollaboratorsInviteController._checkRateLimit = sinon.stub().yields(null, true);
				this.LimitationsManager.canAddXCollaborators = sinon.stub().callsArgWith(2, this.err);
				return this.CollaboratorsInviteController.inviteToProject(this.req, this.res, this.next);
			});
		...
		describe('when inviteToProject produces an error', function() {
			...
			beforeEach(function() {
				...
				this.CollaboratorsInviteController._checkRateLimit = sinon.stub().yields(null, true);
				this.err = new Error('woops');
				this.CollaboratorsInviteHandler.inviteToProject = sinon.stub().callsArgWith(4, this.err);
				return this.CollaboratorsInviteController.inviteToProject(this.req, this.res, this.next);
			});
		...
		describe('when _checkShouldInviteEmail disallows the invite', function() {
			...
			beforeEach(function() {
				this.CollaboratorsInviteController._checkShouldInviteEmail = sinon.stub().callsArgWith(1, null, false);
				this.CollaboratorsInviteController._checkRateLimit = sinon.stub().yields(null, true);
				this.LimitationsManager.canAddXCollaborators = sinon.stub().callsArgWith(2, null, true);
				return this.CollaboratorsInviteController.inviteToProject(this.req, this.res, this.next);
			});
			...
			it('should produce json response with no invite, and an error property', function() {
				this.res.json.callCount.should.equal(1);
				return ({invite: null, error: 'cannot_invite_non_user'}).should.deep.equal(this.res.json.firstCall.args[0]);
			});
		...
		describe('when _checkShouldInviteEmail produces an error', function() {
			...
			beforeEach(function() {
				this.CollaboratorsInviteController._checkShouldInviteEmail = sinon.stub().callsArgWith(1, new Error('woops'));
				this.CollaboratorsInviteController._checkRateLimit = sinon.stub().yields(null, true);
				this.LimitationsManager.canAddXCollaborators = sinon.stub().callsArgWith(2, null, true);
				return this.CollaboratorsInviteController.inviteToProject(this.req, this.res, this.next);
			});
		...
		describe('when the user invites themselves to the project', function() {
			...
			it('should reject action, return json response with error code', function() {
				this.res.json.callCount.should.equal(1);
				return ({invite: null, error: 'cannot_invite_self'}).should.deep.equal(this.res.json.firstCall.args[0]);
			});

			it('should not have called canAddXCollaborators', function() {
				return this.LimitationsManager.canAddXCollaborators.callCount.should.equal(0);
			});

			it('should not have called _checkShouldInviteEmail', function() {
				return this.CollaboratorsInviteController._checkShouldInviteEmail.callCount.should.equal(0);
			});
			...
			return it('should not have called emitToRoom', function() {
				return this.EditorRealTimeController.emitToRoom.callCount.should.equal(0);
			});
		});
		...
		return describe('when _checkRateLimit returns false', function() {
			...
			beforeEach(function() {
				this.CollaboratorsInviteController._checkShouldInviteEmail = sinon.stub().callsArgWith(1, null, true);
				this.CollaboratorsInviteController._checkRateLimit = sinon.stub().yields(null, false);
				this.LimitationsManager.canAddXCollaborators = sinon.stub().callsArgWith(2, null, true);
				return this.CollaboratorsInviteController.inviteToProject(this.req, this.res, this.next);
			});
			...
			it('should not call inviteToProject', function() {
				return this.CollaboratorsInviteHandler.inviteToProject.called.should.equal(false);
			});

			return it('should not call emitToRoom', function() {
				return this.EditorRealTimeController.emitToRoom.called.should.equal(false);
			});
		});
	});
	...
	describe("viewInvite", function() {
		...
		beforeEach(function() {
			...
			this.CollaboratorsHandler.isUserInvitedMemberOfProject = sinon.stub().callsArgWith(2, null, false, null);
			this.CollaboratorsInviteHandler.getInviteByToken = sinon.stub().callsArgWith(2, null, this.invite);
			this.ProjectGetter.getProject = sinon.stub().callsArgWith(2, null, this.fakeProject);
		...
		describe('when the token is valid', function() {
			...
			beforeEach(function() {
				return this.CollaboratorsInviteController.viewInvite(this.req, this.res, this.next);
			});

			it('should render the view template', function() {
				this.res.render.callCount.should.equal(1);
				return this.res.render.calledWith('project/invite/show').should.equal(true);
			});
			...
			it('should call CollaboratorsHandler.isUserInvitedMemberOfProject', function() {
				this.CollaboratorsHandler.isUserInvitedMemberOfProject.callCount.should.equal(1);
				return this.CollaboratorsHandler.isUserInvitedMemberOfProject.calledWith(this.current_user_id, this.project_id).should.equal(true);
			});

			it('should call getInviteByToken', function() {
				this.CollaboratorsInviteHandler.getInviteByToken.callCount.should.equal(1);
				return this.CollaboratorsInviteHandler.getInviteByToken.calledWith(this.fakeProject._id, this.invite.token).should.equal(true);
			});

			it('should call User.getUser', function() {
				this.UserGetter.getUser.callCount.should.equal(1);
				return this.UserGetter.getUser.calledWith({_id: this.fakeProject.owner_ref}).should.equal(true);
			});

			return it('should call ProjectGetter.getProject', function() {
				this.ProjectGetter.getProject.callCount.should.equal(1);
				return this.ProjectGetter.getProject.calledWith(this.project_id).should.equal(true);
			});
		});
		...
		describe('when user is already a member of the project', function() {
			...
			beforeEach(function() {
				this.CollaboratorsHandler.isUserInvitedMemberOfProject = sinon.stub().callsArgWith(2, null, true, null);
				return this.CollaboratorsInviteController.viewInvite(this.req, this.res, this.next);
			});

			it('should redirect to the project page', function() {
				this.res.redirect.callCount.should.equal(1);
				return this.res.redirect.calledWith(`/project/${this.project_id}`).should.equal(true);
			});
			...
			it('should not call getInviteByToken', function() {
				return this.CollaboratorsInviteHandler.getInviteByToken.callCount.should.equal(0);
			});
		...
		describe('when isUserInvitedMemberOfProject produces an error', function() {
			...
			beforeEach(function() {
				this.CollaboratorsHandler.isUserInvitedMemberOfProject = sinon.stub().callsArgWith(2, new Error('woops'));
				return this.CollaboratorsInviteController.viewInvite(this.req, this.res, this.next);
			});
		...
		describe('when the getInviteByToken produces an error', function() {
			...
			beforeEach(function() {
				this.err = new Error('woops');
				this.CollaboratorsInviteHandler.getInviteByToken.callsArgWith(2, this.err);
				return this.CollaboratorsInviteController.viewInvite(this.req, this.res, this.next);
			});
		...
		describe('when the getInviteByToken does not produce an invite', function() {
			...
			beforeEach(function() {
				this.CollaboratorsInviteHandler.getInviteByToken.callsArgWith(2, null, null);
				return this.CollaboratorsInviteController.viewInvite(this.req, this.res, this.next);
			});

			it('should render the not-valid view template', function() {
				this.res.render.callCount.should.equal(1);
				return this.res.render.calledWith('project/invite/not-valid').should.equal(true);
			});
		...
		describe('when User.getUser produces an error', function() {
			...
			it('should call getInviteByToken', function() {
				return this.CollaboratorsInviteHandler.getInviteByToken.callCount.should.equal(1);
			});
	...
	describe("resendInvite", function() {
		...
		beforeEach(function() {
			...
			this.res.render = sinon.stub();
			this.res.sendStatus = sinon.stub();
			this.CollaboratorsInviteHandler.resendInvite = sinon.stub().callsArgWith(3, null);
			this.CollaboratorsInviteController._checkRateLimit = sinon.stub().yields(null, true);
			this.callback = sinon.stub();
			return this.next = sinon.stub();
		});
		...
		describe('when resendInvite does not produce an error', function() {
			...
			beforeEach(function() {
				return this.CollaboratorsInviteController.resendInvite(this.req, this.res, this.next);
			});
			...
			it('should have called resendInvite', function() {
				return this.CollaboratorsInviteHandler.resendInvite.callCount.should.equal(1);
			});

			return it('should check the rate limit', function() {
				return this.CollaboratorsInviteController._checkRateLimit.callCount.should.equal(1);
			});
		});
		...
		return describe('when resendInvite produces an error', function() {
			...
			beforeEach(function() {
				this.err = new Error('woops');
				this.CollaboratorsInviteHandler.resendInvite = sinon.stub().callsArgWith(3, this.err);
				return this.CollaboratorsInviteController.resendInvite(this.req, this.res, this.next);
			});
	...
	describe("revokeInvite", function() {
		...
		beforeEach(function() {
			...
			this.res.render = sinon.stub();
			this.res.sendStatus = sinon.stub();
			this.CollaboratorsInviteHandler.revokeInvite = sinon.stub().callsArgWith(2, null);
			this.callback = sinon.stub();
			return this.next = sinon.stub();
		});
		...
		describe('when revokeInvite does not produce an error', function() {
			...
			beforeEach(function() {
				return this.CollaboratorsInviteController.revokeInvite(this.req, this.res, this.next);
			});
			...
			it('should have called revokeInvite', function() {
				return this.CollaboratorsInviteHandler.revokeInvite.callCount.should.equal(1);
			});
		...
		return describe('when revokeInvite produces an error', function() {
			...
			beforeEach(function() {
				this.err = new Error('woops');
				this.CollaboratorsInviteHandler.revokeInvite = sinon.stub().callsArgWith(2, this.err);
				return this.CollaboratorsInviteController.revokeInvite(this.req, this.res, this.next);
			});
	...
	describe("acceptInvite", function() {
		...
		beforeEach(function() {
			...
			this.res.render = sinon.stub();
			this.res.redirect = sinon.stub();
			this.CollaboratorsInviteHandler.acceptInvite = sinon.stub().callsArgWith(3, null);
			this.callback = sinon.stub();
			return this.next = sinon.stub();
		});
		...
		describe('when acceptInvite does not produce an error', function() {
			...
			beforeEach(function() {
				return this.CollaboratorsInviteController.acceptInvite(this.req, this.res, this.next);
			});
		...
		return describe('when revokeInvite produces an error', function() {
			...
			beforeEach(function() {
				this.err = new Error('woops');
				this.CollaboratorsInviteHandler.acceptInvite = sinon.stub().callsArgWith(3, this.err);
				return this.CollaboratorsInviteController.acceptInvite(this.req, this.res, this.next);
			});
			...
			return it('should have called acceptInvite', function() {
				return this.CollaboratorsInviteHandler.acceptInvite.callCount.should.equal(1);
			});
		});
	});
		...
		return describe('when we should be restricting to existing accounts', function() {
			...
			beforeEach(function() {
				...
				return this.call = callback => {
					return this.CollaboratorsInviteController._checkShouldInviteEmail(this.email, callback);
				};
			});
			...
			describe('when user account is present', function() {
				...
				beforeEach(function() {
					...
					return this.UserGetter.getUserByAnyEmail = sinon.stub().callsArgWith(2, null, this.user);
				});
			...
			describe('when user account is absent', function() {
				...
				return it('should have called getUser', function(done) {
					return this.call((err, shouldAllow) => {
						...
						this.UserGetter.getUserByAnyEmail.calledWith(this.email, {_id: 1}).should.equal(true);
						return done();
					});
				});
			});
			...
			return describe('when getUser produces an error', function() {
				...
				beforeEach(function() {
					this.user = null;
					return this.UserGetter.getUserByAnyEmail = sinon.stub().callsArgWith(2, new Error('woops'));
				});
	...
	return describe('_checkRateLimit', function() {
		beforeEach(function() {
			...
			return this.LimitationsManager.allowedNumberOfCollaboratorsForUser.withArgs(this.sendingUserId).yields(null, 17);
		});
		...
		it('should callback with `true` when rate limit under', function(done) {
			...
			return this.CollaboratorsInviteController._checkRateLimit(this.sendingUserId, (err, result)=> {
		...
		it('should call rate limiter with 200 when collaborators is -1', function(done) {
			this.LimitationsManager.allowedNumberOfCollaboratorsForUser.withArgs(this.sendingUserId).yields(null, -1);
			this.RateLimiter.addCount = sinon.stub().callsArgWith(1, null, true);
			return this.CollaboratorsInviteController._checkRateLimit(this.sendingUserId, (err, result)=> {
		...
		return it('should call rate limiter with 10 when user has no collaborators set', function(done) {
			this.LimitationsManager.allowedNumberOfCollaboratorsForUser.withArgs(this.sendingUserId).yields(null);
			this.RateLimiter.addCount = sinon.stub().callsArgWith(1, null, true);
			return this.CollaboratorsInviteController._checkRateLimit(this.sendingUserId, (err, result)=> {
