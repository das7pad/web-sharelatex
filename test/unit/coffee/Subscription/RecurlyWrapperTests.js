...
const modulePath = "../../../../app/js/Features/Subscription/RecurlyWrapper";
...
describe("RecurlyWrapper", function() {
	...
	before(function() {
		...
		return this.RecurlyWrapper = (RecurlyWrapper = SandboxedModule.require(modulePath, { requires: {
	...
	describe("getSubscription", function() {
		...
		describe("with proper subscription id", function() {
			before(function() {
				this.apiRequest = sinon.stub(this.RecurlyWrapper, "apiRequest", mockApiRequest);
				return this.RecurlyWrapper.getSubscription("44f83d7cba354d5b84812419f923ea96", (error, recurlySubscription) => {
			...
			it("should look up the subscription at the normal API end point", function() {
				return this.apiRequest.args[0][0].url.should.equal("subscriptions/44f83d7cba354d5b84812419f923ea96");
			});

			return it("should return the subscription", function() {
				return this.recurlySubscription.uuid.should.equal("44f83d7cba354d5b84812419f923ea96");
			});
		});
		...
		describe("with ReculyJS token", function() {
			before(function() {
				this.apiRequest = sinon.stub(this.RecurlyWrapper, "apiRequest", mockApiRequest);
				return this.RecurlyWrapper.getSubscription("70db44b10f5f4b238669480c9903f6f5", {recurlyJsResult: true}, (error, recurlySubscription) => {
					return this.recurlySubscription = recurlySubscription;
				});
			});
			after(function() {
			...
			return it("should look up the subscription at the RecurlyJS API end point", function() {
				return this.apiRequest.args[0][0].url.should.equal("recurly_js/result/70db44b10f5f4b238669480c9903f6f5");
			});
		});
		...
		return describe("with includeAccount", function() {
			beforeEach(function() {
				this.apiRequest = sinon.stub(this.RecurlyWrapper, "apiRequest", mockApiRequest);
				return this.RecurlyWrapper.getSubscription("44f83d7cba354d5b84812419f923ea96", {includeAccount: true}, (error, recurlySubscription) => {
					return this.recurlySubscription = recurlySubscription;
				});
			});
			afterEach(function() {
	...
	describe("updateSubscription", function() {
		beforeEach(function(done) {
			...
			this.apiRequest = sinon.stub(this.RecurlyWrapper, "apiRequest", (options, callback) => {
				this.requestOptions = options;
				return callback(null, {}, fixtures["subscriptions/44f83d7cba354d5b84812419f923ea96"]);
		});
			return this.RecurlyWrapper.updateSubscription(this.recurlySubscriptionId, { plan_code : "silver", timeframe: "now" }, (error, recurlySubscription) => {
...
`
			...
			this.requestOptions.url.should.equal(`subscriptions/${this.recurlySubscriptionId}`);
	...
	describe("cancelSubscription", function() {
		beforeEach(function(done) {
			this.recurlySubscriptionId = "subscription-id-123";
			this.apiRequest = sinon.stub(this.RecurlyWrapper, "apiRequest", (options, callback) => {
				options.url.should.equal(`subscriptions/${this.recurlySubscriptionId}/cancel`);
				options.method.should.equal("put");
				return callback();
			});
			return this.RecurlyWrapper.cancelSubscription(this.recurlySubscriptionId, done);
		});
		...
		return describe('when the subscription is already cancelled', function() {
			...
			beforeEach(function() {
				...
				this.recurlySubscriptionId = "subscription-id-123";
				return this.apiRequest = sinon.stub(this.RecurlyWrapper, "apiRequest", (options, callback) => {
					return callback(new Error('woops'), {}, "<error><description>A canceled subscription can't transition to canceled</description></error>");
				});
			});

			return it('should not produce an error', function(done) {
				return this.RecurlyWrapper.cancelSubscription(this.recurlySubscriptionId, err => {
	...
	describe("reactivateSubscription", function() {
		beforeEach(function(done) {
			this.recurlySubscriptionId = "subscription-id-123";
			this.apiRequest = sinon.stub(this.RecurlyWrapper, "apiRequest", (options, callback) => {
				options.url.should.equal(`subscriptions/${this.recurlySubscriptionId}/reactivate`);
				options.method.should.equal("put");
				return callback();
			});
			return this.RecurlyWrapper.reactivateSubscription(this.recurlySubscriptionId, done);
		});
	...
	describe("redeemCoupon", function() {
		...
		beforeEach(function(done) {
			...
			this.apiRequest = sinon.stub(this.RecurlyWrapper, "apiRequest", (options, callback) => {
				...
				options.body.indexOf(`<account_code>${this.recurlyAccountId}</account_code>`).should.not.equal(-1);
				options.body.indexOf("<currency>USD</currency>").should.not.equal(-1);
				options.method.should.equal("post");
				return callback();
			});
			return this.RecurlyWrapper.redeemCoupon(this.recurlyAccountId, this.coupon_code, done);
		});
...
`
	...
	describe('createSubscription', function() {
		...
		beforeEach(function() {
			...
			return this.call = callback => {
				return this.RecurlyWrapper.createSubscription(this.user, this.subscriptionDetails, this.recurly_token_id, callback);
			};
		});
		...
		describe('when paypal', function() {
			...
			beforeEach(function() {
				...
				this._createPaypalSubscription = sinon.stub(this.RecurlyWrapper, '_createPaypalSubscription');
				return this._createPaypalSubscription.callsArgWith(3, null, this.subscription);
			});
			...
			return describe("when _createPaypalSubscription produces an error", function() {
				...
				beforeEach(function() {
					return this._createPaypalSubscription.callsArgWith(3, new Error('woops'));
				});
		...
		return describe('when not paypal', function() {
			...
			beforeEach(function() {
				...
				this._createCreditCardSubscription = sinon.stub(this.RecurlyWrapper, '_createCreditCardSubscription');
				return this._createCreditCardSubscription.callsArgWith(3, null, this.subscription);
			});
			...
			return describe("when _createCreditCardSubscription produces an error", function() {
				...
				beforeEach(function() {
					return this._createCreditCardSubscription.callsArgWith(3, new Error('woops'));
				});
	...
	describe('_createCreditCardSubscription', function() {
		...
		beforeEach(function() {
			...
			this._parseSubscriptionXml = sinon.stub(this.RecurlyWrapper, '_parseSubscriptionXml');
			this._parseSubscriptionXml.callsArgWith(1, null, this.subscription);
			return this.call = callback => {
				return this.RecurlyWrapper._createCreditCardSubscription(this.user, this.subscriptionDetails, this.recurly_token_id, callback);
			};
		});
	...
	describe('_createPaypalSubscription', function() {
		...
		beforeEach(function() {
			this.checkAccountExists = sinon.stub(this.RecurlyWrapper._paypal, 'checkAccountExists');
			this.createAccount = sinon.stub(this.RecurlyWrapper._paypal, 'createAccount');
			this.createBillingInfo = sinon.stub(this.RecurlyWrapper._paypal, 'createBillingInfo');
			this.setAddress = sinon.stub(this.RecurlyWrapper._paypal, 'setAddress');
			this.createSubscription = sinon.stub(this.RecurlyWrapper._paypal, 'createSubscription');
			this.user = {
				_id: 'some_id',
				email: 'user@example.com'
			};
			...
			this.createSubscription.callsArgWith(1, null,
				{user, subscriptionDetails, recurly_token_id,
				userExists: false, account: {accountCode: 'xx'}, billingInfo: {token_id: 'abc'}, subscription: this.subscription}
			);

			return this.call = callback => {
				return this.RecurlyWrapper._createPaypalSubscription(this.user, this.subscriptionDetails, this.recurly_token_id, callback);
			};
		});
	...
	describe('paypal actions', function() {
		...
		beforeEach(function() {
			...
			this._parseBillingInfoXml = sinon.spy(this.RecurlyWrapper, '_parseBillingInfoXml');
			this._parseSubscriptionXml = sinon.spy(this.RecurlyWrapper, '_parseSubscriptionXml');
			return this.cache = {
		...
		describe('_paypal.checkAccountExists', function() {
			...
			beforeEach(function() {
				return this.call = callback => {
					return this.RecurlyWrapper._paypal.checkAccountExists(this.cache, callback);
				};
			});
			...
			describe('when the account exists', function() {
				...
				beforeEach(function() {
					const resultXml = '<account><account_code>abc</account_code></account>';
					return this.apiRequest.callsArgWith(1, null, {statusCode: 200}, resultXml);
				});
			...
			return describe('when apiRequest produces an error', function() {
				...
				beforeEach(function() {
					return this.apiRequest.callsArgWith(1, new Error('woops'), {statusCode: 500});
				});
		...
		describe('_paypal.createBillingInfo', function() {
			...
			beforeEach(function() {
				this.cache.account =
					{account_code: 'abc'};
				return this.call = callback => {
					return this.RecurlyWrapper._paypal.createBillingInfo(this.cache, callback);
				};
			});
		...
		return describe('_paypal.createSubscription', function() {
			...
			beforeEach(function() {
				...
				this.cache.billingInfo = {};
				return this.call = callback => {
					return this.RecurlyWrapper._paypal.createSubscription(this.cache, callback);
				};
			});
	...
	return describe("listAccountActiveSubscriptions", function() {
		beforeEach(function() {
			...
			this.RecurlyWrapper.apiRequest = sinon.stub().yields(null, (this.response = {"mock": "response"}), (this.body = "<mock body/>"));
			return this.RecurlyWrapper._parseSubscriptionsXml = sinon.stub().yields(null, (this.subscriptions = ["mock", "subscriptions"]));
		});
			
		describe("with an account", function() {
			beforeEach(function() {
				return this.RecurlyWrapper.listAccountActiveSubscriptions(this.user_id, this.callback);
			});
			...
			return it("should return the subscriptions", function() {
				return this.callback.calledWith(null, this.subscriptions).should.equal(true);
			});
		});
