...
const modulePath = "../../../../app/js/Features/Subscription/SubscriptionUpdater";
const {
    assert
} = require("chai");
const {
...
describe("SubscriptionUpdater", function() {
	...
	beforeEach(function() {
		...
		this.adminUser =
			{_id: (this.adminuser_id = "5208dd34438843e2db000007")};
		this.otherUserId = "5208dd34438842e2db000005";
		this.allUserIds = ["13213", "dsadas", "djsaiud89"];
		this.userStub = {_id: 'mock-user-stub-id', email: 'mock-stub-email@baz.com'};
		...
		this.findAndModifyStub = sinon.stub().callsArgWith(2, null, this.subscription);
		...
		return this.SubscriptionUpdater = SandboxedModule.require(modulePath, { requires: {
	...
	describe("syncSubscription", function() {
		...
		beforeEach(function() {
			this.SubscriptionLocator.getUsersSubscription.callsArgWith(1, null, this.subscription);
			return this.SubscriptionUpdater._updateSubscriptionFromRecurly = sinon.stub().callsArgWith(2);
		});
		...
		it("should update the subscription if the user already is admin of one", function(done){
			...
			return this.SubscriptionUpdater.syncSubscription(this.recurlySubscription, this.adminUser._id, err=> {
				this.SubscriptionLocator.getUsersSubscription.calledWith(this.adminUser._id).should.equal(true);
				this.SubscriptionUpdater._updateSubscriptionFromRecurly.called.should.equal(true);
				this.SubscriptionUpdater._updateSubscriptionFromRecurly.calledWith(this.recurlySubscription, this.subscription).should.equal(true);
				return done();
			});
		});
	...
	describe("_updateSubscriptionFromRecurly", function() {
		beforeEach(function() {
			...
			return this.SubscriptionUpdater.deleteSubscription = sinon.stub().yields();
		});

		it("should update the subscription with token etc when not expired", function(done){
			return this.SubscriptionUpdater._updateSubscriptionFromRecurly(this.recurlySubscription, this.subscription, err=> {
				this.subscription.recurlySubscription_id.should.equal(this.recurlySubscription.uuid);
				this.subscription.planCode.should.equal(this.recurlySubscription.plan.plan_code);
				this.subscription.save.called.should.equal(true);
				this.FeaturesUpdater.refreshFeatures.calledWith(this.adminUser._id).should.equal(true);
				return done();
			});
		});
		...
		it("should remove the subscription when expired", function(done){
			...
			return this.SubscriptionUpdater._updateSubscriptionFromRecurly(this.recurlySubscription, this.subscription, err=> {
				this.SubscriptionUpdater.deleteSubscription.calledWith(this.subscription._id).should.equal(true);
				return done();
			});
		});
		...
		it("should update all the users features", function(done){
			return this.SubscriptionUpdater._updateSubscriptionFromRecurly(this.recurlySubscription, this.subscription, err=> {
				this.FeaturesUpdater.refreshFeatures.calledWith(this.adminUser._id).should.equal(true);
				this.FeaturesUpdater.refreshFeatures.calledWith(this.allUserIds[0]).should.equal(true);
				this.FeaturesUpdater.refreshFeatures.calledWith(this.allUserIds[1]).should.equal(true);
				this.FeaturesUpdater.refreshFeatures.calledWith(this.allUserIds[2]).should.equal(true);
				return done();
			});
		});

		it("should set group to true and save how many members can be added to group", function(done){
			this.PlansLocator.findLocalPlanInSettings.withArgs(this.recurlySubscription.plan.plan_code).returns({groupPlan:true, membersLimit:5});
			return this.SubscriptionUpdater._updateSubscriptionFromRecurly(this.recurlySubscription, this.subscription, err=> {
	...
	describe("_createNewSubscription", () => it("should create a new subscription then update the subscription", function(done){
        return this.SubscriptionUpdater._createNewSubscription(this.adminUser._id, () => {
	...
	describe("addUserToGroup", function() {
		beforeEach(function() {
			return this.SubscriptionUpdater.addUsersToGroup = sinon.stub().yields(null);
		});

		return it("delegates to addUsersToGroup", function(done){
			return this.SubscriptionUpdater.addUserToGroup(this.subscription._id, this.otherUserId, () => {
				this.SubscriptionUpdater.addUsersToGroup
					.calledWith(this.subscription._id, [this.otherUserId]).should.equal(true);
				return done();
			});
		});
	});
	...
	describe("addUsersToGroup", function() {
		beforeEach(function() {
			return this.FeaturesUpdater.refreshFeatures = sinon.stub().callsArgWith(1);
		});

		it("should add the user ids to the group as a set", function(done){
			return this.SubscriptionUpdater.addUsersToGroup(this.subscription._id, [this.otherUserId], () => {
				const searchOps =
					{_id: this.subscription._id};
				const insertOperation =
					{ $addToSet: { member_ids: { $each: [this.otherUserId] } } };
				this.findAndModifyStub.calledWith(searchOps, insertOperation).should.equal(true);
				return done();
			});
		});
		...
		return it("should update the users features", function(done){
			return this.SubscriptionUpdater.addUserToGroup(this.subscription._id, this.otherUserId, () => {
				this.FeaturesUpdater.refreshFeatures.calledWith(this.otherUserId).should.equal(true);
				return done();
			});
		});
	});
	...
	describe("removeUserFromGroups", function() {
		beforeEach(function() {
			...
			return this.SubscriptionLocator.getMemberSubscriptions.yields(null, this.fakeSubscriptions);
		});

		it("should pull the users id from the group", function(done){
			return this.SubscriptionUpdater.removeUserFromGroup(this.subscription._id, this.otherUserId, () => {
				const searchOps =
					{_id: this.subscription._id};
				const removeOperation =
					{"$pull": {member_ids:this.otherUserId}};
				this.updateManyStub.calledWith(searchOps, removeOperation).should.equal(true);
				return done();
			});
		});

		it("should pull the users id from all groups", function(done){
			return this.SubscriptionUpdater.removeUserFromAllGroups(this.otherUserId, () => {
				const filter =
		...
		return it("should not update features for user stubs", function(done){
			...
			return this.SubscriptionUpdater.removeUserFromGroup(this.subscription._id, this.userStub._id, () => {
	...
	return describe("deleteSubscription", function() {
		beforeEach(function(done) {
			...
			this.SubscriptionLocator.getSubscription = sinon.stub().yields(null, this.subscription);
			this.FeaturesUpdater.refreshFeatures = sinon.stub().yields();
			return this.SubscriptionUpdater.deleteSubscription(this.subscription_id, done);
		});
