...
const modulePath = path.join(__dirname, '../../../../app/js/Features/Subscription/V1SubscriptionManager');
...
describe('V1SubscriptionManager', function() {
	beforeEach(function() {
		this.V1SubscriptionManager = SandboxedModule.require(modulePath, { requires: {
	...
	describe('getPlanCodeFromV1', function() {
		beforeEach(function() {
			...
			this.V1SubscriptionManager._v1Request = sinon.stub()
		...
		return describe('when all goes well', function() {
			it('should call _v1Request', function(done) {
				return this.call((err, planCode) => {
					expect(
						this.V1SubscriptionManager._v1Request.callCount
					).to.equal(1);
					expect(
						this.V1SubscriptionManager._v1Request.calledWith(
							this.userId
						)
					).to.equal(true);
					return done();
	...
	describe('getGrandfatheredFeaturesForV1User', function() {
		describe('when the user ID is greater than the cutoff', () => it('should return an empty feature set', function(done) {
            expect(this.V1SubscriptionManager.getGrandfatheredFeaturesForV1User(100)).to.eql({});
            return done();
        }));

		return describe('when the user ID is less than the cutoff', () => it('should return a feature set with grandfathered properties for github and mendeley', function(done) {
            expect(this.V1SubscriptionManager.getGrandfatheredFeaturesForV1User(1)).to.eql({
                github: true,
                mendeley: true
            });
            return done();
	...
	describe('_v1Request', function() {
		beforeEach(function() {
			return this.UserGetter.getUser = sinon.stub()
				.yields(null, this.user);
		});

		describe('when v1IdForUser produces an error', function() {
			beforeEach(function() {
				this.V1SubscriptionManager.v1IdForUser = sinon.stub()
					.yields(new Error('woops'));
				return this.call = cb => {
					return this.V1SubscriptionManager._v1Request(this.user_id, { url() { return '/foo'; } }, cb);
				};
			});
