...
const modulePath = path.join(__dirname, '../../../../app/js/Features/Announcements/AnnouncementsHandler');
...
describe('AnnouncementsHandler', function() {
	...
	return describe("getUnreadAnnouncements", function() {
		beforeEach(function() {
			...
			return this.BlogHandler.getLatestAnnouncements.callsArgWith(0, null, this.stubbedAnnouncements);
		});
		...
		it("should mark all announcements as read is false", function(done){
			...
			return this.handler.getUnreadAnnouncements(this.user, (err, announcements)=> {
		...
		it("should return older ones marked as read as well", function(done){
			this.AnalyticsManager.getLastOccurrence.callsArgWith(2, null, {segmentation:{blogPostId:"/2008/04/12/title-date-irrelivant"}});
			return this.handler.getUnreadAnnouncements(this.user, (err, announcements)=> {
		...
		it("should return all of them marked as read", function(done){
			this.AnalyticsManager.getLastOccurrence.callsArgWith(2, null, {segmentation:{blogPostId:"/2016/11/01/introducting-latex-code-checker"}});
			return this.handler.getUnreadAnnouncements(this.user, (err, announcements)=> {
		...
		describe("with custom domain announcements", function() {
			beforeEach(function() {
				...
				return this.handler._domainSpecificAnnouncements = sinon.stub().returns(this.stubbedDomainSpecificAnn);
			});
		...
		return describe("_domainSpecificAnnouncements", function() {
			...
			it("should filter announcments which don't have an id", function(done) {
				const result = this.handler._domainSpecificAnnouncements("someone@gmail.com");
