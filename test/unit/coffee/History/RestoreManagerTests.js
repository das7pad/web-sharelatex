...
const modulePath = require('path').join(__dirname, '../../../../app/js/Features/History/RestoreManager');
...
describe('RestoreManager', function() {
	...
	describe('restoreFileFromV2', function() {
		beforeEach(function() {
			this.RestoreManager._writeFileVersionToDisk = sinon.stub().yields(null, (this.fsPath = "/tmp/path/on/disk"));
			this.RestoreManager._findOrCreateFolder = sinon.stub().yields(null, (this.folder_id = 'mock-folder-id'));
			return this.FileSystemImportManager.addEntity = sinon.stub().yields(null, (this.entity = 'mock-entity'));
		});

		describe("with a file not in a folder", function() {
			beforeEach(function() {
				this.pathname = 'foo.tex';
				return this.RestoreManager.restoreFileFromV2(this.user_id, this.project_id, this.version, this.pathname, this.callback);
			});
			...
			it('should add the entity', function() {
				return this.FileSystemImportManager.addEntity
					.calledWith(this.user_id, this.project_id, this.folder_id, 'foo.tex', this.fsPath, false)
					.should.equal(true);
			});
		...
		return describe("with a file in a folder", function() {
			...
			return it('should add the entity by its basename', function() {
				return this.FileSystemImportManager.addEntity
					.calledWith(this.user_id, this.project_id, this.folder_id, 'bar.tex', this.fsPath, false)
					.should.equal(true);
			});
		});
	});
	...
	describe('_findOrCreateFolder', function() {
		beforeEach(function() {
			this.EditorController.mkdirp = sinon.stub().yields(null, [], {_id: (this.folder_id = 'mock-folder-id')});
			return this.RestoreManager._findOrCreateFolder(this.project_id, 'folder/name', this.callback);
		});
	...
	return describe('_addEntityWithUniqueName', function() {
		...
		describe('with a valid name', function() {
			beforeEach(function() {
				...
				return this.RestoreManager._addEntityWithUniqueName(this.addEntityWithName, this.name, this.callback);
			});
		...
		return describe("with an invalid name", function() {
			beforeEach(function() {
				this.addEntityWithName.onFirstCall().yields(new Errors.InvalidNameError());
				this.addEntityWithName.onSecondCall().yields(null, (this.entity = 'mock-entity'));
				return this.RestoreManager._addEntityWithUniqueName(this.addEntityWithName, this.name, this.callback);
			});
			...
			it('should try to add the entity with a unique name', function() {
				...
				return this.addEntityWithName.calledWith(`foo (Restored on ${date}).tex`).should.equal(true);
			});
