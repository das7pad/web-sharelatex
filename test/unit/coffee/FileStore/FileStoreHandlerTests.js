...
describe("FileStoreHandler", function() {
	beforeEach(function() {
		...
		this.settings = {apis:{filestore:{url:"http//filestore.sharelatex.test"}}};
		...
		this.handler = SandboxedModule.require(modulePath, { requires: {
			...
			"./FileHashManager" : (this.FileHashManager = { computeHash: sinon.stub().callsArgWith(1, null, this.hashValue)}),
		...
		return this.handler._buildUrl = sinon.stub().returns("http://filestore.stubbedBuilder.com");
	});
	...
	describe("uploadFileFromDisk", function() {
		...
		it("should create read stream", function(done){
			...
			return this.handler.uploadFileFromDisk(this.project_id, this.file_args, this.fsPath, () => {
		...
		it("builds the correct url", function(done){
			...
			return this.handler.uploadFileFromDisk(this.project_id, this.file_args, this.fsPath, () => {
				this.handler._buildUrl.calledWith(this.project_id, this.file_id).should.equal(true);
				return done();
			});
		});
		...
		it('should callback with the url and fileRef', function(done) {
			...
			return this.handler.uploadFileFromDisk(this.project_id, this.file_args, this.fsPath, (err, url, fileRef) => {
		...
		describe("symlink", () => it("should not read file stat returns nothing", function(done){
		...
		return describe("when upload fails", function() {
			...
			return it('should callback with an error', function(done) {
				...
				return this.handler.uploadFileFromDisk(this.project_id, this.file_args, this.fsPath, err => {
					expect(err).to.exist;
					expect(err).to.be.instanceof(Error);
					expect(this.fs.createReadStream.callCount).to.equal(this.handler.RETRY_ATTEMPTS);
					return done();
				});
			});
		});
	...
	describe("getFileStream", function() {
		...
		it("should get the stream with the correct params", function(done){
			return this.handler.getFileStream(this.project_id, this.file_id, this.query, (err, stream)=> {
		...
		return describe('when range is specified in query', function() {
			...
			return describe('when range is invalid', () => ['0-', '-100', 'one-two', 'nonsense'].forEach(r => {
	...
	return describe("copyFile", function() {
		...
		it("should post json", function(done){
			...
			return this.handler.copyFile(this.project_id, this.file_id, this.newProject_id, this.newFile_id, () => {
				this.request.args[0][0].method.should.equal("put");
				this.request.args[0][0].uri.should.equal(this.handler._buildUrl());
				this.request.args[0][0].json.source.project_id.should.equal(this.project_id);
		...
		it("builds the correct url", function(done){
			this.request.callsArgWith(1, null, {statusCode: 200});
			return this.handler.copyFile(this.project_id, this.file_id, this.newProject_id, this.newFile_id, () => {
				this.handler._buildUrl.calledWith(this.newProject_id, this.newFile_id).should.equal(true);
				return done();
			});
		});
		...
		it("returns the url", function(done){
			this.request.callsArgWith(1, null, {statusCode: 200});
			return this.handler.copyFile(this.project_id, this.file_id, this.newProject_id, this.newFile_id, (err, url) => {
		...
		it("should return the err", function(done){
			const error = "errrror";
			this.request.callsArgWith(1, error);
			return this.handler.copyFile(this.project_id, this.file_id, this.newProject_id, this.newFile_id, err=> {
