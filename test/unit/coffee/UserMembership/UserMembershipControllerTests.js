...
const modulePath = "../../../../app/js/Features/UserMembership/UserMembershipController.js";
...
describe("UserMembershipController", function() {
	beforeEach(function() {
		...
		return this.UserMembershipController = SandboxedModule.require(modulePath, { requires: {
	...
	describe('index', function() {
		...
		it('get users', function(done) {
			return this.UserMembershipController.index(this.req, { render: () => {
		...
		it('render group view', function(done) {
			return this.UserMembershipController.index(this.req, { render: (viewPath, viewParams) => {
				...
				expect(viewParams.paths.addMember).to.equal(`/manage/groups/${this.subscription._id}/invites`);
				return done();
			}
		}
			);
	...
	describe('add', function() {
		...
		it('add user', function(done) {
			return this.UserMembershipController.add(this.req, { json: () => {
				sinon.assert.calledWithMatch(
		...
		it('return user object', function(done) {
			return this.UserMembershipController.add(this.req, { json: payload => {
		...
		it('handle user already added', function(done) {
			...
			return this.UserMembershipController.add(this.req, { status: () => ({
			json: payload => {
	...
	describe('remove', function() {
		...
		it('remove user', function(done) {
			return this.UserMembershipController.remove(this.req, { send: () => {
				sinon.assert.calledWithMatch(
		...
		it('prevent self removal', function(done) {
			...
			return this.UserMembershipController.remove(this.req, { status: () => ({
			json: payload => {
	...
	describe("exportCsv", function() {
		...
		return it("should export the correct csv", function() {
			return assertCalledWith(this.res.send, "mock-email-1@foo.com\nmock-email-2@foo.com\n");
		});
	});
	...
	describe('new', function() {
		...
		return it('renders view', function(done) {
			return this.UserMembershipController.new(this.req, { render: (viewPath, data) => {
	...
	return describe('create', function() {
		...
		it('creates institution', function(done) {
			return this.UserMembershipController.create(this.req, { redirect: path => {
