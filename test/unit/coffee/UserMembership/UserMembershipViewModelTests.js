...
const modulePath = "../../../../app/js/Features/UserMembership/UserMembershipViewModel";
...
describe('UserMembershipViewModel', function() {
	beforeEach(function() {
		...
		this.UserMembershipViewModel = SandboxedModule.require(modulePath, { requires: {
			'mongojs': mongojs,
		...
		this.user = {_id: 'mock-user-id', email: 'mock-email@baz.com', first_name: 'Name'};
		return this.userStub = {_id: 'mock-user-stub-id', email: 'mock-stub-email@baz.com'};
	});
	...
	return describe('build async', function() {
		...
		it('build email', function(done) {
			return this.UserMembershipViewModel.buildAsync(this.email, (error, viewModel) => {
		...
		it('build user', function(done) {
			return this.UserMembershipViewModel.buildAsync(this.user, (error, viewModel) => {
		...
		it('build user id', function(done) {
			...
			return this.UserMembershipViewModel.buildAsync(ObjectId(), (error, viewModel) => {
		...
		return it('build user id with error', function(done) {
			...
			return this.UserMembershipViewModel.buildAsync(userId, (error, viewModel) => {
				should.not.exist(error);
				assertNotCalled(this.UserMembershipViewModel.build);
