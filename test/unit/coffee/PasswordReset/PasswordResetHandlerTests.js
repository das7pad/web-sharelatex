...
const modulePath = path.join(__dirname, "../../../../app/js/Features/PasswordReset/PasswordResetHandler");
const {
    expect
} = require("chai");
...
describe("PasswordResetHandler", function() {
	...
	beforeEach(function() {
		...
		this.V1Api =
			{request: sinon.stub()};
		this.PasswordResetHandler = SandboxedModule.require(modulePath, { requires: {
	...
	describe("generateAndEmailResetToken", function() {
		describe("when in ShareLaTeX", function() {
			it("should check the user exists", function(done){
				...
				return this.PasswordResetHandler.generateAndEmailResetToken(this.user.email, (err, status)=> {
			...
			it("should send the email with the token", function(done){
				...
				return this.PasswordResetHandler.generateAndEmailResetToken(this.user.email, (err, status)=> {
					...
					args[1].setNewPasswordUrl.should.equal(`${this.settings.siteUrl}/user/password/set?passwordResetToken=${this.token}&email=${encodeURIComponent(this.user.email)}`);
					return done();
				});
			});
		...
		return describe("when in overleaf", function() {
			...
			describe("when the email exists", function() {
				beforeEach(function() {
					...
					return this.PasswordResetHandler.generateAndEmailResetToken(this.email, this.callback);
				});

				it('should call the v1 api for the user', function() {
					return this.V1Api.request.calledWith({
				...
				it('should send an email with the token', function() {
					...
					const args = this.EmailHandler.sendEmail.args[0];
					args[0].should.equal("passwordResetRequested");
					return args[1].setNewPasswordUrl.should.equal(`${this.settings.siteUrl}/user/password/set?passwordResetToken=${this.token}&email=${encodeURIComponent(this.user.email)}`);
				});
			...
			describe("when the email doesn't exist", function() {
				beforeEach(function() {
					this.V1Api.request = sinon.stub().yields(null, { statusCode: 404 }, {});
					this.UserGetter.getUserByAnyEmail.callsArgWith(1);
					return this.PasswordResetHandler.generateAndEmailResetToken(this.email, this.callback);
				});
	...
	return describe("setNewUserPassword", function() {
		describe("when no data is found", function() {
			beforeEach(function() {
				...
				return this.PasswordResetHandler.setNewUserPassword(this.token, this.password, this.callback);
			});
		...
		describe('when the data is an old style user_id', function() {
			beforeEach(function() {
				this.AuthenticationManager.setUserPassword.yields(null, true, this.user_id);
				this.OneTimeTokenHandler.getValueFromTokenAndExpire.yields(null, this.user_id);
				return this.PasswordResetHandler.setNewUserPassword(this.token, this.password, this.callback);
			});
			...
			return it('should reset == true and the user_id', function() {
				return this.callback.calledWith(null, true, this.user_id).should.equal(true);
			});
		});
		...
		describe('when the data is a new style user_id', function() {
			beforeEach(function() {
				this.AuthenticationManager.setUserPassword.yields(null, true, this.user_id);
				this.OneTimeTokenHandler.getValueFromTokenAndExpire.yields(null, {user_id: this.user_id});
				return this.PasswordResetHandler.setNewUserPassword(this.token, this.password, this.callback);
			});
		...
		return describe('when the data is v1 id', function() {
			beforeEach(function() {
				...
				this.UserGetter.getUser.withArgs({'overleaf.id': this.v1_user_id}).yields(null, { _id: this.user_id });
				this.OneTimeTokenHandler.getValueFromTokenAndExpire.yields(null, {v1_user_id: this.v1_user_id});
				return this.PasswordResetHandler.setNewUserPassword(this.token, this.password, this.callback);
			});
