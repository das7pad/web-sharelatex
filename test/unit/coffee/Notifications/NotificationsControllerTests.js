...
const modulePath = require('path').join(__dirname, '../../../../app/js/Features/Notifications/NotificationsController.js');
...
describe('NotificationsController', function() {
	...
	it('should ask the handler for all unread notifications', function(done){
		...
		this.handler.getUserNotifications = sinon.stub().callsArgWith(1, null, allNotifications);
		return this.controller.getAllUnreadNotifications(this.req, { send:body=> {
	...
	return it('should send a delete request when a delete has been received to mark a notification', function(done){
		return this.controller.markNotificationAsRead(this.req, { send:() => {
			this.handler.markAsRead.calledWith(user_id, notification_id).should.equal(true);
			return done();
		}
	}
		);
