...
const modulePath = require('path').join(__dirname, '../../../../app/js/Features/Notifications/NotificationsBuilder.js');
...
describe('NotificationsBuilder', function() {
	...
	beforeEach(function() {
		...
		this.request = sinon.stub().returns(this.stubResponse).callsArgWith(1, null, response, this.body);
	...
	return it('should call v1 and create affiliation notifications', function(done){
		...
		return this.controller.ipMatcherAffiliation(user_id).create(ip, callback=> {
			this.request.calledOnce.should.equal(true);
			const expectedOpts = {
				university_name: this.body.name,
				content: this.body.enrolment_ad_html
			};
			this.handler.createNotification.calledWith(
				user_id,
