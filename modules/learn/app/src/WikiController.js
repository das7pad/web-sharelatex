...
const baseWikiUrl = (settings.apis.wiki != null ? settings.apis.wiki.url : undefined) || "https://learn.sharelatex.com";
...
module.exports = (WikiController = { 
	...
	getPage(req, res, next) {
		...
		const isMediaWikiPage = _.some(mediaWikiPages, substring => page.toLowerCase().indexOf(substring) !== -1);
		...
		return async.parallel(jobs, function(error, results){
			...
			if ((pageData.content != null ? pageData.content.length : undefined) > 280) {
				if (_.include(other_lngs, req.lng)) {
	...
	_getPageContent(page, callback) {
		...
		return request({
			...
			const result = { 
				categories: __guard__(data != null ? data.parse : undefined, x => x.categories.map(category => category['*'])),
				content: __guard__(__guard__(data != null ? data.parse : undefined, x2 => x2.text), x1 => x1['*']),
				title: __guard__(data != null ? data.parse : undefined, x3 => x3.title),
				revid: __guard__(data != null ? data.parse : undefined, x4 => x4.revid),
				redirects: __guard__(data != null ? data.parse : undefined, x5 => x5.redirects)
			};
	...
	_renderPage(page, contents, preview, res){
		let title;
		if (!settings.overleaf && ((page.categories != null ? page.categories.length : undefined) > 0) && Array.from(page.categories).includes('Draft') && !preview) {
		...
		if (((page.categories != null ? page.categories.length : undefined) > 0) && Array.from(page.categories).includes('Hide') && !preview) {
			return ErrorController.notFound(null, res);
		}
		if (((page.redirects != null ? page.redirects.length : undefined) > 0) && (page.redirects[0].to !== 'Kb/Knowledge Base')) {
		...
		if (__guard__(settings.cdn != null ? settings.cdn.wiki : undefined, x => x.host) != null) {
			page.content = __guard__(page != null ? page.content : undefined, x1 => x1.replace(/src="(\/[^"]+)"/g, `src='${__guard__(settings.cdn != null ? settings.cdn.web : undefined, x2 => x2.host)}$1'`));
		}
		// width and alt below are Linked_images, but we are not using this and causes a JS error
		page.content = __guard__(page != null ? page.content : undefined, x3 => x3.replace(/width='{{{width}}}'/g, '')); 
		page.content = __guard__(page != null ? page.content : undefined, x4 => x4.replace(/alt='{{{alt}}}'/g, '')); 
