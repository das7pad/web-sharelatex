 ...
 */
define(['base'], App =>
  App.controller('GithubSyncImportModalController', function(
    $scope,
    $modalInstance,
    $http
  ) {
    ...
    $http
      ...
      .then(function(response) {
        ...
        if (!user.enabled) {
          ...
          return $http
            .get('/user/github-sync/repos', { disableAutoLoginRedirect: true })
            .then(function(response) {
              const { data } = response
              $scope.status.loading = false
              return ($scope.status.repos = data.repos)
            })
    ...
    return ($scope.importRepo = function(repo) {
      ...
      return $http
        ...
        .then(function(response) {
          ...
          return (window.location = `/project/${project_id}`)
        })
        .catch(function(response) {
          const { data, status } = response
