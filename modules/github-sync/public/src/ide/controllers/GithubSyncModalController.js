...
define([
	...
	App.controller("GithubSyncModalController", function($scope, $modalInstance, $http, $modal, $window, $interval, ide) {
		...
		$scope.openMergeModal = function(mergeImmediately) {
			let template;
			if (mergeImmediately == null) { mergeImmediately = false; }
			$modalInstance.dismiss();
			...
			return $modal.open({
				...
				resolve: {
					mergeImmediately() { return mergeImmediately; }
				}
			});
		};
		...
		$scope.linkAccount = function() {
			const authWindow = $window.open("/github-sync/beginAuth", "github-sync-auth", "width=700,height=500");
			var poller = $interval(function() {
				// We can get errors when trying to access the URL before it returns 
				// to a ShareLaTeX URL (security exceptions)
				let pathname;
				try {
					pathname = __guard__(authWindow != null ? authWindow.location : undefined, x => x.pathname);
				} catch (e) {
					pathname = null;
				}
				if (__guard__(authWindow != null ? authWindow.location : undefined, x1 => x1.pathname) === "/github-sync/linked") {
					authWindow.close();
					$scope.loadStatus();
		...
		return ($scope.loadStatus = function() {
			...
			return $http.get("/user/github-sync/status", { disableAutoLoginRedirect: true })
				.then(function(response) {
					let { data } = response;
					const userStatus = data;
					$scope.status.user = userStatus;
					return $http.get(`/project/${ide.project_id}/github-sync/status`, { disableAutoLoginRedirect: true })
						.then(function(response) {
							...
							if ($scope.status.project.enabled) {
								return $http.get(`/project/${ide.project_id}/github-sync/commits/unmerged`, { disableAutoLoginRedirect: true })
									.then(function(response) {
										const commits = response.data;
										$scope.status.commits.commits = commits;
										return $scope.status.commits.loading = false;}).catch(function(response) {
										let status;
