...
define([
	...
	App.controller("DropboxRegistrationController", function($scope, $window, $http, $timeout) {
		...
		return $http.post("/dropbox/completeRegistration", data)
			.then(function() {
				$scope.state = "success";
				const redirect = () => $window.location.href = "/user/settings";
				return $timeout(redirect, 5000);}).catch(function() {
				$scope.state = "error";
