 ...
 */
define(['base', 'ide/permissions/PermissionsManager', 'moment'], function(
  App,
  PermissionsManager,
  moment
) {
  ...
  return App.controller('PublishProjectAsTemplateModalController', function(
    $scope,
    $modalInstance,
    ide,
    $http
    ...
    const problemTalkingToTemplateApi = () =>
      ($scope.problemTalkingToTemplateApi = true)

    const refreshPublishedStatus = () =>
      $http
        .get(`/project/${ide.project_id}/template`)
        .then(function(response) {
          const { data } = response
          $scope.templateDetails = data
          $scope.templateDetails.publishedDate = moment(
            data.publishedDate
          ).format('Do MMM YYYY, h:mm a')
          return ($scope.templateDetails.description = data.description)
        })
      ...
      return updateProjectDescription()
        .then(() =>
          $http
            ...
            .then(function() {
              refreshPublishedStatus()
              return ($scope.state.publishInflight = false)
            })
            .catch(() => problemTalkingToTemplateApi())
        )
        .catch(() => problemTalkingToTemplateApi())
    ...
    $scope.unpublishTemplate = function() {
      ...
      return $http
        ...
        .then(function() {
          refreshPublishedStatus()
          return ($scope.state.unpublishInflight = false)
        })
        .catch(() => problemTalkingToTemplateApi())
    }
