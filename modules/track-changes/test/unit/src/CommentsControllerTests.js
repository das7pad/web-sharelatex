...
const modulePath = path.join(__dirname, "../../../app/js/Comments/CommentsController");
const {
    expect
} = require("chai");
...
describe("TrackChanges CommentsController", function() {
	beforeEach(function() {
		...
		this.CommentsController = SandboxedModule.require(modulePath, { requires: {
			"settings-sharelatex": this.settings,
			"logger-sharelatex": { log() {}
		},
			"../../../../../app/js/Features/Chat/ChatApiHandler": this.ChatApiHandler,
	...
	describe("sendComment", function() {
		beforeEach(function() {
			...
			this.UserInfoManager.getPersonalInfo = sinon.stub().yields(null, (this.user = {"unformatted": "user"}));
			this.UserInfoController.formatPersonalInfo = sinon.stub().returns(this.formatted_user = {"formatted": "user"});
			this.ChatApiHandler.sendComment = sinon.stub().yields(null, (this.message = {"mock": "message", user_id: this.user_id}));
		...
		it("should tell the editor real time controller about the update with the data from the chat handler", function() {
			return this.EditorRealTimeController.emitToRoom
				.calledWith(this.project_id, "new-comment", this.thread_id, this.message)
				.should.equal(true);
		});
	...
	describe("getThreads", function() {
		beforeEach(function() {
			this.req.params =
				{project_id: (this.project_id = "mock-project-id")};
			this.ChatApiHandler.getThreads = sinon.stub().yields(null, (this.threads = {"mock": "mock", "threads": "threads"}));
			this.ChatController._injectUserInfoIntoThreads = sinon.stub().yields(null, this.threads);
	...
	describe("resolveThread", function() {
		...
		it("should tell the client the comment was resolved", function() {
			return this.EditorRealTimeController.emitToRoom
				.calledWith(this.project_id, "resolve-thread", this.thread_id, this.formatted_user)
				.should.equal(true);
		});
	...
	describe("editMessage", function() {
		beforeEach(function() {
			...
			this.req.body =
				{content: (this.content = "mock-content")};
		...
		it("should ask the chat handler to edit the comment", function() {
			return this.ChatApiHandler.editMessage
				.calledWith(this.project_id, this.thread_id, this.message_id, this.content)
				.should.equal(true);
		});

		it("should tell the client the comment was edited", function() {
			return this.EditorRealTimeController.emitToRoom
				.calledWith(this.project_id, "edit-message", this.thread_id, this.message_id, this.content)
				.should.equal(true);
		});
	...
	return describe("deleteMessage", function() {
		...
		it("should tell the client the message was deleted", function() {
			return this.EditorRealTimeController.emitToRoom
				.calledWith(this.project_id, "delete-message", this.thread_id, this.message_id)
				.should.equal(true);
		});
