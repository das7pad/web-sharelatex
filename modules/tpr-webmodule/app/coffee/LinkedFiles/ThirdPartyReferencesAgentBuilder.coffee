...
module.exports = buildAgent = (provider) =>
	ThirdPartyReferenceAgent = {
		_buildUrl: (user_id, linkedFileData) ->
			if linkedFileData.group_id?
				"/user/#{user_id}/#{provider}/group/#{linkedFileData.group_id}/bibtex"
			else
				"/user/#{user_id}/#{provider}/bibtex"
		...
		createLinkedFile: (project_id, linkedFileData, name, parent_folder_id, user_id, callback) ->
			...
			url = ThirdPartyReferenceAgent._buildUrl(user_id, linkedFileData)
			ThirdPartyReferenceAgent._doImport(url, project_id, linkedFileData, name, parent_folder_id, user_id, callback)

		refreshLinkedFile: (project_id, linkedFileData, name, parent_folder_id, user_id, callback) ->
			callback = _.once(callback)
			url = ThirdPartyReferenceAgent._buildUrl(user_id, linkedFileData)
			if ThirdPartyReferenceAgent._isOrphanedImport(linkedFileData)
				return callback(new FileCannotRefreshError())
			ThirdPartyReferenceAgent._userIsImporter user_id, linkedFileData, (err, isImporter) ->
		...
		_doImport: (url, project_id, linkedFileData, name, parent_folder_id, user_id, callback) ->
			callback = _.once(callback)
			ReferencesApiHandler.userCanMakeRequest user_id, provider, (err, canMakeRequest) ->
				return callback(err) if err?
				return callback(new FeatureNotAvailableError()) if !canMakeRequest
				opts =
					method:"get"
					url: url
				logger.log {user_id, provider}, "[ThirdPartyReferenceAgent] getting bibtex from third-party-references"
	...
	return ThirdPartyReferenceAgent
