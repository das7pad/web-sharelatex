...
module.exports = ZoteroAgent = {

	_buildUrl: (user_id, linkedFileData) ->
		"/user/#{user_id}/zotero/bibtex"
	...
	createLinkedFile: (project_id, linkedFileData, name, parent_folder_id, user_id, callback) ->
		...
		url = ZoteroAgent._buildUrl(user_id, linkedFileData)
		ZoteroAgent._doImport(url, project_id, linkedFileData, name, parent_folder_id, user_id, callback)

	refreshLinkedFile: (project_id, linkedFileData, name, parent_folder_id, user_id, callback) ->
		callback = _.once(callback)
		url = ZoteroAgent._buildUrl(user_id, linkedFileData)
		if ZoteroAgent._isOrphanedImport(linkedFileData)
			return callback(new FileCannotRefreshError())
		ZoteroAgent._userIsImporter user_id, linkedFileData, (err, isImporter) ->
	...
	_doImport: (url, project_id, linkedFileData, name, parent_folder_id, user_id, callback) ->
		callback = _.once(callback)
		ReferencesApiHandler.userCanMakeRequest user_id, 'zotero', (err, canMakeRequest) ->
			return callback(err) if err?
			return callback(new FeatureNotAvailableError()) if !canMakeRequest
			opts =
				method:"get"
				url: url
			logger.log {user_id, 'zotero'}, "[ZoteroAgent] getting bibtex from third-party-references"
			ReferencesApiHandler.make3rdRequest opts, (err, response, body)->
				if err
					logger.err {user_id}, "[ZoteroAgent] error getting bibtex from third-party-references"
					return callback(err)
				if 200 <= response.statusCode < 300
					# Do import with bibtex content
					logger.log {user_id}, "[ZoteroAgent] got bibtex from third-party-references, importing"
					...
					logger.log {user_id, project_id, statusCode:response.statusCode},
						"[ZoteroAgent] error code from tpr api"
