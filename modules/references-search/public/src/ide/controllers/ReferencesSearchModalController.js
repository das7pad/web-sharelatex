 ...
 */
define(['base'], App =>
  App.controller('ReferencesSearchModalController', function(
    $scope,
    $modalInstance,
    $window,
    $timeout,
    ...
    $scope.moveSelectionForward = function() {
      if ($scope.state.selectedIndex === null) {
        if (
          ...
          $scope.state.searchResults.length > 0
        ) {
    ...
    $scope.doSearch = function() {
      ...
      ide.$http
        .post(`/project/${$scope.project_id}/references/search`, {
          query: $scope.state.queryText,
          _csrf: window.csrfToken
        })
        .then(
      ...
      return $timeout(() => ($scope.state.currentlySearching = false), 30000)
    }
