...
define([
	...
	return App.controller("ReferencesSearchController", function($scope, $modal, ide, $timeout, sixpack, event_tracking, $rootScope) {
		...
		$scope.updateHintNode = function(editor) {
			if (__guard__(__guard__(editor != null ? editor.completer : undefined, x1 => x1.popup), x => x.isOpen) === true) {
				if (!$scope._hintNode) {
					if ($scope.isCursorAtCitation(editor)) {
						...
						const container = __guard__(__guard__(__guard__(editor != null ? editor.completer : undefined, x4 => x4.popup), x3 => x3.renderer), x2 => x2.container);
						if (!container) {
		...
		$scope.setup = function() {
			...
			const startAutocomplete = __guard__(__guard__(editor != null ? editor.commands : undefined, x1 => x1.commands), x => x.startAutocomplete);
			...
			if (!startAutocomplete._sl_patched) {
				...
				const handleControlSpace = function(ed) {
					...
					if ((__guard__(__guard__(ed != null ? ed.completer : undefined, x3 => x3.popup), x2 => x2.isOpen) === true) && (isAtCitation === true)) {
							
						event_tracking.sendMB("bib-search-modal-opened");

						if ($scope.shouldABTestBibSearch) { sixpack.convert('bib-search-highlight'); }

						$scope.searchEnabled = __guard__(__guard__(__guard__(ide != null ? ide.$scope : undefined, x6 => x6.project), x5 => x5.features), x4 => x4.referencesSearch) === true;
		...
		$scope.insertKeyAtCursor = function(key) {
			...
			const lineUpToCursor = editor.getSession().getTextRange(new Range(pos.row, 0, pos.row, pos.column));
			const lineBeyondCursor = editor.getSession().getTextRange(new Range(pos.row, pos.column, pos.row, 99999));
		...
		return ide.referencesSearchManager = {
			openReferencesModal(providerStr) { return $scope.openReferencesSearchModal(providerStr); }
		};
});
});
