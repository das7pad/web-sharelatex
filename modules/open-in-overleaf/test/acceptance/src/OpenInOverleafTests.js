	...
	return describe("when creating a project from a snippet", function() {
		...
		describe("when POSTing a snippet with a valid csrf token via xhr", function() {
			...
			return it("should create a project with the returned id", function(done) {
				const projectId = JSON.parse(this.body).redirect.match(this.uri_regex)[1];
		...
		describe("when GETing with a snippet in the query", function() {
			beforeEach(function(done) {
				return this.user.request.get("/docs?snip=test"
				, (_err, _res, _body) => {
					this.err = _err;
					this.res = _res;
					this.body = _body;
		...
		describe("when GETing with a csrf token", function() {
			beforeEach(function(done) {
				return this.user.request.get(`/docs?snip=test?_csrf=${this.user.csrfToken}`
				, (_err, _res, _body) => {
					this.err = _err;
					this.res = _res;
					this.body = _body;
		...
		describe("when POSTing a snippet for a non-logged-in user", () => it("should render the gateway page", function(done) {
		...
		describe("when POSTing without a snippet", () => it("should render an error page", function(done) {
            return this.user.request.post({
                url: "/docs",
                form: {
                    _csrf: this.user.csrfToken
		...
		describe("when POSTing an encoded snippet with valid csrf", function() {
			...
			return it("should create a project containing the decoded snippet", function(done) {
				...
				return ProjectGetter.getProject(projectId, function(error, project) {
					...
					expect(project).to.exist;
					return ProjectEntityHandler.getDoc(project._id, project.rootDoc_id, function(error, lines) {
						if (error != null) { return done(error); }
		...
		describe("when POSTing a snip_uri that does not exist", function() {
			...
			return it("should return a 'not found' error", function() {
				...
				return expect(JSON.parse(this.body).error).to.equal(translations.i18n.translate('not_found_error_from_the_supplied_url'));
			});
		});
		...
		describe("when POSTing a template name that does not exist", function() {
			...
			return it("should return a 'not found' error", function() {
				expect(this.res.statusCode).to.equal(404);
				return expect(JSON.parse(this.body).error).to.equal('the_requested_template_was_not_found');
			});
		});
		...
		describe("when sending more than one kind of snippet parameter", function() {
			...
			return it("should return an 'ambiguous parameters' error", function() {
				...
				return expect(JSON.parse(this.body).error).to.equal(translations.i18n.translate('more_than_one_kind_of_snippet_was_requested'));
			});
		});
		...
		describe("when the document has a title", function() {
			...
			return it("should ensure that the project name is unique", function(done) {
				...
				, (err, res, body) => {
					expect(err).not.to.exist;
					const newProjectId = JSON.parse(body).redirect.match(this.uri_regex)[1];
					expect(newProjectId).to.exist;

					return ProjectGetter.getProject(newProjectId, function(error, project) {
						if (error != null) { return done(error); }
