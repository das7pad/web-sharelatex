...
const DocumentHelper = require('../../../../app/src/Features/Documents/DocumentHelper')
const ProjectUploadManager = require('../../../../app/src/Features/Uploads/ProjectUploadManager')
...
module.exports = OpenInOverleafController = {
  ...
  openInOverleaf(req, res, next) {
    ...
    if (paramCount > 1) {
      return next(new OpenInOverleafErrors.AmbiguousParametersError())
    }
    ...
    return OpenInOverleafController._populateSnippetFromRequest(
      req,
      function (err, snippet) {
        ...
        if (snippet.snip != null) {
          return OpenInOverleafController._createProjectFromPostedSnippet(
            user_id,
            snippet,
            sendResponse
          )
        } else if (snippet.projectFile != null) {
          return OpenInOverleafController._createProjectFromZipArchive(
            user_id,
            snippet,
            sendResponse
          )
        } else if (snippet.files != null) {
          return OpenInOverleafController._createProjectFromFileList(
            user_id,
            snippet,
            sendResponse
          )
        ...
        function (cb) {
          const projectName =
            typeof snippet.snip_name === 'string'
              ? snippet.snip_name
              : DocumentHelper.getTitleFromTexContent(content) ||
                snippet.defaultTitle
          return ProjectDetailsHandler.generateUniqueName(
            user_id,
            ProjectDetailsHandler.fixProjectName(projectName),
            (err, name) => cb(err, name)
          )
        },
        (projectName, cb) =>
          ProjectCreationHandler.createProjectFromSnippet(
            user_id,
            projectName,
            content,
            (err, project) => cb(err, project)
          ),
        (project, cb) =>
        ...
        cb =>
          ProjectDetailsHandler.generateUniqueName(
            ...
            ProjectDetailsHandler.fixProjectName(
              snippet.title || snippet.defaultTitle
            ),
            (err, name) => cb(err, name)
          ),
        (projectName, cb) =>
  ...
  _populateSnippetFromRequest(req, cb) {
    ...
    return OpenInOverleafController._getSnippetContentsFromRequest(
      ...
      function (error, snippet) {
        ...
        if (
          snippet.snip == null &&
          snippet.projectFile == null &&
          snippet.files == null
        ) {
          return cb(new Errors.InvalidError())
        }

        snippet.comment = comment
        if (req.body.engine != null) {
          snippet.engine = req.body.engine
        }
        if (req.body.publisher_slug != null) {
          snippet.publisherSlug = req.body.publisher_slug
        }
        snippet.defaultTitle = OpenInOverleafController._getDefaultTitleFromSnipRequest(
          req
        )
  ...
  _getSnippetContentsFromRequest(req, cb) {
    ...
    if (req.body.snip_name != null) {
      snippet.snip_name = req.body.snip_name
    }
    if (Array.isArray(snippet.snip_name) && snippet.snip_name.length === 1) {
      snippet.snip_name = snippet.snip_name[0]
    }
    if (req.body.brand_variation_id != null) {
      snippet.brandVariationId = req.body.brand_variation_id
    }
    ...
    if (req.body.snip != null) {
      ...
      if (req.body.snip_uri.length === 1) {
        return OpenInOverleafHelper.populateSnippetFromUri(
          req.body.snip_uri[0],
          snippet,
          cb
        )
      } else {
        return OpenInOverleafHelper.populateSnippetFromUriArray(
          req.body.snip_uri,
          snippet,
          cb
        )
      ...
      return OpenInOverleafHelper.populateSnippetFromUri(
        req.body.snip_uri || req.body.zip_uri,
        snippet,
        cb
      )
    } else if (req.body.template != null) {
      return OpenInOverleafHelper.populateSnippetFromTemplate(
        req.body.template,
        snippet,
        cb
      )
    } else if (req.body.partner != null && req.body.client_media_id != null) {
      return OpenInOverleafHelper.populateSnippetFromConversionJob(
        req.body.partner,
        req.body.client_media_id,
        snippet,
        cb
      )
    } else {
