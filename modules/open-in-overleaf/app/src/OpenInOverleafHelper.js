...
const UrlHelper = require('../../../../app/src/Features/Helpers/UrlHelper')
const ProjectHelper = require('../../../../app/src/Features/Project/ProjectHelper')
const ProjectRootDocManager = require('../../../../app/src/Features/Project/ProjectRootDocManager')
...
const V1Api = require('../../../../app/src/Features/V1/V1Api')
...
module.exports = OpenInOverleafHelper = {
  ...
  getDocumentLinesFromSnippet(snippet, content = null) {
    return (
      snippet.comment +
      OpenInOverleafHelper._normalizeMainSrcContent(snippet, content)
    )
      .trim()
      .split('\n')
  },
  ...
  populateSnippetFromTemplate(template, source_snippet, cb) {
    ...
    if (templateData == null) {
      return cb(new OpenInOverleafErrors.TemplateNotFoundError())
    }

    const snippet = OpenInOverleafHelper._cloneSnippet(source_snippet)

    if (templateData.brand_variation_id != null) {
      snippet.brandVariationId = templateData.brand_variation_id
    }
    return OpenInOverleafHelper.populateSnippetFromUri(
      `${
        settings.openInOverleaf != null
          ? settings.openInOverleaf.templateUriPrefix
          : undefined
      }${template}.zip`,
      snippet,
      cb
    ...
    return V1Api.request(
      ...
      function(err, response, body) {
        if (
          ...
          (body != null ? body.input_file_uri : undefined) == null
        ) {
          return cb(new OpenInOverleafErrors.ConversionNotFoundError())
        }
        if (err != null) {
          return cb(err)
        }
        ...
        const snippet = OpenInOverleafHelper._cloneSnippet(source_snippet)
        if (body.brand_variation_id != null) {
          snippet.brandVariationId = body.brand_variation_id
        }
        return OpenInOverleafHelper.populateSnippetFromUri(
          body.input_file_uri,
          snippet,
      ...
      function(file, cb) {
        if (file.content != null) {
          return ProjectEntityUpdateHandler.addDoc(
            ...
            file.name,
            OpenInOverleafHelper.getDocumentLinesFromSnippet(
              snippet,
              file.content
            ),
            project.owner_ref,
          ...
          V1Api.request(
            ...
            function(err, response, body) {
              if (
                (err != null ? err.statusCode : undefined) === 404 ||
                (response != null ? response.statusCode : undefined) === 404 ||
                (body != null ? body.default_variation_id : undefined) == null
              ) {
                return cb(new OpenInOverleafErrors.PublisherNotFoundError())
              }
              if (err != null) {
                return cb(err)
              }
...
`
  ...
  _normalizeMainSrcContent(snippet, content = null) {
    const r = OpenInOverleafHelper._wrapSnippetIfNoDocumentClass(
      OpenInOverleafHelper.normalizeLatexContent(content || snippet.snip),
...
`
  ...
  _ensureFilesHaveUniqueNames(files, callback) {
    ...
    for (var file of Array.from(files)) {
      ...
      if (filenamesByExtension[ext] == null) {
        filenamesByExtension[ext] = []
      }
      const base = file.name.substring(0, file.name.length - ext.length)
      ProjectHelper.ensureNameIsUnique(
        filenamesByExtension[ext],
        base,
        [],
        100,
      ...
      for (const file of Array.from(snippet.files)) {
        if (file.content != null) {
          if (
            ...
            DocumentHelper.contentHasDocumentclass(file.content)
          ) {
            snippet.rootDoc = file.name
          }
          const title = DocumentHelper.getTitleFromTexContent(file.content)
          if (title != null) {
            snippet.title = title
            if (file.name === snippet.rootDoc) {
              break
            } else {
