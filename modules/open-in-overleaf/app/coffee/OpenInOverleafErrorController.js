...
module.exports = (OpenInOverleafErrorController = {
	handleError(err, req, res, next){
		const e = OpenInOverleafErrorController._statusAndTranslatableTextForError(err, req);
		...
		if (req.xhr || ((req.headers.accept != null ? req.headers.accept.indexOf('json') : undefined) > -1)) {
			...
			return res.status(e.status).render(Path.resolve(__dirname, '../views/gateway'), {error: errorText});
		}
	},
