...
module.exports = OpenInOverleafErrorController =
	handleError: (err, req, res, next)->
		e = OpenInOverleafErrorController._statusAndTranslatableTextForError(err, req)
		errorText = req.i18n.translate(e.text)

		if req.xhr || req.headers.accept?.indexOf('json') > -1
			res.setHeader('Content-Type', 'application/json')
			res.status(e.status).send(JSON.stringify({error: errorText}))
		else
			res.status(e.status).render(Path.resolve(__dirname, '../views/gateway'), error: errorText)
	...
	_statusAndTranslatableTextForError: (error, req) ->
		...
		if error instanceof OpenInOverleafErrors.InvalidFileTypeError
			...
			return {status: 422, text: 'the_file_supplied_is_of_an_unsupported_type'}
		...
		logger.error err: error, url:req.url, method:req.method, user:user, "unhandled error"
		return {status: 500, text: 'sorry_something_went_wrong_opening_the_document_please_try_again'}
