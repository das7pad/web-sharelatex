...
module.exports = {
  apply(webRouter, apiRouter) {
    ...
    adminRouter.delete(
      '/user/:user_id/overleaf',
      UserAdminController.deleteOverleafV1Link
    )
    adminRouter.delete(
      '/user/:user_id/secondaryemail',
      UserAdminController.deleteSecondaryEmail
    )
    adminRouter.post(
      '/user/:user_id/refresh_features',
      UserAdminController.refreshFeatures
    )

    adminRouter.post('/user/:user_id/email', UserAdminController.updateEmail)

    adminRouter.get(
      '/user/:user_id/subscription/new',
      SubscriptionAdminController.new
    )
    adminRouter.get(
      '/user/:user_id/subscription/:subscription_id',
      SubscriptionAdminController.show
    )
    adminRouter.post('/subscription', SubscriptionAdminController.create)
    adminRouter.post(
      '/subscription/:subscription_id',
      SubscriptionAdminController.update
    )
    adminRouter.delete(
      '/subscription/:subscription_id',
      SubscriptionAdminController.delete
    )
