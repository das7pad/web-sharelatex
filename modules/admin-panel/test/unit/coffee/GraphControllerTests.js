...
describe("GraphController", function() {
	beforeEach(function() {
		...
		this.GraphController = SandboxedModule.require(modulePath, { requires: {
			"logger-sharelatex": { 
				log() {},
				err() {}
			},
		...
		this.projects = [
			{_id:1, owner_ref: this.users[0]._id, readOnly_refs: [], collaberator_refs: []}, 
			{_id:2, owner_ref: this.users[0]._id, readOnly_refs: this.users[2]._id, collaberator_refs: this.users[3]._id},
			{_id:3, owner_ref: this.users[0]._id, readOnly_refs: this.users[1]._id, collaberator_refs: []},
			{_id:4, owner_ref: this.users[1]._id, readOnly_refs: this.users[0]._id, collaberator_refs: []},
		];
	...
	describe("userGraph", function() {
		...
		beforeEach(function() {
			...
			return this.GraphController._nextLevel = sinon.stub().callsArgWith(3, null, this.SigmaJSGraph);
		});
	...
	describe("_genGraph", function() {
		...
		it("should create graph with nodes", function(done){ 
			return this.GraphController._genGraph(this.projects, [this.users[0]._id.toString()], this.SigmaJSGraph, function(err, graph) {
	...
	describe("_nextLevel", function() {
		...
		beforeEach(function() {
			...
			this.GraphController._genGraph = sinon.stub().callsArgWith(3, null, this.OneLevelGraph);
			return this.GraphController._getNames = sinon.stub().callsArgWith(1, null, this.OneLevelGraph);
		});

		return it("should create 1-level graph", function(done){ 
			return this.GraphController._nextLevel([this.users[0]._id.toString()], this.emptyGraph, 1, function(err, graph) {
	...
	return describe("_getNames", () => it("should add name to nodes", function(done){ 
        return this.GraphController._getNames({nodes: this.nodes, edges:[]}, function(err, graph) {
