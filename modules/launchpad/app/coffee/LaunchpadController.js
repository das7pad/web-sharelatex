...
module.exports = (LaunchpadController = {
	...
	launchpadPage(req, res, next) {
		...
		return LaunchpadController._atLeastOneAdminExists(function(err, adminUserExists) {
			if (err != null) {
				return next(err);
			}
			if (!sessionUser) {
				if (!adminUserExists) {
					return res.render(Path.resolve(__dirname, "../views/launchpad"), {adminUserExists, authMethod});
				} else {
					return AuthenticationController._redirectToLoginPage(req, res);
				}
			} else {
				return UserGetter.getUser(sessionUser._id, {isAdmin: 1}, function(err, user) {
					if (err != null) {
						return next(err);
					}
	...
	_atLeastOneAdminExists(callback) {
		...
		return UserGetter.getUser({isAdmin: true}, {_id: 1, isAdmin: 1}, function(err, user) {
			if (err != null) {
				return callback(err);
			}
		...
		return function(req, res, next) {
			...
			return LaunchpadController._atLeastOneAdminExists(function(err, exists) {
				...
				return UserRegistrationHandler.registerNewUser(body, function(err, user) {
					...
					return User.update({_id: user._id}, {$set: {isAdmin: true}}, function(err) {
						if (err != null) {
	...
	registerAdmin(req, res, next) {
		...
		return LaunchpadController._atLeastOneAdminExists(function(err, exists) {
			...
			return UserRegistrationHandler.registerNewUser(body, function(err, user){
				...
				const proceed = () => User.update({_id: user._id}, {$set: {isAdmin: true}}, function(err) {
                    if (err != null) {
                        logger.err({user_id: user._id, err}, "error setting user to admin");
                        return next(err);
                    }
				...
				if ((Settings.overleaf != null) && (Settings.createV1AccountOnLogin != null)) {
					logger.log({user_id: user._id}, "Creating backing account in v1 for new admin user");
					const SharelatexAuthController = require(
					...
					return UserGetter.getUser(user._id, function(err, user) {
						...
						return SharelatexAuthController._createBackingAccountIfNeeded(user, req, function(err) {
							if (err != null) { return next(err); }
							return proceed();
						});
					});
