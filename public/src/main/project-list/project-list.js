...
define([
	...
	App.controller("ProjectPageController", function($scope, $modal, $q, $window, queuedHttp, event_tracking, $timeout, localStorage) {
		let project_id;
		...
		$timeout(() => recalculateProjectListHeight()
		, 10);

		$scope.$watch((
			() => $scope.projects.filter(project => ((project.tags == null) || (project.tags.length === 0)) && !project.archived).length), newVal => $scope.nUntagged = newVal);
		...
		$scope.$on("project-list:notifications-received", () => $scope.$applyAsync(() => recalculateProjectListHeight()));
		...
		$scope.updateSelectedProjects = function() {
			...
			return $scope.isArchiveableProjectSelected = $scope.selectedProjects.some(project => window.user_id === project.owner._id);
		};

		$scope.getSelectedProjects = () => $scope.selectedProjects;

		$scope.getSelectedProjectIds = () => $scope.selectedProjects.map(project => project.id);
		...
		$scope.updateVisibleProjects = function() {
			...
			for (project of Array.from($scope.projects)) {
				...
				if (($scope.filter === "tag") && (selectedTag != null) && !Array.from(selectedTag.project_ids).includes(project.id)) {
					visible = false;
				}
		...
		$scope.removeSelectedProjectsFromTag = function(tag) {
			...
			const removed_project_ids = $scope._removeProjectIdsFromTagArray(tag, selected_project_ids);
		...
		$scope.addSelectedProjectsToTag = function(tag) {
			...
			return (() => {
				...
				for (project_id of Array.from(added_project_ids)) {
					result.push(queuedHttp.post(`/tag/${tag._id}/project/${project_id}`, {
		...
		$scope.createProject = function(name, template) {
			if (template == null) { template = "none"; }
			return queuedHttp
				.post("/project/new", {
					_csrf: window.csrfToken,
					projectName: name,
		...
		$scope.openCreateProjectModal = function(template) {
			...
			return modalInstance.result.then(project_id => window.location = `/project/${project_id}`);
		};
		...
		$scope.openRenameProjectModal = function() {
			...
			if ((project == null) || (project.accessLevel !== "owner")) { return; }
		...
		$scope.openArchiveProjectsModal = function() {
			const modalInstance = $modal.open({
				...
				resolve: {
					projects() { return $scope.getSelectedProjects(); }
				}
			});
			event_tracking.send('project-list-page-interaction', 'project action', 'Delete');
			return modalInstance.result.then(() => $scope.archiveOrLeaveSelectedProjects());
		};

		$scope.archiveOrLeaveSelectedProjects = () => $scope.archiveOrLeaveProjects($scope.getSelectedProjects());
		...
		$scope.restoreSelectedProjects = () => $scope.restoreProjects($scope.getSelectedProjects());
		...
		$scope.downloadSelectedProjects = () => $scope.downloadProjectsById($scope.getSelectedProjectIds());
	...
	return App.controller("ProjectListItemController", function($scope, $modal, queuedHttp) {

		$scope.shouldDisableCheckbox = project => ($scope.filter === 'archived') && (project.accessLevel !== 'owner');
		...
		$scope.ownerName = function() {
			if ($scope.project.accessLevel === "owner") {
				...
				return [$scope.project.owner.first_name, $scope.project.owner.last_name].filter(n => n != null).join(" ");
			} else {
				return "None";
			}
		};
		...
		$scope.clone = function(e) {
			...
			return $scope.cloneProject($scope.project, `${$scope.project.name} (Copy)`)
		...
		return $scope.deleteProject = function(e) {
			...
			return modalInstance.result.then(function() {
				...
				return queuedHttp({
					...
					return $scope.updateVisibleProjects();}).catch(() => $scope.project.isTableActionInflight = false);
			});
		};
	});
});
