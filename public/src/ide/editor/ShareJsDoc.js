	...
	return ShareJsDoc = (function() {
		ShareJsDoc = class ShareJsDoc extends EventEmitter {
			...
			constructor(doc_id, docLines, version, socket) {
				...
				this.socket = socket;
				this.type = "text";
				docLines = (Array.from(docLines).map((line) => decodeURIComponent(escape(line))));
				...
				this.connection = {
					send: update => {
						...
						if ((window.disconnectOnUpdate != null) && (Math.random() < window.disconnectOnUpdate)) {
						...
						if (this.track_changes) {
							if (update.meta == null) { update.meta = {}; }
							update.meta.tc = this.track_changes_id_seeds.inflight;
						}
						return this.socket.emit("applyOtUpdate", this.doc_id, update, error => {
							if (error != null) { return this._handleError(error); }
						});
					},
					state: "ok",
			...
			submitOp(...args) { return this._doc.submitOp(...Array.from(args || [])); }
			...
			processUpdateFromServer(message) {
				...
				if (__guard__(message != null ? message.meta : undefined, x => x.type) === "external") {
			...
			getSnapshot() { return this._doc.snapshot; }
			getVersion() { return this._doc.version; }
			...
			getInflightOp() { return this._doc.inflightOp; }
			getPendingOp() { return this._doc.pendingOp; }
			getRecentAck() {
				// check if we have received an ack recently (within a factor of two of the single user flush delay)
				return (this.lastAcked != null) && ((new Date() - this.lastAcked) <  (2 * SINGLE_USER_FLUSH_DELAY));
			}
			getOpSize(op) {
			...
			attachToAce(ace) { return this._doc.attach_ace(ace, false, window.maxDocLength); }
			detachFromAce() { return (typeof this._doc.detach_ace === 'function' ? this._doc.detach_ace() : undefined); }

			attachToCM(cm) { return this._doc.attach_cm(cm, false); }
			detachFromCM() { return (typeof this._doc.detach_cm === 'function' ? this._doc.detach_cm() : undefined); } // If we're waiting for the project to join, try again in 0.5 seconds
			_startInflightOpTimeout(update) {
				...
				var retryOp = () => {
					...
					if (this._doc.inflightOp != null) {
						...
						update.dupIfSource = [this.connection.id, ...Array.from(this._doc.inflightSubmittedIds)];
						...
						if (this.connection.state !== "ok") {
							...
							return timer = setTimeout(retryOp, this.WAIT_FOR_CONNECTION_TIMEOUT);
						} else {
