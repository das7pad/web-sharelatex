...
define([
	...
	return (AutoCompleteManager = class AutoCompleteManager {
			...
			const ReferencesCompleter = {
				getCompletions(editor, session, pos, prefix, callback) {
					...
					if (commandFragment) {
						...
						if (citeMatch) {
							let [_, commandName, previousArgs, currentArg] = Array.from(citeMatch);
		...
		onChange(change) {
			...
			const { end } = change;
			const {lineUpToCursor, commandFragment} = Helpers.getContext(this.editor, end);
			if (((i = lineUpToCursor.indexOf('%')) > -1) && (lineUpToCursor[i-1] !== '\\')) {
				return;
			}
			...
			if (/^\\[^a-zA-Z]$/.test(lastTwoChars)) {
				__guardMethod__(this.editor != null ? this.editor.completer : undefined, 'detach', o => o.detach());
				return;
			}
		...
		monkeyPatchAutocomplete() {
			...
			if ((Autocomplete.prototype._insertMatch == null)) {
						...
						data.completer = {
							insertMatch(editor, matchData) {
								...
								if (matchData.snippet) {
									return aceSnippetManager.insertSnippet(editor, matchData.snippet);
								} else {
									return editor.execCommand("insertstring", matchData.value || matchData);
								}
							}
						};
					}
				...
				Autocomplete.startCommand = {
					...
					exec: editor => {
						...
						if (filtered = __guard__(__guard__(editor != null ? editor.completer : undefined, x1 => x1.completions), x => x.filtered)) {
						...
						if (__guard__(__guard__(editor.completer != null ? editor.completer.completions : undefined, x3 => x3.filtered), x2 => x2.length) === 0) {
			...
			return Util.retrievePrecedingIdentifier = function(text, pos, regex) {
				...
				for (let start = pos-1, i = start, asc = start <= 0; asc ? i <= 0 : i >= 0; asc ? i++ : i--) {
					if (text[i] === "\n") {
