...
define([
	...
	var Helpers = {
		...
		getLastCommandFragmentIndex(lineUpToCursor) {
			...
			let m;
			const blankArguments = lineUpToCursor.replace(/\[([^\]]*)\]/g, args => Array(args.length + 1).join('.'));
		...
		getCommandNameFromFragment(commandFragment) {
			return __guard__(commandFragment != null ? commandFragment.match(/\\(\w+)\{/) : undefined, x => x[1]);
		},
