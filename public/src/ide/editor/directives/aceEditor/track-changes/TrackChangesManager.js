		...
		TrackChangesManager = class TrackChangesManager {
			...
			rejectChangeIds(change_ids) {
				...
				for (let change of Array.from(changes)) {
					if (change.op.d != null) {
						...
						const end = this.adapter.shareJsOffsetToAcePosition(change.op.p + change.op.i.length);
			...
			checkMapping() {
				...
				for (var change of Array.from(this.rangesTracker.changes)) {
					if (this.adapter.changeIdToMarkerIdMap[change.id] != null) {
						({ op } = change);
						({background_marker_id, callout_marker_id} = this.adapter.changeIdToMarkerIdMap[change.id]);
				...
				for (let comment of Array.from(this.rangesTracker.comments)) {
					if (this.adapter.changeIdToMarkerIdMap[comment.id] != null) {
						({background_marker_id, callout_marker_id} = this.adapter.changeIdToMarkerIdMap[comment.id]);
						start = this.adapter.shareJsOffsetToAcePosition(comment.op.p);
						end = this.adapter.shareJsOffsetToAcePosition(comment.op.p + comment.op.c.length);
						expected_markers.push({ marker_id: background_marker_id, start, end });
						expected_markers.push({ marker_id: callout_marker_id, start, end: start });
					}
				}
			...
			recalculateReviewEntriesScreenPositions() {
				...
				for (let entry_id in object) {
					...
					const doc_position = this.adapter.shareJsOffsetToAcePosition(entry.offset);
			...
			_onCommentRemoved(comment) {
				if (this.adapter.changeIdToMarkerIdMap[comment.id] != null) {
					...
					const {background_marker_id, callout_marker_id} = this.adapter.changeIdToMarkerIdMap[comment.id];
			...
			_onChangeMoved(change) {
				...
				const start = this.adapter.shareJsOffsetToAcePosition(change.op.p);
				if (change.op.i != null) {
					end = this.adapter.shareJsOffsetToAcePosition(change.op.p + change.op.i.length);
				} else {
					end = start;
				}
			...
			_updateMarker(change_id, start, end) {
				if ((this.adapter.changeIdToMarkerIdMap[change_id] == null)) { return; }
				const session = this.editor.getSession();
				const markers = session.getMarkers();
				const {background_marker_id, callout_marker_id} = this.adapter.changeIdToMarkerIdMap[change_id];
