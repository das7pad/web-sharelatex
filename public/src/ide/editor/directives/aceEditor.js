...
define([
...
], function(App, Ace, SearchBox, Vim, ModeList, UndoManager, AutoCompleteManager, SpellCheckManager, SpellCheckAdapter, HighlightsManager, CursorPositionManager, CursorPositionAdapter, TrackChangesManager, TrackChangesAdapter, MetadataManager) {
	...
	App.directive("aceEditor", function($timeout, $compile, $rootScope, event_tracking, localStorage, $cacheFactory, metadata, graphics, preamble, files, $http, $q, $window) {
		...
		return  {
			...
			link(scope, element, attrs) {
				...
				if (!window._debug_editors) { window._debug_editors = []; }
				...
				if (scope.spellCheck) { // only enable spellcheck when explicitly required
					const spellCheckCache = $cacheFactory.get(`spellCheck-${scope.name}`) || $cacheFactory(`spellCheck-${scope.name}`, {capacity: 1000});
				...
				const cursorPositionManager = new CursorPositionManager(scope, new CursorPositionAdapter(editor), localStorage);
				const trackChangesManager   = new TrackChangesManager(scope, editor, element, new TrackChangesAdapter(editor));
				const metadataManager = new MetadataManager(scope, editor, element, metadata);
				const autoCompleteManager = new AutoCompleteManager(scope, editor, element, metadataManager, graphics, preamble, files);
				...
				const getCursorScreenPosition = function() {
					...
					const sessionPos = session.documentToScreenPosition(cursorPosition.row, cursorPosition.column);
					const screenPos = editor.renderer.textToScreenCoordinates(sessionPos.row, sessionPos.column);
				...
				if (attrs.resizeOn != null) {
					for (let event of Array.from(attrs.resizeOn.split(","))) {
						scope.$on(event, function() {
							...
							const session = editor.getSession();
							return session.setScrollTop((session.getScrollTop() + newScreenPosition) - previousScreenPosition);
						});
					}
				}
				...
				scope.$watch("fontFamily", function(value) {
					if (value != null) {
						switch (value) {
							case 'monaco':
								return editor.setOption('fontFamily', '"Monaco", "Menlo", "Ubuntu Mono", "Consolas", "source-code-pro", monospace');
							case 'lucida':
								return editor.setOption('fontFamily', '"Lucida Console", monospace');
							default:
				...
				const onScroll = function(scrollTop) {
					if ((scope.eventsBridge == null)) { return; }
				...
				const onScrollbarVisibilityChanged = function(event, vRenderer) {
					if ((scope.eventsBridge == null)) { return; }
					return scope.eventsBridge.emit("aceScrollbarVisibilityChanged", vRenderer.scrollBarV.isVisible, vRenderer.scrollBarV.width);
				};

				if (scope.eventsBridge != null) {
					editor.renderer.on("scrollbarVisibilityChanged", onScrollbarVisibilityChanged);

					scope.eventsBridge.on("externalScroll", position => editor.getSession().setScrollTop(position));
				...
				const onSessionChangeForCursorPosition = function(e) {
					...
					return e.session.selection.on('changeCursor', cursorPositionManager.onCursorChange);
				};

				const onUnloadForCursorPosition = () => cursorPositionManager.onUnload(editor.getSession());
				...
				var attachToAce = function(sharejs_doc) {
					...
					if ((scope.syntaxValidation != null) && syntaxValidationEnabled && !/\.bib$/.test(scope.fileName)) {
				...
				editor.renderer.on("changeCharacterSize", () =>
					scope.$apply(() => scope.rendererData.lineHeight = editor.renderer.lineHeight)
				);
				...
				scope.$on('$destroy', function() {
					if (scope.sharejsDoc != null) {
						...
						return scope.eventsBridge.emit("aceScrollbarVisibilityChanged", false, 0);
					}
				});
