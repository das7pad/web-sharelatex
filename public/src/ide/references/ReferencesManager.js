	...
	return (ReferencesManager = class ReferencesManager {
		constructor(ide, $scope) {
			...
			this.$scope.$on('document:closed', (e, doc) => {
				...
				if (__guard__(entity != null ? entity.name : undefined, x => x.match(/.*\.bib$/))) {
		...
		indexReferences(docIds, shouldBroadcast) {
			...
			return this.ide.$http.post(
				`/project/${this.$scope.project_id}/references/index`,
				opts
			).then(response => {
		...
		indexAllReferences(shouldBroadcast) {
			...
			return this.ide.$http.post(
				`/project/${this.$scope.project_id}/references/indexAll`,
				opts
			).then(response => {
				return this._storeReferencesKeys(response.data.keys);
			});
