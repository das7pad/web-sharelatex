...
define([
	...
	App.factory('PDFRenderer', ['$q', '$timeout', 'pdfAnnotations', 'pdfTextLayer', 'pdfSpinner', function($q, $timeout, pdfAnnotations, pdfTextLayer, pdfSpinner) {
		...
		return PDFRenderer = (function() {
			PDFRenderer = class PDFRenderer {
				...
				constructor(url, options) {
					...
					if (__guard__(window.location != null ? window.location.search : undefined, x => x.indexOf("disable-font-face=true")) >= 0) {
					...
					this.pdfjs = PDFJS.getDocument({url: this.url, rangeChunkSize: 2*65536});
				...
				resetState() {
					...
					for (let page of Array.from(this.pageState || [])) {
						...
						__guard__(page != null ? page.renderTask : undefined, x1 => x1.cancel());
					}
				...
				getPdfViewport(pageNum, scale) {
					...
					return this.document.then(pdfDocument => {
						...
						, error => {
							return (typeof this.errorCallback === 'function' ? this.errorCallback(error) : undefined);
						});
					});
				}

				getDestinations() {
					return this.document.then(pdfDocument => pdfDocument.getDestinations());
				}

				getDestination(dest) {
					return this.document.then(pdfDocument => pdfDocument.getDestination(dest)
					, error => {
						return (typeof this.errorCallback === 'function' ? this.errorCallback(error) : undefined);
					});
				}
				...
				getPageIndex(ref) {
					return this.document.then(pdfDocument => {
						return pdfDocument.getPageIndex(ref).then(idx => idx
						, error => {
							return (typeof this.errorCallback === 'function' ? this.errorCallback(error) : undefined);
						});
					});
				...
				triggerRenderQueue(interval) {
					if (interval == null) { interval = this.JOB_QUEUE_INTERVAL; }
				...
				startIndicators() {
					...
					this.queuedPages = [];
					for (var page of Array.from(this.renderQueue)) { this.queuedPages[page.pagenum] = true; }
					...
					return (() => {
						...
						for (page of Array.from(this.renderQueue)) {
							if (!this.spinTimer[page.pagenum] && !this.spinTimerDone[page.pagenum]) {
				...
				processRenderQueue() {
					...
					let timedOut = false;
					const timer = $timeout(() => { // page load timed out
						if (loadTask.cancelled) { return; } // return from cancelled page load
						__guardMethod__(Raven, 'captureMessage', o => o.captureMessage(`pdfng page load timed out after ${this.PAGE_LOAD_TIMEOUT}ms`));
						timedOut = true;
						this.clearIndicator(page);
						// @jobs = @jobs - 1
						// @triggerRenderQueue(0)
						return (typeof this.errorCallback === 'function' ? this.errorCallback('timeout') : undefined);
					}
					...
					return loadTask.then(pageObject => {
						// page load success
				...
				doRender(element, pagenum, page) {
					...
					result.then(function() {
						...
						, error => typeof self.errorCallback === 'function' ? self.errorCallback(error) : undefined);
						return page.getAnnotations().then(annotations => annotationsLayer.setAnnotations(annotations)
