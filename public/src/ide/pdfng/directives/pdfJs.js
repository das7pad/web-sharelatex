			...
			link(scope, element, attrs) {
				...
				const flashControls = () =>
					scope.$evalAsync(function() {
						...
						return $timeout(() => scope.flashControls = false
						, 1000);
					})
				;

				scope.$on('pdfDoubleClick', (event, e) => typeof scope.dblClickCallback === 'function' ? scope.dblClickCallback({page: e.page - 1, offset: { top: e.y, left: e.x }}) : undefined);
				...
				scope.$on('progress', (event, progress) =>
					scope.$apply(function() {
						...
						if (scope.progress > 100) { scope.progress = 100; }
						if (scope.progress < 0) { return scope.progress = 0; }
					})
				);
