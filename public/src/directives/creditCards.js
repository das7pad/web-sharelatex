		...
		const cardFromNumber = function(num) {
			...
			for (let card of Array.from(cards)) {
				for (let pattern of Array.from(card.patterns)) {
					const p = pattern + "";
					if (num.substr(0, p.length) === p) { return card; }
				}
			}
		};

		const cardFromType = function(type) {
			for (let card of Array.from(cards)) { if (card.type === type) { return card; } }
		};
	...
	App.factory('ccFormat', function(ccUtils, $filter) {
		const hasTextSelected = function($target) {
			...
			if (($target.prop('selectionStart') != null) &&
				($target.prop('selectionStart') !== $target.prop('selectionEnd'))) { return true; }

			// If some text is selected in IE
			if (__guard__(typeof document !== 'undefined' && document !== null ? document.selection : undefined, x => x.createRange) != null) {
		...
		const formatCardNumber = function(e) {
			...
			if (($target.prop('selectionStart') != null) &&
				($target.prop('selectionStart') !== value.length)) { return; }
		...
		const parseCardNumber = function(value) {
    		if (value != null) { return value.replace(/\s/g, ''); } else { return value; }
   };
