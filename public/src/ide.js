...
define([
	...
	App.controller("IdeController", function($scope, $timeout, ide, localStorage, sixpack, event_tracking, metadata, $q) {
		...
		ide.toggleReviewPanel = ($scope.toggleReviewPanel = function() {
			...
			return event_tracking.sendMB("rp-toggle-panel", { value : $scope.ui.reviewPanelOpen });
		});
		...
		$scope.$watch("ui.chatOpen", function(isOpen) {
			if (isOpen) { return event_tracking.sendMBOnce("ide-open-chat-once"); }
		});

		$scope.$watch("ui.leftMenuShown", function(isOpen) {
			if (isOpen) { return event_tracking.sendMBOnce("ide-open-left-menu-once"); }
		});
		...
		$scope.$on("project:joined", function() {
			if (inited) { return; }
			inited = true;
			if (__guard__($scope != null ? $scope.project : undefined, x => x.deletedByExternalDataSource)) {
...
`);
		...
		if (pdfLayout = localStorage("pdf.layout")) {
			if (pdfLayout === "split") { $scope.switchToSideBySideLayout(); }
			if (pdfLayout === "flat") { $scope.switchToFlatLayout(); }
		} else {
		...
		try {
			const chromeVersion = parseFloat(navigator.userAgent.split(" Chrome/")[1]) || null;
		...
		ide.featureToggle = __guard__(__guard__(typeof location !== 'undefined' && location !== null ? location.search : undefined, x1 => x1.match(/^\?ft=(\w+)$/)), x => x[1]);
