...
app.config(['$provide', $provide =>
	$provide.decorator('$exceptionHandler', ['$log', '$delegate', ($log, $delegate) =>
		function(exception, cause) {
			if ((typeof Raven !== 'undefined' && Raven !== null ? Raven.captureException : undefined) != null) {
...
app.factory('unAuthHttpResponseInterceptor', ['$q','$location', ($q, $location) =>
		({
			responseError(response) {
				...
				if ([401, 403].includes(response.status) && !(response.config != null ? response.config.disableAutoLoginRedirect : undefined)) {
...
app.config(['$httpProvider', $httpProvider => $httpProvider.interceptors.push('unAuthHttpResponseInterceptor')
]);
