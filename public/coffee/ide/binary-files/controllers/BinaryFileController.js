...
define([
	...
	App.controller("BinaryFileController", ["$scope", "$rootScope", "$http", "$timeout", "$element", "ide", "waitFor", function($scope, $rootScope, $http, $timeout, $element, ide, waitFor) {
		...
		const extension = file => __guard__(file.name.split(".").pop(), x => x.toLowerCase());
		...
		$scope.refreshFile = function(file) {
			...
			$scope.refreshError = null;
			return ide.fileTreeManager.refreshLinkedFile(file)
					...
					$timeout(
						() =>
							waitFor(
								...
								.then(newFile => ide.binaryFilesManager.openFile(newFile)).catch(err => console.warn(err))
						
						, 0
					);
					return $scope.refreshError = null;}).catch(response => $scope.refreshError = response.data).finally(() => $scope.refreshing = false);
		};
		...
		(loadTextFileFilePreview = function() {
			if (!$scope.isTextFile()) { return; }
			const url = `/project/${project_id}/file/${$scope.openFile.id}?range=0-${TWO_MEGABYTES}`;
			...
			return $http({
				...
				.then(function(response) {
					...
					if (data.length >= (TWO_MEGABYTES - 200)) {
						...
						data = __guardMethod__(data, 'replace', o => o.replace(/\n.*$/, ''));
					}
					$scope.textPreview.data = data;
					return $timeout(setHeight, 0);}).catch(function(error) {
					console.error(error);
