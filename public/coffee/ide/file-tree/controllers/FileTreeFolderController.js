...
define([
	...
	App.controller("FileTreeFolderController", function($scope, ide, $modal, localStorage) {
		$scope.expanded = localStorage(`folder.${$scope.entity.id}.expanded`) || false;

		$scope.toggleExpanded = function() {
			$scope.expanded = !$scope.expanded;
			return localStorage(`folder.${$scope.entity.id}.expanded`, $scope.expanded);
		};
		...
		$scope.openNewDocModal = () =>
			$modal.open({
				...
				size: 'lg',
				resolve: {
					parent_folder() { return $scope.entity; },
					type() { return 'doc'; }
				}
			})
		;
