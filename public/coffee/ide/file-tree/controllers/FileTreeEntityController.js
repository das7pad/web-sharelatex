...
define([
	...
	App.controller("FileTreeEntityController", ["$scope", "ide", "$modal", function($scope, ide, $modal) {
		...
		$scope.finishRenaming = function() {
			...
			if (!$scope.entity.renaming) { return; }
		...
		$scope.$on("rename:selected", function() {
			if ($scope.entity.selected) { return $scope.startRenaming(); }
		});
		...
		$scope.$on("delete:selected", function() {
			if ($scope.entity.selected) { return $scope.openDeleteModal(); }
		});
