	...
	return (FileTreeManager = class FileTreeManager {
		...
		_bindToSocketEvents() {
			this.ide.socket.on("reciveNewDoc", (parent_folder_id, doc) => {
				const parent_folder = this.findEntityById(parent_folder_id) || this.$scope.rootFolder;
			...
			this.ide.socket.on("reciveNewFile", (parent_folder_id, file, source, linkedFileData) => {
				const parent_folder = this.findEntityById(parent_folder_id) || this.$scope.rootFolder;
				return this.$scope.$apply(() => {
					parent_folder.children.push({
						name: file.name,
		...
		clearMultiSelectedEntities() {
			if (this.$scope.multiSelectedCount === 0) { return; } // Be efficient, this is called a lot on 'click'
		...
		findSelectedEntity() {
			let selected = null;
			this.forEachEntity(function(entity) {
				if (entity.selected) { return selected = entity; }
			});
			return selected;
		}
		...
		findEntityById(id, options) {
			...
			if (this.$scope.rootFolder.id === id) { return this.$scope.rootFolder; }
		...
		forEachEntity(callback) {
			if (callback == null) { callback = function(entity, parent_folder, path) {}; }
		...
		loadRootFolder() {
			return this.$scope.rootFolder = this._parseFolder(__guard__(this.$scope != null ? this.$scope.project : undefined, x => x.rootFolder[0]));
		}
		...
		_getCurrentFolder(startFolder) {
			if (startFolder == null) { startFolder = this.$scope.rootFolder; }
		...
		existsInThisFolder(folder, name) {
			for (let entity of Array.from((folder != null ? folder.children : undefined) || [])) {
		...
		nameExistsError(message) {
			if (message == null) { message = "already exists"; }
		...
		createDoc(name, parent_folder) {
			...
			if (parent_folder == null) { parent_folder = this.getCurrentFolder(); }
		...
		refreshLinkedFile(file) {
			...
			const provider = file.linkedFileData != null ? file.linkedFileData.provider : undefined;
			if ((provider == null)) {
				console.warn(`>> no provider for ${file.name}`, file);
				return;
			}
			return this.ide.$http.post(`/project/${this.ide.project_id}/linked_file/${file.id}/refresh`, {
				_csrf: window.csrfToken
			}, {
				disableAutoLoginRedirect: true
			});
		...
		renameEntity(entity, name, callback) {
			...
			return this.ide.$http.post(`/project/${this.ide.project_id}/${entity.type}/${entity.id}/rename`, {
				...
				.then(() => entity.name = name).finally(() => entity.renamingToName = null);
		}
		...
		moveEntity(entity, parent_folder, callback) {
			...
			if (callback == null) { callback = function(error) {}; }
			if (this._isChildFolder(entity, parent_folder)) { return; }
			// check if a doc/file/folder already exists with this name
		...
		_deleteEntityFromScope(entity, options) {
			if (options == null) { options = { moveToDeleted: true }; }
			if ((entity == null)) { return; }
		...
		_moveEntityInScope(entity, parent_folder) {
			if (Array.from(parent_folder.children).includes(entity)) { return; }
