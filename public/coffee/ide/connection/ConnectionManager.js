			...
			constructor(ide, $scope) {
				...
				if ((typeof io === 'undefined' || io === null)) {
					...
					this.ide.socket =
						{on() {}};
					this.$scope.$apply(() => {
						return this.$scope.state.error = "Could not connect to websocket server :(";
					});
					return;
				}
				...
				this.ide.socket = io.connect(null, {
					reconnect: false,
				...
				this.ide.socket.on("connect_failed", () => {
					...
					return this.$scope.$apply(() => {
						return this.$scope.state.error = "Unable to connect, please view the <u><a href='/learn/Kb/Connection_problems'>connection problems guide</a></u> to fix the issue.";
					});
				});
				...
				this.ide.socket.on('disconnect', () => {
					...
					if (!this.$scope.connection.forced_disconnect && !this.userIsInactive && !this.gracefullyReconnecting) {
				...
				this.ide.socket.on('forceDisconnect', message => {
					...
					this.ide.showGenericMessageModal("Please Refresh", `\
...
`);
					return setTimeout(() => location.reload()
					, 10 * 1000);
				});
			...
			reportConnectionError(err) {
				...
				if (((err != null ? err.message : undefined) === "not authorized") || ((err != null ? err.message : undefined) === "invalid session")) {
					return window.location = `/login?redir=${encodeURI(window.location.pathname)}`;
				} else {
					this.ide.socket.disconnect();
					return this.ide.showGenericMessageModal("Something went wrong connecting", `\
...
`);
			...
			joinProject() {
				...
				return this.ide.socket.emit('joinProject', data, (err, project, permissionsLevel, protocolVersion) => {
					if ((err != null) || (project == null)) {
						return this.reportConnectionError(err);
					}

					if ((this.$scope.protocolVersion != null) && (this.$scope.protocolVersion !== protocolVersion)) {
			...
			startAutoReconnectCountdown() {
				...
				if ((this.lastUserAction != null) && ((new Date() - this.lastUserAction) > twoMinutes)) {
				...
				return setTimeout(() => {
					if (!this.connected) {
						return this.timeoutId = setTimeout((() => this.decreaseCountdown()), 1000);
					}
				}
				, 200);
			}
			...
			decreaseCountdown() {
				this.timeoutId = null;
				if ((this.$scope.connection.reconnection_countdown == null)) { return; }
				sl_console.log("[ConnectionManager] decreasing countdown", this.$scope.connection.reconnection_countdown);
				this.$scope.$apply(() => {
			...
			tryReconnect() {
				...
				return setTimeout((() => { if (!this.connected) { return this.startAutoReconnectCountdown(); } }), 2000); // ms, rate limit on reconnects for other user activity (e.g. cursor moves)
			}

			tryReconnectWithRateLimit(options) {
				// bail out if the reconnect is already in progress
				if (this.$scope.connection != null ? this.$scope.connection.reconnecting : undefined) { return; }
				// bail out if we are going to reconnect soon anyway
				const reconnectingSoon = ((this.$scope.connection != null ? this.$scope.connection.reconnection_countdown : undefined) != null) && (this.$scope.connection.reconnection_countdown <= 5);
				const clickedTryNow = options != null ? options.force : undefined;  // user requested reconnection
				if (reconnectingSoon && !clickedTryNow) { return; }
				// bail out if we tried reconnecting recently
				const allowedInterval = clickedTryNow ? this.MIN_RETRY_INTERVAL : this.BACKGROUND_RETRY_INTERVAL;
				if ((this.lastConnectionAttempt != null) && ((new Date() - this.lastConnectionAttempt) < allowedInterval)) { return; }
				return this.tryReconnect();
			}

			disconnectIfInactive(){
				this.userIsInactive = (new Date() - this.lastUserAction) > this.disconnectAfterMs;
			...
			reconnectGracefully() {
				if (this.reconnectGracefullyStarted == null) { this.reconnectGracefullyStarted = new Date(); }
				const userIsInactive = (new Date() - this.lastUserAction) > this.RECONNECT_GRACEFULLY_RETRY_INTERVAL;
				const maxIntervalReached = (new Date() - this.reconnectGracefullyStarted) > this.MAX_RECONNECT_GRACEFULLY_INTERVAL;
