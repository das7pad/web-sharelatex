	...
	App.directive("aggregateChangeEntry", $timeout =>
		({
			...
			link(scope, element, attrs) {
				...
				scope.$watch("entry.metadata.replaced_content.length", deletionContentLength => scope.deletionNeedsCollapsing = deletionContentLength > scope.contentLimit);

				return scope.$watch("entry.content.length", insertionContentLength => scope.insertionNeedsCollapsing = insertionContentLength > scope.contentLimit);
			}
		})
	)
);