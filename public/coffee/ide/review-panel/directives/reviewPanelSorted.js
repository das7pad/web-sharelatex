		...
		({
			link(scope, element, attrs) {
				...
				const layout = function(animate) {
					let entry, height, i, original_top, overflowTop, OVERVIEW_TOGGLE_HEIGHT, PADDING, TOOLBAR_HEIGHT, top;
					...
					let focused_entry_index = Math.min(previous_focused_entry_index, entries.length - 1);
					...
					const positionLayoutEl = function($callout_el, original_top, top) {
						if (original_top <= top) {
							...
							return $callout_el.css({top: (original_top + line_height) - 1, height: top - original_top});
						} else {
							$callout_el.addClass("rp-entry-callout-inverted");
							return $callout_el.css({top: top + line_height, height: original_top - top});
						}
					};

					// Put the focused entry as close to where it wants to be as possible
					const focused_entry_top = Math.max(focused_entry.scope.entry.screenPos.y, TOOLBAR_HEIGHT);
				...
				ace.require("ace/lib/event").addMouseWheelListener(scroller[0], function(e) {
				...
				var scrollAce = scrollTop => scope.reviewPanelEventsBridge.emit("externalScroll", scrollTop);
