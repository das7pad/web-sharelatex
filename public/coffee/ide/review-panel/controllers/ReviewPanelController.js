...
define([
	...
	App.controller("ReviewPanelController", function($scope, $element, ide, $timeout, $http, $modal, event_tracking, localStorage) {
		let UserTCSyncState;
		...
		$scope.reviewPanel = {
			...
			overview: {
				loading: false,
				docsCollapsedState: JSON.parse(localStorage(`docs_collapsed_state:${$scope.project_id}`)) || {}
			},
			dropdown: {
				loading: false
			},
		...
		window.addEventListener("beforeunload", function() {
			...
			const valToStore = Object.keys(collapsedStates).length > 0 ? JSON.stringify(collapsedStates) : null;
			return localStorage(`docs_collapsed_state:${$scope.project_id}`, valToStore);
		});

		$scope.$on("layout:pdf:linked", (event, state) => $scope.$broadcast("review-panel:layout"));

		$scope.$on("layout:pdf:resize", (event, state) => $scope.$broadcast("review-panel:layout", false));

		$scope.$on("expandable-text-area:resize", event =>
			$timeout(() => $scope.$broadcast("review-panel:layout"))
		);

		$scope.$on("review-panel:sizes", (e, sizes) => $scope.$broadcast("editor:set-scroll-size", sizes));
				...
				return (() => {
					...
					for (let member of Array.from(members)) {
						if (member.privileges === "readAndWrite") {
							result.push($scope.reviewPanel.formattedProjectMembers[member._id] = formatUser(member));
						} else {
		...
		ide.socket.on("resolve-thread", (thread_id, user) => _onCommentResolved(thread_id, user));
		...
		const getDocEntries = function(doc_id) {
			if ($scope.reviewPanel.entries[doc_id] == null) { $scope.reviewPanel.entries[doc_id] = {}; }
			return $scope.reviewPanel.entries[doc_id];
		};

		const getDocResolvedComments = function(doc_id) {
			if ($scope.reviewPanel.resolvedComments[doc_id] == null) { $scope.reviewPanel.resolvedComments[doc_id] = {}; }
			return $scope.reviewPanel.resolvedComments[doc_id];
		};
		
		var getThread = function(thread_id) {
			if ($scope.reviewPanel.commentThreads[thread_id] == null) { $scope.reviewPanel.commentThreads[thread_id] = { messages: [] }; }
		...
		let scrollbar = {};
		$scope.reviewPanelEventsBridge.on("aceScrollbarVisibilityChanged", function(isVisible, scrollbarWidth) {
			scrollbar = {isVisible, scrollbarWidth};
			return updateScrollbar();
		});

		var updateScrollbar = function() {
			if (scrollbar.isVisible && ($scope.reviewPanel.subView === $scope.SubViews.CUR_FILE) && !$scope.editor.showRichText) {
		...
		$scope.$watch("!ui.reviewPanelOpen && reviewPanel.hasEntries", function(open, prevVal) {
			if ((open == null)) { return; }
		...
		$scope.$watch((function() {
			const entries = $scope.reviewPanel.entries[$scope.editor.open_doc_id] || {};
			const permEntries = {};
		...
		}), nEntries => $scope.reviewPanel.hasEntries = (nEntries > 0) && $scope.project.features.trackChangesVisible);
					...
					return (() => {
						...
						for (let doc of Array.from(docs)) {
							...
							if (doc.id !== $scope.editor.open_doc_id) { // this is kept up to date in real-time, don't overwrite
								...
								rangesTracker.comments = (doc.ranges != null ? doc.ranges.comments : undefined) || [];
								rangesTracker.changes = (doc.ranges != null ? doc.ranges.changes : undefined) || [];
							}
		...
		var refreshOverviewPanel = function() {
			...
			return refreshRanges()
				.then(() => $scope.reviewPanel.overview.loading = false).catch(() => $scope.reviewPanel.overview.loading = false);
		};
		...
		var updateEntries = function(doc_id) {
			...
			for (change of Array.from(rangesTracker.changes)) {
				...
				if (
					...
					if (entries[change.id] == null) { entries[change.id] = {}; }
			...
			for (let comment of Array.from(rangesTracker.comments)) {
				...
				if ($scope.reviewPanel.resolvedThreadIds[comment.op.t]) {
					new_comment = resolvedComments[comment.id] != null ? resolvedComments[comment.id] : (resolvedComments[comment.id] = {});
					delete entries[comment.id];
				} else {
					new_comment = entries[comment.id] != null ? entries[comment.id] : (entries[comment.id] = {});
		...
		$scope.$on("editor:focus:changed", function(e, selection_offset_start, selection_offset_end, selection) {
			const doc_id = $scope.editor.open_doc_id;
			const entries = getDocEntries(doc_id);
			...
			for (let id in entries) {
				...
				if ((entry.type === "comment") && !$scope.reviewPanel.resolvedThreadIds[entry.thread_id]) {
					entry.focused = (entry.offset <= selection_offset_start && selection_offset_start <= entry.offset + entry.content.length);
				} else if (entry.type === "insert") {
					isChangeEntryAndWithinSelection = (entry.offset >= selection_offset_start) && ((entry.offset + entry.content.length) <= selection_offset_end);
					entry.focused = (entry.offset <= selection_offset_start && selection_offset_start <= entry.offset + entry.content.length);
				} else if (entry.type === "delete") {
					isChangeEntryAndWithinSelection = selection_offset_start <= entry.offset && entry.offset <= selection_offset_end;
					entry.focused = (entry.offset === selection_offset_start);
				} else if (entry.type === "aggregate-change") {
					isChangeEntryAndWithinSelection = (entry.offset >= selection_offset_start) && ((entry.offset + entry.content.length) <= selection_offset_end);
					entry.focused = (entry.offset <= selection_offset_start && selection_offset_start <= entry.offset + entry.content.length);
				} else if ([ "add-comment", "bulk-actions" ].includes(entry.type) && selection) {
					entry.focused = true;
				}
		...
		$scope.acceptChanges = function(change_ids) {
			...
			return event_tracking.sendMB("rp-changes-accepted", { view: $scope.ui.reviewPanelOpen ? $scope.reviewPanel.subView : 'mini' });
		};

		$scope.rejectChanges = function(change_ids) {
			_doRejectChanges(change_ids);
			return event_tracking.sendMB("rp-changes-rejected", { view: $scope.ui.reviewPanelOpen ? $scope.reviewPanel.subView : 'mini' });
		};
		...
		var _doRejectChanges = change_ids => $scope.$broadcast("changes:reject", change_ids);
		...
		var showBulkActionsDialog = isAccept =>
			$modal.open({
				...
				resolve: {
					isAccept() { return isAccept; },
					nChanges() { return $scope.reviewPanel.nVisibleSelectedChanges; }
				},
				scope: $scope.$new()
		...
		$scope.submitNewComment = function(content) {
			...
			const doc_id = $scope.editor.open_doc_id;
			const entries = getDocEntries(doc_id);
			if ((entries["add-comment"] == null)) { return; }
			...
			$http.post(`/project/${$scope.project_id}/thread/${thread_id}/messages`, {content, _csrf: window.csrfToken})
				.catch(() => ide.showGenericMessageModal("Error submitting comment", "Sorry, there was a problem submitting your comment"));
		...
		$scope.resolveComment = function(entry, entry_id) {
			...
			return event_tracking.sendMB("rp-comment-resolve", { view: $scope.ui.reviewPanelOpen ? $scope.reviewPanel.subView : 'mini' });
		};
		...
		$scope.gotoEntry = (doc_id, entry) => ide.editorManager.openDocId(doc_id, { gotoOffset: entry.offset });

		$scope.toggleFullTCStateCollapse = function() {
			if ($scope.project.features.trackChanges) {
				return $scope.reviewPanel.fullTCStateCollapsed = !$scope.reviewPanel.fullTCStateCollapsed;
			} else {
		...
		const _setUserTCState = function(userId, newValue, isLocal) {
			...
			if ($scope.reviewPanel.trackChangesState[userId] == null) { $scope.reviewPanel.trackChangesState[userId] = {}; }
			...
			} else if ((state.syncState === UserTCSyncState.PENDING) && (state.value === newValue)) {
				state.syncState = UserTCSyncState.SYNCED;
			} else if (isLocal) {
				state.value = newValue;
		...
		var _setGuestsTCState = function(newValue, isLocal) {
			...
			if ((currentUserType() === UserTypes.GUEST) || (currentUserType() === UserTypes.ANONYMOUS)) {
				return $scope.editor.wantTrackChanges = newValue;
			}
		};
		...
		const applyTrackChangesStateToClient = function(state) {
			if (typeof state === "boolean") {
				...
				for (let member of Array.from(project.members)) {
					_setUserTCState(member._id, state[member._id] != null ? state[member._id] : false);
				}
				return _setUserTCState($scope.project.owner._id, state[$scope.project.owner._id] != null ? state[$scope.project.owner._id] : false);
			}
		};
		...
		$scope.toggleTrackChangesFromKbdShortcut = function() {
			...
			return $scope.toggleTrackChangesForUser(!$scope.reviewPanel.trackChangesState[ide.$scope.user.id].value, ide.$scope.user.id);
		};

		const setGuestFeatureBasedOnProjectAccessLevel = projectPublicAccessLevel => $scope.reviewPanel.trackChangesForGuestsAvailable = (projectPublicAccessLevel === 'tokenBased');
		...
		const onToggleTrackChangesForGuestsAvailability = function(available) {
			...
			if (available) { return; }
			if (!$scope.reviewPanel.trackChangesOnForGuests) { return; } // Already turned off
			if ($scope.reviewPanel.trackChangesOnForEveryone) { return; } // Overrides guest setting
		...
		$scope.$watch('reviewPanel.trackChangesForGuestsAvailable', function(available) {
			if (available != null) {
		...
		ide.$scope.$on("project:joined", function() {
			...
			if (project.features.trackChanges) {
				if (window.trackChangesState == null) { window.trackChangesState = false; }
		...
		var refreshChangeUsers = function(refresh_for_user_id) {
			...
			return $http.get(`/project/${$scope.project_id}/changes/users`)
				.then(function(response) {
		...
		var ensureThreadsAreLoaded = function() {
			...
			return $http.get(`/project/${$scope.project_id}/threads`)
				.then(function(response) {
		...
		var formatUser = function(user) {
			let isSelf, name;
			const id = (user != null ? user._id : undefined) || (user != null ? user.id : undefined);
			...
			if (id === window.user_id) {
				...
				name = [user.first_name, user.last_name].filter(n => (n != null) && (n !== "")).join(" ");
				if (name === "") {
					name = (user.email != null ? user.email.split("@")[0] : undefined) || "Unknown";
				}
				isSelf = false;
			}
			return {
				...
				avatar_text: [user.first_name, user.last_name].filter(n => n != null).map(n => n[0]).join("")
			};
		};
