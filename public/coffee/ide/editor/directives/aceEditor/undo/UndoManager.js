	...
	return (UndoManager = class UndoManager {
		...
		execute(options) {
			...
			for (var deltaSet of Array.from(aceDeltaSets || [])) {
				if (deltaSet.group === "doc") { // ignore code folding etc.
					for (let delta of Array.from(deltaSet.deltas)) {
						if ((currentDeltaSet.remote != null) && (currentDeltaSet.remote !== !!delta.remote)) {
							newDeltaSet();
						}
			...
			for (deltaSet of Array.from(splitDeltaSets)) {
				var simpleDeltaSet;
				({simpleDeltaSet, docLines}  = this._aceDeltaSetToSimpleDeltaSet(deltaSet, docLines));
				const frame = {
		...
		undo(dontSelect) {
			...
			const linesBeforeDelta = this._revertSimpleDeltaSetsOnDocLines(update.deltaSets, lines);
			const deltaSets = this._simpleDeltaSetsToAceDeltaSets(update.deltaSets, linesBeforeDelta);
		...
		redo(dontSelect) {
			...
			if ((update == null)) { return; }
			const lines = this.session.getDocument().getAllLines();
			const deltaSets = this._simpleDeltaSetsToAceDeltaSets(update.deltaSets, lines);
		...
		_shiftLocalChangeToTopOfUndoStack() {
			...
			if (!localChangeExists) {
				...
				while (head.length > 0) {
					...
					const result = this._swapSimpleDeltaSetsOrder(localChange.deltaSets, remoteChange.deltaSets);
					if (result != null) {
		...
		_swapSimpleDeltaSetsOrder(firstDeltaSets, secondDeltaSets) {
			...
			for (let firstDeltaSet of Array.from(newFirstDeltaSets.slice(0).reverse())) {
				for (let firstDelta of Array.from(firstDeltaSet.deltas.slice(0).reverse())) {
					for (let secondDeltaSet of Array.from(newSecondDeltaSets)) {
						for (let secondDelta of Array.from(secondDeltaSet.deltas)) {
							const success = this._swapSimpleDeltaOrderInPlace(firstDelta, secondDelta);
		...
		_copyDeltaSets(deltaSets) {
			...
			for (let deltaSet of Array.from(deltaSets)) {
				...
				for (let delta of Array.from(deltaSet.deltas)) {
					...
					if (delta.insert != null) { newDelta.insert = delta.insert; }
					if (delta.remove != null) { newDelta.remove = delta.remove; }
		...
		_aceDeltaToSimpleDelta(aceDelta, docLines) {
			...
			if ((start == null)) {
				...
				const error = new Error(`aceDelta had no start event: ${JSONstringifyWithCycles(aceDelta)}`);
				throw error;
			}
		...
		_simpleDeltaToAceDeltas(simpleDelta, docLines) {
			let aceDelta, end;
			const {row, column} = this._simplePositionToAcePosition(simpleDelta.position, docLines);
