...
define([
	...
	return (SpellCheckAdapter = class SpellCheckAdapter {
		...
		normalizeChangeEvent(e) { return e; }
		...
		getHighlightFromCoords(coords) {
			const position = this.editor.renderer.screenToTextCoordinates(coords.x, coords.y);
		...
		selectHighlightedWord(highlight) {
			...
			return this.editor.getSession().getSelection().setSelectionRange(
				new Range(
					row, startColumn,
					row, endColumn
				)
		...
		replaceWord(highlight, newWord) {
			...
			this.editor.getSession().replace(new Range(
				row, startColumn,
				row, endColumn
			), newWord);
