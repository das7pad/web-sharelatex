	...
	return (HighlightsManager = class HighlightsManager {
		constructor($scope, editor, element) {
			...
			this.$scope.gotoHighlightBelow = () => {
				if ((this.firstHiddenHighlightAfter == null)) { return; }
				return this.editor.scrollToLine(this.firstHiddenHighlightAfter.end.row, true, false);
			};

			this.$scope.gotoHighlightAbove = () => {
				if ((this.lastHiddenHighlightBefore == null)) { return; }
				return this.editor.scrollToLine(this.lastHiddenHighlightBefore.start.row, true, false);
			};
		}
		...
		redrawAnnotations() {
			...
			for (let annotation of Array.from(this.$scope.highlights || [])) {
				(annotation => {
					const colorScheme = ColorManager.getColorScheme(annotation.hue, this.element);
		...
		showAnnotationLabels(position) {
			...
			if ((labelToShow == null)) {
				...
				if (__guard__(this.$scope != null ? this.$scope.annotationLabel : undefined, x => x.show) !== false) {
				...
				if (labelToShow.snapToStartOfRange) {
					coords = this.editor.renderer.textToScreenCoordinates(labelToShow.range.start.row, labelToShow.range.start.column);
				} else {
		...
		updateShowMoreLabels() {
			if (!this.$scope.navigateHighlights) { return; }
			return setTimeout(() => {
				...
				for (let annotation of Array.from(this.$scope.highlights || [])) {
					...
					if (range.end.row > lastRow) {
						highlightsAfter += 1;
						if (!this.firstHiddenHighlightAfter) { this.firstHiddenHighlightAfter = range; }
					}
				}
		...
		_drawCursor(annotation, colorScheme) {
			return this.markerIds.push(this.editor.getSession().addMarker(new Range(
				annotation.cursor.row, annotation.cursor.column,
				annotation.cursor.row, annotation.cursor.column + 1
			), "annotation remote-cursor", function(html, range, left, top, config) {
				const div = `\
<div
...
`
		...
		_addMarkerWithCustomStyle(range, klass, foreground, style) {
			...
			return this.markerIds.push(this.editor.getSession().addMarker(range, klass, function(html, range, left, top, config) {
				if (range.isMultiLine()) {
					return markerLayer.drawTextMarker(html, range, klass, config, style);
				} else {
					return markerLayer.drawSingleLineMarker(html, range, `${klass} ace_start`, config, 0, style);
				}
			}
			, foreground)
			);
