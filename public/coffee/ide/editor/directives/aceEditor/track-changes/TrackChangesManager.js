	...
	return TrackChangesManager = (function() {
		...
		TrackChangesManager = class TrackChangesManager {
			...
			constructor($scope, editor, element) {
				...
				if (window.trackChangesManager == null) { window.trackChangesManager = this; }

				this.$scope.$watch("trackChanges", track_changes => {
					if ((track_changes == null)) { return; }
			...
			setTrackChanges(value) {
				if (value) {
					return (this.$scope.sharejsDoc != null ? this.$scope.sharejsDoc.track_changes_as = window.user.id || "anonymous" : undefined);
				} else {
			...
			rejectChangeIds(change_ids) {
				...
				for (let change of Array.from(changes)) {
					if (change.op.d != null) {
						...
						const end = this._shareJsOffsetToAcePosition(change.op.p + change.op.i.length);
			...
			hideCommentsByThreadIds(thread_ids) {
				...
				for (let comment of Array.from((this.rangesTracker != null ? this.rangesTracker.comments : undefined) || [])) {
				...
				return (() => {
					...
					for (let comment of Array.from(this.rangesTracker.comments)) {
						...
						if ((selection_start <= comment_start) && (comment_end <= selection_end)) {
							result.push(this._cutState.comments.push({
			...
			onPaste() {
				return this.editor.once("change", change => {
					if (change.action !== "insert") { return; }
					...
					return this.$scope.sharejsDoc.on("ranges:dirty.paste", () => {
						...
						if ((pasted_text === this._cutState.text) && (this.$scope.docId === this._cutState.docId)) {
							for (let {comment, offset, text} of Array.from(this._cutState.comments)) {
			...
			checkMapping() {
				...
				for (var change of Array.from(this.rangesTracker.changes)) {
					if (this.changeIdToMarkerIdMap[change.id] != null) {
						({ op } = change);
						({background_marker_id, callout_marker_id} = this.changeIdToMarkerIdMap[change.id]);
				...
				for (let comment of Array.from(this.rangesTracker.comments)) {
					if (this.changeIdToMarkerIdMap[comment.id] != null) {
						({background_marker_id, callout_marker_id} = this.changeIdToMarkerIdMap[comment.id]);
						start = this._shareJsOffsetToAcePosition(comment.op.p);
						end = this._shareJsOffsetToAcePosition(comment.op.p + comment.op.c.length);
						expected_markers.push({ marker_id: background_marker_id, start, end });
						expected_markers.push({ marker_id: callout_marker_id, start, end: start });
					}
				}
				...
				for ({marker_id, start, end} of Array.from(expected_markers)) {
					...
					if ((marker.range.start.row !== start.row) ||
			...
			updateFocus() {
				...
				return this.$scope.$emit("editor:focus:changed", selection_start, selection_end, is_selection);
			}

			broadcastChange() {
				return this.$scope.$emit("editor:track-changes:changed", this.$scope.docId);
			}
			...
			recalculateReviewEntriesScreenPositions() {
				...
				for (let entry_id in object) {
					...
					const screen_position = session.documentToScreenPosition(doc_position.row, doc_position.column);
					const y = screen_position.row * renderer.lineHeight;
					if (entry.screenPos == null) { entry.screenPos = {}; }
					entry.screenPos.y = y;
			...
			recalculateVisibleEntries() {
				...
				for (let entry_id in entries) {
					...
					entry.visible = (entriesLength < CULL_AFTER) || (firstRow - OFFSCREEN_ROWS <= entry.docPos.row && entry.docPos.row <= lastRow + OFFSCREEN_ROWS);
			...
			_getCurrentDocEntries() {
				...
				const entries = this.$scope.reviewPanel.entries[doc_id] != null ? this.$scope.reviewPanel.entries[doc_id] : (this.$scope.reviewPanel.entries[doc_id] = {});
				return entries;
			}
			...
			_onInsertAdded(change) {
				...
				const callout_marker_id = this._createCalloutMarker(start, "track-changes-added-marker-callout");
			...
			_onDeleteAdded(change) {
				...
				const callout_marker_id = this._createCalloutMarker(position, "track-changes-deleted-marker-callout");
				return this.changeIdToMarkerIdMap[change.id] = { background_marker_id, callout_marker_id };
			}

			_onInsertRemoved(change) {
				const {background_marker_id, callout_marker_id} = this.changeIdToMarkerIdMap[change.id];
			...
			_onCommentAdded(comment) {
				...
				if ((this.changeIdToMarkerIdMap[comment.id] == null)) {
					...
					const end = this._shareJsOffsetToAcePosition(comment.op.p + comment.op.c.length);
					const session = this.editor.getSession();
					const doc = session.getDocument();
					const background_range = new Range(start.row, start.column, end.row, end.column);
					const background_marker_id = session.addMarker(background_range, "track-changes-marker track-changes-comment-marker", "text");
					const callout_marker_id = this._createCalloutMarker(start, "track-changes-comment-marker-callout");
			...
			_onCommentRemoved(comment) {
				if (this.changeIdToMarkerIdMap[comment.id] != null) {
					...
					const {background_marker_id, callout_marker_id} = this.changeIdToMarkerIdMap[comment.id];
			...
			_aceChangeToShareJs(delta) {
				const lines = this.editor.getSession().getDocument().getLines(0, delta.start.row);
			...
			_onChangeMoved(change) {
				...
				const start = this._shareJsOffsetToAcePosition(change.op.p);
				if (change.op.i != null) {
					end = this._shareJsOffsetToAcePosition(change.op.p + change.op.i.length);
				} else {
					end = start;
				}
			...
			_updateMarker(change_id, start, end) {
				if ((this.changeIdToMarkerIdMap[change_id] == null)) { return; }
				const session = this.editor.getSession();
				const markers = session.getMarkers();
				const {background_marker_id, callout_marker_id} = this.changeIdToMarkerIdMap[change_id];
