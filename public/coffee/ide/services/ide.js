...
define([
	...
	App.factory("ide", ["$http", "queuedHttp", "$modal", "$q", "$filter", "$timeout", function($http, queuedHttp, $modal, $q, $filter, $timeout) {
		const ide = {};
		ide.$http = $http;
		...
		ide.reportError = (error, meta) => {
			if (meta == null) { meta = {}; }
			meta.client_id = __guard__(this.socket != null ? this.socket.socket : undefined, x => x.sessionid);
			meta.transport = __guard__(__guard__(this.socket != null ? this.socket.socket : undefined, x2 => x2.transport), x1 => x1.name);
	...
	return App.controller("GenericMessageModalController", ["$scope", "$modalInstance", "title", "message", function($scope, $modalInstance, title, message) {
		$scope.title = title;
