...
define([
	...
	App.controller('pdfViewerController', ['$scope', '$q', '$timeout', 'PDFRenderer', '$element', 'pdfHighlights', 'pdfSpinner', function($scope, $q, $timeout, PDFRenderer, $element, pdfHighlights, pdfSpinner) {
		this.load = function() {
			// $scope.pages = []

			if ($scope.document != null) { $scope.document.destroy(); }
			...
			$scope.document = new PDFRenderer(url, {
				...
				errorCallback(error) {
					__guardMethod__(Raven, 'captureMessage', o => o.captureMessage(`pdfng error ${error}`));
			...
			return $scope.loaded = $q.all({
				...
				pdfViewport: $scope.document.getPdfViewport(1, 1)
					...
					$scope.$emit('loaded');
					return $scope.numPages = result.numPages;}).catch(function(error) {
		...
		this.zoomIn = function() {
			...
			return $scope.forceScale = { scaleMode: 'scale_mode_value', scale: newScale };
		};
		...
		this.computeOffset = function(page, position) {
			...
			return $scope.document.getPdfViewport(page.pageNum).then(function(viewport) {
				...
				const pageOffset = viewport.convertToViewportPoint(offset.left, offset.top);
				// if the passed-in position doesn't have the page height/width add them now
				if (position.pageSize == null) { position.pageSize = {"height": viewport.viewBox[3], "width": viewport.viewBox[2]}; }
	...
	return App.directive('pdfViewer', ['$q', '$timeout', 'pdfSpinner', ($q, $timeout, pdfSpinner) =>
		({
			...
			link(scope, element, attrs, ctrl) {
				...
				var getVisiblePages = function() {
					...
					const visiblePages = _.filter(scope.pages, function(page) {
						if ((page.element == null)) { return false; }
				...
				const queueRescale = function(scale) {
					...
					if ((rescaleTimer != null) || (layoutTimer != null) || (elementTimer != null)) { return; }
				...
				var doRescale = function(scale) {
					...
					return layoutReady.promise.then(function(parentSize) {
						...
						return ctrl.setScale(scale, h, w).then(() =>
				...
				var _setMouseUpHandler = function() {
					if ((mouseUpHandler == null)) {
						return mouseUpHandler = $(document.body).one('mouseup', _handleSelectionMouseUp);
					}
				};
				...
				var _handleSelectionMouseUp = function() {
					...
					window.setTimeout(function() {
						...
						if (!removedClass) { return _setMouseUpHandler(); }
					}
					, 10);
					return true;
				};
				...
				var _hasSelection = function() {
					const selection = typeof window.getSelection === 'function' ? window.getSelection() : undefined;
					...
					return (selection != null) && _isSelectionWithinPDF(selection) && !selection.isCollapsed;
				};
				...
				var _isSelectionWithinPDF = function(selection) {
					...
					const selectionAncestorNode = selection.getRangeAt(0).commonAncestorContainer;
					return (element.find(selectionAncestorNode).length > 0) || element.is(selectionAncestorNode);
				};
				...
				scope.$watch('pdfSrc', function(newVal, oldVal) {
					...
					return ctrl.load().then(() =>
						// trigger a redraw
						scope.scale = angular.copy((scope.scale))).catch(error => scope.$emit('pdf:error', error));
				});
...
//					console.log 'got change in position watcher', newVal, oldVal
				...
				scope.$watch("highlights", function(areas) {
					...
					const pageNum = scope.pages[first.page] != null ? scope.pages[first.page].pageNum : undefined;
				...
				return scope.$on('$destroy', function() {
					...
					if (elementTimer != null) { clearTimeout(elementTimer); }
					if (layoutTimer != null) { clearTimeout(layoutTimer); }
					if (rescaleTimer != null) { clearTimeout(rescaleTimer); }
					if (spinnerTimer != null) { return clearTimeout(spinnerTimer); }
				});
			}
		})
