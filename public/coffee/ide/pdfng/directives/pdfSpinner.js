...
define([
	...
	App.factory('pdfSpinner', [function() {
		...
		return (pdfSpinner = class pdfSpinner {
			...
			add(element, options) {
				const size = 64;
				const spinner = $(`<div class="pdfng-spinner" style="position: absolute; top: 50%; left:50%; transform: translateX(-50%) translateY(-50%);"><i class="fa fa-spinner${(options != null ? options.static : undefined) ? '' : ' fa-spin'}" style="color: #999"></i></div>`);
