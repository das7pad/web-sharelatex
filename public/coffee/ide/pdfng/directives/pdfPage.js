			...
			link(scope, element, attrs, ctrl) {
				...
				if (!scope.page.sized) {
					if (scope.defaultPageSize != null) {
						...
						var handler = scope.$watch('defaultPageSize', function(defaultPageSize) {
				...
				element.on('dblclick', function(e) {
					...
					return scope.document.getPdfViewport(scope.page.pageNum).then(function(viewport) {
				...
				scope.$on('pdf:highlights', function(event, highlights) {
					...
					const pageHighlights = ((() => {
						const result = [];
						for (h of Array.from(highlights)) { 							if (h.page === scope.page.pageNum) {
								result.push(h);
							}
						}
						return result;
					...
					scope.document.getPdfViewport(scope.page.pageNum).then(viewport =>
						(() => {
							...
							// console.log 'adding highlight', h, viewport
								...
								result1.push(highlightsLayer.addHighlight(viewport, hl.h, top, hl.width, hl.height));
							}
							return result1;
						})()
					);
