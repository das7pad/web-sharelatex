...
define([
	...
	const historyFileEntityController = function($scope, $element, $attrs) {
		...
		const _handleFileClick = () => ctrl.historyFileTreeController.handleEntityClick(ctrl.fileEntity);
		var _getFolderIcon = function() {
			if (ctrl.isOpen) { return "fa-folder-open"; } else { return "fa-folder"; }
		};
		ctrl.$onInit = function() {
			if (ctrl.fileEntity.type === "folder") {
				...
				return $scope.$watch((() => ctrl.historyFileTreeController.selectedPathname), newPathname => ctrl.isSelected = ctrl.fileEntity.pathname === newPathname);
			}
		};
	};
