...
define([
	...
	App.controller("HistoryV2FileTreeController", ["$scope", "ide", "_", function($scope, ide, _) {
		let _reducePathsToTree;
		let _previouslySelectedPathname = null;
		$scope.currentFileTree = [];

		const _pathnameExistsInFiles = (pathname, files) => _.any(files, file => file.pathname === pathname);

		const _getSelectedDefaultPathname = function(files) {
			let selectedPathname = null;
			if ((_previouslySelectedPathname != null) && _pathnameExistsInFiles(_previouslySelectedPathname, files)) {
				selectedPathname = _previouslySelectedPathname;
			} else { 
				const mainFile = _.find(files, file => /main\.tex$/.test(file.pathname));
				if (mainFile != null) {
		...
		$scope.handleFileSelection = file => $scope.history.selection.pathname = (_previouslySelectedPathname = file.pathname);

		$scope.$watch('history.files', function(files) {
			if ((files != null) && (files.length > 0)) {
				$scope.currentFileTree = _.reduce(files, _reducePathsToTree, []);
				return $scope.history.selection.pathname = _getSelectedDefaultPathname(files);
			}
		});
