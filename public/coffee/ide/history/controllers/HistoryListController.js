...
], function(App, displayNameForUser) {

	App.controller("HistoryListController", ["$scope", "$modal", "ide", function($scope, $modal, ide) {
		...
		return $scope.$watch("history.updates.length", () => $scope.recalculateSelectedUpdates());
	}
	]);

	return App.controller("HistoryListItemController", ["$scope", "event_tracking", function($scope, event_tracking) {
		$scope.$watch("update.selectedFrom", function(selectedFrom, oldSelectedFrom) {
			if (selectedFrom) {
				for (let update of Array.from($scope.history.updates)) {
					if (update !== $scope.update) { update.selectedFrom = false; }
				}
		...
		$scope.$watch("update.selectedTo", function(selectedTo, oldSelectedTo) {
			if (selectedTo) {
				for (let update of Array.from($scope.history.updates)) {
					if (update !== $scope.update) { update.selectedTo = false; }
				}
				return $scope.recalculateSelectedUpdates();
			}
		});
