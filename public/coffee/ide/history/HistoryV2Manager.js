	...
	return HistoryManager = (function() {
		HistoryManager = class HistoryManager {
			...
			constructor(ide, $scope) {
				...
				this.$scope.$watch("history.showOnlyLabels", (showOnlyLabels, prevVal) => {
			...
			reset() {
				return this.$scope.history = {
					...
					atEnd: false,
					userHasFullFeature: __guard__(this.$scope.project != null ? this.$scope.project.features : undefined, x => x.versioning) || false,
			...
			autoSelectRecentUpdates() {
				...
				for (let i = 0; i < this.$scope.history.updates.length; i++) {
					...
					if (this._updateContainsUserId(update, this.$scope.user.id) || (i > this.MAX_RECENT_UPDATES_TO_SELECT)) {
						break;
					}
			...
			autoSelectLastLabel() {
				if (this.$scope.history.labels.length === 0) { return; }
			...
			selectUpdate(update) {
				...
				return this.loadFileTreeForVersion(this.$scope.history.updates[selectedUpdateIndex].toV);
			}

			selectedLabelFromUpdatesSelection() {
				// Get the number of labels associated with the currently selected update
				const nSelectedLabels = __guard__(__guard__(this.$scope.history.selection.updates != null ? this.$scope.history.selection.updates[0] : undefined, x1 => x1.labels), x => x.length);
				...
				// If the update has one label, select it 
					...
					return this.selectLabel(this.$scope.history.selection.updates[0].labels[0]);
				// If there are multiple labels for the update, select the latest
				} else if (nSelectedLabels > 1) {
					const sortedLabels = this.ide.$filter("orderBy")(this.$scope.history.selection.updates[0].labels, '-created_at');
			...
			fetchNextBatchOfUpdates() {
				...
				const requests = 
					{updates: this.ide.$http.get(updatesURL)};
			
				if ((this.$scope.history.labels == null)) {
					requests.labels = this.ide.$http.get(labelsURL);
				}

				return this.ide.$q.all(requests)
					.then(response => {
						const updatesData = response.updates.data;
						if (response.labels != null) {
							this.$scope.history.labels = this._sortLabelsByVersionAndDate(response.labels.data);
						}
						this._loadUpdates(updatesData.updates);
						this.$scope.history.nextBeforeTimestamp = updatesData.nextBeforeTimestamp;
						if ((updatesData.nextBeforeTimestamp == null) || this.$scope.history.freeHistoryLimitHit) {
			...
			loadFileAtPointInTime() {
				...
				if ((this.$scope.history.selection.updates != null ? this.$scope.history.selection.updates[0] : undefined) != null) {
				...
				const query = [`pathname=${encodeURIComponent(pathname)}`, `from=${toV}`, `to=${toV}`];
				url += `?${query.join("&")}`;
				this.$scope.history.selectedFile =
					{loading: true};
				return this.ide.$http
					.get(url)
					.then(response => {
			...
			labelCurrentVersion(labelComment) { 
				return this._labelVersion(labelComment, this.$scope.history.selection.updates[0].toV);
			}
			...
			_isLabelSelected(label) {
				return label.id === (this.$scope.history.selection.label != null ? this.$scope.history.selection.label.id : undefined);
			}
			...
			_deleteLabelLocally(labelToDelete) {
				for (let i = 0; i < this.$scope.history.updates.length; i++) {
					...
					if (update.toV === labelToDelete.version) {
						update.labels = _.filter(update.labels, label => label.id !== labelToDelete.id);
						break; 
					}
				}
				return this.$scope.history.labels = _.filter(this.$scope.history.labels, label => label.id !== labelToDelete.id);
			}
			...
			_parseDiff(diff) {
				...
				for (let i = 0; i < iterable.length; i++) {
					...
					if ((entry.i != null) || (entry.d != null)) {
						const user = entry.meta.users != null ? entry.meta.users[0] : undefined;
						...
						if (entry.i != null) {
							highlights.push({
								...
								highlight: range,
								hue: ColorManager.getHueForUserId(user != null ? user.id : undefined)
							});
			...
			_loadUpdates(updates) {
				...
				for (let i = 0; i < iterable.length; i++) {
					...
					if (!this.$scope.history.userHasFullFeature && (update.meta.end_ts < timestamp24hoursAgo)) {
			...
			_addLabelToLocalUpdate(label) {
				const localUpdate = _.find(this.$scope.history.updates, update => update.toV === label.version);
				if (localUpdate != null) {
					localUpdate.labels = this._sortLabelsByVersionAndDate(localUpdate.labels.concat(label));
				}
				return this.$scope.history.labels = this._sortLabelsByVersionAndDate(this.$scope.history.labels.concat(label));
			}
			...
			_perDocSummaryOfUpdates(updates) {
				...
				const updatePathnameWithUpdateVersions = function(pathname, update, deletedAtV) {
					...
					const doc_summary = docs_summary[original_pathname] != null ? docs_summary[original_pathname] : (docs_summary[original_pathname] = {
			...
			_calculateDiffDataFromSelection() {
				...
				const object = this._perDocSummaryOfUpdates(this.$scope.history.selection.updates);
			...
			_selectDocFromUpdates() {
				let pathname;
				const affected_docs = this._perDocSummaryOfUpdates(this.$scope.history.selection.updates);
