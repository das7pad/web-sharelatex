...
define([
	...
	App.controller("PdfController", function($scope, $http, ide, $modal, synctex, event_tracking, localStorage) {
		...
		$scope.pdf.view = __guard__($scope != null ? $scope.pdf : undefined, x => x.url) ? 'pdf' : 'uncompiled';
		...
		const filesDropdownEl	= logsContainerEl != null ? logsContainerEl.querySelector(".files-dropdown") : undefined;
		...
		const getFilesDropdownTopCoordAsRatio = () => (filesDropdownEl != null ? filesDropdownEl.getBoundingClientRect().top : undefined) / (logsContainerEl != null ? logsContainerEl.getBoundingClientRect().height : undefined);

		$scope.$watch("shouldShowLogs", function(shouldShow) {
			if (shouldShow) {
				return $scope.$applyAsync(() => $scope.shouldDropUp = getFilesDropdownTopCoordAsRatio() > 0.65);
			}
		});

		$scope.trackLogHintsLearnMore = () => event_tracking.sendMB("logs-hints-learn-more");
		...
		const createQueryString = function(args) {
			...
			if (qs_args.length) { return `?${qs_args.join("&")}`; } else { return ""; }
		};
		...
		const autoCompileIfReady = function() {
			...
			const autoCompileLintingError = ide.$scope.hasLintingError && ide.$scope.settings.syntaxValidation;
			...
			const startedTryingAt = Math.max($scope.startedTryingAutoCompileAt, $scope.lastFinishedCompileAt || 0);
		...
		var triggerAutoCompile = () => $scope.recompile({isAutoCompileOnChange: true});

		const startTryingAutoCompile = function() {
			if (autoCompileInterval != null) { return; }
			$scope.startedTryingAutoCompileAt = Date.now();
		...
		$scope.$watch("autocompile_enabled", function(newValue, oldValue) {
			if ((newValue != null) && (oldValue !== newValue)) {
				...
				return event_tracking.sendMB("autocompile-setting-changed", { value: newValue });
			}
	});
		...
		if ($scope.stop_on_validation_error == null) { $scope.stop_on_validation_error = true; } // turn on for all users by default
		...
		const sendCompileRequest = function(options) {
			...
			let checkType = (() => { switch (false) {
				case !$scope.check: return "validate"; // validate only
				case !options.try: return "silent"; // allow use to try compile once
				case !$scope.stop_on_validation_error: return "error"; // try to compile
				default: return "silent"; // ignore errors
			} })();
		...
		const parseCompileResponse = function(response) {
			...
			if ((response != null ? response.outputFiles : undefined) != null) {
				for (file of Array.from((response != null ? response.outputFiles : undefined))) {
			...
			if (response.status === "timedout") {
				...
				if (response.status === "validation-fail") { $scope.pdf.failedCheck = true; }
			...
			return $scope.pdf.outputFiles.sort((a,b) => (b.main - a.main) || a.name.localeCompare(b.name));
		};
		...
		var fetchLogs = function(fileByPath, options) {
			...
			const getFile = function(name, file) {
				...
				if ((file != null ? file.url : undefined) != null) {  // FIXME clean this up when we have file.urls out consistently
					opts.url = file.url;
				...
				(() => {
					...
					for (let key of ['all', 'errors', 'warnings']) {
						if (newEntries.type != null) {
							for (let entry of Array.from(newEntries[key])) { entry.type = newEntries.type; }
						}
			...
			const processLog = function(log) {
				...
				const {errors, warnings, typesetting} = HumanReadableLogs.parse(log, {ignoreDuplicates: true});
			...
			const processChkTex = function(log) {
				...
				for (let line of Array.from(log.split("\n"))) {
					...
					if (m = line.match(/^(\S+):(\d+):(\d+): (Error|Warning): (.*)/)) {
						const result = { file:m[1], line:m[2], column:m[3], level:m[4].toLowerCase(), message: `${m[4]}: ${m[5]}`};
				...
				const logHints = HumanReadableLogs.parse({type: "Syntax", all, errors, warnings});
				...
				return (() => {
					...
					for (let entry of Array.from(logEntries.all)) {
						if (entry.file != null) {
							...
							if (entity != null) {
								if (!$scope.pdf.logEntryAnnotations[entity.id]) { $scope.pdf.logEntryAnnotations[entity.id] = []; }
			...
			if (logFile != null) {
				...
				if (blgFile != null) {	// retrieve the blg file if present
		...
		$scope.recompile = function(options) {
			...
			return sendCompileRequest(options)
				.then(function(response) {
					...
					return parseCompileResponse(data);}).catch(function(response) {
					...
					return $scope.pdf.view = 'errors';}).finally(() => $scope.lastFinishedCompileAt = Date.now());
		};
		...
		$scope.toggleLogs = function() {
			...
			if ($scope.shouldShowLogs) { return event_tracking.sendMBOnce("ide-open-logs-once"); }
		};
		...
		$scope.toggleRawLog = function() {
			...
			if ($scope.pdf.showRawLog) { return event_tracking.sendMB("logs-view-raw"); }
		};
		...
		const synctex = {
			syncToPdf(cursorPosition) {
				...
				$http({
					...
					.then(function(response) {
						const { data } = response;
						return deferred.resolve(data.pdf || []);}).catch(function(response) {
	...
	App.controller("PdfLogEntryController", ["$scope", "ide", "event_tracking", ($scope, ide, event_tracking) =>
		$scope.openInEditor = function(entry) {
			...
			const entity = ide.fileTreeManager.findEntityByPath(entry.file);
			if ((entity == null) || (entity.type !== "doc")) { return; }
			if (entry.line != null) {
				({ line } = entry);
			}
			...
			return ide.editorManager.openDoc(entity, {gotoLine: line, gotoColumn: column});
		}
