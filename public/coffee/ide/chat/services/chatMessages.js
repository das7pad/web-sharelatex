...
define([
	...
	App.factory("chatMessages", function($http, ide) {
		...
		ide.socket.on("new-chat-message", message => {
			if ((__guard__(message != null ? message.user : undefined, x => x.id) === ide.$scope.user.id) && justSent) {
				// Nothing to do
			} else {
		...
		chat.loadMoreMessages = function() {
			if (chat.state.atEnd) { return; }
			if (chat.state.errored) { return; }
			...
			return $http
				...
				.then(function(response) {
					...
					if (typeof messages.reverse !== 'function') {
						...
						return chat.state.nextBeforeTimestamp = chat.state.messages[0] != null ? chat.state.messages[0].timestamp : undefined;
					}
			});
		};
		...
		const prependMessage = function(message) {
			...
			const shouldGroup = (firstMessage != null) &&
				(firstMessage.user.id === __guard__(message != null ? message.user : undefined, x => x.id)) &&
		...
		var appendMessage = function(message) {
			...
			const shouldGroup = (lastMessage != null) &&
				(lastMessage.user.id === __guard__(message != null ? message.user : undefined, x => x.id)) &&
