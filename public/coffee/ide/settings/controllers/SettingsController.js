...
define([
	...
	App.controller("SettingsController", ["$scope", "settings", "ide", "_", function($scope, settings, ide, _) {
		...
		if (($scope.settings.fontFamily != null) && !["monaco", "lucida"].includes($scope.settings.fontFamily)) {
			delete $scope.settings.fontFamily;
		}

		if (($scope.settings.lineHeight != null) && !["compact", "normal", "wide"].includes($scope.settings.lineHeight)) {
		...
		$scope.$watch("settings.overallTheme", (overallTheme, oldOverallTheme) => {
			if (overallTheme !== oldOverallTheme) {
				const chosenTheme = _.find($scope.overallThemesList, theme => theme.val === overallTheme);
				if (chosenTheme != null) {
		...
		$scope.$watch("project.rootDoc_id", (rootDoc_id, oldRootDoc_id) => {
			...
			if (typeof oldRootDoc_id === "undefined") { return; }
