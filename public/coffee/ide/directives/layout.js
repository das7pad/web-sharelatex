			...
			compile() {
				return {
					pre(scope, element, attrs) {
						...
						const hasCustomToggler = (customTogglerPane != null) && (customTogglerMsgWhenOpen != null) && (customTogglerMsgWhenClosed != null);
						...
						if ((state = ide.localStorage(`layout.${name}`)) != null) {
							if (state.east != null) {
								if ((attrs.minimumRestoreSizeEast == null) || ((state.east.size >= attrs.minimumRestoreSizeEast) && !state.east.initClosed)) {
									options.east = state.east;
								}
							}
							if (state.west != null) {
								if ((attrs.minimumRestoreSizeWest == null) || ((state.west.size >= attrs.minimumRestoreSizeWest) && !state.west.initClosed)) {
						...
						const repositionCustomToggler = function() {
							...
							if (paneState != null) {
								return customTogglerEl.css(positionAnchor, paneState.initClosed ? 0 : paneState.size);
							}
						};
						...
						const onExternalResize = function() {
							if ((attrs.resizeProportionally != null) && scope.$eval(attrs.resizeProportionally)) {
								const eastState = element.layout().readState().east;
								if (eastState != null) {
									const newInternalWidth = (eastState.size / oldWidth) * element.width();
						...
						if (hasCustomToggler) {
							...
							if ((state[customTogglerPane] != null ? state[customTogglerPane].initClosed : undefined) === true) {
							...
							customTogglerScope.tooltipPlacement = customTogglerPane === "east" ? "left" : "right";
...
`)(customTogglerScope);
						...
						var onPaneOpen = function(pane) {
							...
							return customTogglerEl.scope().$applyAsync(() => customTogglerEl.scope().isOpen = true);
						};
						...
						var onPaneClose = function(pane) {
							...
							return customTogglerEl.scope().$applyAsync(() => customTogglerEl.scope().isOpen = false);
						};

						// Save state when exiting
						$(window).unload(() => ide.localStorage(`layout.${name}`, element.layout().readState()));
						...
						if (attrs.openEast != null) {
							scope.$watch(attrs.openEast, function(value, oldValue) {
								...
								return setTimeout(() => scope.$digest()
								, 0);
							});
						}
