...
define([
	...
	App.factory("abTestManager", function($http, ipCookie) {
		...
		const _persistCookieStep = function(testName, bucket, newStep){
			...
			return ga('send', 'event', 'ab_tests', `${testName}:${bucket}`, `step-${newStep}`);
		};
		...
		return {
			processTestWithStep: (processTestWithStep = function(testName, bucket, newStep){
				const currentCookieStep = __guard__(_getTestCookie(testName, bucket), x => x.step);
			...
			getABTestBucket: (getABTestBucket = function(test_name, buckets) {
				...
				const bucketIndex = parseInt(hash.toString().slice(0,2), 16) % ((buckets != null ? buckets.length : undefined) || 2);
	...
	return App.controller("AbTestController", function($scope, abTestManager){
		...
		return _.each(window.ab, event=> abTestManager.processTestWithStep(event.testName, event.bucket, event.step));
	});
});
