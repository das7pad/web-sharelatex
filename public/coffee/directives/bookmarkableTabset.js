		...
		({
			...
			link(scope, el, attrs, tabset) {
				...
				const _makeActive = function(hash) {
					if ((hash != null) && (hash !== "")) {
						const matchingTab = _.find(tabset.tabs, tab => tab.bookmarkableTabId === hash);
						if (matchingTab != null) {
							matchingTab.select();
				...
				return scope.$applyAsync(function() {
					...
					const _clickLinkToTab = event => _makeActive(event.currentTarget.getAttribute("href").replace('#', ''));
