...
define([
	...
	const defaultRoleHints = [ "Undergraduate Student", "Masters Student (MSc, MA, ...)", "Doctoral Student (PhD, EngD, ...)", "Postdoc", "Lecturer", "Senior Lecturer", "Reader", "Associate Professor ", "Assistant Professor ", "Professor", "Emeritus Professor" ];
	...
	const commonTLDs = [ "br", "cn", "co", "co.jp",  "co.uk", "com", "com.au", "de","fr", "in", "info", "io", "net", "no", "ru", "se", "us", "com.tw", "com.br", "pl", "it", "co.in", "com.mx" ];
	const commonDomains = [ "gmail", "googlemail", "icloud", "me", "yahoo", "ymail", "yahoomail", "hotmail", "live", "msn", "outlook", "gmx", "mail", "aol", "163", "mac",  "qq", "o2", "libero", "126" ];
	...
	return App.factory("UserAffiliationsDataService", ["$http", "$q", "_", function($http, $q, _) {
		...
		const getUserDefaultEmail = () =>
			getUserEmails().then(userEmails => _.find(userEmails, userEmail => userEmail.default))
		;
		...
		const getUniversityDomainFromPartialDomainInput = function(partialDomainInput) {
			if (universitiesByDomain[partialDomainInput] != null) {
				return $q.resolve(universitiesByDomain[partialDomainInput]);
			} else { 
				return $http.get("/institutions/domains" , { params: { hostname: partialDomainInput, limit: 1 } })
		...
		const addUserAffiliationWithUnknownUniversity = (email, unknownUniversityName, unknownUniversityCountryCode, role, department) =>
			$http.post("/user/emails", {
				email,
				university: {
