...
define([
	"base",
	"libs/algolia-2.5.2"
], function(App, algolia){
	App.factory("Institutions", () => new AlgoliaSearch(window.algolia.institutions.app_id, window.algolia.institutions.api_key).initIndex("institutions"));
	...
	App.controller("UserProfileController", function($scope, $modal, $http){
		...
		$scope.getPercentComplete = function() {
			const results = _.filter($scope.userInfoForm, value=> (value == null) || ((value != null ? value.length : undefined) !== 0));
	...
	return App.controller("UserProfileModalController", function($scope, $modalInstance, $http, Institutions) {
		...
		$scope.updateInstitutionsList = function(inputVal){
			...
			return Institutions.search($scope.userInfoForm.institution, (err, response)=>
				$scope.institutions = _.map(response.hits, institution=> `${institution.name} (${institution.domain})`)
			);
		};
