...
define([
	...
	const App = angular.module("SharelatexApp", [
		"ui.bootstrap",
		"autocomplete",
		"RecursionHelper",
		"ng-context-menu",
		...
		"ngTagsInput",
		"ui.select"
	]).config(function($qProvider, sixpackProvider, $httpProvider, uiSelectConfig) {
		...
		return __guard__(typeof MathJax !== 'undefined' && MathJax !== null ? MathJax.Hub : undefined, x => x.Config({
			messageStyle: "none",
			imageFont:null,
			"HTML-CSS": {
				availableFonts: ["TeX"],
	...
	const sl_debugging = (__guard__(window.location != null ? window.location.search : undefined, x => x.match(/debug=true/)) != null);
	window.sl_console =
		{log(...args) { if (sl_debugging) { return console.log(...Array.from(args || [])); } }};
