...
module.exports = (ProjectEntityMongoUpdateHandler = (self = {
	...
	addDoc: wrapWithLock(function(project_id, folder_id, doc, callback) {
		...
		return ProjectGetter.getProjectWithoutLock(project_id, {rootFolder:true, name:true, overleaf:true}, function(err, project) {
			if (err != null) {
				logger.err({project_id, err}, "error getting project for add doc");
				return callback(err);
			}
			logger.log({project_id, folder_id, doc_name: doc.name}, "adding doc to project with project");
	...
	addFile: wrapWithLock(function(project_id, folder_id, fileRef, callback){
		...
		return ProjectGetter.getProjectWithoutLock(project_id, {rootFolder:true, name:true, overleaf:true}, function(err, project) {
			...
			logger.log({project_id: project._id, folder_id, file_name: fileRef.name}, "adding file");
			return self._confirmFolder(project, folder_id, folder_id => self._putElement(project, folder_id, fileRef, "file", callback));
		});
	}),

	replaceFileWithNew: wrapWithLock((project_id, file_id, newFileRef, callback) => ProjectGetter.getProjectWithoutLock(project_id, {rootFolder:true, name:true, overleaf:true}, function(err, project) {
        if (err != null) { return callback(err); }
        return ProjectLocator.findElement({project, element_id: file_id, type: 'file'}, (err, fileRef, path)=> {
            if (err != null) { return callback(err); }
            return ProjectEntityMongoUpdateHandler._insertDeletedFileReference(project_id, fileRef, function(err) {
                if (err != null) { return callback(err); }
                ...
                return Project.findOneAndUpdate(conditions, update, {new:true}, function(err, newProject) {
                    if (err != null) { return callback(err); }
	...
	mkdirp: wrapWithLock(function(project_id, path, options, callback) {
		...
		return ProjectGetter.getProjectWithOnlyFolders(project_id, (err, project)=> {
			if (path === '/') {
			...
			const procesFolder = (previousFolders, folderName, callback)=> {
				...
				return ProjectLocator.findElementByPath({project, path: builtUpPath, exactCaseMatch: (options != null ? options.exactCaseMatch : undefined)}, (err, foundFolder)=> {
					if ((foundFolder == null)) {
						logger.log({path, project_id:project._id, folderName}, "making folder from mkdirp");
						return self.addFolder.withoutLock(project_id, parentFolder_id, folderName, function(err, newFolder, parentFolder_id){
							if (err != null) { return callback(err); }
	...
	moveEntity: wrapWithLock(function(project_id, entity_id, destFolderId, entityType, callback) {
		if (callback == null) { callback = function(error) {}; }
		return ProjectGetter.getProjectWithoutLock(project_id, {rootFolder:true, name:true, overleaf:true}, function(err, project) {
			if (err != null) { return callback(err); }
			return ProjectLocator.findElement({project, element_id: entity_id, type: entityType}, function(err, entity, entityPath){
				if (err != null) { return callback(err); }
				...
				return self._checkValidMove(project, entityType, entity, entityPath, destFolderId, function(error) {
					if (error != null) { return callback(error); }
					return ProjectEntityHandler.getAllEntitiesFromProject(project, function(error, oldDocs, oldFiles) {
						if (error != null) { return callback(error); }
						...
						// opposite order.
						return self._putElement(project, destFolderId, entity, entityType, function(err, result){
							if (err != null) { return callback(err); }
							...
							return self._removeElementFromMongoArray(Project, project_id, entityPath.mongo, entity_id, function(err, newProject){
								if (err != null) { return callback(err); }
								return ProjectEntityHandler.getAllEntitiesFromProject(newProject, function(err, newDocs, newFiles) {
									if (err != null) { return callback(err); }
									const startPath = entityPath.fileSystem;
									const endPath = result.path.fileSystem;
									const changes = {oldDocs, newDocs, oldFiles, newFiles, newProject};
									// check that no files have been lost (or duplicated)
									if ((oldFiles.length !== newFiles.length) || (oldDocs.length !== newDocs.length)) {
										logger.err({
											project_id,
									...
									return callback(null, project, startPath, endPath, entity.rev, changes, callback);
								});
							});
						});
					});
	...
	deleteEntity: wrapWithLock((project_id, entity_id, entityType, callback) => ProjectGetter.getProjectWithoutLock(project_id, {name:true, rootFolder:true, overleaf:true}, function(error, project) {
        if (error != null) { return callback(error); }
        return ProjectLocator.findElement({project, element_id: entity_id, type: entityType}, function(error, entity, path) {
            if (error != null) { return callback(error); }
            return self._removeElementFromMongoArray(Project, project_id, path.mongo, entity_id, function(error, newProject) {
                if (error != null) { return callback(error); }
	...
	renameEntity: wrapWithLock((project_id, entity_id, entityType, newName, callback) => ProjectGetter.getProjectWithoutLock(project_id, {rootFolder:true, name:true, overleaf:true}, (error, project)=> {
        if (error != null) { return callback(error); }
        return ProjectEntityHandler.getAllEntitiesFromProject(project, (error, oldDocs, oldFiles) => {
            if (error != null) { return callback(error); }
            return ProjectLocator.findElement({project, element_id:entity_id, type:entityType}, (error, entity, entPath, parentFolder)=> {
                if (error != null) { return callback(error); }
                ...
                return self._checkValidElementName(parentFolder, newName, error => {
                    if (error != null) { return callback(error); }
                    ...
                    return Project.findOneAndUpdate(conditions, update, { "new": true}, function(error, newProject) {
                        if (error != null) { return callback(error); }
                        return ProjectEntityHandler.getAllEntitiesFromProject(newProject, (error, newDocs, newFiles) => {
                            if (error != null) { return callback(error); }
	...
	addFolder: wrapWithLock((project_id, parentFolder_id, folderName, callback) => ProjectGetter.getProjectWithoutLock(project_id, {rootFolder:true, name:true, overleaf:true}, function(err, project) {
        if (err != null) {
            logger.err({project_id, err}, "error getting project for add folder");
            return callback(err);
        }
        return self._confirmFolder(project, parentFolder_id, parentFolder_id => {
            const folder = new Folder({name: folderName});
            logger.log({project: project._id, parentFolder_id, folderName}, "adding new folder");
            return self._putElement(project, parentFolder_id, folder, "folder", err=> {
                if (err != null) {
                    logger.err({err, project_id:project._id}, "error adding folder to project");
                    return callback(err);
                }
	...
	_removeElementFromMongoArray(model, model_id, path, element_id, callback){
		if (callback == null) { callback = function(err, project) {}; }
		...
		return model.findOneAndUpdate(conditions, pullUpdate, {"new": true}, callback);
	},
	...
	_countElements(project){
		var countFolder = function(folder){
			...
			for (let subfolder of Array.from((folder != null ? folder.folders : undefined) || [])) {
				total += countFolder(subfolder);
			}

			if (__guard__(folder != null ? folder.folders : undefined, x => x.length) != null) {
				total += folder.folders.length;
			}

			if (__guard__(folder != null ? folder.docs : undefined, x1 => x1.length) != null) {
				total += folder.docs.length;
			}

			if (__guard__(folder != null ? folder.fileRefs : undefined, x2 => x2.length) != null) {
	...
	_putElement(project, folder_id, element, type, callback){
		let e;
		if (callback == null) { callback = function(err, path, project){}; }
		...
		if ((element == null) || (element._id == null)) {
			...
			logger.err({project_id:project._id, folder_id, element, type}, "failed trying to insert element as it was null");
			return callback(e);
		}
		...
		if (!SafePath.isCleanFilename(element.name)) {
			...
			logger.err({project_id:project._id, folder_id, element, type}, "failed trying to insert element as name was invalid");
			return callback(e);
		}
		...
		return ProjectLocator.findElement({project, element_id:folder_id, type:"folders"}, (err, folder, path)=> {
			if (err != null) {
				logger.err({err, project_id:project._id, folder_id, type, element}, "error finding folder for _putElement");
				return callback(err);
			}
			const newPath = {
			...
			return self._checkValidElementName(folder, element.name, err => {
				...
				logger.log({project_id: project._id, element_id: element._id, fileType: type, folder_id, mongopath}, "adding element to project");
				// We are using Mongoose here, but if we ever switch to a direct mongo call
				// the next line will need to be updated to {returnNewDocument:true}
				return Project.findOneAndUpdate(conditions, update, {"new": true}, function(err, newProject){
					if (err != null) {
						logger.err({err, project_id: project._id}, 'error saving in putElement project');
						return callback(err);
					}
	...
	_blockedFilename(entityPath, entityType) {
		...
		const [dir, file] = Array.from([path.dirname(entityPath.fileSystem), path.basename(entityPath.fileSystem)]);
	...
	_checkValidElementName(folder, name, callback) {
		...
		for (let doc of Array.from((folder != null ? folder.docs : undefined) || [])) {
			if (doc.name === name) { return callback(err); }
		}
		for (let file of Array.from((folder != null ? folder.fileRefs : undefined) || [])) {
			if (file.name === name) { return callback(err); }
		}
		for (folder of Array.from((folder != null ? folder.folders : undefined) || [])) {
			if (folder.name === name) { return callback(err); }
		}
		return callback();
	},

	_confirmFolder(project, folder_id, callback){
		logger.log({folder_id, project_id:project._id}, "confirming folder in project");
	...
	_checkValidMove(project, entityType, entity, entityPath, destFolderId, callback) {
		if (callback == null) { callback = function(error) {}; }
		return ProjectLocator.findElement({ project, element_id: destFolderId, type:"folder"}, function(err, destEntity, destFolderPath) {
			if (err != null) { return callback(err); }
			// check if there is already a doc/file/folder with the same name
			// in the destination folder
			return self._checkValidElementName(destEntity, entity.name, function(err){
				if (err != null) { return callback(err); }
				if (/folder/.test(entityType)) {
					...
					const isNestedFolder = destFolderPath.fileSystem.slice(0, entityPath.fileSystem.length) === entityPath.fileSystem;
					if (isNestedFolder) {
