...
module.exports = (BrandVariationsHandler = {
	getBrandVariationById(brandVariationId, callback){
		if (callback == null) { callback = function(error, brandVariationDetails) {}; }
...
var _formatBrandVariationDetails = function(details) {
	...
	if (details.publish_menu_icon != null) {
		return details.publish_menu_icon = _setV1AsHostIfRelativeURL(details.publish_menu_icon);
	}
};

var _setV1AsHostIfRelativeURL = urlString => // The first argument is the base URL to resolve against if the second argument is not absolute.
...
url.resolve(
    __guard__(__guard__(settings != null ? settings.apis : undefined, x1 => x1.v1), x => x.url),
    urlString
);
