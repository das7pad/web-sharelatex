...
module.exports = {
	index(req, res, next){
		...
		return entity.fetchV1Data(function(error, entity) {
			...
			return UserMembershipHandler.getUsers(entity, entityConfig, function(error, users){
				let entityName;
				if (error != null) { return next(error); }
				const entityPrimaryKey = entity[entityConfig.fields.primaryKey].toString();
				if (entityConfig.fields.name) { entityName = entity[entityConfig.fields.name]; }
				return res.render("user_membership/index", {
					name: entityName,
					users,
					groupSize: (entityConfig.hasMembersLimit ? entity.membersLimit : undefined),
	...
	add(req, res, next){
		...
		return UserMembershipHandler.addUser(entity, entityConfig, email, function(error, user){
	...
	remove(req, res, next){
		...
		return UserMembershipHandler.removeUser(entity, entityConfig, userId, function(error, user){
	...
	create(req, res, next){
		...
		return UserMembershipHandler.createEntity(entityId, entityConfig, function(error, entity) {
			if (error != null) { return next(error); }
