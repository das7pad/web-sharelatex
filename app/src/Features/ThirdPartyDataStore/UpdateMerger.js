...
module.exports = (UpdateMerger = {
	mergeUpdate(user_id, project_id, path, updateRequest, source, callback){
		...
		return FileWriter.writeStreamToDisk(project_id, updateRequest, function(err, fsPath){
			if (err != null) { return callback(err); }
			return UpdateMerger._mergeUpdate(user_id, project_id, path, fsPath, source, mergeErr => fs.unlink(fsPath, function(deleteErr) {
                if (deleteErr != null) {
	...
	_determineFileType(project_id, path, fsPath, callback) {
		if (callback == null) { callback = function(err, fileType) {}; }
		return ProjectEntityHandler.getAllEntities(project_id, function(err, docs, files) {
			if (err != null) { return callback(err); }
	...
	_mergeUpdate(user_id, project_id, path, fsPath, source, callback){
		if (callback == null) { callback = function(error) {}; }
		return UpdateMerger._determineFileType(project_id, path, fsPath, function(err, fileType){
			if (err != null) { return callback(err); }
			if (["existing-file", "new-file"].includes(fileType)) {
				return UpdateMerger.p.processFile(project_id, fsPath, path, source, user_id, callback);
			} else if (["existing-doc", "new-doc"].includes(fileType)) {
				return UpdateMerger.p.processDoc(project_id, user_id, fsPath, path, source, callback);
			} else {
	...
	deleteUpdate(user_id, project_id, path, source, callback){
		...
		return EditorController.deleteEntityWithPath(project_id, path, source, user_id, function() {
			logger.log({project_id, path}, "finished processing update to delete entity from tpds");
			return callback();
		});
	},
	...
	p: {
		...
		processDoc(project_id, user_id, fsPath, path, source, callback){
			return UpdateMerger.p.readFileIntoTextArray(fsPath, function(err, docLines){
				if (err != null) {
					logger.err({project_id},  "error reading file into text array for process doc update");
					return callback(err);
				}
				logger.log({docLines},  "processing doc update from tpds");
				return EditorController.upsertDocWithPath(project_id, path, docLines, source, user_id, function(err) {
					logger.log({project_id}, "completed processing file update from tpds");
					return callback(err);
				});
			});
		},
		...
		processFile(project_id, fsPath, path, source, user_id, callback){
			...
			return EditorController.upsertFileWithPath(project_id, path, fsPath, null, source, user_id, function(err) {
				logger.log({project_id}, "completed processing file update from tpds");
				return callback(err);
			});
		},
