...
module.exports = (CollaboratorsHandler = {
	...
	getMemberIdsWithPrivilegeLevels(project_id, callback) {
		if (callback == null) { callback = function(error, members) {}; }
		const projection = {
			owner_ref: 1,
			collaberator_refs: 1,
			readOnly_refs: 1,
		...
		return ProjectGetter.getProject(project_id, projection, function(error, project) {
			let member_id;
			if (error != null) { return callback(error); }
			if ((project == null)) { return callback(new Errors.NotFoundError(`no project found with id ${project_id}`)); }
			const members = [];
			members.push({
			...
			if (project.publicAccesLevel === PublicAccessLevels.TOKEN_BASED) {
				for (member_id of Array.from(project.tokenAccessReadAndWrite_refs || [])) {
					members.push({
						id: member_id.toString(),
						privilegeLevel: PrivilegeLevels.READ_AND_WRITE,
	...
	getMemberIds(project_id, callback) {
		if (callback == null) { callback = function(error, member_ids) {}; }
		return CollaboratorsHandler.getMemberIdsWithPrivilegeLevels(project_id, function(error, members) {
			if (error != null) { return callback(error); }
	...
	_loadMembers(members, callback) {
		...
		const result = [];
		return async.mapLimit(members, 3,
			(member, cb) => UserGetter.getUserOrUserStubById(member.id, CollaboratorsHandler.USER_PROJECTION, function(error, user) {
	...
	getMembersWithPrivilegeLevels(project_id, callback) {
		...
		return CollaboratorsHandler.getMemberIdsWithPrivilegeLevels(project_id, function(error, members) {
			...
			if (error != null) { return callback(error); }
			return CollaboratorsHandler._loadMembers(members, (error, users) => callback(error, users));
		});
	},
	...
	getMemberIdPrivilegeLevel(user_id, project_id, callback) {
		...
		// then optimise this.
		if (callback == null) { callback = function(error, privilegeLevel) {}; }
		return CollaboratorsHandler.getMemberIdsWithPrivilegeLevels(project_id, function(error, members) {
			if (members == null) { members = []; }
			if (error != null) { return callback(error); }
			for (let member of Array.from(members)) {
				if (member.id === (user_id != null ? user_id.toString() : undefined)) {
	...
	getInvitedMemberCount(project_id, callback) {
		if (callback == null) { callback = function(error, count) {}; }
		return CollaboratorsHandler.getMemberIdsWithPrivilegeLevels(project_id, function(error, members) {
			if (error != null) { return callback(error); }
			return callback(null, (members || []).filter(m => m.source !== Sources.TOKEN).length);
		});
	},

	getInvitedCollaboratorCount(project_id, callback) {
		if (callback == null) { callback = function(error, count) {}; }
		return CollaboratorsHandler.getInvitedMemberCount(project_id, function(error, count) {
			if (error != null) { return callback(error); }
	...
	isUserInvitedMemberOfProject(user_id, project_id, callback) {
		if (callback == null) { callback = function(error, isMember, privilegeLevel) {}; }
		return CollaboratorsHandler.getMemberIdsWithPrivilegeLevels(project_id, function(error, members) {
			if (members == null) { members = []; }
			if (error != null) { return callback(error); }
			for (let member of Array.from(members)) {
				if ((member.id.toString() === user_id.toString()) && (member.source !== Sources.TOKEN)) {
	...
	getProjectsUserIsMemberOf(user_id, fields, callback) {
		if (callback == null) { callback = function(error, ...rest) {}; }
		const obj = rest[0], array = obj.readAndWrite, array1 = obj.readOnly, array2 = obj.tokenReadAndWrite, array3 = obj.tokenReadOnly;
		return;
	...
	removeUserFromProject(project_id, user_id, callback){
		...
		return Project.update(conditions, update, function(err){
			if (err != null) {
				logger.error({err}, "problem removing user from project collaberators");
			}
			return callback(err);
		});
	},
	...
	removeUserFromAllProjets(user_id, callback) {
		if (callback == null) { callback = function(error) {}; }
		return CollaboratorsHandler.getProjectsUserIsMemberOf(user_id, { _id: 1 }, function(error, {readAndWrite, readOnly, tokenReadAndWrite, tokenReadOnly}) {
			if (error != null) { return callback(error); }
			const allProjects =
				readAndWrite
				.concat(readOnly)
			...
			for (let project of Array.from(allProjects)) {
				((project => jobs.push(function(cb) {
                    if ((project == null)) { return cb(); }
                    return CollaboratorsHandler.removeUserFromProject(project._id, user_id, cb);
                })))(project);
			}
	...
	addUserIdToProject(project_id, adding_user_id, user_id, privilegeLevel, callback){
		if (callback == null) { callback = function(error) {}; }
		return ProjectGetter.getProject(project_id, { collaberator_refs: 1, readOnly_refs: 1 }, function(error, project) {
			let level;
			if (error != null) { return callback(error); }
			...
			return Project.update({ _id: project_id }, { $addToSet: level }, function(error) {
				if (error != null) { return callback(error); }
				// Flush to TPDS in background to add files to collaborator's Dropbox
				const ProjectEntityHandler = require("../Project/ProjectEntityHandler");
				ProjectEntityHandler.flushProjectToThirdPartyDataStore(project_id, function(error) {
					if (error != null) {
						return logger.error({err: error, project_id, user_id}, "error flushing to TPDS after adding collaborator");
					}
				});
				return callback();
			});
	...
	getAllInvitedMembers(projectId, callback) {
		if (callback == null) { callback = function(err, members){}; }
		logger.log({projectId}, "fetching all members");
		return CollaboratorsHandler.getInvitedMembersWithPrivilegeLevels(projectId, function(error, rawMembers) {
			if (error != null) {
				logger.err({projectId, error}, "error getting members for project");
				return callback(error);
			}
			const {owner, members} = ProjectEditorHandler.buildOwnerAndMembersViews(rawMembers);
			return callback(null, members);
		});
	},

	userIsTokenMember(userId, projectId, callback) {
		if (callback == null) { callback = function(err, isTokenMember){}; }
	...
	transferProjects(from_user_id, to_user_id, callback) {
		if (callback == null) { callback = function(err, projects) {}; }
		...
		return Project.find(query, { _id: 1 }, function(error, projects) {
			...
			const update_jobs = [];
			update_jobs.push(cb => Project.update({ owner_ref: from_user_id }, { $set: { owner_ref: to_user_id }}, { multi: true }, cb));
			for (let key of Array.from(MEMBER_KEYS)) {
				((key => update_jobs.push(function(cb) {
                    query = {};
                    ...
                    return Project.update(query, addNewUserUpdate, { multi: true }, function(error) {
                        if (error != null) { return cb(error); }
                        return Project.update(query, removeOldUserUpdate, { multi: true }, cb);
                    });
                })))(key);
			}
			...
			for (let project_id of Array.from(project_ids)) {
				((project_id => flush_jobs.push(cb => ProjectEntityHandler.flushProjectToThirdPartyDataStore(project_id, cb))))(project_id);
			}

			// Flush in background, no need to block on this
			async.series(flush_jobs, function(error) {
				if (error != null) {
					return logger.err({err: error, project_ids, from_user_id, to_user_id}, "error flushing tranferred projects to TPDS");
				}
			});
