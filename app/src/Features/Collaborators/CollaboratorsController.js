...
module.exports = (CollaboratorsController = {
	removeUserFromProject(req, res, next) {
		...
		return CollaboratorsController._removeUserIdFromProject(project_id, user_id, function(error) {
	...
	removeSelfFromProject(req, res, next) {
		...
		const project_id = req.params.Project_id;
		const user_id    = __guard__(req.session != null ? req.session.user : undefined, x => x._id);
		return CollaboratorsController._removeUserIdFromProject(project_id, user_id, function(error) {
			if (error != null) { return next(error); }
			return res.sendStatus(204);
		});
	...
	_removeUserIdFromProject(project_id, user_id, callback) {
		...
		return CollaboratorsHandler.removeUserFromProject(project_id, user_id, function(error){
	...
	getAllMembers(req, res, next) {
		...
		return CollaboratorsHandler.getAllInvitedMembers(projectId, function(err, members) {
			if (err != null) {
