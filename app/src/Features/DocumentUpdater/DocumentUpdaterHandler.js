...
module.exports = (DocumentUpdaterHandler = {
	...
	flushMultipleProjectsToMongo(project_ids, callback) {
		...
		for (let project_id of Array.from(project_ids)) {
			((project_id => jobs.push(callback => DocumentUpdaterHandler.flushProjectToMongo(project_id, callback))))(project_id);
		}
	...
	setDocument(project_id, doc_id, user_id, docLines, source, callback){
		if (callback == null) { callback = function(error) {}; }
		logger.log({project_id, doc_id, source, user_id}, "setting doc in document updater");
		return DocumentUpdaterHandler._makeRequest({
			path: `/project/${project_id}/doc/${doc_id}`,
			method: "POST",
			json: {
	...
	getProjectDocsIfMatch(project_id, projectStateHash, callback) {
		...
		return request.post(url, function(error, res, body){
			timer.done();
			if (error != null) {
				logger.error({err:error, url, project_id}, "error getting project docs from doc updater");
				return callback(error);
			}
			if (res.statusCode === 409) { // HTTP response code "409 Conflict"
				...
				let docs;
				logger.log({project_id}, "got project docs from document document updater");
				try {
	...
	updateProjectStructure(project_id, projectHistoryId, userId, changes, callback){
		if (callback == null) { callback = function(error) {}; }
		if (!(settings.apis.project_history != null ? settings.apis.project_history.sendProjectStructureOps : undefined)) { return callback(); }

		const docUpdates = DocumentUpdaterHandler._getUpdates('doc', changes.oldDocs, changes.newDocs);
		const fileUpdates = DocumentUpdaterHandler._getUpdates('file', changes.oldFiles, changes.newFiles);
		const projectVersion = __guard__(changes != null ? changes.newProject : undefined, x => x.version);

		if ((docUpdates.length + fileUpdates.length) < 1) { return callback(); }
	...
	_getUpdates(entityType, oldEntities, newEntities) {
		...
		if (!oldEntities) { oldEntities = []; }
		if (!newEntities) { newEntities = []; }
		const updates = [];

		const oldEntitiesHash = _.indexBy(oldEntities, entity => entity[entityType]._id.toString());
		const newEntitiesHash = _.indexBy(newEntities, entity => entity[entityType]._id.toString());
...
const keys = {
	pendingUpdates(op) {  return `${PENDINGUPDATESKEY}:${op.doc_id}`; },
	docsWithPendingUpdates: DOCIDSWITHPENDINGUPDATES,
	docLines(op) { return `${DOCLINESKEY}:${op.doc_id}`; },
