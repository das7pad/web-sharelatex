...
module.exports = (EditorController = {
	addDoc(project_id, folder_id, docName, docLines, source, user_id, callback){
		...
		return EditorController.addDocWithRanges(project_id, folder_id, docName, docLines, {}, source, user_id, callback);
	},

	addDocWithRanges(project_id, folder_id, docName, docLines, docRanges, source, user_id, callback){
		if (callback == null) { callback = function(error, doc){}; }
		docName = docName.trim();
		logger.log({project_id, folder_id, docName, source}, "sending new doc to project");
		Metrics.inc("editor.add-doc");
		return ProjectEntityUpdateHandler.addDocWithRanges(project_id, folder_id, docName, docLines, docRanges, user_id, (err, doc, folder_id)=> {
			if (err != null) {
				logger.err({err, project_id, docName}, "error adding doc without lock");
				return callback(err);
			}
	...
	addFile(project_id, folder_id, fileName, fsPath, linkedFileData, source, user_id, callback){
		...
		return ProjectEntityUpdateHandler.addFile(project_id, folder_id, fileName, fsPath, linkedFileData, user_id, (err, fileRef, folder_id)=> {
			if (err != null) {
				logger.err({err, project_id, folder_id, fileName}, "error adding file without lock");
				return callback(err);
			}
	...
	upsertDoc(project_id, folder_id, docName, docLines, source, user_id, callback){
		if (callback == null) { callback = function(err){}; }
		return ProjectEntityUpdateHandler.upsertDoc(project_id, folder_id, docName, docLines, source, user_id, function(err, doc, didAddNewDoc) {
			if (didAddNewDoc) {
				EditorRealTimeController.emitToRoom(project_id, 'reciveNewDoc', folder_id, doc, source);
			}
			return callback(err, doc);
	...
	upsertFile(project_id, folder_id, fileName, fsPath, linkedFileData, source, user_id, callback) {
		if (callback == null) { callback = function(err, file) {}; }
		return ProjectEntityUpdateHandler.upsertFile(project_id, folder_id, fileName, fsPath, linkedFileData, user_id, function(err, newFile, didAddFile, existingFile) {
			if (err != null) { return callback(err); }
			if (!didAddFile) { // replacement, so remove the existing file from the client
	...
	upsertDocWithPath(project_id, elementPath, docLines, source, user_id, callback) {
		return ProjectEntityUpdateHandler.upsertDocWithPath(project_id, elementPath, docLines, source, user_id, function(err, doc, didAddNewDoc, newFolders, lastFolder) {
			if (err != null) { return callback(err); }
			return EditorController._notifyProjectUsersOfNewFolders(project_id, newFolders, function(err) {
				if (err != null) { return callback(err); }
				if (didAddNewDoc) {
	...
	upsertFileWithPath(project_id, elementPath, fsPath, linkedFileData, source, user_id, callback) {
		return ProjectEntityUpdateHandler.upsertFileWithPath(project_id, elementPath, fsPath, linkedFileData, user_id, function(err, newFile, didAddFile, existingFile, newFolders, lastFolder) {
			if (err != null) { return callback(err); }
			return EditorController._notifyProjectUsersOfNewFolders(project_id, newFolders, function(err) {
				if (err != null) { return callback(err); }
				if (!didAddFile) { // replacement, so remove the existing file from the client
	...
	addFolder(project_id, folder_id, folderName, source, callback){
		if (callback == null) { callback = function(error, folder){}; }
		folderName = folderName.trim();
		logger.log({project_id, folder_id, folderName, source}, "sending new folder to project");
		Metrics.inc("editor.add-folder");
		return ProjectEntityUpdateHandler.addFolder(project_id, folder_id, folderName, (err, folder, folder_id)=> {
			if (err != null) {
				logger.err({err, project_id, folder_id, folderName, source}, "could not add folder");
				return callback(err);
			}
			return EditorController._notifyProjectUsersOfNewFolder(project_id, folder_id, folder, function(err) {
				if (err != null) { return callback(err); }
	...
	mkdirp(project_id, path, callback){
		if (callback == null) { callback = function(error, newFolders, lastFolder){}; }
		logger.log({project_id, path}, "making directories if they don't exist");
		return ProjectEntityUpdateHandler.mkdirp(project_id, path, (err, newFolders, lastFolder)=> {
			if (err != null) {
	...
	deleteEntity(project_id, entity_id, entityType, source, userId, callback){
		...
		return ProjectEntityUpdateHandler.deleteEntity(project_id, entity_id, entityType, userId, function(err){
			if (err != null) {
	...
	deleteEntityWithPath(project_id, path, source, user_id, callback) {
		return ProjectEntityUpdateHandler.deleteEntityWithPath(project_id, path, user_id, function(err, entity_id) {
			if (err != null) { return callback(err); }
			EditorRealTimeController.emitToRoom(project_id, 'removeEntity', entity_id, source);
	...
	updateProjectDescription(project_id, description, callback){
		...
		return ProjectDetailsHandler.setProjectDescription(project_id, description, function(err){
			if (err != null) {
				logger.err({err, project_id, description}, "something went wrong setting the project description");
				return callback(err);
			}
	...
	renameEntity(project_id, entity_id, entityType, newName, userId, callback){
		...
		logger.log({entity_id, entity_id, entity_id}, "reciving new name for entity for project");
		return ProjectEntityUpdateHandler.renameEntity(project_id, entity_id, entityType, newName, userId, function(err) {
			if (err != null) {
				logger.err({err, project_id, entity_id, entityType, newName}, "error renaming entity");
				return callback(err);
			}
			if (newName.length > 0) {
	...
	moveEntity(project_id, entity_id, folder_id, entityType, userId, callback){
		...
		return ProjectEntityUpdateHandler.moveEntity(project_id, entity_id, folder_id, entityType, userId, function(err) {
			if (err != null) {
				logger.err({err, project_id, entity_id, folder_id}, "error moving entity");
				return callback(err);
			}
	...
	renameProject(project_id, newName, callback) {
		if (callback == null) { callback = function(err) {}; }
		return ProjectDetailsHandler.renameProject(project_id, newName, function(err) {
			if (err != null) {
	...
	setCompiler(project_id, compiler, callback) {
		if (callback == null) { callback = function(err) {}; }
		return ProjectOptionsHandler.setCompiler(project_id, compiler, function(err) {
			if (err != null) { return callback(err); }
	...
	setImageName(project_id, imageName, callback) {
		if (callback == null) { callback = function(err) {}; }
		return ProjectOptionsHandler.setImageName(project_id, imageName, function(err) {
			if (err != null) { return callback(err); }
	...
	setSpellCheckLanguage(project_id, languageCode, callback) {
		if (callback == null) { callback = function(err) {}; }
		return ProjectOptionsHandler.setSpellCheckLanguage(project_id, languageCode, function(err) {
			if (err != null) { return callback(err); }
			logger.log({languageCode, project_id}, "setting languageCode for spell check");
	...
	setPublicAccessLevel(project_id, newAccessLevel, callback) {
		if (callback == null) { callback = function(err) {}; }
		return ProjectDetailsHandler.setPublicAccessLevel(project_id, newAccessLevel, function(err) {
			if (err != null) { return callback(err); }
			...
			if (newAccessLevel === PublicAccessLevels.TOKEN_BASED) {
				return ProjectDetailsHandler.ensureTokensArePresent(project_id, function(err, tokens) {
					if (err != null) { return callback(err); }
					EditorRealTimeController.emitToRoom(
						project_id,
	...
	setRootDoc(project_id, newRootDocID, callback) {
		if (callback == null) { callback = function(err) {}; }
		return ProjectEntityUpdateHandler.setRootDoc(project_id, newRootDocID, function(err) {
			if (err != null) { return callback(err); }
	...
	_notifyProjectUsersOfNewFolders(project_id, folders, callback){
		if (callback == null) { callback = function(error){}; }
		return async.eachSeries(folders,
	...
	_notifyProjectUsersOfNewFolder(project_id, folder_id, folder, callback){
		if (callback == null) { callback = function(error){}; }
		logger.log({project_id, folder, parentFolder_id:folder_id}, "sending newly created folder out to users");
