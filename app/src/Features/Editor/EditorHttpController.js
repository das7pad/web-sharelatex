...
module.exports = (EditorHttpController = {
	joinProject(req, res, next) {
		...
		return EditorHttpController._buildJoinProjectView(req, project_id, user_id, function(error, project, privilegeLevel) {
	...
	_buildJoinProjectView(req, project_id, user_id, callback) {
		...
		return ProjectGetter.getProjectWithoutDocLines(project_id, function(error, project) {
			if (error != null) { return callback(error); }
			if ((project == null)) { return callback(new Error("not found")); }
			return CollaboratorsHandler.getInvitedMembersWithPrivilegeLevels(project_id, function(error, members) {
				if (error != null) { return callback(error); }
				...
				return AuthorizationManager.getPrivilegeLevelForProject(user_id, project_id, token, function(error, privilegeLevel) {
					...
					return CollaboratorsInviteHandler.getAllInvites(project_id, function(error, invites) {
						if (error != null) { return callback(error); }
	...
	addDoc(req, res, next) {
		...
		logger.log({project_id, docName:name, parent_folder_id}, "getting request to add doc to project");
		if (!EditorHttpController._nameIsAcceptableLength(name)) {
			return res.sendStatus(400);
		}
		return EditorController.addDoc(project_id, parent_folder_id, name, [], "editor", user_id, function(error, doc) {
			if (error === "project_has_to_many_files") {
				return res.status(400).json(req.i18n.translate("project_has_to_many_files"));
			} else if (error != null) {
				return next(error);
			} else {
				return res.json(doc);
	...
	addFolder(req, res, next) {
		...
		return EditorController.addFolder(project_id, parent_folder_id, name, "editor", function(error, doc) {
			if (error === "project_has_to_many_files") {
				return res.status(400).json(req.i18n.translate("project_has_to_many_files"));
			} else if ((error != null ? error.message : undefined) === 'invalid element name') {
	...
	renameEntity(req, res, next) {
		...
		const user_id = AuthenticationController.getLoggedInUserId(req);
		return EditorController.renameEntity(project_id, entity_id, entity_type, name, user_id, function(error) {
			if (error != null) { return next(error); }
	...
	moveEntity(req, res, next) {
		...
		const user_id = AuthenticationController.getLoggedInUserId(req);
		return EditorController.moveEntity(project_id, entity_id, folder_id, entity_type, user_id, function(error) {
			if (error != null) { return next(error); }
			return res.sendStatus(204);
		});
	},
	...
	deleteEntity(req, res, next) {
		...
		const user_id = AuthenticationController.getLoggedInUserId(req);
		return EditorController.deleteEntity(project_id, entity_id, entity_type, "editor", user_id, function(error) {
			if (error != null) { return next(error); }
			return res.sendStatus(204);
		});
	}
