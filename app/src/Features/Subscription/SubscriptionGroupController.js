...
module.exports = {
	...
	removeUserFromGroup(req, res, next){
		...
		logger.log({subscriptionId: subscription._id, userToRemove_id}, "removing user from group subscription");
		return SubscriptionGroupHandler.removeUserFromGroup(subscription._id, userToRemove_id, function(err){
			if (err != null) {
				logger.err({err, subscriptionId: subscription._id, userToRemove_id}, "error removing user from group");
				return next(err);
			}
			return res.send();
		});
	...
	removeSelfFromGroup(req, res, next){
		...
		return getManagedSubscription(adminUserId, function(error, subscription) {
			...
			logger.log({adminUserId, userToRemove_id}, "removing user from group subscription after self request");
			return SubscriptionGroupHandler.removeUserFromGroup(subscription._id, userToRemove_id, function(err){
				if (err != null) {
					logger.err({err, userToRemove_id, adminUserId}, "error removing self from group");
...
var getManagedSubscription = (managerId, callback) => SubscriptionLocator.findManagedSubscription(managerId, function(err, subscription){
    if (subscription != null) {
        logger.log({managerId}, "got managed subscription");
    } else {
        if (!err) { err = new Error(`No subscription found managed by user ${managerId}`); }
    }
