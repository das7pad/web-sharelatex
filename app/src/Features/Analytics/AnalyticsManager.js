...
const makeFaultTolerantRequest = function(userId, options, callback) {
	if (!__guard__(settings.apis != null ? settings.apis.analytics : undefined, x => x.enabled)) {
		return callback();
	}

	if ((userId+"") === ((settings.smokeTest != null ? settings.smokeTest.userId : undefined)+"")) {
		return callback();
	}
...
var makeRequest = function(opts, callback){
	...
	if (__guard__(settings.apis != null ? settings.apis.analytics : undefined, x1 => x1.url) != null) {
		const urlPath = opts.url;
		opts.url = `${settings.apis.analytics.url}${urlPath}`;
		return request(opts, callback);
	} else {
		return callback(new Errors.ServiceNotConfiguredError('Analytics service not configured'));
	}
};
