...
module.exports = (UserController = {
	getLoggedInUsersPersonalInfo(req, res, next) {
		...
		logger.log({user_id}, "reciving request for getting logged in users personal info");
		if ((user_id == null)) { return next(new Error("User is not logged in")); }
	...
	getPersonalInfo(req, res, next) {
		...
		return UserGetter.getUser(query, { _id: true, first_name: true, last_name: true, email: true}, function(error, user) {
	...
	formatPersonalInfo(user, callback) {
		if (callback == null) { callback = function(error, info) {}; }
		if ((user == null)) {
			return {};
		}
		const formatted_user = { id: user._id.toString() };
		for (let key of ["first_name", "last_name", "email", "signUpDate", "role", "institution"]) {
			if (user[key] != null) {
