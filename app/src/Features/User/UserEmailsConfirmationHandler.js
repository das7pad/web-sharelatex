...
module.exports = (UserEmailsConfirmationHandler = {
	sendConfirmationEmail(user_id, email, emailTemplate, callback) {
		...
		if (process.env['SHARELATEX_NO_CONFIRMATION_MESSAGES'] != null) { return callback(null); }

		email = EmailHelper.parseEmail(email);
		if ((email == null)) { return callback(new Error('invalid email')); }
		const data = {user_id, email};
		return OneTimeTokenHandler.getNewToken('email_confirmation', data, {expiresIn: ONE_YEAR_IN_S}, function(err, token){
	...
	confirmEmailFromToken(token, callback) {
		...
		return OneTimeTokenHandler.getValueFromTokenAndExpire('email_confirmation', token, function(error, data) {
