...
module.exports = (UserSessionsManager = {
	...
	getAllUserSessions(user, exclude, callback) {
		if (callback == null) { callback = function(err, sessionKeys){}; }
		...
		return rclient.smembers(sessionSetKey, function(err, sessionKeys) {
			...
			return Async.mapSeries(sessionKeys, ((k, cb) => rclient.get(k, cb)), function(err, sessions) {
				if (err != null) {
				...
				for (let session of Array.from(sessions)) {
					...
					const session_user = (session != null ? session.user : undefined) || __guard__(session != null ? session.passport : undefined, x => x.user);
					result.push({
	...
	revokeAllUserSessions(user, retain, callback) {
		...
		return rclient.smembers(sessionSetKey, function(err, sessionKeys) {
			...
			const keysToDelete = _.filter(sessionKeys, k => !Array.from(retain).includes(k));
	...
	touch(user, callback) {
		...
		const sessionSetKey = UserSessionsRedis.sessionSetKey(user);
		return rclient.expire(sessionSetKey, `${Settings.cookieSessionLength}`, function(err, response) {
			if (err != null) {
	...
	_checkSessions(user, callback) {
		...
		return rclient.smembers(sessionSetKey, function(err, sessionKeys) {
			...
			return Async.series(
				sessionKeys.map(
					key => next => rclient.get(key, function(err, val) {
                        if (err != null) {
                            return next(err);
                        }
                        if ((val == null)) {
