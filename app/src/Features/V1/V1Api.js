...
const DEFAULT_V1_PARAMS = {
	baseUrl: __guard__(__guard__(settings != null ? settings.apis : undefined, x1 => x1.v1), x => x.url),
	auth: {
		user: __guard__(__guard__(settings != null ? settings.apis : undefined, x3 => x3.v1), x2 => x2.user),
		pass: __guard__(__guard__(settings != null ? settings.apis : undefined, x5 => x5.v1), x4 => x4.pass)
	},
	json: true,
	timeout: 30 * 1000
};
...
const DEFAULT_V1_OAUTH_PARAMS = {
	baseUrl: __guard__(__guard__(settings != null ? settings.apis : undefined, x7 => x7.v1), x6 => x6.url),
	json: true,
	timeout: 30 * 1000
};
...
module.exports = (V1Api = {
	request(options, callback) {
		if ((callback == null)) { return request(options); }
		return v1Request(options, (error, response, body) => V1Api._responseHandler(options, error, response, body, callback));
	},

	oauthRequest(options, token, callback) {
		if (options.uri == null) { return callback(new Error("uri required")); }
		if (options.method == null) { options.method = "GET"; }
		options.auth = {bearer: token};
		return v1OauthRequest(options, (error, response, body) => V1Api._responseHandler(options, error, response, body, callback));
	},
