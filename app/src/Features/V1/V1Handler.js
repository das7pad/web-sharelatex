...
module.exports = (V1Handler = {
	...
	authWithV1(email, password, callback) {
		if (callback == null) { callback = function(err, isValid, v1Profile){}; }
		return V1Api.request({
			...
			if ([200, 403].includes(response.statusCode)) {
				...
				logger.log({email, isValid, v1UserId: __guard__(body != null ? body.user_profile : undefined, x => x.id)},
	...
	doPasswordReset(v1_user_id, password, callback) {
		if (callback == null) { callback = function(err, created){}; }
	...
	getDocExported(token, callback) {
		...
		return V1Api.request({ url: `/api/v1/sharelatex/docs/${token}/exported_to_v2` }, function(err, response, body) {
