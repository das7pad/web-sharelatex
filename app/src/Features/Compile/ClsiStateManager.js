...
const buildState = s => crypto.createHash('sha1').update(s, 'utf8').digest('hex');

module.exports = (ClsiStateManager = {

	computeHash(project, options, callback) {
		if (callback == null) { callback = function(err, hash) {}; }
		return ProjectEntityHandler.getAllEntitiesFromProject(project, function(err, docs, files) {
			const fileList = (Array.from(files || []).map((f) => `${f.file._id}:${f.file.rev}:${f.file.created}:${f.path}`));
			const docList = (Array.from(docs || []).map((d) => `${d.doc._id}:${d.path}`));
			const sortedEntityList = [...Array.from(docList), ...Array.from(fileList)].sort();
			...
			const hash = buildState([...Array.from(sortedEntityList), ...Array.from(sortedOptionsList)].join("\n"));
