...
module.exports = (ProjectOutputFileAgent = {
	...
	createLinkedFile(project_id, linkedFileData, name, parent_folder_id, user_id, callback) {
		...
		return this._prepare(project_id, linkedFileData, user_id, (err, linkedFileData) => {
			if (err != null) { return callback(err); }
			return this._getFileStream(linkedFileData, user_id, (err, readStream) => {
				if (err != null) { return callback(err); }
	...
	refreshLinkedFile(project_id, linkedFileData, name, parent_folder_id, user_id, callback) {
		return this._prepare(project_id, linkedFileData, user_id, (err, linkedFileData) => {
			if (err != null) { return callback(err); }
			return this._compileAndGetFileStream(linkedFileData, user_id, (err, readStream, new_build_id) => {
				if (err != null) { return callback(err); }
				readStream.on("error", callback);
				return readStream.on("response", response => {
					if (response.statusCode >= 200 && response.statusCode < 300) {
	...
	_checkAuth(project_id, data, current_user_id, callback) {
		if (callback == null) { callback = function(err, allowed){}; }
		...
		return this._getSourceProject(data, function(err, project) {
			if (err != null) { return callback(err); }
			return AuthorizationManager.canUserReadProject(current_user_id,
				project._id,
				null,
	...
	_getFileStream(linkedFileData, user_id, callback) {
		if (callback == null) { callback = function(err, fileStream){}; }
		callback = _.once(callback);
		...
		return this._getSourceProject(linkedFileData, function(err, project) {
			...
			return ClsiManager.getOutputFileStream(source_project_id,
				user_id,
				build_id,
	...
	_compileAndGetFileStream(linkedFileData, user_id, callback) {
		if (callback == null) { callback = function(err, stream, build_id){}; }
		callback = _.once(callback);
		const { source_output_file_path } = linkedFileData;
		return this._getSourceProject(linkedFileData, function(err, project) {
			if (err != null) { return callback(err); }
			const source_project_id = project._id;
			return CompileManager.compile(source_project_id,
				user_id,
				{},
