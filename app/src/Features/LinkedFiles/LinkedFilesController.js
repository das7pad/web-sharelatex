...
module.exports = (LinkedFilesController = {
	...
	createLinkedFile(req, res, next) {
		...
		return Agent.createLinkedFile(project_id,
			data,
			name,
			parent_folder_id,
			user_id,
			function(err, newFileId) {
				if (err != null) { return LinkedFilesController.handleError(err, req, res, next); }
	...
	refreshLinkedFile(req, res, next) {
		...
		return LinkedFilesHandler.getFileById(project_id, file_id, function(err, file, path, parentFolder) {
			...
			if ((linkedFileData == null) || ((linkedFileData != null ? linkedFileData.provider : undefined) == null)) {
				return res.send(409);
			}
			const {
                provider
			...
			return Agent.refreshLinkedFile(project_id,
				linkedFileData,
				name,
				parent_folder_id,
				user_id,
	...
	handleError(error, req, res, next) {
		if (error instanceof BadDataError) {
			...
			return res.status(409).send("Sorry, the source project is not yet imported to Overleaf v2. Please import it to Overleaf v2 to refresh this file");
