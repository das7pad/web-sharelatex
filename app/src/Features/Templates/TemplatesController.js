...
module.exports = (TemplatesController = {
	...
	getV1Template(req, res){
		...
		if (!/^[0-9]+$/.test(templateVersionId) || !/^[0-9]+$/.test(templateId)) {
			logger.err({templateVersionId, templateId}, "invalid template id or version");
		...
		return res.render(path.resolve(__dirname, "../../../views/project/editor/new_from_template"), data);
	},

	createProjectFromV1Template(req, res, next){
		const user_id = AuthenticationController.getLoggedInUserId(req);
		return TemplatesManager.createProjectFromV1Template(req.body.brandVariationId, req.body.compiler, req.body.mainFile, req.body.templateId, req.body.templateName, req.body.templateVersionId, user_id, req.body.imageName, function(err, project) {
