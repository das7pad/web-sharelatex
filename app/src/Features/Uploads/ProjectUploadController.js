...
module.exports = (ProjectUploadController = {
	uploadProject(req, res, next) {
		...
		return ProjectUploadManager.createProjectFromZipArchive(user_id, name, path, function(error, project) {
			...
			if (error != null) {
				...
				if ((error.name != null) && (error.name === 'InvalidError')) {
					return res.status(422).json({ success: false, error: req.i18n.translate(error.message) });
				} else {
					return res.status(500).json({ success: false, error: req.i18n.translate("upload_failed") });
				}
			} else {
				logger.log(
	...
	uploadFile(req, res, next) {
		...
		if ((name == null) || (name.length === 0) || (name.length > 150)) {
			logger.err({project_id, originalName:name}, "bad name when trying to upload file");
			return res.send({success: false});
		}
		logger.log({folder_id, project_id}, "getting upload file request");
		const user_id = AuthenticationController.getLoggedInUserId(req);

		return FileSystemImportManager.addEntity(user_id, project_id, folder_id, name, path, true, function(error, entity) {
			fs.unlink(path, function() {});
			timer.done();
			if (error != null) {
				logger.error({
					err: error, project_id, file_path: path,
					file_name: name, folder_id
				},
				...
				logger.log(
					...
					"uploaded file");
				return res.send({success: true, entity_id: (entity != null ? entity._id : undefined), entity_type: (entity != null ? entity.type : undefined)});
			}
		});
	},

	multerMiddleware(req, res, next) {
		if (upload == null) { return res.status(500).json({success: false, error: req.i18n.translate("upload_failed")}); }
