...
webRouter.use(function(req, res, next) {
	...
	if (AuthenticationController.isUserLoggedIn(req)) {
		UserSessionsManager.touch(AuthenticationController.getSessionUser(req), function(err){});
	}
	return next();
});
...
webRouter.use(function(req, res, next) {
	...
	return helmet({ // note that more headers are added by default
...
privateApiRouter.get("/heapdump", (req, res) => require('heapdump').writeSnapshot('/tmp/' + Date.now() + '.web.heapsnapshot', (err, filename) => res.send(filename)));
...
const enableApiRouter = Settings.web != null ? Settings.web.enableApiRouter : undefined;
...
const enableWebRouter = Settings.web != null ? Settings.web.enableWebRouter : undefined;
