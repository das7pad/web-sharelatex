...
const EuroCountries = ["AT", "BE", "BG", "HR", "CY", "CZ",  
	...
	getCurrencyCode(ip, callback){
		return GeoIpLookup.getDetails(ip, function(err, ipDetails){
			if ((err != null) || (ipDetails == null)) {
				logger.err({err, ip}, "problem getting currencyCode for ip, defaulting to USD");
				return callback(null, "USD");
			}
			const countryCode = __guard__(ipDetails != null ? ipDetails.country_code : undefined, x => x.toUpperCase());
