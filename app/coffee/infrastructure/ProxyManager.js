...
module.exports = (ProxyManager = {
	apply(publicApiRouter) {
		return (() => {
			...
			for (var proxyUrl in settings.proxyUrls) {
				...
				result.push((function(target) {
					const method = (target.options != null ? target.options.method : undefined) || 'get';
					return publicApiRouter[method](proxyUrl, ProxyManager.createProxy(target));
				})(target));
			}
			return result;
		})();
	...
	createProxy(target) {
		return function(req, res, next) {
			...
			const options =
				{url: targetUrl};
			if (req.headers != null ? req.headers.cookie : undefined) { options.headers = { Cookie: req.headers.cookie }; }
			if ((target != null ? target.options : undefined) != null) { Object.assign(options, target.options); }
			if (['post', 'put'].includes(options.method)) { options.form = req.body; }
			const upstream = request(options);
			upstream.on("error", error => logger.error({err: error}, "error in ProxyManager"));
