	...
	collabratecUsers	: [
		{
			...
			added_at					: { type: Date, default() { return new Date(); } }
		}
	]});
...
var applyToAllFilesRecursivly = (ProjectSchema.statics.applyToAllFilesRecursivly = function(folder, fun){
	_.each(folder.fileRefs, file => fun(file));
	return _.each(folder.folders, folder => applyToAllFilesRecursivly(folder, fun));
});
