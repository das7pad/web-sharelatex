...
const ClsiCookieManager = require("./Features/Compile/ClsiCookieManager")(Settings.apis.clsi != null ? Settings.apis.clsi.backendGroupName : undefined);
...
module.exports = (Router = class Router {
	constructor(webRouter, privateApiRouter, publicApiRouter){
		...
		publicApiRouter.get('/status', (req, res) => res.send("web sharelatex is alive (web)"));
		privateApiRouter.get('/status', (req, res) => res.send("web sharelatex is alive (api)"));
		...
		webRouter.get("/status/compiler/:Project_id", AuthorizationMiddleware.ensureUserCanReadProject, function(req, res) {
			...
			// run the compile
			return CompileManager.compile(project_id, test_user_id, {}, function(error, status) {
		...
		webRouter.get('/oops-mongo', (req, res, next) => require("./models/Project").Project.findOne({}, function() {
