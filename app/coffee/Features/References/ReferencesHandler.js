...
if ((__guard__(settings.apis != null ? settings.apis.references : undefined, x => x.url) == null)) {
...
module.exports = (ReferencesHandler = {
	...
	_findBibFileIds(project) {
		...
		var _process = function(folder) {
			_.each((folder.fileRefs || []), function(file) {
				if (__guard__(file != null ? file.name : undefined, x1 => x1.match(/^.*\.bib$/))) {
	...
	_findBibDocIds(project) {
		...
		var _process = function(folder) {
			_.each((folder.docs || []), function(doc) {
				if (__guard__(doc != null ? doc.name : undefined, x1 => x1.match(/^.*\.bib$/))) {
	...
	_isFullIndex(project, callback) {
		...
		return UserGetter.getUser(project.owner_ref, { features: true }, function(err, owner) {
			if (err != null) { return callback(err); }
			const features = owner != null ? owner.features : undefined;
			return callback(null, ((features != null ? features.references : undefined) === true) || ((features != null ? features.referencesSearch : undefined) === true));
		});
	},

	indexAll(projectId, callback) {
		if (callback == null) { callback = function(err, data){}; }
		return ProjectGetter.getProject(projectId, {rootFolder: true, owner_ref: 1}, function(err, project) {
			if (err) {
			...
			return ReferencesHandler._doIndexOperation(projectId, project, docIds, fileIds, callback);
		});
	},
	...
	index(projectId, docIds, callback) {
		...
		return ProjectGetter.getProject(projectId, {rootFolder: true, owner_ref: 1}, function(err, project) {
			...
			return ReferencesHandler._doIndexOperation(projectId, project, docIds, [], callback);
		});
	},

	_doIndexOperation(projectId, project, docIds, fileIds, callback) {
		if ((__guard__(settings.apis != null ? settings.apis.references : undefined, x1 => x1.url) == null)) {
			return callback();
		}
		return ReferencesHandler._isFullIndex(project, function(err, isFullIndex) {
			...
			return Async.series(
				...
				function(err) {
					...
					const bibDocUrls = docIds.map(docId => ReferencesHandler._buildDocUrl(projectId, docId));
					const bibFileUrls = fileIds.map(fileId => ReferencesHandler._buildFileUrl(projectId, fileId));
