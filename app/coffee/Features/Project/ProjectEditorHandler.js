...
module.exports = (ProjectEditorHandler = {
	...
	buildProjectModelView(project, members, invites) {
		...
		const result = {
			...
			imageName:   (project.imageName != null) ? Path.basename(project.imageName) : undefined
		};
		...
		({owner, ownerFeatures, members} = this.buildOwnerAndMembersViews(members));
		result.owner = owner;
		...
		result.features.referencesSearch = result.features.referencesSearch || result.features.references;
		result.features.mendeley = result.features.mendeley || result.features.references;
	...
	buildOwnerAndMembersViews(members) {
		...
		for (let member of Array.from((members || []))) {
			if (member.privilegeLevel === "owner") {
				...
				filteredMembers.push(this.buildUserModelView(member.user, member.privilegeLevel));
			}
		}
		return {
			owner,
	...
	buildFolderModelView(folder) {
		...
		return {
			_id        : folder._id,
			name       : folder.name,
			folders    : ((Array.from(folder.folders || [])).map((childFolder) => this.buildFolderModelView(childFolder))),
			fileRefs   : ((() => {
				const result = [];
				for (file of Array.from(fileRefs)) { 					result.push(this.buildFileModelView(file));
				}
				return result;
			})()),
			docs       : ((Array.from(folder.docs || [])).map((doc) => this.buildDocModelView(doc)))
		};
	},
