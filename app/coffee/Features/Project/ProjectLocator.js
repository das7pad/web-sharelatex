...
module.exports = (ProjectLocator = {
	findElement(options, _callback){
		if (_callback == null) { _callback = function(err, element, path, parentFolder){}; }
		...
		if (project != null) {
			...
			return ProjectGetter.getProject(project_id, {rootFolder:true, rootDoc_id:true}, function(err, project){
	...
	findRootDoc(opts, callback){
		const getRootDoc = project=> {
			if (project.rootDoc_id != null) {
				return this.findElement({project, element_id:project.rootDoc_id, type:"docs"}, function(error, ...args) {
					if (error != null) {
	...
	findElementByPath(options, callback){
		if (callback == null) { callback = function(err, foundEntity, type){}; }
		...
		if (project != null) {
			return ProjectLocator._findElementByPathWithProject(project, path, exactCaseMatch, callback);
		} else {
			return ProjectGetter.getProject(project_id, {rootFolder:true, rootDoc_id:true}, function(err, project){
				if (err != null) { return callback(err); }
				return ProjectLocator._findElementByPathWithProject(project, path, exactCaseMatch, callback);
	...
	_findElementByPathWithProject(project, needlePath, exactCaseMatch, callback){
		...
		if (exactCaseMatch) {
			...
			matchFn = (a, b) => (a != null ? a.toLowerCase() : undefined) === (b != null ? b.toLowerCase() : undefined);
		}
		...
		var getParentFolder = function(haystackFolder, foldersList, level, cb){
			...
			if (!found) {
				return cb(`not found project: ${project._id} search path: ${needlePath}, folder ${foldersList[level]} could not be found`);
			}
		};
		...
		const getEntity = function(folder, entityName, cb){
			...
			for (let childFolder of Array.from(folder.folders || [])) {
				if (matchFn(childFolder != null ? childFolder.name : undefined, entityName)) {
					result = childFolder;
					type = "folder";
				}
			}
			...
			if (result != null) {
				...
				return cb(`not found project: ${project._id} search path: ${needlePath}, entity ${entityName} could not be found`);
			}
		};


		if (typeof err !== 'undefined' && err !== null) {
			logger.err({err, project_id:project._id}, "error getting project for finding element");
			return callback(err);
		}
		if ((project == null)) {
		...
		logger.log({project_id:project._id, path:needlePath, foldersList}, "looking for element by path");
		const jobs = new Array();
		jobs.push(
			cb => getParentFolder(rootFolder, foldersList, 0, cb));
		jobs.push(
			(folder, cb) => getEntity(folder, needleName, cb));
		return async.waterfall(jobs, callback);
	},

	findUsersProjectByName(user_id, projectName, callback){
		return ProjectGetter.findAllUsersProjects(user_id, 'name archived', function(err, allProjects){
			if (typeof error !== 'undefined' && error !== null) { return callback(error); }
			const {owned, readAndWrite} = allProjects;
			const projects = owned.concat(readAndWrite);
			projectName = projectName.toLowerCase();
			const project = _.find(projects, project => (project.name.toLowerCase() === projectName) && (project.archived !== true));
			logger.log({user_id, projectName, totalProjects:projects.length, project}, "looking for project by name");
...
var getIndexOf = function(searchEntity, id){
	...
	let count = 0;
	while(count < length) {
		if (((searchEntity[count] != null ? searchEntity[count]._id : undefined)+"") === (id+"")) {
			return count;
		}
		count++;
	}
