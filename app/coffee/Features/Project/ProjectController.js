...
module.exports = (ProjectController = {
	...
	updateProjectSettings(req, res, next) {
		...
		if (req.body.compiler != null) {
			jobs.push(callback => editorController.setCompiler(project_id, req.body.compiler, callback));
		}

		if (req.body.imageName != null) {
			jobs.push(callback => editorController.setImageName(project_id, req.body.imageName, callback));
		}

		if (req.body.name != null) {
			jobs.push(callback => editorController.renameProject(project_id, req.body.name, callback));
		}

		if (req.body.spellCheckLanguage != null) {
			jobs.push(callback => editorController.setSpellCheckLanguage(project_id, req.body.spellCheckLanguage, callback));
		}

		if (req.body.rootDocId != null) {
			jobs.push(callback => editorController.setRootDoc(project_id, req.body.rootDocId, callback));
		}
	...
	updateProjectAdminSettings(req, res, next) {
		...
		if (req.body.publicAccessLevel != null) {
			jobs.push(callback => editorController.setPublicAccessLevel(project_id, req.body.publicAccessLevel, callback));
		}
	...
	deleteProject(req, res) {
		...
		if (forever) {
			return projectDeleter.deleteProject(project_id, {deleterUser: user, ipAddress:req.ip} , cb);
		} else {
	...
	cloneProject(req, res, next){
		...
		return projectDuplicator.duplicate(currentUser, project_id, projectName, function(err, project){
			if (err != null) {
				logger.error({err, project_id, user_id: currentUser._id}, "error cloning project");
				return next(err);
			}
			return res.send({name:project.name, project_id:project._id, owner_ref:project.owner_ref});
		});
	},
	...
	newProject(req, res, next){
		...
		const projectName = req.body.projectName != null ? req.body.projectName.trim() : undefined;
		const {
            template
        } = req.body;
		logger.log({user: user_id, projectType: template, projectName}, "creating project");
		return async.waterfall([
			function(cb){
				if (template === 'example') {
					return projectCreationHandler.createExampleProject(user_id, projectName, cb);
				} else {
					return projectCreationHandler.createBasicProject(user_id, projectName, cb);
				}
			}
		], function(err, project){
			if (err != null) { return next(err); }
			logger.log({project, user: user_id, projectName, templateType: template}, "created project");
	...
	userProjectsJson(req, res, next) {
		...
		return ProjectGetter.findAllUsersProjects(user_id,
			'name lastUpdated publicAccesLevel archived owner_ref tokens', function(err, projects) {
				if (err != null) { return next(err); }
	...
	projectEntitiesJson(req, res, next) {
		...
		return ProjectGetter.getProject(project_id, function(err, project) {
			if (err != null) { return next(err); }
			return ProjectEntityHandler.getAllEntitiesFromProject(project, function(err, docs, files) {
				if (err != null) { return next(err); }
				const entities = docs.concat(files)
	...
	projectListPage(req, res, next){
		...
		return async.parallel({
			...
			projects(cb){
				return ProjectGetter.findAllUsersProjects(user_id, 'name lastUpdated lastUpdatedBy publicAccesLevel archived owner_ref tokens', cb);
			},
			v1Projects(cb) {
				return Modules.hooks.fire("findAllV1Projects", user_id, function(error, projects) {
			...
			hasSubscription(cb){
				return LimitationsManager.hasPaidSubscription(currentUser, function(error, hasPaidSubscription) {
					if ((error != null) && error instanceof V1ConnectionError) {
						return cb(null, true);
					}
			...
			user(cb) {
				return User.findById(user_id, "featureSwitches overleaf awareOfV2 features", cb);
			},
			userAffiliations(cb) {
				...
				const v1Tags = (results.v1Projects != null ? results.v1Projects.tags : undefined) || [];
				const tags = results.tags[0].concat(v1Tags);
				const notifications = require("underscore").map(results.notifications, function(notification){
					notification.html = req.i18n.translate(notification.templateKey, notification.messageOpts);
					return notification;
				});
				const portalTemplates = ProjectController._buildPortalTemplatesList(results.userAffiliations);
				const projects = ProjectController._buildProjectList(results.projects, results.v1Projects != null ? results.v1Projects.projects : undefined);
				const {
                    user
                } = results;
				const {
				...
				const warnings = ProjectController._buildWarningsList(results.v1Projects);

				// in v2 add notifications for matching university IPs
				if (Settings.overleaf != null) {
					UserGetter.getUser(user_id, { 'lastLoginIp': 1 }, function(error, user) {
						if (req.ip !== user.lastLoginIp) {
							return NotificationsBuilder.ipMatcherAffiliation(user._id, req.ip).create();
						}
					});
				}

				return ProjectController._injectProjectUsers(projects, function(error, projects) {
					if (error != null) { return next(error); }
					const viewModel = {
						title:'your_projects',
						priority_title: true,
					...
					if ((__guard__(Settings != null ? Settings.algolia : undefined, x => x.app_id) != null) && (__guard__(Settings != null ? Settings.algolia : undefined, x1 => x1.read_only_api_key) != null)) {
					...
					const paidUser = (user.features != null ? user.features.github : undefined) && (user.features != null ? user.features.dropbox : undefined); // use a heuristic for paid account
					const freeUserProportion = 0.10;
					const sampleFreeUser = parseInt(user._id.toString().slice(-2), 16) < (freeUserProportion * 255);
					...
					if (showFrontWidget) {
						viewModel.frontChatWidgetRoomId = Settings.overleaf != null ? Settings.overleaf.front_chat_widget_room_id : undefined;
					}
	...
	loadEditor(req, res, next){
		...
		return async.auto({
			project(cb){
				return ProjectGetter.getProject(
					...
					function(err, project) {
						...
						if (((project.overleaf != null ? project.overleaf.id : undefined) == null) || ((project.tokens != null ? project.tokens.readAndWrite : undefined) == null) || (Settings.projectImportingCheckMaxCreateDelta == null)) { return cb(null, project); }
						const createDelta = (new Date().getTime() - new Date(project._id.getTimestamp()).getTime()) / 1000;
						if (!(createDelta < Settings.projectImportingCheckMaxCreateDelta)) { return cb(null, project); }
						return V1Handler.getDocExported(project.tokens.readAndWrite, function(err, doc_exported) {
							if (err != null) { return next(err); }
			...
			brandVariation: [ "project", function(cb, results) {
				if (((results.project != null ? results.project.brandVariationId : undefined) == null)) {
					return cb();
				}
				return BrandVariationsHandler.getBrandVariationById(results.project.brandVariationId, (error, brandVariationDetails) => cb(error, brandVariationDetails));
			}
			]
		}, function(err, results){
			if (err != null) {
			...
			const daysSinceLastUpdated =  (new Date() - project.lastUpdated) / 86400000;
			logger.log({project_id, daysSinceLastUpdated}, "got db results for loading editor");

			const token = TokenAccessHandler.getRequestToken(req, project_id);
			const {
                isTokenMember
            } = results;
			return AuthorizationManager.getPrivilegeLevelForProject(user_id, project_id, token, function(error, privilegeLevel){
				...
				if ((subscription != null) && (subscription.freeTrial != null) && (subscription.freeTrial.expiresAt != null)) {
				...
				res.render('project/editor', {
					...
					useV2History: !!__guard__(project.overleaf != null ? project.overleaf.history : undefined, x => x.display),
					richTextTrackChangesEnabled: ((req.query != null ? req.query.rttc : undefined) === 'true') || user.betaProgram,
					showTestControls: ((req.query != null ? req.query.tc : undefined) === 'true') || user.isAdmin,
					brandVariation,
	...
	_buildProjectList(allProjects, v1Projects){
		let project;
		if (v1Projects == null) { v1Projects = []; }
		const {owned, readAndWrite, readOnly, tokenReadAndWrite, tokenReadOnly} = allProjects;
		const projects = [];
	...
	_buildV1ProjectViewModel(project) {
		...
		if (((project.owner != null) && project.owner.user_is_owner) || ((project.creator != null) && project.creator.user_is_creator)) {
	...
	_injectProjectUsers(projects, callback) {
		let project;
		if (callback == null) { callback = function(error, projects) {}; }
		const users = {};
		...
		for (let user_id in users) {
			...
			((user_id => jobs.push(callback => UserGetter.getUserOrUserStubById(user_id, { first_name: 1, last_name: 1, email: 1 }, function(error, user) {
	...
	_buildWarningsList(v1ProjectData) {
		if (v1ProjectData == null) { v1ProjectData = {}; }
		const warnings = [];
...
(generateThemeList = function() {
	const files = fs.readdirSync(__dirname + '/../../../../public/js/' + PackageVersions.lib('ace'));
	return (() => {
		const result = [];
