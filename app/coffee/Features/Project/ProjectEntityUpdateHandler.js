...
const wrapWithLock = function(methodWithoutLock) {
	...
	if (typeof methodWithoutLock === 'function') {
		methodWithLock = function(project_id, ...rest) {
			...
			return LockManager.runWithLock(LOCK_NAMESPACE, project_id,
		...
		methodWithLock = wrapWithSetup(function(project_id, ...rest) {
			const adjustedLength = Math.max(rest.length, 1), args = rest.slice(0, adjustedLength - 1), callback = rest[adjustedLength - 1];
			return LockManager.runWithLock(LOCK_NAMESPACE, project_id, cb => mainTask(project_id, ...Array.from(args), cb),
			callback);
		});
...
module.exports = (ProjectEntityUpdateHandler = (self = {
	copyFileFromExistingProjectWithProject: wrapWithLock({
		beforeLock(next) {
			return function(project_id, project, folder_id, originalProject_id, origonalFileRef, userId, callback){
				if (callback == null) { callback = function(error, fileRef, folder_id) {}; }
				logger.log({ project_id, folder_id, originalProject_id, origonalFileRef }, "copying file in s3 with project");
				return ProjectEntityMongoUpdateHandler._confirmFolder(project, folder_id, function(folder_id) {
					...
					const fileProperties = {name : SafePath.clean(origonalFileRef.name)};
					...
					return FileStoreHandler.copyFile(originalProject_id, origonalFileRef._id, project._id, fileRef._id, function(err, fileStoreUrl){
						...
						return next(project_id, project, folder_id, originalProject_id, origonalFileRef, userId, fileRef, fileStoreUrl, callback);
					});
				});
			};
		},
		withLock(project_id, project, folder_id, originalProject_id, origonalFileRef, userId, fileRef, fileStoreUrl, callback){
			if (callback == null) { callback = function(error, fileRef, folder_id) {}; }
			const projectHistoryId = __guard__(project.overleaf != null ? project.overleaf.history : undefined, x => x.id);
			return ProjectEntityMongoUpdateHandler._putElement(project, folder_id, fileRef, "file", function(err, result, newProject) {
				if (err != null) {
					logger.err({ err, project_id, folder_id }, "error putting element as part of copy");
					return callback(err);
				}
				return TpdsUpdateSender.addFile({ project_id, file_id:fileRef._id, path:__guard__(result != null ? result.path : undefined, x1 => x1.fileSystem), rev:fileRef.rev, project_name:project.name}, function(err) {
					if (err != null) {
						logger.err({ err, project_id, folder_id, originalProject_id, origonalFileRef }, "error sending file to tpds worker");
					}
					const newFiles = [{
						file: fileRef,
						path: __guard__(result != null ? result.path : undefined, x2 => x2.fileSystem),
						url: fileStoreUrl
					}
					];
					return DocumentUpdaterHandler.updateProjectStructure(project_id, projectHistoryId, userId, {newFiles, newProject}, function(error) {
	...
	updateDocLines(project_id, doc_id, lines, version, ranges, lastUpdatedAt, lastUpdatedBy, callback){
		if (callback == null) { callback = function(error) {}; }
		return ProjectGetter.getProjectWithoutDocLines(project_id, function(err, project){
			if (err != null) { return callback(err); }
			if ((project == null)) { return callback(new Errors.NotFoundError("project not found")); }
			logger.log({project_id, doc_id}, "updating doc lines");
			return ProjectLocator.findElement({project, element_id:doc_id, type:"docs"}, function(err, doc, path){
				...
				if (err != null) {
					if (err instanceof Errors.NotFoundError) {
						...
						// the doc is deleted.
						isDeletedDoc = true;
						doc = _.find(project.deletedDocs, doc => doc._id.toString() === doc_id.toString());
					} else {
						return callback(err);
					}
				}
				...
				return DocstoreManager.updateDoc(project_id, doc_id, lines, version, ranges, function(err, modified, rev) {
					...
					if (modified && !isDeletedDoc) {
						...
						return TpdsUpdateSender.addDoc({project_id, path:path.fileSystem, doc_id, project_name:project.name, rev}, callback);
					} else {
						return callback();
					}
				});
	...
	setRootDoc(project_id, newRootDocID, callback){
		...
		return Project.update({_id:project_id}, {rootDoc_id:newRootDocID}, {}, callback);
	},

	unsetRootDoc(project_id, callback) {
		if (callback == null) { callback = function(error) {}; }
		logger.log({project_id}, "removing root doc");
		return Project.update({_id:project_id}, {$unset: {rootDoc_id: true}}, {}, callback);
	},

	_addDocAndSendToTpds(project_id, folder_id, doc, callback){
		if (callback == null) { callback = function(error, result, project) {}; }
		return ProjectEntityMongoUpdateHandler.addDoc(project_id, folder_id, doc, function(err, result, project) {
			if (err != null) {
				logger.err({err, project_id, folder_id, doc_name: (doc != null ? doc.name : undefined), doc_id:(doc != null ? doc._id : undefined)}, "error adding file with project");
				return callback(err);
			}
			return TpdsUpdateSender.addDoc({
				project_id,
				doc_id:       (doc != null ? doc._id : undefined),
				path:         __guard__(result != null ? result.path : undefined, x => x.fileSystem),
	...
	addDocWithRanges: wrapWithLock({
		beforeLock(next) {
			return function(project_id, folder_id, docName, docLines, ranges, userId, callback) {
				if (callback == null) { callback = function(error, doc, folder_id) {}; }
				...
				return DocstoreManager.updateDoc(project_id.toString(), doc._id.toString(), docLines, 0, ranges, function(err, modified, rev) {
					if (err != null) { return callback(err); }
					return next(project_id, folder_id, doc, docName, docLines, ranges, userId, callback);
				});
			};
		},
		withLock(project_id, folder_id, doc, docName, docLines, ranges, userId, callback) {
			if (callback == null) { callback = function(error, doc, folder_id) {}; }
			return ProjectEntityUpdateHandler._addDocAndSendToTpds(project_id, folder_id, doc, function(err, result, project) {
				if (err != null) { return callback(err); }
				const docPath = __guard__(result != null ? result.path : undefined, x => x.fileSystem);
				const projectHistoryId = __guard__(project.overleaf != null ? project.overleaf.history : undefined, x1 => x1.id);
				const newDocs = [{
					doc,
					path: docPath,
				...
				return DocumentUpdaterHandler.updateProjectStructure(project_id, projectHistoryId, userId, {newDocs, newProject: project}, function(error) {
					if (error != null) { return callback(error); }
	...
	_uploadFile(project_id, folder_id, fileName, fsPath, linkedFileData, callback){
		if (callback == null) { callback = function(error, fileStoreUrl, fileRef) {}; }
		...
		return FileStoreHandler.uploadFileFromDisk(project_id, fileArgs, fsPath, function(err, fileStoreUrl, fileRef){
			if (err != null) {
				logger.err({err, project_id, folder_id, file_name: fileName, fileRef}, "error uploading image to s3");
				return callback(err);
			}
	...
	_addFileAndSendToTpds(project_id, folder_id, fileRef, callback){
		...
		return ProjectEntityMongoUpdateHandler.addFile(project_id, folder_id, fileRef, function(err, result, project) {
			if (err != null) {
				logger.err({err, project_id, folder_id, file_name: fileRef.name, fileRef}, "error adding file with project");
				return callback(err);
			}
			return TpdsUpdateSender.addFile({project_id, file_id:fileRef._id, path:__guard__(result != null ? result.path : undefined, x => x.fileSystem), project_name:project.name, rev:fileRef.rev}, function(err) {
				if (err != null) { return callback(err); }
				return callback(null, result, project);
			});
		});
	...
	addFile: wrapWithLock({
		beforeLock(next) {
			return function(project_id, folder_id, fileName, fsPath, linkedFileData, userId, callback) {
				if (!SafePath.isCleanFilename(fileName)) {
					return callback(new Errors.InvalidNameError("invalid element name"));
				}
				return ProjectEntityUpdateHandler._uploadFile(project_id, folder_id, fileName, fsPath, linkedFileData, function(error, fileStoreUrl, fileRef) {
					if (error != null) { return callback(error); }
					return next(project_id, folder_id, fileName, fsPath, linkedFileData, userId, fileRef, fileStoreUrl, callback);
				});
			};
		},
		withLock(project_id, folder_id, fileName, fsPath, linkedFileData, userId, fileRef, fileStoreUrl, callback){
			if (callback == null) { callback = function(error, fileRef, folder_id) {}; }
			return ProjectEntityUpdateHandler._addFileAndSendToTpds(project_id, folder_id, fileRef, function(err, result, project) {
				if (err != null) { return callback(err); }
				const projectHistoryId = __guard__(project.overleaf != null ? project.overleaf.history : undefined, x => x.id);
				const newFiles = [{
					file: fileRef,
					path: __guard__(result != null ? result.path : undefined, x1 => x1.fileSystem),
					url: fileStoreUrl
				}
				];
				return DocumentUpdaterHandler.updateProjectStructure(project_id, projectHistoryId, userId, {newFiles, newProject: project}, function(error) {
					if (error != null) { return callback(error); }
					return callback(null, fileRef, folder_id);
				});
			});
	...
	replaceFile: wrapWithLock({
		beforeLock(next) {
			return function(project_id, file_id, fsPath, linkedFileData, userId, callback){
				// create a new file
				const fileArgs = {
				...
				return FileStoreHandler.uploadFileFromDisk(project_id, fileArgs, fsPath, function(err, fileStoreUrl, fileRef){
					if (err != null) { return callback(err); }
					return next(project_id, file_id, fsPath, linkedFileData, userId, fileRef, fileStoreUrl, callback);
				});
			};
		},
		withLock(project_id, file_id, fsPath, linkedFileData, userId, newFileRef, fileStoreUrl, callback){
			return ProjectEntityMongoUpdateHandler.replaceFileWithNew(project_id, file_id, newFileRef, function(err, oldFileRef, project, path, newProject) {
				if (err != null) { return callback(err); }
				const oldFiles = [{
					file: oldFileRef,
				...
				return TpdsUpdateSender.addFile({project_id:project._id, file_id:newFileRef._id, path:path.fileSystem, rev:oldFileRef.rev + 1, project_name:project.name}, function(err) {
					if (err != null) { return callback(err); }
					return DocumentUpdaterHandler.updateProjectStructure(project_id, projectHistoryId, userId, {oldFiles, newFiles, newProject}, callback);
				});
			});
		}
	}),
	...
	upsertDoc: wrapWithLock(function(project_id, folder_id, docName, docLines, source, userId, callback){
		if (callback == null) { callback = function(err, doc, folder_id, isNewDoc){}; }
		if (!SafePath.isCleanFilename(docName)) {
			return callback(new Errors.InvalidNameError("invalid element name"));
		}
		return ProjectLocator.findElement({project_id, element_id: folder_id, type: "folder"}, function(error, folder) {
			if (error != null) { return callback(error); }
			if ((folder == null)) { return callback(new Error("Couldn't find folder")); }
			...
			if (existingDoc != null) {
				return DocumentUpdaterHandler.setDocument(project_id, existingDoc._id, userId, docLines, source, err=> {
					logger.log({project_id, doc_id:existingDoc._id}, "notifying users that the document has been updated");
				...
				return self.addDocWithRanges.withoutLock(project_id, folder_id, docName, docLines, {}, userId, function(err, doc) {
					if (err != null) { return callback(err); }
	...
	upsertFile: wrapWithLock({
		...
		withLock(project_id, folder_id, fileName, fsPath, linkedFileData, userId, newFileRef, fileStoreUrl, callback){
			if (callback == null) { callback = function(err, file, isNewFile, existingFile){}; }
			return ProjectLocator.findElement({project_id, element_id: folder_id, type: "folder"}, function(error, folder) {
				if (error != null) { return callback(error); }
				if ((folder == null)) { return callback(new Error("Couldn't find folder")); }
				...
				if (existingFile != null) {
					...
					return self.replaceFile.mainTask(project_id, existingFile._id, fsPath, linkedFileData, userId, newFileRef, fileStoreUrl, function(err) {
						if (err != null) { return callback(err); }
						return callback(null, newFileRef, (existingFile == null), existingFile);
					});
				} else {
					// this calls directly into the addFile main task (without the beforeLock part)
					return self.addFile.mainTask(project_id, folder_id, fileName, fsPath, linkedFileData, userId, newFileRef, fileStoreUrl, function(err) {
						if (err != null) { return callback(err); }
						return callback(null, newFileRef, (existingFile == null), existingFile);
					});
				}
	...
	upsertFileWithPath: wrapWithLock({
		beforeLock(next) {
			return function(project_id, elementPath, fsPath, linkedFileData, userId, callback){
				if (!SafePath.isCleanPath(elementPath)) {
					return callback(new Errors.InvalidNameError("invalid element name"));
				}
				...
				return FileStoreHandler.uploadFileFromDisk(project_id, fileArgs, fsPath, function(err, fileStoreUrl, fileRef){
					if (err != null) { return callback(err); }
					return next(project_id, folderPath, fileName, fsPath, linkedFileData, userId, fileRef, fileStoreUrl, callback);
				});
			};
		},
		withLock(project_id, folderPath, fileName, fsPath, linkedFileData, userId, fileRef, fileStoreUrl, callback) {
			return self.mkdirp.withoutLock(project_id, folderPath, function(err, newFolders, folder) {
				...
				return self.upsertFile.mainTask(project_id, folder._id, fileName, fsPath, linkedFileData, userId, fileRef, fileStoreUrl, function(err, newFile, isNewFile, existingFile) {
					if (err != null) { return callback(err); }
					return callback(null, newFile, isNewFile, existingFile, newFolders, folder);
				});
			});
		}
	}),
	...
	deleteEntity: wrapWithLock(function(project_id, entity_id, entityType, userId, callback){
		...
		return ProjectEntityMongoUpdateHandler.deleteEntity(project_id, entity_id, entityType, function(error, entity, path, projectBeforeDeletion, newProject) {
			if (error != null) { return callback(error); }
			return self._cleanUpEntity(projectBeforeDeletion, newProject, entity, entityType, path.fileSystem, userId, function(error) {
				if (error != null) { return callback(error); }
				return TpdsUpdateSender.deleteEntity({project_id, path:path.fileSystem, project_name:projectBeforeDeletion.name}, function(error) {
					if (error != null) { return callback(error); }
	...
	deleteEntityWithPath: wrapWithLock((project_id, path, userId, callback) => ProjectLocator.findElementByPath({project_id, path}, function(err, element, type){
        if (err != null) { return callback(err); }
        if ((element == null)) { return callback(new Errors.NotFoundError("project not found")); }
	...
	mkdirp: wrapWithLock(function(project_id, path, callback){
		if (callback == null) { callback = function(err, newlyCreatedFolders, lastFolderInPath){}; }
		for (let folder of Array.from(path.split('/'))) {
			if ((folder.length > 0) && !SafePath.isCleanFilename(folder)) {
				return callback(new Errors.InvalidNameError("invalid element name"));
			}
		}
		return ProjectEntityMongoUpdateHandler.mkdirp(project_id, path, {exactCaseMatch: false}, callback);
	}),
	...
	mkdirpWithExactCase: wrapWithLock(function(project_id, path, callback){
		...
		return ProjectEntityMongoUpdateHandler.mkdirp(project_id, path, {exactCaseMatch: true}, callback);
	}),
	...
	moveEntity: wrapWithLock(function(project_id, entity_id, destFolderId, entityType, userId, callback){
		...
		entityType = entityType.toLowerCase();
		return ProjectEntityMongoUpdateHandler.moveEntity(project_id, entity_id, destFolderId, entityType, function(err, project, startPath, endPath, rev, changes) {
			if (err != null) { return callback(err); }
			const projectHistoryId = __guard__(project.overleaf != null ? project.overleaf.history : undefined, x => x.id);
			TpdsUpdateSender.moveEntity({ project_id, project_name: project.name, startPath, endPath, rev });
			return DocumentUpdaterHandler.updateProjectStructure(project_id, projectHistoryId, userId, changes, callback);
		});
	}),
	...
	renameEntity: wrapWithLock(function(project_id, entity_id, entityType, newName, userId, callback){
		...
		return ProjectEntityMongoUpdateHandler.renameEntity(project_id, entity_id, entityType, newName, function(err, project, startPath, endPath, rev, changes) {
			if (err != null) { return callback(err); }
			const projectHistoryId = __guard__(project.overleaf != null ? project.overleaf.history : undefined, x => x.id);
			TpdsUpdateSender.moveEntity({ project_id, project_name: project.name, startPath, endPath, rev });
			return DocumentUpdaterHandler.updateProjectStructure(project_id, projectHistoryId, userId, changes, callback);
	...
	resyncProjectHistory: wrapWithLock((project_id, callback) => ProjectGetter.getProject(project_id, {rootFolder: true, overleaf: true}, function(error, project) {
        if (error != null) { return callback(error); }

        const projectHistoryId = __guard__(__guard__(project != null ? project.overleaf : undefined, x1 => x1.history), x => x.id);
        if ((projectHistoryId == null)) {
            error = new Errors.ProjectHistoryDisabledError(`project history not enabled for ${project_id}`);
            return callback(error);
        }

        return ProjectEntityHandler.getAllEntitiesFromProject(project, function(error, docs, files) {
            if (error != null) { return callback(error); }
            ...
            return DocumentUpdaterHandler.resyncProjectHistory(project_id, projectHistoryId, docs, files, callback);
        });
    })),

	_cleanUpEntity(project, newProject, entity, entityType, path, userId, callback) {
		if (callback == null) { callback = function(error) {}; }
		return self._updateProjectStructureWithDeletedEntity(project, newProject, entity, entityType, path, userId, function(error) {
			if (error != null) { return callback(error); }
	...
	_updateProjectStructureWithDeletedEntity(project, newProject, entity, entityType, entityPath, userId, callback) {
	...
	_cleanUpDoc(project, doc, path, userId, callback) {
		...
		return unsetRootDocIfRequired(function(error) {
			if (error != null) { return callback(error); }
			return ProjectEntityMongoUpdateHandler._insertDeletedDocReference(project._id, doc, function(error) {
				if (error != null) { return callback(error); }
	...
	_cleanUpFile(project, file, path, userId, callback) {
		if (callback == null) { callback = function(error) {}; }
		return ProjectEntityMongoUpdateHandler._insertDeletedFileReference(project._id, file, callback);
	},
	...
	_cleanUpFolder(project, folder, folderPath, userId, callback) {
		...
		for (let doc of Array.from(folder.docs)) {
			(function(doc) {
				...
				return jobs.push(callback => self._cleanUpDoc(project, doc, docPath, userId, callback));
			})(doc);
		}
		...
		for (let file of Array.from(folder.fileRefs)) {
			(function(file) {
				...
				return jobs.push(callback => self._cleanUpFile(project, file, filePath, userId, callback));
			})(file);
		}
		...
		for (let childFolder of Array.from(folder.folders)) {
			(function(childFolder) {
				...
				return jobs.push(callback => self._cleanUpFolder(project, childFolder, folderPath, userId, callback));
			})(childFolder);
		}
