...
module.exports = (ProjectDetailsHandler = {
	getDetails(project_id, callback){
		return ProjectGetter.getProject(project_id, {name:true, description:true, compiler:true, features:true, owner_ref:true, overleaf:true}, function(err, project){
			if (err != null) {
			...
			return UserGetter.getUser(project.owner_ref, function(err, user) {
				...
				const details = {
					...
					features: (user != null ? user.features : undefined) || settings.defaultFeatures
				};
	...
	getProjectDescription(project_id, callback){
		return ProjectGetter.getProject(project_id, {description: true}, (err, project) => callback(err, project != null ? project.description : undefined));
	},

	setProjectDescription(project_id, description, callback){
		const conditions = {_id:project_id};
		const update = {description};
		logger.log({conditions, update, project_id, description}, "setting project description");
	...
	transferOwnership(project_id, user_id, suffix, callback){
		...
		return ProjectGetter.getProject(project_id, {owner_ref: true, name: true}, function(err, project){
			if (err != null) { return callback(err); }
			if (project == null) { return callback(new Errors.NotFoundError("project not found")); }
			if (project.owner_ref === user_id) { return callback(); }

			return UserGetter.getUser(user_id, function(err, user) {
				if (err != null) { return callback(err); }
				if (user == null) { return callback(new Errors.NotFoundError("user not found")); }
				...
				return CollaboratorsHandler.removeUserFromProject(project_id, user_id, function(err) {
					if (err != null) { return callback(err); }
					return ProjectDetailsHandler.generateUniqueName(user_id, project.name + suffix, function(err, name) {
						if (err != null) { return callback(err); }
						return Project.update({_id: project_id},
							{
								$set: {
									owner_ref: user_id,
									name
								...
								if (err != null) { return callback(err); }
								return ProjectEntityHandler.flushProjectToThirdPartyDataStore(project_id, callback);
						});
					});
				});
			});
	...
	renameProject(project_id, newName, callback){
		...
		return ProjectDetailsHandler.validateProjectName(newName, function(error) {
			...
			return ProjectGetter.getProject(project_id, {name:true}, function(err, project){
				if ((err != null) || (project == null)) {
					logger.err({err,  project_id}, "error getting project or could not find it todo project rename");
					return callback(err);
				}
				const oldProjectName = project.name;
				return Project.update({_id:project_id}, {name: newName}, (err, project)=> {
					if (err != null) {
						return callback(err);
					}
					return tpdsUpdateSender.moveEntity({project_id, project_name:oldProjectName, newProjectName:newName}, callback);
				});
			});
		});
	},
	...
	generateUniqueName(user_id, name, suffixes, callback ) {
		...
		if (callback == null) { callback = function(error, newName) {}; }
		if ((arguments.length === 3) && (typeof suffixes === 'function')) { // make suffixes an optional argument
			callback = suffixes;
			suffixes = [];
		}
		return ProjectDetailsHandler.ensureProjectNameIsUnique(user_id, name, suffixes, callback);
	},
	...
	ensureProjectNameIsUnique(user_id, name, suffixes, callback) {
		if (suffixes == null) { suffixes = []; }
		if (callback == null) { callback = function(error, name, changed){}; }
		return ProjectGetter.findAllUsersProjects(user_id, {name: 1}, function(error, allUsersProjectNames) {
			if (error != null) { return callback(error); }
			...
			const projectNameList = _.pluck(_.flatten(_.values(allUsersProjectNames)),'name');
			return ProjectHelper.ensureNameIsUnique(projectNameList, name, suffixes, ProjectDetailsHandler.MAX_PROJECT_NAME_LENGTH, callback);
		});
	},
	...
	setPublicAccessLevel(project_id, newAccessLevel, callback){
		...
		if ((project_id != null) && (newAccessLevel != null) && _.include([
			PublicAccessLevels.READ_ONLY,
			PublicAccessLevels.READ_AND_WRITE,
			PublicAccessLevels.PRIVATE,
			PublicAccessLevels.TOKEN_BASED
		], newAccessLevel)) {
			return Project.update({_id:project_id},{publicAccesLevel:newAccessLevel}, err => callback(err));
		}
	},

	ensureTokensArePresent(project_id, callback) {
		if (callback == null) { callback = function(err, tokens){}; }
		return ProjectGetter.getProject(project_id, {tokens: 1}, function(err, project) {
			if (err != null) { return callback(err); }
			if ((project.tokens != null) && (project.tokens.readOnly != null) && (project.tokens.readAndWrite != null)) {
				...
				logger.log({
					...
					has_readOnly: (__guard__(project != null ? project.tokens : undefined, x => x.readOnly) != null),
					has_readAndWrite: (__guard__(project != null ? project.tokens : undefined, x1 => x1.readAndWrite) != null)
				}, "generating tokens for project");
				return ProjectDetailsHandler._generateTokens(project, function(err) {
					if (err != null) { return callback(err); }
					return Project.update({_id: project_id}, {$set: {tokens: project.tokens}}, function(err) {
						if (err != null) { return callback(err); }
						return callback(null, project.tokens);
					});
				});
	...
	_generateTokens(project, callback) {
		...
		if (!project.tokens) { project.tokens = {}; }
		const {
            tokens
        } = project;
		...
		if ((tokens.readOnly == null)) {
			return ProjectTokenGenerator.generateUniqueReadOnlyToken(function(err, token) {
				if (err != null) { return callback(err); }
