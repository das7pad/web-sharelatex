...
module.exports = (ProjectDeleter = {
	...
	markAsDeletedByExternalSource(project_id, callback){
		...
		logger.log({project_id}, "marking project as deleted by external data source");
		const conditions = {_id:project_id};
		const update = {deletedByExternalDataSource:true};

		return Project.update(conditions, update, {}, err => require('../Editor/EditorController').notifyUsersProjectHasBeenDeletedOrRenamed(project_id, () => callback()));
	},
		...
		return Project.find({owner_ref: user_id}, function(error, projects) {
			...
			return async.each(
				...
				function(err) {
					...
					return CollaboratorsHandler.removeUserFromAllProjets(user_id, callback);
			});
		});
	},
	...
	deleteProject(project_id, options, callback) {
		...
		return async.waterfall([
			cb => Project.findOne({_id: project_id}, (err, project) => cb(err, project)),
			function(project, cb) {
				const deletedProject = new DeletedProject();
				deletedProject.project = project;
				deletedProject.deleterData = {
					deletedAt: new Date(),
					deleterId: (options.deleterUser != null ? options.deleterUser._id : undefined),
			...
			(deletedProject, cb) => documentUpdaterHandler.flushProjectToMongoAndDelete(project_id, err => cb(err, deletedProject)),
			function(deletedProject, cb) {
				CollaboratorsHandler.getMemberIds(project_id, function(error, member_ids) {
					if (member_ids == null) { member_ids = []; }
			...
			logger.log({project_id}, "successfully deleting project from user request");
	...
	archiveProject(project_id, callback){
		...
		return Project.update({_id:project_id}, { $set: { archived: true }}, function(err){
			if (err != null) {
				logger.err({err}, "problem archived project");
				return callback(err);
			}
			logger.log({project_id}, "successfully archived project from user request");
			return callback();
		});
	},
	...
	restoreProject(project_id, callback) {
		if (callback == null) { callback = function(error) {}; }
		return Project.update({_id:project_id}, { $unset: { archived: true }}, callback);
	}
});
