...
module.exports = (ProjectRootDocManager = {
	setRootDocAutomatically(project_id, callback) {
		...
		return ProjectEntityHandler.getAllDocs(project_id, function(error, docs) {
			...
			const jobs = _.map(docs, (doc, path) => (function(cb) {
			...
			return async.series(jobs, function(root_doc_id){
				if (root_doc_id != null) {
					return ProjectEntityUpdateHandler.setRootDoc(project_id, root_doc_id, callback);
				} else {
					return callback();
				}
			});
	...
	findRootDocFileFromDirectory(directoryPath, callback) {
		if (callback == null) { callback = function(error, path, content) {}; }
		const filePathsPromise = globby([
				'**/*.{tex,Rtex}'
			], {
				cwd: directoryPath,
		...
		filePathsPromise.then(
			unsortedFiles => ProjectRootDocManager._sortFileList(unsortedFiles, directoryPath, function(err, files) {
                ...
                return async.until(
                    ...
                    function(cb) {
                        ...
                        return fs.readFile(Path.join(directoryPath, file), 'utf8', function(error, content) {
                    ...
                    err => callback(err, doc != null ? doc.path : undefined, doc != null ? doc.content : undefined));
            }),
			err => callback(err));
	...
	setRootDocFromName(project_id, rootDocName, callback) {
		if (callback == null) { callback = function(error) {}; }
		return ProjectEntityHandler.getAllDocPathsFromProjectById(project_id, function(error, docPaths) {
			let doc_id, path;
			if (error != null) { return callback(error); }
			...
			if (rootDocName[0] !== '/') { rootDocName = `/${rootDocName}`; }
	...
	ensureRootDocumentIsSet(project_id, callback) {
		if (callback == null) { callback = function(error) {}; }
		return ProjectGetter.getProject(project_id, {rootDoc_id: 1}, function(error, project) {
			if (error != null) { return callback(error); }
			if ((project == null)) {
			...
			if (project.rootDoc_id != null) {
				return ProjectEntityHandler.getAllDocPathsFromProjectById(project_id, function(error, docPaths) {
					...
					if (rootDocValid) {
						...
						return ProjectEntityUpdateHandler.setRootDoc(project_id, null, () => ProjectRootDocManager.setRootDocAutomatically(project_id, callback));
					}
				});
			} else {
				return ProjectRootDocManager.setRootDocAutomatically(project_id, callback);
	...
	_sortFileList(listToSort, rootDirectory, callback) {
		...
		return async.mapLimit(
			listToSort,
			5,
			(filePath, cb) => fs.stat(Path.join(rootDirectory, filePath), function(err, stat) {
			...
			function(err, files) {
				...
				return callback(null, _.map(files.sort(ProjectRootDocManager._rootDocSort), file => file.path));
		});
	},
	...
	_rootDocSort(a, b) {
		...
		if ((a.name === 'main.tex') && (b.name !== 'main.tex')) { return -1; }
		if ((a.name !== 'main.tex') && (b.name === 'main.tex')) { return 1; }
