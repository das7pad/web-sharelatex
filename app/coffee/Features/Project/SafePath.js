...
`);
	...
	return SafePath = {
		...
		clean(filename) {
			...
			filename = filename.replace(BADFILE_RX, match => new Array(match.length + 1).join("_"));
		...
		isCleanPath(path) {
			...
			if (lastElementIsEmpty) { return false; }

			for (let element of Array.from(elements)) {
				if ((element.length > 0) && !SafePath.isCleanFilename(element)) { return false; }
			}

			// check for a top-level reserved name
			if (BLOCKEDFILE_RX.test(path.replace(/^\/?/,''))) { return false; }  // remove leading slash if present
