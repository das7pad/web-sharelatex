...
module.exports = (ProjectHelper = {
	...
	ensureNameIsUnique(nameList, name, suffixes, maxLength, callback) {
		...
		for (let suffix of Array.from(suffixes)) {
			const candidateName = ProjectHelper._addSuffixToProjectName(name, suffix, maxLength);
			if (isUnique(candidateName)) {
				return callback(null, candidateName, true);
			}
		}
		// if there are no (more) suffixes, use a numeric one
		const uniqueName = ProjectHelper._addNumericSuffixToProjectName(name, allNames, maxLength);
		if (uniqueName != null) {
			return callback(null, uniqueName, true);
		} else {
			return callback(new Error(`Failed to generate a unique name for: ${name}`));
		}
	},
	...
	_addNumericSuffixToProjectName(name, allProjectNames, maxLength) {
		...
		while (n <= last) {
			...
			if (!allProjectNames.has(candidate)) { return candidate; }
			n += 1;
		}
