...
module.exports = (ProjectCollabratecDetailsHandler = {
	initializeCollabratecProject(project_id, user_id, collabratec_document_id, collabratec_privategroup_id, callback) {
		if (callback == null) { callback = function(err){}; }
		return ProjectCollabratecDetailsHandler.setCollabratecUsers(project_id, [ { user_id, collabratec_document_id, collabratec_privategroup_id } ], callback);
	},

	isLinkedCollabratecUserProject(project_id, user_id, callback) {
		if (callback == null) { callback = function(err, isLinked){}; }
		try {
	...
	linkCollabratecUserProject(project_id, user_id, collabratec_document_id, callback) {
		if (callback == null) { callback = function(err){}; }
		try {
			project_id = ObjectId(project_id);
			user_id = ObjectId(user_id);
	...
	setCollabratecUsers(project_id, collabratec_users, callback) {
		...
		if (!Array.isArray(collabratec_users)) { callback(new Error("collabratec_users must be array")); }
