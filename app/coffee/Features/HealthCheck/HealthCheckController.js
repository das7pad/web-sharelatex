...
module.exports = (HealthCheckController = {
	check(req, res, next) {
		...
		return d.run(function() {
			...
			return mocha.run(function() {
				...
				const path = require.resolve(__dirname + "/../../../../test/smoke/js/SmokeTests.js");
	...
	checkMongo(req, res, next){
		...
		return UserGetter.getUserEmail(settings.smokeTest.userId, function(err, email){
			if (err != null) {
				logger.err({err}, "mongo health check failed, error present");
				return res.sendStatus(500);
			} else if ((email == null)) {
				logger.err({err}, "mongo health check failed, no emai present in find result");
				return res.sendStatus(500);
			} else {
...
var Reporter = res => (function(runner) {
