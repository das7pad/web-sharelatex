...
module.exports = (ProjectDownloadsController = {
	...
	downloadMultipleProjects(req, res, next) {
		...
		return DocumentUpdaterHandler.flushMultipleProjectsToMongo(project_ids, function(error) {
			if (error != null) { return next(error); }
			return ProjectZipStreamManager.createZipStreamForMultipleProjects(project_ids, function(error, stream) {
				if (error != null) { return next(error); }
				res.setContentDisposition(
					'attachment',
