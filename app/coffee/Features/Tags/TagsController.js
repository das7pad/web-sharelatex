...
module.exports = {
	...
	addProjectToTag(req, res, next) {
		...
		return TagsHandler.addProjectToTag(user_id, tag_id, project_id, function(error) {
			if (error != null) { return next(error); }
	...
	removeProjectFromTag(req, res, next) {
		...
		return TagsHandler.removeProjectFromTag(user_id, tag_id, project_id, function(error) {
			if (error != null) { return next(error); }
			return res.status(204).end();
		});
	},
