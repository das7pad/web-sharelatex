...
module.exports = (ProjectFileAgent = {
	...
	createLinkedFile(project_id, linkedFileData, name, parent_folder_id, user_id, callback) {
		...
		return this._go(project_id, linkedFileData, name, parent_folder_id, user_id, callback);
	},
	...
	_prepare(project_id, linkedFileData, user_id, callback) {
		if (callback == null) { callback = function(err, linkedFileData){}; }
		return this._checkAuth(project_id, linkedFileData, user_id, (err, allowed) => {
			if (err != null) { return callback(err); }
			if (!allowed) { return callback(new AccessDeniedError()); }
	...
	_go(project_id, linkedFileData, name, parent_folder_id, user_id, callback) {
		...
		return this._prepare(project_id, linkedFileData, user_id, (err, linkedFileData) => {
			if (err != null) { return callback(err); }
			if (!this._validate(linkedFileData)) {
				return callback(new BadDataError());
			}
			return this._getEntity(linkedFileData, user_id, (err, source_project, entity, type) => {
				if (err != null) { return callback(err); }
				if (type === 'doc') {
					return DocstoreManager.getDoc(source_project._id, entity._id, function(err, lines) {
						if (err != null) { return callback(err); }
						return LinkedFilesHandler.importContent(project_id,
							lines.join('\n'),
							linkedFileData,
							name,
							parent_folder_id,
					...
					return FileStoreHandler.getFileStream(source_project._id, entity._id, null, function(err, fileStream) {
						if (err != null) { return callback(err); }
	...
	_getEntity(linkedFileData, current_user_id, callback) {
			if (callback == null) { callback = function(err, entity, type) {}; }
			...
			return this._getSourceProject(linkedFileData, function(err, project) {
				...
				return DocumentUpdaterHandler.flushProjectToMongo(source_project_id, function(err) {
					if (err != null) { return callback(err); }
	...
	_checkAuth(project_id, data, current_user_id, callback) {
		...
		return this._getSourceProject(data, function(err, project) {
			if (err != null) { return callback(err); }
			return AuthorizationManager.canUserReadProject(current_user_id, project._id, null, function(err, canRead) {
				if (err != null) { return callback(err); }
