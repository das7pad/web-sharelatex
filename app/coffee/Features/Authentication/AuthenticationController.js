...
module.exports = (AuthenticationController = {
		...
		return req.login(user, function(err) {
			...
			return req.session.destroy(function(err) {
				...
				for (let key in oldSession) {
					...
					if (key !== '__tmp') { req.session[key] = value; }
				}
	...
	finishLogin(user, req, res, next) {
		if (user === false) { return res.redirect('/login'); } // OAuth2 'state' mismatch
		if (user.must_reconfirm) {
			...
			return AuthenticationController.afterLoginSessionSetup(req, user, function(err) {
				if (err != null) {
					return next(err);
				}
				return SudoModeHandler.activateSudoMode(user._id, function(err) {
					...
					if (__guard__(req.headers != null ? req.headers['accept'] : undefined, x => x.match(/^application\/json.*$/))) {
	...
	doPassportLogin(req, username, password, done) {
		...
		return Modules.hooks.fire('preDoPassportLogin', req, email, function(err, infoList) {
			if (err != null) { return next(err); }
			const info = infoList.find(i => (i != null));
			if (info != null) {
				return done(null, false, info);
			}
			return LoginRateLimiter.processLoginRequest(email, function(err, isAllowed){
				...
				return AuthenticationManager.authenticate({email}, password, function(error, user) {
	...
	setInSessionUser(req, props) {
		return (() => {
			...
			for (let key in props) {
				...
				if (__guard__(__guard__(req != null ? req.session : undefined, x1 => x1.passport), x => x.user) != null) {
					req.session.passport.user[key] = value;
				}
				if (__guard__(req != null ? req.session : undefined, x2 => x2.user) != null) {
	...
	getSessionUser(req) {
		...
		} else if (__guard__(__guard__(req != null ? req.session : undefined, x2 => x2.passport), x1 => x1.user)) {
	...
	requireOauth(allowUserStub) {
		...
		if (allowUserStub == null) { allowUserStub = false; }
		const Oauth2Server = require("../../../../modules/oauth2-server/app/js/Oauth2Server");
		return function(req, res, next) {
			if (next == null) { next = function(error) {}; }
			const request = new Oauth2Server.Request(req);
			const response = new Oauth2Server.Response(res);
			return Oauth2Server.server.authenticate(request, response, {}, function(err, token) {
				if (err != null) {
					// use a 401 status code for malformed header for git-bridge
					if ((err.code === 400) && (err.message === 'Invalid request: malformed authorization header')) { err.code = 401; }
					// fall back to v1 on invalid token
					if (err.code === 401) { return AuthenticationController._requireOauthV1Fallback(req, res, next); }
					// send all other errors
					return res.status(err.code).json({error: err.name, error_description: err.message});
				}
				if ((token.user.constructor.modelName === "UserStub") && !allowUserStub) { return res.sendStatus(401); }
				req.oauth =
	...
	_requireOauthV1Fallback(req, res, next) {
		...
		return V1Api.request(options, function(error, response, body) {
			...
			if (!__guard__(body != null ? body.user_profile : undefined, x => x.id)) { return res.status(401).json({error: "invalid_token"}); }
			return User.findOne({ "overleaf.id": body.user_profile.id }, function(error, user) {
				if (error != null) { return next(error); }
				if (user == null) { return res.status(401).json({error: "invalid_token"}); }
				req.oauth =
	...
	requireGlobalLogin(req, res, next) {
		if (Array.from(AuthenticationController._globalLoginWhitelist).includes(req._parsedUrl.pathname)) {
			return next();
		}
	...
	setRedirectInSession(req, value) {
		if ((value == null)) {
			value = Object.keys(req.query).length > 0 ? `${req.path}?${querystring.stringify(req.query)}` : `${req.path}`;
		}
		if (
			(req.session != null) &&
	...
	_redirectToLoginOrRegisterPage(req, res){
		if ((req.query.zipUrl != null) || (req.query.project_name != null) || (req.path === '/user/subscription/new')) {
	...
	_recordSuccessfulLogin(user_id, callback) {
		...
		return UserUpdater.updateUser(user_id.toString(), {
	...
	_getRedirectFromSession(req) {
		let safePath;
		const value = __guard__(req != null ? req.session : undefined, x => x.postLoginRedirect);
		if (value) { safePath = AuthenticationController._getSafeRedirectPath(value); }
	...
	_getSafeRedirectPath(value) {
		...
		if (safePath === '/') { safePath = undefined; }
		return safePath;
	}
});
