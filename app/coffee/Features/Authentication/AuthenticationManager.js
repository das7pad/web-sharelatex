...
const BCRYPT_ROUNDS = __guard__(Settings != null ? Settings.security : undefined, x => x.bcryptRounds) || 12;

const _checkWriteResult = function(result, callback) {
	// for MongoDB
	if (callback == null) { callback = function(error, updated) {}; }
...
module.exports = (AuthenticationManager = {
		...
		return User.findOne(query, (error, user) => {
			...
			if (user != null) {
				if (user.hashedPassword != null) {
					return bcrypt.compare(password, user.hashedPassword, function(error, match) {
						if (error != null) { return callback(error); }
						if (match) {
							return AuthenticationManager.checkRounds(user, user.hashedPassword, password, function(err) {
	...
	validatePassword(password) {
		if (password == null) { return { message: 'password not set' }; }

		const allowAnyChars = (Settings.passwordStrengthOptions != null ? Settings.passwordStrengthOptions.allowAnyChars : undefined) === true;
		const min = __guard__(Settings.passwordStrengthOptions != null ? Settings.passwordStrengthOptions.length : undefined, x1 => x1.min) || 6;
		let max = __guard__(Settings.passwordStrengthOptions != null ? Settings.passwordStrengthOptions.length : undefined, x2 => x2.max) || 72;

		// we don't support passwords > 72 characters in length, because bcrypt truncates them
		if (max > 72) { max = 72; }

		if (!(password.length >= min)) { return { message: 'password is too short' }; }
		if (!(password.length <= max)) { return { message: 'password is too long' }; }
		if (!allowAnyChars && !AuthenticationManager._passwordCharactersAreValid(password)) { return { message: 'password contains an invalid character' }; }
		return null;
	},

	setUserPassword(user_id, password, callback) {
		if (callback == null) { callback = function(error, changed) {}; }
		const validation = this.validatePassword(password);
		if (validation != null) { return callback(validation.message); }

		return UserGetter.getUser(user_id, { email:1, overleaf: 1 }, function(error, user) {
			if (error != null) { return callback(error); }
			const v1IdExists = ((user.overleaf != null ? user.overleaf.id : undefined) != null);
			if (v1IdExists && (Settings.overleaf != null)) { // v2 user in v2
				// v2 user in v2, change password in v1
				return AuthenticationManager.setUserPasswordInV1(user.overleaf.id, password, callback);
				...
				return AuthenticationManager.setUserPasswordInV2(user_id, password, callback);
			} else if (!v1IdExists && (Settings.overleaf != null)) {
	...
	checkRounds(user, hashedPassword, password, callback) {
		...
		if (__guard__(Settings != null ? Settings.security : undefined, x1 => x1.disableBcryptRoundsUpgrades)) {
			return callback();
		}
		...
		return bcrypt.genSalt(BCRYPT_ROUNDS, minorVersion, function(error, salt) {
			...
			return bcrypt.hash(password, salt, function(error, hash) {
				...
				return db.users.update({
					...
					if (updateError != null) { return callback(updateError); }
	...
	setUserPasswordInV1(v1_user_id, password, callback) {
		...
		return V1Handler.doPasswordReset(v1_user_id, password, function(error, reset){
			if (error != null) { return callback(error); }
	...
	_passwordCharactersAreValid(password) {
		const digits = __guard__(Settings.passwordStrengthOptions != null ? Settings.passwordStrengthOptions.chars : undefined, x1 => x1.digits) || '1234567890';
		const letters = __guard__(Settings.passwordStrengthOptions != null ? Settings.passwordStrengthOptions.chars : undefined, x2 => x2.letters) || 'abcdefghijklmnopqrstuvwxyz';
		...
		for (let charIndex = 0, end = password.length - 1, asc = 0 <= end; asc ? charIndex <= end : charIndex >= end; asc ? charIndex++ : charIndex--) {
			if (!(digits.indexOf(password[charIndex]) > -1) &&
				...
				!(symbols.indexOf(password[charIndex]) > -1)) { return false; }
		}
		return true;
	}
});
