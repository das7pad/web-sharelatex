...
module.exports = (RecurlyWrapper = {
	apiUrl : __guard__(Settings.apis != null ? Settings.apis.recurly : undefined, x => x.url) || "https://api.recurly.com/v2",

	_addressToXml(address) {
		const allowedKeys = ['address1', 'address2', 'city', 'country', 'state', 'zip', 'postal_code'];
		...
		for (let k in address) {
			...
			if (v && (Array.from(allowedKeys).includes(k))) {
				resultString += `<${k}${k === 'address2' ? ' nil="nil"' : ''}>${v || ''}</${k}>\n`;
			}
		}
	...
	_paypal: {
		checkAccountExists(cache, next) {
			...
			return RecurlyWrapper.apiRequest({
				...
				return RecurlyWrapper._parseAccountXml(responseBody, function(err, account) {
					if (err) {
...
`;
		...
		createBillingInfo(cache, next) {
			...
			const accountCode = __guard__(cache != null ? cache.account : undefined, x1 => x1.account_code);
			if (!accountCode) {
...
`;
			return RecurlyWrapper.apiRequest({
				...
				return RecurlyWrapper._parseBillingInfoXml(responseBody, function(err, billingInfo) {
					if (err) {
...
`;  // TODO: check account details and billing
			return RecurlyWrapper.apiRequest({
				...
				return RecurlyWrapper._parseSubscriptionXml(responseBody, function(err, subscription) {
					if (err) {
...
`;
	...
	createSubscription(user, subscriptionDetails, recurly_token_id, callback){
		...
		const fn = isPaypal ? RecurlyWrapper._createPaypalSubscription : RecurlyWrapper._createCreditCardSubscription;
	...
	apiRequest(options, callback) {
		...
		options.headers = {
			"Authorization" : "Basic " + Buffer.from(Settings.apis.recurly.apiKey).toString("base64"),
		...
		return request(options, function(error, response, body) {
			if ((error == null) && (response.statusCode !== 200) && (response.statusCode !== 201) && (response.statusCode !== 204) && ((response.statusCode !== 404) || !expect404)) {
				logger.err({err:error, body, options, statusCode:(response != null ? response.statusCode : undefined)}, "error returned from recurly");
	...
	getSubscription(subscriptionId, options, callback) {
		...
		return RecurlyWrapper.apiRequest({
			...
			if (error != null) { return callback(error); }
			return RecurlyWrapper._parseSubscriptionXml(body, (error, recurlySubscription) => {
				if (error != null) { return callback(error); }
				if (options.includeAccount) {
					let accountId;
					if ((recurlySubscription.account != null) && (recurlySubscription.account.url != null)) {
						accountId = recurlySubscription.account.url.match(/accounts\/(.*)/)[1];
					} else {
						return callback("I don't understand the response from Recurly");
					}

					return RecurlyWrapper.getAccount(accountId, function(error, account) {
						if (error != null) { return callback(error); }
		...
		var getPageOfAccounts = (cursor = null)=> {
			...
			return RecurlyWrapper.apiRequest(opts, (error, response, body) => {
				...
				return RecurlyWrapper._parseXml(body, function(err, data){
					...
					cursor = __guard__(response.headers.link != null ? response.headers.link.match(/cursor=([0-9]+)&/) : undefined, x1 => x1[1]);
					if (cursor != null) {
	...
	getAccountActiveCoupons(accountId, callback) {
		return RecurlyWrapper.apiRequest({
			...
			if (error != null) { return callback(error); }
			return RecurlyWrapper._parseRedemptionsXml(body, function(error, redemptions) {
				if (error != null) { return callback(error); }
				const activeRedemptions = redemptions.filter(redemption => redemption.state === 'active');
				const couponCodes = activeRedemptions.map(redemption => redemption.coupon_code);
				return Async.map(couponCodes, RecurlyWrapper.getCoupon, function(error, coupons) {
					if (error != null) { return callback(error); }
	...
	getCoupon(couponCode, callback) {
		...
		return RecurlyWrapper.apiRequest(opts, (error, response, body) => RecurlyWrapper._parseCouponXml(body, callback));
	},
	...
	updateSubscription(subscriptionId, options, callback) {
		logger.log({subscriptionId, options}, "telling recurly to update subscription");
		const requestBody = `\
<subscription>
...
`;
		...
		return RecurlyWrapper.apiRequest({
			...
			if (error != null) {
				return RecurlyWrapper._parseXml(body, function(_err, parsed) {
					if (__guard__(parsed != null ? parsed.error : undefined, x1 => x1.description) === "A canceled subscription can't transition to canceled") {
	...
	reactivateSubscription(subscriptionId, callback) {
		logger.log({subscriptionId}, "telling recurly to reactivating subscription");
		return RecurlyWrapper.apiRequest({
...
`;
		logger.log({account_code, coupon_code, requestBody}, "redeeming coupon for user");
		return RecurlyWrapper.apiRequest({
			...
			if (error != null) {
				logger.err({err:error, account_code, coupon_code}, "error redeeming coupon");
			}
			return callback(error);
		});
	},
	...
	listAccountActiveSubscriptions(account_id, callback) {
		if (callback == null) { callback = function(error, subscriptions) {}; }
		return RecurlyWrapper.apiRequest({
	...
	_parseSubscriptionsXml(xml, callback) {
		return RecurlyWrapper._parseXmlAndGetAttribute(xml, "subscriptions", callback);
	},

	_parseSubscriptionXml(xml, callback) {
		return RecurlyWrapper._parseXmlAndGetAttribute(xml, "subscription", callback);
	},
	...
	_parseBillingInfoXml(xml, callback) {
		return RecurlyWrapper._parseXmlAndGetAttribute(xml, "billing_info", callback);
	},
