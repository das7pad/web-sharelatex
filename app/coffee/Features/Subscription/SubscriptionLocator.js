...
module.exports = (SubscriptionLocator = {
	...
	getUsersSubscription(user_or_id, callback){
		...
		return Subscription.findOne({admin_id:user_id}, function(err, subscription){
	...
	getMemberSubscriptions(user_or_id, callback) {
		...
		return Subscription.find({member_ids: user_id}).populate("admin_id").exec(callback);
	},
	...
	getSubscriptionByMemberIdAndId(user_id, subscription_id, callback){
		return Subscription.findOne({member_ids: user_id, _id:subscription_id}, {_id:1}, callback);
	},

	getGroupSubscriptionsMemberOf(user_id, callback){
		return Subscription.find({member_ids: user_id}, {_id:1, planCode:1}, callback);
	},
