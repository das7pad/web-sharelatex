...
module.exports = (SubscriptionController = {
	...
	plansPage(req, res, next) {
		...
		return GeoIpLookup.getCurrencyCode((req.query != null ? req.query.ip : undefined) || req.ip, function(err, recomendedCurrency){
			if (err != null) { return next(err); }
			const render = () => res.render(viewName, {
			...
			if (user_id != null) {
				return UserGetter.getUser(user_id, {signUpDate: 1}, function(err, user) {
					if (err != null) { return next(err); }
					currentUser = user;
					return render();
				});
	...
	paymentPage(req, res, next) {
		...
		return LimitationsManager.userHasV1OrV2Subscription(user, function(err, hasSubscription){
			if (err != null) { return next(err); }
			if (hasSubscription || (plan == null)) {
				...
				return SubscriptionHandler.validateNoSubscriptionInRecurly(user._id, function(error, valid) {
	...
	userSubscriptionPage(req, res, next) {
		const user = AuthenticationController.getSessionUser(req);
		return SubscriptionViewModelBuilder.buildUsersSubscriptionViewModel(user, function(error, results) {
			if (error != null) { return next(error); }
			const {
	...
	createSubscription(req, res, next){
		...
		logger.log({recurly_token_id, user_id:user._id, subscriptionDetails}, "creating subscription");
		...
		return LimitationsManager.userHasV1OrV2Subscription(user, function(err, hasSubscription){
			...
			return SubscriptionHandler.createSubscription(user, subscriptionDetails, recurly_token_id, function(err){
				if (err != null) {
					logger.err({err, user_id:user._id}, "something went wrong creating subscription");
					return next(err);
				}
	...
	successful_subscription(req, res, next){
		const user = AuthenticationController.getSessionUser(req);
		return SubscriptionViewModelBuilder.buildUsersSubscriptionViewModel(user, function(error, {personalSubscription}) {
			if (error != null) { return next(error); }
	...
	cancelSubscription(req, res, next) {
		...
		return SubscriptionHandler.cancelSubscription(user, function(err){
			if (err != null) {
				logger.err({err, user_id:user._id}, "something went wrong canceling subscription");
				return next(err);
			}
	...
	cancelV1Subscription(req, res, next) {
		...
		return V1SubscriptionManager.cancelV1Subscription(user_id, function(err){
			if (err != null) {
				logger.err({err, user_id}, "something went wrong canceling v1 subscription");
				return next(err);
			}
	...
	updateSubscription(req, res, next){
		const _origin = __guard__(req != null ? req.query : undefined, x => x.origin) || null;
		const user = AuthenticationController.getSessionUser(req);
		...
		return SubscriptionHandler.updateSubscription(user, planCode, null, function(err){
			if (err != null) {
				logger.err({err, user_id:user._id}, "something went wrong updating subscription");
				return next(err);
			}
			return res.redirect("/user/subscription");
		});
	...
	reactivateSubscription(req, res, next){
		...
		return SubscriptionHandler.reactivateSubscription(user, function(err){
			if (err != null) {
				logger.err({err, user_id:user._id}, "something went wrong reactivating subscription");
				return next(err);
			}
			return res.redirect("/user/subscription");
		});
	...
	recurlyCallback(req, res, next){
		...
		if ((req.body != null) && (req.body["expired_subscription_notification"] != null)) {
			const recurlySubscription = req.body["expired_subscription_notification"].subscription;
			return SubscriptionHandler.recurlyCallback(recurlySubscription, function(err){
				if (err != null) { return next(err); }
	...
	renderUpgradeToAnnualPlanPage(req, res, next){
		const user = AuthenticationController.getSessionUser(req);
		return LimitationsManager.userHasV2Subscription(user, function(err, hasSubscription, subscription){
			let planName;
			if (err != null) { return next(err); }
			const planCode = subscription != null ? subscription.planCode.toLowerCase() : undefined;
			if ((planCode != null ? planCode.indexOf("annual") : undefined) !== -1) {
				planName = "annual";
			} else if ((planCode != null ? planCode.indexOf("student") : undefined) !== -1) {
				planName = "student";
			} else if ((planCode != null ? planCode.indexOf("collaborator") : undefined) !== -1) {
				planName = "collaborator";
			}
			if (!hasSubscription) {
				return res.redirect("/user/subscription/plans");
			}
			logger.log({planName, user_id:user._id}, "rendering upgrade to annual page");
	...
	processUpgradeToAnnualPlan(req, res, next){
		...
		return SubscriptionHandler.updateSubscription(user, annualPlanName, coupon_code, function(err){
			if (err != null) {
	...
	recurlyNotificationParser(req, res, next) {
		...
		return req.on("end", () => RecurlyWrapper._parseXml(xml, function(error, body) {
            if (error != null) { return next(error); }
            req.body = body;
            return next();
        }));
