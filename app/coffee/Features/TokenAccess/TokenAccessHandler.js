...
module.exports = (TokenAccessHandler = {
	...
	_getProjectByEitherToken(token, callback) {
		if (callback == null) { callback = function(err, project){}; }
		return TokenAccessHandler._getProjectByReadOnlyToken(token, function(err, project) {
			if (err != null) { return callback(err); }
			if (project != null) {
				return callback(null, project);
			}
			return TokenAccessHandler._getProjectByReadAndWriteToken(token, function(err, project) {
				if (err != null) { return callback(err); }
				return callback(null, project);
			});
		});
	...
	_getProjectByReadAndWriteToken(token, callback) {
		...
		}, {_id: 1, tokens: 1, publicAccesLevel: 1, owner_ref: 1}, function(err, project) {
			if (err != null) { return callback(err); }
			if ((project == null)) {
				return callback(null, null);
			}
	...
	findProjectWithReadOnlyToken(token, callback) {
		if (callback == null) { callback = function(err, project, projectExists){}; }
		return TokenAccessHandler._getProjectByReadOnlyToken(token, function(err, project) {
			if (err != null) {
				return callback(err);
			}
	...
	_userIsMember(userId, projectId, callback) {
		if (callback == null) { callback = function(err, isMember){}; }
		return CollaboratorsHandler.isUserInvitedMemberOfProject(userId, projectId, callback);
	},

	findProjectWithHigherAccess(token, userId, callback) {
		if (callback == null) { callback = function(err, project){}; }
		return TokenAccessHandler._getProjectByEitherToken(token, function(err, project) {
			if (err != null) { return callback(err); }
			if ((project == null)) {
				return callback(null, null);
			}
			const projectId = project._id;
			return TokenAccessHandler._userIsMember(userId, projectId, function(err, isMember) {
				if (err != null) { return callback(err); }
				return callback(
					null,
					isMember === true ? project : null
				);
			});
		});
	},
	...
	grantSessionTokenAccess(req, projectId, token) {
		if (req.session != null) {
			...
			return req.session.anonTokenAccess[projectId.toString()] = token.toString();
		}
	},

	getRequestToken(req, projectId) {
		const token = (
			__guard__(__guard__(req != null ? req.session : undefined, x1 => x1.anonTokenAccess), x => x[projectId.toString()]) ||
	...
	isValidToken(projectId, token, callback) {
		if (callback == null) { callback = function(err, isValidReadAndWrite, isValidReadOnly){}; }
		if (!token) {
			return callback(null, false, false);
		}
		const _validate = project => (project != null) &&
        (project.publicAccesLevel === PublicAccessLevels.TOKEN_BASED) &&
        (project._id.toString() === projectId.toString());
		return TokenAccessHandler.findProjectWithReadAndWriteToken(token, function(err, readAndWriteProject) {
			if (err != null) { return callback(err); }
			const isValidReadAndWrite = _validate(readAndWriteProject);
			return TokenAccessHandler.findProjectWithReadOnlyToken(token, function(err, readOnlyProject) {
				if (err != null) { return callback(err); }
	...
	getV1DocPublishedInfo(token, callback) {
		...
		if (callback == null) { callback = function(err, publishedInfo) {}; }
		if ((Settings.apis != null ? Settings.apis.v1 : undefined) == null) { return callback(null, {
			allow: true
		}); }

		return V1Api.request({ url: `/api/v1/sharelatex/docs/${token}/is_published` }, function(err, response, body) {
			if (err != null) { return callback(err); }
	...
	getV1DocInfo(token, v2UserId, callback) {
		...
		if (callback == null) { callback = function(err, info){}; }
		if ((Settings.apis != null ? Settings.apis.v1 : undefined) == null) { return callback(null, {
			exists: true,
			exported: false
		}); }
		...
		return UserGetter.getUser(v2UserId, { overleaf: 1 }, function(err, user) {
			...
			return V1Api.request({ url: `/api/v1/sharelatex/users/${v1UserId}/docs/${token}/info` }, function(err, response, body) {
				if (err != null) { return callback(err); }
				return callback(null, body);
			});
		});
