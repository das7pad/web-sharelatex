...
module.exports = (TokenAccessController = {
	...
	_tryHigherAccess(token, userId, req, res, next) {
		return TokenAccessHandler.findProjectWithHigherAccess(token, userId, function(err, project) {
			if (err != null) {
	...
	readAndWriteToken(req, res, next) {
		...
		return TokenAccessHandler.findProjectWithReadAndWriteToken(token, function(err, project, projectExists) {
			if (err != null) {
				logger.err({err, token, userId},
			...
			if (!projectExists && settings.overleaf) {
				...
				return TokenAccessController._tryHigherAccess(token, userId, req, res, next);
			} else {
				if ((userId == null)) {
				...
				return TokenAccessHandler.addReadAndWriteUserToProject(userId, project._id, function(err) {
					if (err != null) {
	...
	readOnlyToken(req, res, next) {
		...
		return TokenAccessHandler.getV1DocPublishedInfo(token, function(err, doc_published_info) {
			if (err != null) { return next(err); }
			if (doc_published_info.allow === false) { return res.redirect(doc_published_info.published_path); }

			return TokenAccessHandler.findProjectWithReadOnlyToken(token, function(err, project, projectExists) {
				if (err != null) {
					logger.err({err, token, userId},
				...
				if (!projectExists && settings.overleaf) {
					...
					if ((userId == null)) {
						...
						return TokenAccessHandler.addReadOnlyUserToProject(userId, project._id, function(err) {
							if (err != null) {
								logger.err({err, token, userId, projectId: project._id},
									"[TokenAccess] error adding user to project with readAndWrite token");
								return next(err);
	...
	_handleV1Project(token, userId, redirectPath, res, next) {
		if ((userId == null)) {
			...
			return TokenAccessHandler.getV1DocInfo(token, userId, function(err, doc_info) {
				if (err != null) { return next(err); }
				if (!doc_info.exists) { return next(new Errors.NotFoundError()); }
				if (doc_info.exported) { return next(new Errors.NotFoundError()); }
				if (Features.hasFeature('force-import-to-v2')) {
