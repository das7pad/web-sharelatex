...
var updateOpenConnetionsMetrics = function(){
	metrics.gauge("open_connections.socketio", __guard__(__guard__(__guard__(require("../../infrastructure/Server").io, x2 => x2.sockets), x1 => x1.clients()), x => x.length));
	metrics.gauge("open_connections.http", _.size(__guard__(require('http').globalAgent, x3 => x3.sockets)));
	metrics.gauge("open_connections.https", _.size(__guard__(require('https').globalAgent, x4 => x4.sockets)));
...
module.exports = (AdminController = {
	...
	index : (req, res, next)=> {
		...
		for (url in object) {
			...
			openSockets[`http://${url}`] = ((() => {
				const result = [];
				for (agent of Array.from(agents)) { 					result.push(agent._httpMessage.path);
				}
				return result;
			})());
		}
		...
		for (url in object1) {
			...
			openSockets[`https://${url}`] = ((() => {
				const result1 = [];
				for (agent of Array.from(agents)) { 					result1.push(agent._httpMessage.path);
				}
				return result1;
			})());
		}

		return SystemMessageManager.getMessagesFromDB(function(error, systemMessages) {
	...
	syncUserToSubscription(req, res){
		...
		return RecurlyWrapper.getSubscription(subscription_id, {}, (err, subscription) => User.findById(user_id, (err, user) => SubscriptionHandler.syncSubscriptionToUser(subscription, user._id, function(err){
            logger.log({user_id, subscription_id}, "linked account to subscription");
            return res.send();
        })));
	},

	flushProjectToTpds(req, res){
		return projectEntityHandler.flushProjectToThirdPartyDataStore(req.body.project_id, err => res.sendStatus(200));
	},
	...
	pollDropboxForUser(req, res){
		...
		return TpdsUpdateSender.pollDropboxForUser(user_id, () => res.sendStatus(200));
	},
			
	createMessage(req, res, next) {
		return SystemMessageManager.createMessage(req.body.content, function(error) {
			if (error != null) { return next(error); }
			return res.sendStatus(200);
		});
	},
