...
// Note: notification keys should be url-safe
	...
	featuresUpgradedByAffiliation(affiliation, user) {
		return {
			...
			create(callback) {
				...
				return NotificationsHandler.createNotification(user._id, this.key, "notification_features_upgraded_by_affiliation", messageOpts, null, false, callback);
			},
			read(callback) {
				if (callback == null) { callback = function(){}; }
	...
	redundantPersonalSubscription(affiliation, user) {
		return {
			...
			create(callback) {
				...
				const messageOpts =
					{institutionName: affiliation.institutionName};
				return NotificationsHandler.createNotification(user._id, this.key, "notification_personal_subscription_not_required_due_to_affiliation", messageOpts, null, false, callback);
			},
			read(callback) {
				if (callback == null) { callback = function(){}; }
				return NotificationsHandler.markAsRead(this.key, callback);
	...
	projectInvite(invite, project, sendingUser, user) {
		return {
			...
			create(callback) {
				...
				logger.log({user_id: user._id, project_id: project._id, invite_id: invite._id, key: this.key}, "creating project invite notification for user");
				return NotificationsHandler.createNotification(user._id, this.key, "notification_project_invite", messageOpts, invite.expires, callback);
			},
			read(callback) {
				if (callback == null) { callback = function(){}; }
	...
	ipMatcherAffiliation(userId) {
		return {
			create(ip, callback) {
				if (callback == null) { callback = function(){}; }
				if (!__guard__(__guard__(settings != null ? settings.apis : undefined, x1 => x1.v1), x => x.url)) { return null; } // service is not configured
				return request({
					method: 'GET',
					...
					logger.log({user_id:userId, key}, "creating notification key for user");
					return NotificationsHandler.createNotification(userId, key, "notification_ip_matched_affiliation", messageOpts, null, false, callback);
				});
			},
