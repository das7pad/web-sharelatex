...
const CTAEmailTemplate = function(content) {
	if (content.greeting == null) { content.greeting = () => 'Hi,'; }
	if (content.secondaryMessage == null) { content.secondaryMessage = () => ""; }
	return {
		subject(opts) { return content.subject(opts); },
		layout: BaseWithHeaderEmailLayout,
		plainTextTemplate(opts) { return `\
...
${(typeof content.secondaryMessage === 'function' ? content.secondaryMessage(opts).trim() : undefined) || ""}
...
`; },
		compiledTemplate(opts) {
			return SingleCTAEmailBody({
				title: (typeof content.title === 'function' ? content.title(opts) : undefined),
				...
				gmailGoToAction: (typeof content.gmailGoToAction === 'function' ? content.gmailGoToAction(opts) : undefined),
				StringHelper
			});
		}
	};
...
templates.accountMergeToOverleafAddress = CTAEmailTemplate({
	subject() { return `Confirm Account Merge - ${settings.appName}`; },
	title() { return "Confirm Account Merge"; },
	message() {
		return `\
...
`;
	...
	ctaText() { return "Confirm Account Merge"; },
	ctaURL(opts) { return opts.tokenLinkUrl; }
});

templates.accountMergeToSharelatexAddress = templates.accountMergeToOverleafAddress;

templates.registered = CTAEmailTemplate({
	subject() { return `Activate your ${settings.appName} Account`; },
	message(opts) { return `\
...
`; },
	secondaryMessage() { return `If you have any questions or problems, please contact ${settings.adminEmail}`; },
	ctaText() { return "Set password"; },
	ctaURL(opts) { return opts.setNewPasswordUrl; }
});

templates.canceledSubscription = CTAEmailTemplate({
	subject() { return `${settings.appName} thoughts`; },
	message() { return `\
...
`; },
	secondaryMessage() { return "Thank you in advance!"; },
	ctaText() { return "Leave Feedback"; },
	ctaURL(opts) { return "https://docs.google.com/forms/d/e/1FAIpQLScqU6Je1r4Afz6ul6oY0RAfN7RabdWv_oL1u7Rj1YBmXS4fiQ/viewform?usp=sf_link"; }
});

templates.reactivatedSubscription = CTAEmailTemplate({
	subject() { return `Subscription Reactivated - ${settings.appName}`; },
	message(opts) { return `\
Your subscription was reactivated successfully.\
`; },
	ctaText() { return "View Subscription Dashboard"; },
	ctaURL(opts) { return `${settings.siteUrl}/user/subscription`; }
});

templates.passwordResetRequested = CTAEmailTemplate({
	subject() { return `Password Reset - ${settings.appName}`; },
	title() { return "Password Reset"; },
	message() { return `We got a request to reset your ${settings.appName} password.`; },
	secondaryMessage() { return `\
If you ignore this message, your password won't be changed.

If you didn't request a password reset, let us know.\
`; },
	ctaText() { return "Reset password"; },
	ctaURL(opts) { return opts.setNewPasswordUrl; }
});

templates.confirmEmail = CTAEmailTemplate({
	subject() { return `Confirm Email - ${settings.appName}`; },
	title() { return "Confirm Email"; },
	message() { return `Please confirm your email on ${settings.appName}.`; },
	ctaText() { return "Confirm Email"; },
	ctaURL(opts) { return opts.confirmEmailUrl; }
});

templates.projectInvite = CTAEmailTemplate({
	subject(opts) { return `${ _.escape(SpamSafe.safeProjectName(opts.project.name, "New Project")) } - shared by ${ _.escape(SpamSafe.safeEmail(opts.owner.email, "a collaborator")) }`; },
	title(opts) { return `${ _.escape(SpamSafe.safeProjectName(opts.project.name, "New Project")) } - shared by ${ _.escape(SpamSafe.safeEmail(opts.owner.email, "a collaborator")) }`; },
	message(opts) { return `${ _.escape(SpamSafe.safeEmail(opts.owner.email, "a collaborator")) } wants to share ${ _.escape(SpamSafe.safeProjectName(opts.project.name, "a new project")) } with you.`; },
	ctaText() { return "View project"; },
	ctaURL(opts) { return opts.inviteUrl; },
...
templates.verifyEmailToJoinTeam = CTAEmailTemplate({
	subject(opts) { return `${ _.escape(SpamSafe.safeUserName(opts.inviterName, "A collaborator")) } has invited you to join a team on ${ settings.appName }`; },
	title(opts) { return `${ _.escape(SpamSafe.safeUserName(opts.inviterName, "A collaborator")) } has invited you to join a team on ${ settings.appName }`; },
	message(opts) { return `Please click the button below to join the team and enjoy the benefits of an upgraded ${ settings.appName } account.`; },
	ctaText(opts) { return "Join now"; },
	ctaURL(opts) { return opts.acceptInviteUrl; }
});

templates.dropboxUnlinkedDuplicate = CTAEmailTemplate({
	subject() { return `Your Dropbox Account has been Unlinked - ${settings.appName}`; },
	message(opts) { return `\
...
`; },
	ctaText() { return "Link Dropbox Account"; },
	ctaURL(opts) { return `${settings.siteUrl}/user/settings`; }
});

templates.testEmail = CTAEmailTemplate({
	subject() { return `A Test Email from ${settings.appName}`; },
	title() { return `A Test Email from ${settings.appName}`; },
	greeting() { return "Hi,"; },
	message() { return `This is a test Email from ${settings.appName}`; },
	ctaText() { return `Open ${settings.appName}`; },
	ctaURL() { return settings.siteUrl; }
});

templates.projectsTransferredFromSharelatex = CTAEmailTemplate({
	subject() { return "ShareLaTeX projects transferred to your Overleaf account"; },
	title() { return "ShareLaTeX projects transferred to your Overleaf account"; },
	message(opts) { return `\
...
`; },
	ctaText() { return `Log in to ${ settings.appName }`; },
	ctaURL() { return settings.siteUrl + "/login"; }
});

templates.emailAddressPoachedEmail = CTAEmailTemplate({
	subject() { return `One of your email addresses has been moved to another ${ settings.appName } account`; },
	title() { return `One of your email addresses has been moved to another ${ settings.appName } account`; },
	message(opts) {
		let message = `\
...
module.exports = {
	...
	buildEmail(templateName, opts){
		...
		if (__guard__(settings.email != null ? settings.email.templates : undefined, x => x.customFooter) != null) {
			opts.body += __guard__(settings.email != null ? settings.email.templates : undefined, x1 => x1.customFooter);
		}
		return {
			subject : template.subject(opts),
			html: template.layout(opts),
			text: __guardMethod__(template, 'plainTextTemplate', o => o.plainTextTemplate(opts))
		};
	}
};
