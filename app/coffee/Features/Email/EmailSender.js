...
let client = {
	sendMail(options, callback) {
		if (callback == null) { callback = function(err,status) {}; }
		logger.log({options}, "Would send email if enabled.");
		return callback();
	}
};
if ((__guard__(__guard__(Settings != null ? Settings.email : undefined, x1 => x1.parameters), x => x.AWSAccessKeyID) != null) || (__guard__(Settings != null ? Settings.email : undefined, x2 => x2.driver) === 'ses')) {
	logger.log("using aws ses for email");
	nm_client = nodemailer.createTransport(sesTransport(Settings.email.parameters));
} else if (__guard__(__guard__(Settings != null ? Settings.email : undefined, x4 => x4.parameters), x3 => x3.sendgridApiKey) != null) {
	logger.log("using sendgrid for email");
	nm_client = nodemailer.createTransport(sgTransport({auth:{api_key:__guard__(__guard__(Settings != null ? Settings.email : undefined, x6 => x6.parameters), x5 => x5.sendgridApiKey)}}));
} else if (__guard__(__guard__(Settings != null ? Settings.email : undefined, x8 => x8.parameters), x7 => x7.MandrillApiKey) != null) {
	logger.log("using mandril for email");
	nm_client = nodemailer.createTransport(mandrillTransport({auth:{apiKey:__guard__(__guard__(Settings != null ? Settings.email : undefined, x10 => x10.parameters), x9 => x9.MandrillApiKey)}}));
} else if (__guard__(Settings != null ? Settings.email : undefined, x11 => x11.parameters) != null) {
	logger.log("using smtp for email");
	const smtp = _.pick(__guard__(Settings != null ? Settings.email : undefined, x12 => x12.parameters), "host", "port", "secure", "auth", "ignoreTLS");
...
module.exports = {
	sendEmail(options, callback){
		...
		return checkCanSendEmail(options, function(err, canContinue){
			...
			if (!canContinue) {
				logger.log({sendingUser_id:options.sendingUser_id, to:options.to, subject:options.subject, canContinue}, "rate limit hit for sending email, not sending");
