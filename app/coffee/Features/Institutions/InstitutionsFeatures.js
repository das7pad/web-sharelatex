...
module.exports = (InstitutionsFeatures = {
	getInstitutionsFeatures(userId, callback) {
		...
		return InstitutionsFeatures.getInstitutionsPlan(userId, function(error, plan) {
	...
	getInstitutionsPlan(userId, callback) {
		if (callback == null) { callback = function(error, plan) {}; }
		return InstitutionsFeatures.hasLicence(userId, function(error, hasLicence) {
			if (error != null) { return callback(error); }
			if (!hasLicence) { return callback(null, null); }
	...
	hasLicence(userId, callback) {
		if (callback == null) { callback = function(error, hasLicence) {}; }
		return InstitutionsGetter.getConfirmedInstitutions(userId, function(error, institutions) {
			if (error != null) { return callback(error); }

			const hasLicence = institutions.some(institution => institution.licence && (institution.licence !== 'free'));
