...
module.exports = (ReferalFeatures = {
	getBonusFeatures(user_id, callback) {
		...
		return User.findOne(query, function(error, user) {
			...
			if ((user == null)) { return callback(new Error(`user not found ${user_id} for assignBonus`)); }
			logger.log({user_id, refered_user_count: user.refered_user_count}, "assigning bonus");
	...
	_calculateFeatures(user){
		...
		return (Settings.bonus_features != null ? Settings.bonus_features[`${bonusLevel}`] : undefined) || {};
	},
