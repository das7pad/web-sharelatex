...
module.exports = { 
	...
	use(req, res, next){
		if (req.query != null) {
			...
			} else if (req.query.r != null) { // Short hand for referal
				req.session.referal_id = req.query.r;
			} else if (req.query.fb_ref != null) {
				req.session.referal_id = req.query.fb_ref;
			}

			if (req.query.rm != null) { // referal medium e.g. twitter, facebook, email
				switch (req.query.rm) {
					case "fb":
			...
			if (req.query.rs != null) { // referal source e.g. project share, bonus
				switch (req.query.rs) {
					case "b":
