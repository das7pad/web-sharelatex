...
module.exports = (ChatController = {
	sendMessage(req, res, next){
		...
		return ChatApiHandler.sendGlobalMessage(project_id, user_id, content, function(err, message) {
			if (err != null) { return next(err); }
			return UserInfoManager.getPersonalInfo(message.user_id, function(err, user) {
				if (err != null) { return next(err); }
	...
	getMessages(req, res, next){
		...
		return ChatApiHandler.getGlobalMessages(project_id, query.limit, query.before, function(err, messages) {
			if (err != null) { return next(err); }
			return ChatController._injectUserInfoIntoThreads({global: { messages }}, function(err) {
				if (err != null) { return next(err); }
				logger.log({length: (messages != null ? messages.length : undefined)}, "sending messages to client");
	...
	_injectUserInfoIntoThreads(threads, callback) {
		...
		if (callback == null) { callback = function(error, threads) {}; }
		const user_ids = {};
		...
		for (user_id in user_ids) {
			...
			((user_id => jobs.push(cb => UserInfoManager.getPersonalInfo(user_id, function(err, user) {
                if (typeof error !== 'undefined' && error !== null) { return cb(error); }
