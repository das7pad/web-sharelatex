...
module.exports = (DocumentHelper = {
	getTitleFromTexContent(content, maxContentToScan) {
		if (maxContentToScan == null) { maxContentToScan = 30000; }
		const TITLE_WITH_CURLY_BRACES = /\\[tT]itle\*?\s*{([^}]+)}/;
		const TITLE_WITH_SQUARE_BRACES = /\\[tT]itle\s*\[([^\]]+)\]/;
		for (let line of Array.from(DocumentHelper._getLinesFromContent(content, maxContentToScan))) {
			var match;
	...
	contentHasDocumentclass(content, maxContentToScan) {
		...
		for (let line of Array.from(DocumentHelper._getLinesFromContent(content, maxContentToScan))) {
			...
			if (line.match(/^\s*\\documentclass/)) { return true; }
		}
	...
	_getLinesFromContent(content, maxContentToScan) {
		if (typeof content === 'string') { return content.substring(0, maxContentToScan).split("\n"); } else { return content; }
	}
});
