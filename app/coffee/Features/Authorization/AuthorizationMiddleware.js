...
module.exports = (AuthorizationMiddleware = {
	ensureUserCanReadMultipleProjects(req, res, next) {
		...
		return AuthorizationMiddleware._getUserId(req, function(error, user_id) {
			...
			// errors in callbacks
			return async.rejectSeries(project_ids, function(project_id, cb) {
				const token = TokenAccessHandler.getRequestToken(req, project_id);
				return AuthorizationManager.canUserReadProject(user_id, project_id, token, function(error, canRead) {
					if (error != null) { return next(error); }
					return cb(canRead);
				});
			}
	...
	ensureUserCanReadProject(req, res, next) {
		return AuthorizationMiddleware._getUserAndProjectId(req, function(error, user_id, project_id) {
			if (error != null) { return next(error); }
			const token = TokenAccessHandler.getRequestToken(req, project_id);
			return AuthorizationManager.canUserReadProject(user_id, project_id, token, function(error, canRead) {
				if (error != null) { return next(error); }
	...
	ensureUserCanWriteProjectSettings(req, res, next) {
		return AuthorizationMiddleware._getUserAndProjectId(req, function(error, user_id, project_id) {
			if (error != null) { return next(error); }
			const token = TokenAccessHandler.getRequestToken(req, project_id);
			return AuthorizationManager.canUserWriteProjectSettings(user_id, project_id, token, function(error, canWrite) {
				if (error != null) { return next(error); }
				if (canWrite) {
	...
	ensureUserCanWriteProjectContent(req, res, next) {
		return AuthorizationMiddleware._getUserAndProjectId(req, function(error, user_id, project_id) {
			if (error != null) { return next(error); }
			const token = TokenAccessHandler.getRequestToken(req, project_id);
			return AuthorizationManager.canUserWriteProjectContent(user_id, project_id, token, function(error, canWrite) {
				if (error != null) { return next(error); }
				if (canWrite) {
	...
	ensureUserCanAdminProject(req, res, next) {
		return AuthorizationMiddleware._getUserAndProjectId(req, function(error, user_id, project_id) {
			if (error != null) { return next(error); }
			const token = TokenAccessHandler.getRequestToken(req, project_id);
			return AuthorizationManager.canUserAdminProject(user_id, project_id, token, function(error, canAdmin) {
				if (error != null) { return next(error); }
				if (canAdmin) {
	...
	_getUserAndProjectId(req, callback) {
		if (callback == null) { callback = function(error, user_id, project_id) {}; }
		const project_id = (req.params != null ? req.params.project_id : undefined) || (req.params != null ? req.params.Project_id : undefined);
		if ((project_id == null)) {
	...
	_getUserId(req, callback) {
		if (callback == null) { callback = function(error, user_id) {}; }
		const user_id = AuthenticationController.getLoggedInUserId(req) || __guard__(req != null ? req.oauth_user : undefined, x => x._id) || null;
