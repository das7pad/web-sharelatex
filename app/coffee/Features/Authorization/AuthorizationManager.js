...
module.exports = (AuthorizationManager = {
	...
	getPublicAccessLevel(project_id, callback) {
		if (callback == null) { callback = function(err, level){}; }
		...
		return ProjectGetter.getProject(project_id, {publicAccesLevel: 1}, function(error, project) {
	...
	getPrivilegeLevelForProject(
		...
		if (callback == null) { callback = function(error, privilegeLevel, becausePublic, becauseSiteAdmin) {}; }
		if ((user_id == null)) {
			// User is Anonymous, Try Token-based access
			return AuthorizationManager.getPublicAccessLevel(project_id, function(err, publicAccessLevel) {
				...
				if (publicAccessLevel === PublicAccessLevels.TOKEN_BASED) {
					...
					return TokenAccessHandler.isValidToken(project_id, token, function(err, isValidReadAndWrite, isValidReadOnly) {
						if (err != null) { return callback(err); }
						if (isValidReadOnly) {
			...
			return CollaboratorsHandler.getMemberIdPrivilegeLevel(user_id, project_id, function(error, privilegeLevel) {
				if (error != null) { return callback(error); }
				if ((privilegeLevel != null) && (privilegeLevel !== PrivilegeLevels.NONE)) {
					...
					return AuthorizationManager.isUserSiteAdmin(user_id, function(error, isAdmin) {
						if (error != null) { return callback(error); }
						if (isAdmin) {
	...
	canUserReadProject(user_id, project_id, token, callback) {
		if (callback == null) { callback = function(error, canRead) {}; }
		return AuthorizationManager.getPrivilegeLevelForProject(user_id, project_id, token, function(error, privilegeLevel) {
			if (error != null) { return callback(error); }
			return callback(null, ([PrivilegeLevels.OWNER, PrivilegeLevels.READ_AND_WRITE, PrivilegeLevels.READ_ONLY].includes(privilegeLevel)));
		});
	},
		
	canUserWriteProjectContent(user_id, project_id, token, callback) {
		if (callback == null) { callback = function(error, canWriteContent) {}; }
		return AuthorizationManager.getPrivilegeLevelForProject(user_id, project_id, token, function(error, privilegeLevel) {
			if (error != null) { return callback(error); }
			return callback(null, ([PrivilegeLevels.OWNER, PrivilegeLevels.READ_AND_WRITE].includes(privilegeLevel)));
		});
	},
		
	canUserWriteProjectSettings(user_id, project_id, token, callback) {
		if (callback == null) { callback = function(error, canWriteSettings) {}; }
		return AuthorizationManager.getPrivilegeLevelForProject(user_id, project_id, token, function(error, privilegeLevel, becausePublic) {
			if (error != null) { return callback(error); }
			if (privilegeLevel === PrivilegeLevels.OWNER) {
				return callback(null, true);
			} else if ((privilegeLevel === PrivilegeLevels.READ_AND_WRITE) && !becausePublic) {
				return callback(null, true);
			} else {
	...
	canUserAdminProject(user_id, project_id, token, callback) {
		if (callback == null) { callback = function(error, canAdmin, becauseSiteAdmin) {}; }
		return AuthorizationManager.getPrivilegeLevelForProject(user_id, project_id, token, function(error, privilegeLevel, becausePublic, becauseSiteAdmin) {
			if (error != null) { return callback(error); }
	...
	isUserSiteAdmin(user_id, callback) {
		if (callback == null) { callback = function(error, isAdmin) {}; }
		if ((user_id == null)) {
			return callback(null, false);
		}
		return User.findOne({ _id: user_id }, { isAdmin: 1 }, function(error, user) {
			if (error != null) { return callback(error); }
