...
module.exports = (ErrorController = {
	...
	handleError(error, req, res, next) {
		...
		if ((error != null ? error.code : undefined) === 'EBADCSRFTOKEN') {
			logger.warn({err: error,url:req.url, method:req.method, user}, "invalid csrf");
			res.sendStatus(403);
			return;
		}
		if (error instanceof Errors.NotFoundError) {
			...
			logger.error({err: error, url:req.url, method:req.method, user}, "error passed to top level next middleware");
