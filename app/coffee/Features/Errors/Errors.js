...
var ServiceDisabledError = function(message) {
	const error = new Error(message);
...
const ThirdPartyUserNotFoundError = function(message) {
	if (message == null) { message = "user not found for provider and external id"; }
	const error = new Error(message);
...
var SubscriptionAdminDeletionError = function(message) {
	if (message == null) { message = "subscription admins cannot be deleted"; }
	const error = new Error(message);
