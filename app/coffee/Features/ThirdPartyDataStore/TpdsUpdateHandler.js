...
module.exports = {
	...
	newUpdate(user_id, projectName, path, updateRequest, source, callback){
		const getOrCreateProject = cb=> {
			return projectLocator.findUsersProjectByName(user_id, projectName, (err, project)=> {
				logger.log({user_id, filePath:path, projectName}, "handling new update from tpds");
				if ((project == null)) {
					return projectCreationHandler.createBlankProject(user_id, projectName, (err, project)=> {
						...
						setTimeout((() => ProjectRootDocManager.setRootDocAutomatically(project._id)), this._rootDocTimeoutLength);
		...
		return getOrCreateProject(function(err, project){
			...
			return CooldownManager.isProjectOnCooldown(project._id, function(err, projectIsOnCooldown) {
				if (err != null) { return callback(err); }
				...
				return FileTypeManager.shouldIgnore(path, function(err, shouldIgnore){
					...
					return updateMerger.mergeUpdate(user_id, project._id, path, updateRequest, source, callback);
				});
			});
		});
	},
	...
	deleteUpdate(user_id, projectName, path, source, callback){	
		...
		return projectLocator.findUsersProjectByName(user_id, projectName, function(err, project){
			if ((project == null)) {
				logger.log({user_id, filePath:path, projectName}, "project not found from tpds update, ignoring folder or project");
				return callback();
			}
			if (path === "/") {
				logger.log({user_id, filePath:path, projectName, project_id:project._id}, "project found for delete update, path is root so marking project as deleted");
				return projectDeleter.markAsDeletedByExternalSource(project._id, callback);
			} else {
				return updateMerger.deleteUpdate(user_id, project._id, path, source, err => callback(err));
			}
		});
	},
