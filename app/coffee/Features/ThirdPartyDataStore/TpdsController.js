...
module.exports = {
	...
	mergeUpdate(req, res){
		...
		logger.log({user_id, filePath, fullPath:req.params[0], projectName, source}, "reciving update request from tpds");
		return tpdsUpdateHandler.newUpdate(user_id, projectName, filePath, req, source, function(err){
			logger.log({user_id, filePath, fullPath:req.params[0]}, "sending response that tpdsUpdate has been completed");
			if (err != null) {
				logger.err({err, user_id, filePath}, "error reciving update from tpds");
				return res.sendStatus(500);
			} else {
				logger.log({user_id, filePath, projectName}, "telling tpds update has been processed");
	...
	deleteUpdate(req, res){
		...
		const {filePath, user_id, projectName} = parseParams(req);
		const source = req.headers["x-sl-update-source"] || "unknown";
		logger.log({user_id, filePath, projectName, fullPath:req.params[0], source}, "reciving delete request from tpds");
		return tpdsUpdateHandler.deleteUpdate(user_id, projectName, filePath, source, function(err){
			if (err != null) {
				logger.err({err, user_id, filePath}, "error reciving update from tpds");
				return res.sendStatus(500);
			} else {
				logger.log({user_id, filePath, projectName}, "telling tpds delete has been processed");
				return res.sendStatus(200);
			}
		});
	},
	...
	updateProjectContents(req, res, next) {
		...
		return UpdateMerger.mergeUpdate(null, project_id, path, req, source, function(error) {
	...
	deleteProjectContents(req, res, next) {
		...
		const path = "/" + req.params[0]; // UpdateMerger expects leading slash
		const source = req.headers["x-sl-update-source"] || "unknown";
		logger.log({project_id, path, source}, "received project contents delete request");
		return UpdateMerger.deleteUpdate(null, project_id, path, source, function(error) {
			if (error != null) { return next(error); }
			return res.sendStatus(200);
		});
	},
