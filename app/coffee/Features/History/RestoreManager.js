...
module.exports = (RestoreManager = {
	restoreDocFromDeletedDoc(user_id, project_id, doc_id, name, callback) {
		...
		return ProjectEntityHandler.getDoc(project_id, doc_id, {include_deleted: true}, function(error, lines) {
			if (error != null) { return callback(error); }
			const addDocWithName = (name, callback) => EditorController.addDoc(project_id, null, name, lines, 'restore', user_id, callback);
			return RestoreManager._addEntityWithUniqueName(addDocWithName, name, callback);
		});
	},

	restoreFileFromV2(user_id, project_id, version, pathname, callback) {
		if (callback == null) { callback = function(error, entity) {}; }
		return RestoreManager._writeFileVersionToDisk(project_id, version, pathname, function(error, fsPath) {
			if (error != null) { return callback(error); }
			...
			return RestoreManager._findOrCreateFolder(project_id, dirname, function(error, parent_folder_id) {
				if (error != null) { return callback(error); }
				const addEntityWithName = (name, callback) => FileSystemImportManager.addEntity(user_id, project_id, parent_folder_id, name, fsPath, false, callback);
				return RestoreManager._addEntityWithUniqueName(addEntityWithName, basename, callback);
			});
		});
	},

	_findOrCreateFolder(project_id, dirname, callback) {
		if (callback == null) { callback = function(error, folder_id) {}; }
		return EditorController.mkdirp(project_id, dirname, function(error, newFolders, lastFolder) {
			if (error != null) { return callback(error); }
	...
	_writeFileVersionToDisk(project_id, version, pathname, callback) {
		...
		const url = `${Settings.apis.project_history.url}/project/${project_id}/version/${version}/${encodeURIComponent(pathname)}`;
