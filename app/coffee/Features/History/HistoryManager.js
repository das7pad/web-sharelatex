...
module.exports = (HistoryManager = {
	initializeProject(callback) {
		if (callback == null) { callback = function(error, history_id) {}; }
		if (!(settings.apis.project_history != null ? settings.apis.project_history.initializeHistoryForNewProjects : undefined)) { return callback(); }
		return request.post({
			...
			if ((res.statusCode >= 200) && (res.statusCode < 300)) {
				...
				const overleaf_id = __guard__(project != null ? project.project : undefined, x => x.id);
				if (!overleaf_id) {
	...
	injectUserDetails(data, callback) {
		...
		// service
		let entry, user;
		if (callback == null) { callback = function(error, data_with_users) {}; }
		let user_ids = new Set();
		for (entry of Array.from(data.diff || data.updates || [])) {
			for (user of Array.from((entry.meta != null ? entry.meta.users : undefined) || [])) {
		...
		return UserGetter.getUsers(user_ids, { first_name: 1, last_name: 1, email: 1 }, function(error, users_array) {
			if (error != null) { return callback(error); }
			const users = {};
			...
			for (entry of Array.from(data.diff || data.updates || [])) {
				if (entry.meta != null) {
					entry.meta.users = ((entry.meta != null ? entry.meta.users : undefined) || []).map(function(user) {
					if (typeof user === "string") {
						return users[user];
					} else {
						return user;
