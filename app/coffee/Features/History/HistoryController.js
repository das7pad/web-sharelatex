...
module.exports = (HistoryController = {
	selectHistoryApi(req, res, next) {
		...
		return ProjectDetailsHandler.getDetails(project_id, function(err, project) {
			...
			const history = project.overleaf != null ? project.overleaf.history : undefined;
			if (((history != null ? history.id : undefined) != null) && (history != null ? history.display : undefined)) {
	...
	proxyToHistoryApi(req, res, next) {
		...
		const url = HistoryController.buildHistoryServiceUrl(req.useProjectHistory) + req.url;
	...
	resyncProjectHistory(req, res, next) {
		...
		return ProjectEntityUpdateHandler.resyncProjectHistory(project_id, function(error) {
			if (error instanceof Errors.ProjectHistoryDisabledError) { return res.sendStatus(404); }
			if (error != null) { return next(error); }
	...
	restoreFileFromV2(req, res, next) {
		...
		return RestoreManager.restoreFileFromV2(user_id, project_id, version, pathname, function(error, entity) {
			if (error != null) { return next(error); }
			return res.json({
				type: entity.type,
				id: entity._id
	...
	restoreDocFromDeletedDoc(req, res, next) {
		...
		return RestoreManager.restoreDocFromDeletedDoc(user_id, project_id, doc_id, name, (err, doc) => {
	...
	downloadZipOfVersion(req, res, next) {
		...
		return ProjectDetailsHandler.getDetails(project_id, function(err, project) {
			if (err != null) { return next(err); }
			const v1_id = __guard__(project.overleaf != null ? project.overleaf.history : undefined, x => x.id);
			if ((v1_id == null)) {
				logger.err({project_id, version}, 'got request for zip version of non-v1 history project');
				return res.sendStatus(402);
			}
			return HistoryController._pipeHistoryZipToResponse(v1_id, version, `${project.name} (Version ${version})`, res, next);
		});
	},
	...
	_pipeHistoryZipToResponse(v1_project_id, version, name, res, next) {
		...
		return request(options, function(err, response, body) {
			...
			return async.retry(40,
				callback => setTimeout(function() {
                    // increase delay by 1 second up to 10
                    if (retryDelay < 10000) { retryDelay += 1000; }
                    retryAttempt++;
                    const getReq = request({
                        url: body.zipUrl,
                    ...
                    getReq.on('response', function(response) {
                        if (response.statusCode !== 200) { return callback(new Error("invalid response")); }
