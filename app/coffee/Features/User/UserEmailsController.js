...
module.exports = (UserEmailsController = {
	...
	add(req, res, next) {
		...
		if (email == null) { return res.sendStatus(422); }
		...
		return UserUpdater.addEmailAddress(userId, email, affiliationOptions, function(error){
			if (error != null) {
				return UserEmailsController._handleEmailError(error, req, res, next);
			}
			return UserEmailsConfirmationHandler.sendConfirmationEmail(userId, email, function(err) {
				if (error != null) { return next(error); }
	...
	setDefault(req, res, next) {
		...
		return UserUpdater.updateV1AndSetDefaultEmailAddress(userId, email, function(error){
			if (error != null) {
				return UserEmailsController._handleEmailError(error, req, res, next);
			} else {
				return res.sendStatus(200);
	...
	endorse(req, res, next) {
		...
		return endorseAffiliation(userId, email, req.body.role, req.body.department, function(error){
			if (error != null) { return next(error); }
			return res.sendStatus(204);
		});
	},
	...
	resendConfirmation(req, res, next) {
		...
		const email = EmailHelper.parseEmail(req.body.email);
		if (email == null) { return res.sendStatus(422); }
		return UserGetter.getUserByAnyEmail(email, {_id:1}, function(error, user) {
			if (error != null) { return next(error); }
			if ((user == null) || (__guard__(user != null ? user._id : undefined, x => x.toString()) !== userId)) {
				logger.log({userId, email, foundUserId: (user != null ? user._id : undefined)}, "email doesn't match logged in user");
				return res.sendStatus(422);
			}
			logger.log({userId, email}, 'resending email confirmation token');
			return UserEmailsConfirmationHandler.sendConfirmationEmail(userId, email, function(error) {
				if (error != null) { return next(error); }
				return res.sendStatus(200);
			});
		});
	...
	confirm(req, res, next) {
		...
		return UserEmailsConfirmationHandler.confirmEmailFromToken(token, function(error) {
			if (error != null) {
