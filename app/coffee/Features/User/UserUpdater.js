...
const { addAffiliation, removeAffiliation } = require("../Institutions/InstitutionsAPI");
...
module.exports = (UserUpdater = {
	...
	changeEmailAddress(userId, newEmail, callback){
		...
		if ((newEmail == null)) { return callback(new Error('invalid email')); }
		logger.log({userId, newEmail}, "updaing email address of user");

		let oldEmail = null;
		return async.series([
			cb => UserGetter.getUserEmail(userId, function(error, email) {
                oldEmail = email;
                return cb(error);
            }),
	...
	addEmailAddress(userId, newEmail, affiliationOptions, callback) {
		if (callback == null) { // affiliationOptions is optional
		...
		return UserGetter.ensureUniqueEmailAddress(newEmail, error => {
			...
			return addAffiliation(userId, newEmail, affiliationOptions, error => {
				...
				const update = {$push: {emails: {email: newEmail, createdAt: new Date(), reversedHostname}}};
	...
	updateEmailAddressInV1(userId, newEmail, callback) {
		if ((__guard__(Settings.apis != null ? Settings.apis.v1 : undefined, x => x.url) == null)) {
			return callback();
		}
		return UserGetter.getUser(userId, { 'overleaf.id': 1, emails: 1 }, function(error, user) {
			let email;
			if (error != null) { return callback(error); }
			if ((user == null)) { return callback(new Errors.NotFoundError('no user found')); }
			...
			return request({
				...
				if (error != null) {
					if (error.code === 'ECONNREFUSED') { error = new Errors.V1ConnectionError('No V1 connection'); }
					return callback(error);
				}
	...
	confirmEmail(userId, email, confirmedAt, callback) {
		...
		return addAffiliation(userId, email, {confirmedAt}, error => {
			if (error != null) {
				logger.err({error}, 'problem adding affiliation while confirming email');
				return callback(error);
			}
...
[
	...
	'removeEmailAddress',
	'removeReconfirmFlag'
].map(method => metrics.timeAsyncMethod(UserUpdater, method, 'mongo.UserUpdater', logger));
