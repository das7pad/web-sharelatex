...
module.exports = (UserController = {
	...
	trySoftDeleteUser(req, res, next) {
		return UserController._tryDeleteUser(UserDeleter.softDeleteUser, req, res, next);
	},
	...
	_tryDeleteUser(deleteMethod, req, res, next) {
		...
		return AuthenticationManager.authenticate({_id: user_id}, password, function(err, user) {
			if (err != null) {
	...
	unsubscribe(req, res){
		const user_id = AuthenticationController.getLoggedInUserId(req);
		return UserGetter.getUser(user_id, (err, user) => newsLetterManager.unsubscribe(user, () => res.send()));
	},
	...
	updateUserSettings(req, res){
		...
		return User.findById(user_id, function(err, user){
			...
			return user.save(function(err){
				const newEmail = req.body.email != null ? req.body.email.trim().toLowerCase() : undefined;
				if ((newEmail == null) || (newEmail === user.email) || req.externalAuthenticationSystemUsed()) {
					...
					return UserUpdater.changeEmailAddress(user_id, newEmail, function(err){
						if (err != null) {
							let message;
							logger.err({err, user_id, newEmail}, "problem updaing users email address");
						...
						return User.findById(user_id, function(err, user){
							if (err != null) {
								logger.err({err, user_id}, "error getting user for email update");
								return res.sendStatus(500);
							}
	...
	logout(req, res, next){
		return UserController._doLogout(req, function(err) {
			...
			const redirect_url = (settings.overleaf != null) ? settings.overleaf.host + '/users/ensure_signed_out' : '/login';
	...
	register(req, res, next){
		...
		return UserRegistrationHandler.registerNewUserAndSendActivationEmail(email, function(error, user, setNewPasswordUrl) {
	...
	clearSessions(req, res, next) {
		...
		return UserSessionsManager.revokeAllUserSessions(user, [req.sessionID], function(err) {
			if (err != null) { return next(err); }
	...
	changePassword(req, res, next){
		...
		const user_id = AuthenticationController.getLoggedInUserId(req);
		return AuthenticationManager.authenticate({_id:user_id}, oldPass, function(err, user){
			if (err != null) { return next(err); }
			if(user) {
				...
				const validationError = AuthenticationManager.validatePassword(newPassword1);
				if (newPassword1 !== newPassword2) {
					...
					return AuthenticationManager.setUserPassword(user._id, newPassword1, function(error) {
						if (error != null) { return next(error); }
						return UserSessionsManager.revokeAllUserSessions(user, [req.sessionID], function(err) {
							if (err != null) { return next(err); }
							return res.send({
