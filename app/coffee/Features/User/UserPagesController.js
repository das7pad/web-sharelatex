...
module.exports = (UserPagesController = {
	...
	activateAccountPage(req, res) {
		...
		if (((req.query != null ? req.query.user_id : undefined) == null) || ((req.query != null ? req.query.token : undefined) == null)) {
			return ErrorController.notFound(req, res);
		}

		return UserGetter.getUser(req.query.user_id, {email: 1, loginCount: 1}, function(error, user) {
			if (error != null) { return next(error); }
			if (!user) {
				return ErrorController.notFound(req, res);
			}
			if (user.loginCount > 0) {
				logger.log({user}, "user has already logged in so is active, sending them to /login");
	...
	loginPage(req, res){
		...
		if ((req.query.redir != null) && (AuthenticationController._getRedirectFromSession(req) == null)) {
	...
	renderReconfirmAccountPage(req, res) {
		const page_data = {
			reconfirm_email: __guard__(req != null ? req.session : undefined, x => x.reconfirm_email)
		};
	...
	settingsPage(req, res, next){
		...
		const shouldAllowEditingDetails = !(__guard__(Settings != null ? Settings.ldap : undefined, x => x.updateUserDetailsOnLogin)) && !(__guard__(Settings != null ? Settings.saml : undefined, x1 => x1.updateUserDetailsOnLogin));
		const oauthProviders = Settings.oauthProviders || {};

		return UserGetter.getUser(user_id, function(err, user){
			if (err != null) { return next(err); }

			return UserPagesController._hasPassword(user, function(err, passwordPresent) {
				if (err) {
	...
	sessionsPage(req, res, next) {
		...
		return UserSessionsManager.getAllUserSessions(user, [req.sessionID], function(err, sessions) {
			if (err != null) {
	...
	_hasPassword(user, callback) {
		if ((Settings.apis.v1 != null ? Settings.apis.v1.url : undefined) == null) { return callback(null, true); }
		return request.get({
			url: `${Settings.apis.v1.url}/api/v1/sharelatex/has_password`,
			auth: { user: Settings.apis.v1.user, pass: Settings.apis.v1.pass },
			body: { user_id: __guard__(user != null ? user.overleaf : undefined, x => x.id) },
			timeout: 20 * 1000,
			json: true
	...
	_restructureThirdPartyIds(user) {
		...
		if (!user.thirdPartyIdentifiers || (user.thirdPartyIdentifiers.length === 0)) { return null; }
