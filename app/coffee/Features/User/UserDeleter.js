...
module.exports = (UserDeleter = {
	...
	deleteUser(user_id, callback){
		...
		return User.findById(user_id, function(err, user){
			...
			return async.series([
				...
				if (err != null) {
					logger.err({err, user_id}, "something went wrong deleteing the user");
				}
				return callback(err);
			});
		});
	...
	_cleanupUser(user, callback) {
		if (user == null) { return callback(new Error("no user supplied")); }
		return async.series([
			cb => NewsletterManager.unsubscribe(user, function(err) {
	...
	_ensureCanDeleteUser(user, callback) {
		return SubscriptionLocator.getUsersSubscription(user, function(error, subscription) {
			if (subscription != null) {
				if (!error) { error = new Errors.SubscriptionAdminDeletionError(); }
			}
