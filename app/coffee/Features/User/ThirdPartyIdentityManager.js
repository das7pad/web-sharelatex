...
module.exports = (ThirdPartyIdentityManager = {
	getUser(providerId, externalUserId, callback) {
		...
		const query = ThirdPartyIdentityManager._getUserQuery(providerId, externalUserId);
		return User.findOne(query, function(err, user) {
			if (err != null) { return callback(err); }
			if (!user) { return callback(new Errors.ThirdPartyUserNotFoundError()); }
			return callback(null, user);
		});
	},

	login(providerId, externalUserId, externalData, callback) {
		return ThirdPartyIdentityManager.getUser(providerId, externalUserId, function(err, user) {
			if (err != null) { return callback(err); }
			if (!externalData) { return callback(null, user); }
			const query = ThirdPartyIdentityManager._getUserQuery(providerId, externalUserId);
			const update = ThirdPartyIdentityManager._thirdPartyIdentifierUpdate(user, providerId, externalUserId, externalData);
	...
	loginUserStub(providerId, externalUserId, externalData, callback) {
		return ThirdPartyIdentityManager.login(providerId, externalUserId, externalData, function(err, user) {
			if (err == null) { return callback(null, user); }
			if (err.name !== "ThirdPartyUserNotFoundError") { return callback(err); }
			const query = ThirdPartyIdentityManager._getUserQuery(providerId, externalUserId);
			return UserStub.findOne(query, function(err, userStub) {
				if (err != null) { return callback(err); }
				if (!userStub) { return callback(new Errors.ThirdPartyUserNotFoundError()); }
				if (!externalData) { return callback(null, userStub); }
				const update = ThirdPartyIdentityManager._thirdPartyIdentifierUpdate(userStub, providerId, externalUserId, externalData);
				return UserStub.findOneAndUpdate(query, update, {new: true}, callback);
			});
		});
	},
	...
	_thirdPartyIdentifierUpdate(user, providerId, externalUserId, externalData) {
		...
		const thirdPartyIdentifier = user.thirdPartyIdentifiers.find(tpi => (tpi.externalUserId === externalUserId) && (tpi.providerId === providerId));
	...
	link(user_id, providerId, externalUserId, externalData, callback, retry) {
		...
		const update = { $push: { thirdPartyIdentifiers: {
			externalUserId,
			externalData,
			providerId
		}
		...
		return UserUpdater.updateUser(query, update, function(err, res) {
			if (err != null) { return callback(err); }
			if (res.nModified === 1) { return callback(null, res); }
			// if already retried then throw error
			if (retry) { return callback(new Error("update failed")); }
			// attempt to clear existing entry then retry
			return ThirdPartyIdentityManager.unlink(user_id, providerId, function(err) {
				if (err != null) { return callback(err); }
				return ThirdPartyIdentityManager.link(user_id, providerId, externalUserId, externalData, callback, true);
			});
		});
	},

	unlink(user_id, providerId, callback) {
		const update = { $pull: { thirdPartyIdentifiers: {
			providerId
		}
	}
	};
	...
	unlinkUserStub(user_id, providerId, callback) {
		return ThirdPartyIdentityManager.unlink(user_id, providerId, function(err, res) {
			if (err != null) { return callback(err); }
			if (res.nModified === 1) { return callback(null, res); }
			const update = { $pull: { thirdPartyIdentifiers: {
				providerId
