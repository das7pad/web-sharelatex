...
module.exports = (FileStoreHandler = {
	...
	uploadFileFromDisk(project_id, file_args, fsPath, callback){
		if (callback == null) { callback = function(error, url, fileRef) {}; }
		return fs.lstat(fsPath, function(err, stat){
			...
			if ((stat == null)) {
				logger.err({project_id, file_args, fsPath}, "stat is not available, can not check file from disk");
				return callback(new Error("error getting stat, not available"));
			}
			if (!stat.isFile()) {
				logger.log({project_id, file_args, fsPath}, "tried to upload symlink, not contining");
				return callback(new Error("can not upload symlink"));
			}
			return Async.retry(FileStoreHandler.RETRY_ATTEMPTS, (cb, results) => FileStoreHandler._doUploadFileFromDisk(project_id, file_args, fsPath, cb)
			, function(err, result) {
				if (err != null) {
	...
	_doUploadFileFromDisk(project_id, file_args, fsPath, callback) {
			...
			return FileHashManager.computeHash(fsPath, function(err, hashValue) {
				...
				const fileRef = new File(Object.assign({}, file_args, {hash: hashValue}));
				const file_id = fileRef._id;
				logger.log({project_id, file_id, fsPath, hash: hashValue, fileRef}, "uploading file from disk");
				const readStream = fs.createReadStream(fsPath);
				readStream.on("error", function(err){
					logger.err({err, project_id, file_id, fsPath}, "something went wrong on the read stream of uploadFileFromDisk");
					return callback(err);
				});
				return readStream.on("open", function() {
					...
					writeStream.on("error", function(err){
						logger.err({err, project_id, file_id, fsPath}, "something went wrong on the write stream of uploadFileFromDisk");
						return callback(err);
					});
	...
	getFileStream(project_id, file_id, query, callback){
		logger.log({project_id, file_id, query}, "getting file stream from file store");
		let queryString = "";
		...
		readStream.on("error", err => logger.err({err, project_id, file_id, query, opts}, "error in file stream"));
	...
	deleteFile(project_id, file_id, callback){
		...
		return request(opts, function(err, response){
			if (err != null) {
				logger.err({err, project_id, file_id}, "something went wrong deleting file from filestore");
			}
			return callback(err);
		});
	},
	...
	copyFile(oldProject_id, oldFile_id, newProject_id, newFile_id, callback){
		logger.log({oldProject_id, oldFile_id, newProject_id, newFile_id}, "telling filestore to copy a file");
		const opts = {
			method : "put",
			json: {
				source: {
