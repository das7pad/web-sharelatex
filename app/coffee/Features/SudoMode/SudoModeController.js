...
const {
    ObjectId
} = require('../../infrastructure/Mongoose').mongo;
...
module.exports = (SudoModeController = {
	...
	sudoModePrompt(req, res, next) {
		...
		return SudoModeHandler.isSudoModeActive(userId, function(err, isActive) {
			...
			return res.render('sudo_mode/sudo_mode_prompt', {title: 'confirm_password_to_continue'});
		});
	},
	...
	submitPassword(req, res, next) {
		...
		return UserGetter.getUser(ObjectId(userId), {email: 1}, function(err, userRecord) {
			if (err != null) {
			...
			return SudoModeHandler.authenticate(userRecord.email, password, function(err, user) {
				if (err != null) {
