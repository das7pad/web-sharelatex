...
module.exports = (BlogController = { 
	...
	getPage(req, res, next){
		...
		const extensionsToProxy = [".png", ".xml", ".jpeg", ".jpg", ".json", ".zip", ".eps", ".gif"];
		...
		return request.get(blogUrl, function(err, r, data){
			if (((r != null ? r.statusCode : undefined) === 404) || ((r != null ? r.statusCode : undefined) === 403)) {
			...
			try {
				...
				if (__guard__(settings.cdn != null ? settings.cdn.web : undefined, x => x.host) != null) {
					if (data != null) {
						data.content = __guard__(data != null ? data.content : undefined, x1 => x1.replace(/src="(\/[^"]+)"/g, `src='${__guard__(settings.cdn != null ? settings.cdn.web : undefined, x2 => x2.host)}$1'`));
					}
				}
			} catch (error) {
				err = error;
	...
	_directProxy(originUrl, res){
		...
		upstream.on("error", error => logger.error({err: error}, "blog proxy error"));
