...
if (((Settings.mailchimp != null ? Settings.mailchimp.api_key : undefined) == null)) {
	logger.info("Using newsletter provider: none");
	mailchimp =
		{request(opts, cb){ return cb(); }};
} else {
	logger.info("Using newsletter provider: mailchimp");
	mailchimp = new Mailchimp(Settings.mailchimp != null ? Settings.mailchimp.api_key : undefined);
}
...
module.exports = {
	...
	subscribe(user, callback){
		...
		logger.log({options, user, email:user.email}, "subscribing user to the mailing list");
	...
	unsubscribe(user, callback){
		if (callback == null) { callback = function() {}; }
		logger.log({user, email:user.email}, "trying to unsubscribe user to the mailing list");
	...
	changeEmail(oldEmail, newEmail, callback){
		...
		return mailchimp.request(options, function(err){
			if ((err != null) && (__guard__(err != null ? err.message : undefined, x => x.indexOf("merge fields were invalid")) !== -1)) {
				logger.log({oldEmail, newEmail}, "unable to change email in newsletter, user has never subscribed");
				return callback();
			} else if ((err != null) && (__guard__(err != null ? err.message : undefined, x1 => x1.indexOf("could not be validated")) !== -1)) {
				...
				return callback();
			} else if ((err != null) && (err.message.indexOf("is already a list member") !== -1)) {
				logger.log({oldEmail, newEmail},
					"unable to change email in newsletter, new email is already on mailing list");
				return callback();
			} else if ((err != null) && (__guard__(err != null ? err.message : undefined, x2 => x2.indexOf("looks fake or invalid")) !== -1)) {
				logger.log({oldEmail, newEmail},
...
const hashEmail = email => crypto.createHash('md5').update(email.toLowerCase()).digest("hex");
...
var buildOptions = function(user, is_subscribed){
	...
	const opts = {
		method: "PUT",
		path: `/lists/${(Settings.mailchimp != null ? Settings.mailchimp.list_id : undefined)}/members/${subscriber_hash}`,
		body: {
