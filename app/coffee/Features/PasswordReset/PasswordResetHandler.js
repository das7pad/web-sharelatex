...
module.exports = (PasswordResetHandler = {
	...
	generateAndEmailResetToken(email, callback){
		if (callback == null) { callback = function(error, status) {}; }
		return PasswordResetHandler._getPasswordResetData(email, function(error, exists, data) {
			if (error != null) {
				return callback(error, null);
			} else if (exists) {
				return OneTimeTokenHandler.getNewToken('password', data, function(err, token){
					if (err) { return callback(err); }
					const emailOptions = {
						to : email,
						setNewPasswordUrl : `${settings.siteUrl}/user/password/set?passwordResetToken=${token}&email=${encodeURIComponent(email)}`
					};
					return EmailHandler.sendEmail("passwordResetRequested", emailOptions, function(error) {
				...
				return UserGetter.getUserByAnyEmail(email, function(err, user) {
					if (!user) {
						return callback(error, null);
					} else if (((user.overleaf != null ? user.overleaf.id : undefined) == null)) {
	...
	setNewUserPassword(token, password, callback){
		if (callback == null) { callback = function(error, found, user_id) {}; }
		return OneTimeTokenHandler.getValueFromTokenAndExpire('password', token, function(err, data){
			if (err) { return callback(err); }
			if ((data == null)) {
			...
			if (data.user_id != null) {
				return AuthenticationManager.setUserPassword(data.user_id, password, function(err, reset) {
					if (err) { return callback(err); }
					return callback(null, reset, data.user_id);
				});
			} else if (data.v1_user_id != null) {
				return AuthenticationManager.setUserPasswordInV1(data.v1_user_id, password, function(error, reset) {
					if (error != null) { return callback(error); }
					return UserGetter.getUser({ 'overleaf.id': data.v1_user_id }, {_id:1}, function(error, user) {
						if (error != null) { return callback(error); }
						return callback(null, reset, user != null ? user._id : undefined);
					});
				});
			}
		});
	...
	_getPasswordResetData(email, callback) {
		if (callback == null) { callback = function(error, exists, data) {}; }
