...
module.exports = (AnnouncementsHandler = { 
	...
	_domainSpecificAnnouncements(email){
		const domainSpecific = _.filter(settings != null ? settings.domainAnnouncements : undefined, function(domainAnnouncment){
			const matches = _.filter(domainAnnouncment.domains, domain => email.indexOf(domain) !== -1);
	...
	getUnreadAnnouncements(user, callback){
		if (callback == null) { callback = function(err, announcements){}; }
		...
		return async.parallel({
			lastEvent(cb){
				return AnalyticsManager.getLastOccurrence(user._id, "announcement-alert-dismissed", cb);
			},
			announcements(cb){
			...
			if (err != null) {
				logger.err({err, user_id:user._id}, "error getting unread announcements");
				return callback(err);
			}
			
			let domainSpecific = AnnouncementsHandler._domainSpecificAnnouncements(user != null ? user.email : undefined);
			...
			const lastSeenBlogId = __guard__(__guard__(results != null ? results.lastEvent : undefined, x1 => x1.segmentation), x => x.blogPostId);

			const announcementIndex = _.findIndex(announcements, announcement => announcement.id === lastSeenBlogId);
			...
			logger.log({announcementsLength:(announcements != null ? announcements.length : undefined), user_id:(user != null ? user._id : undefined)}, "returning announcements");
