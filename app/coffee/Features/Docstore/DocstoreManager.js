...
module.exports = (DocstoreManager = {
	deleteDoc(project_id, doc_id, callback) {
		...
		return request.del(url, function(error, res, body) {
			...
			if (200 <= res.statusCode && res.statusCode < 300) {
				...
				logger.error({err: error, project_id, doc_id}, "tried to delete doc not in docstore");
				...
				logger.error({err: error, project_id, doc_id}, "error deleting doc in docstore");
	...
	getAllDocs(project_id, callback) {
		...
		return request.get({
			...
			if (200 <= res.statusCode && res.statusCode < 300) {
				...
				error = new Error(`docstore api responded with non-success code: ${res.statusCode}`);
				logger.error({err: error, project_id}, "error getting all docs from docstore");
				return callback(error);
			}
		});
	},
	...
	getAllRanges(project_id, callback) {
		...
		return request.get({
			...
			if (200 <= res.statusCode && res.statusCode < 300) {
				...
				error = new Error(`docstore api responded with non-success code: ${res.statusCode}`);
				logger.error({err: error, project_id}, "error getting all doc ranges from docstore");
				return callback(error);
			}
		});
	},
	...
	getDoc(project_id, doc_id, options, callback) {
		...
		if (callback == null) { callback = function(error, lines, rev, version) {}; }
		...
		return request.get({
			...
			if (200 <= res.statusCode && res.statusCode < 300) {
				...
				logger.error({err: error, project_id, doc_id}, "doc not found in docstore");
				return callback(error);
			} else {
				error = new Error(`docstore api responded with non-success code: ${res.statusCode}`);
				logger.error({err: error, project_id, doc_id}, "error getting doc from docstore");
				return callback(error);
			}
		});
	},
	...
	updateDoc(project_id, doc_id, lines, version, ranges, callback) {
		...
		return request.post({
			...
			if (200 <= res.statusCode && res.statusCode < 300) {
				...
				error = new Error(`docstore api responded with non-success code: ${res.statusCode}`);
				logger.error({err: error, project_id, doc_id}, "error updating doc in docstore");
				return callback(error);
			}
		});
	},
	...
	archiveProject(project_id, callback){
		...
		return request.post(url, function(err, res, docs) {
			...
			if (200 <= res.statusCode && res.statusCode < 300) {
				...
				logger.err({err: error, project_id}, "error archiving project in docstore");
				return callback(error);
			}
		});
	},
	...
	unarchiveProject(project_id, callback){
		...
		return request.post(url, function(err, res, docs) {
			...
			if (200 <= res.statusCode && res.statusCode < 300) {
				...
				const error = new Error(`docstore api responded with non-success code: ${res.statusCode}`);
				logger.err({err: error, project_id}, "error unarchiving project in docstore");
				return callback(error);
			}
		});
	}
