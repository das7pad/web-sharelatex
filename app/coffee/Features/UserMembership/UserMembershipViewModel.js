...
module.exports = (UserMembershipViewModel = {
	...
	buildAsync(userOrIdOrEmail, callback) {
		if (callback == null) { callback = function(error, viewModel){}; }
		...
		return UserGetter.getUserOrUserStubById(userId, projection, function(error, user, isStub) {
...
var buildUserViewModel = function(user, isInvite) {
	if (isInvite == null) { isInvite = false; }
	return {
		_id: user._id || null,
...
var buildUserViewModelWithStub = user => // user stubs behave as invites
