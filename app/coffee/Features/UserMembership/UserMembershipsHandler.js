...
module.exports = (UserMembershipsHandler = {
	removeUserFromAllEntities(userId, callback) {
		...
		for (let key in UserMembershipEntityConfigs) {
			...
			if (entityConfig.fields.write != null) { entityConfigs.push(entityConfig); }
		}

		// remove the user from all entities types
		return async.map(entityConfigs, ((entityConfig, innerCallback) => UserMembershipsHandler.removeUserFromEntities(entityConfig, userId, innerCallback)), callback);
	},
	...
	removeUserFromEntities(entityConfig, userId, callback) {
		...
		return EntityModels[entityConfig.modelName].updateMany({}, removeOperation, callback);
	},

	getEntitiesByUser(entityConfig, userId, callback) {
		if (callback == null) { callback = function(error, entities) {}; }
		const query = Object.assign({}, entityConfig.baseQuery);
		query[entityConfig.fields.access] = userId;
		return EntityModels[entityConfig.modelName].find(query, function(error, entities) {
			if (entities == null) { entities = []; }
			if (error != null) { return callback(error); }
			return async.mapSeries(entities,
