...
const clsiCookiesEnabled = ((Settings.clsiCookie != null ? Settings.clsiCookie.key : undefined) != null) && (Settings.clsiCookie.key.length !== 0);
...
module.exports = function(backendGroup){
	...
	return {
		...
		_getServerId(project_id, callback){
			if (callback == null) { callback = function(err, serverId){}; }
		...
		_populateServerIdViaRequest(project_id, callback){
			...
			return request.get(url, (err, res, body)=> {
				if (err != null) {
					logger.err({err, project_id}, "error getting initial server id for project");
					return callback(err);
				}
				return this.setServerId(project_id, res, function(err, serverId){
					if (err != null) {
						logger.err({err, project_id}, "error setting server id via populate request");
					}
		...
		_parseServerIdFromResponse(response){
			const cookies = Cookie.parse((response.headers["set-cookie"] != null ? response.headers["set-cookie"][0] : undefined) || "");
		...
		setServerId(project_id, response, callback){
			...
			if ((serverId == null)) { // We don't get a cookie back if it hasn't changed
				return rclient.expire(this.buildKey(project_id), Settings.clsiCookie.ttl, callback);
			}
			if (rclient_secondary != null) {
				this._setServerIdInRedis(rclient_secondary, project_id, serverId);
			}
			return this._setServerIdInRedis(rclient, project_id, serverId, err => callback(err, serverId));
		},
		...
		getCookieJar(project_id, callback){
			if (callback == null) { callback = function(err, jar){}; }
			if (!clsiCookiesEnabled) {
			...
			return this._getServerId(project_id, (err, serverId)=> {
				...
				const serverCookie = request.cookie(`${Settings.clsiCookie.key}=${serverId}`);
