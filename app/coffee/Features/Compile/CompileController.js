...
module.exports = (CompileController = {
	...
	compile(req, res, next) {
		...
		const isAutoCompile = !!(req.query != null ? req.query.auto_compile : undefined);
		...
		} else if (__guard__(req.body != null ? req.body.settingsOverride : undefined, x => x.rootDoc_id) != null) { // Can be removed after deploy
		...
		if (['validate', 'error', 'silent'].includes(req.body != null ? req.body.check : undefined)) {
		...
		return CompileManager.compile(project_id, user_id, options, function(error, status, outputFiles, clsiServerId, limits, validationProblems) {
	...
	compileSubmission(req, res, next) {
		...
		options.compileGroup = (req.body != null ? req.body.compileGroup : undefined) || Settings.defaultFeatures.compileGroup;
		options.timeout = (req.body != null ? req.body.timeout : undefined) || Settings.defaultFeatures.compileTimeout;
		logger.log({options, submission_id}, "got compileSubmission request");
		return ClsiManager.sendExternalRequest(submission_id, req.body, options, function(error, status, outputFiles, clsiServerId, validationProblems) {
			if (error != null) { return next(error); }
			logger.log({submission_id, files:outputFiles}, "compileSubmission output files");
			res.contentType("application/json");
			return res.status(200).send(JSON.stringify({
				status,
				outputFiles,
	...
	downloadPdf(req, res, next){
		...
		const project_id = req.params.Project_id;
		const isPdfjsPartialDownload = req.query != null ? req.query.pdfng : undefined;
		...
		return ProjectGetter.getProject(project_id, {name: 1}, function(err, project) {
			...
			return rateLimit(function(err, canContinue){
				if (err != null) {
					...
					logger.log({project_id, ip:req.ip}, "rate limit hit downloading pdf");
					return res.sendStatus(500);
				} else {
					return CompileController._downloadAsUser(req, function(error, user_id) {
						const url = CompileController._getFileUrl(project_id, user_id, req.params.build_id, "output.pdf");
	...
	deleteAuxFiles(req, res, next) {
		...
		return CompileController._compileAsUser(req, function(error, user_id) {
			if (error != null) { return next(error); }
			return CompileManager.deleteAuxFiles(project_id, user_id, function(error) {
				if (error != null) { return next(error); }
	...
	compileAndDownloadPdf(req, res, next){
		...
		return CompileManager.compile(project_id, null, {}, function(err){
			if (err != null) {
				logger.err({err, project_id}, "something went wrong compile and downloading pdf");
				res.sendStatus(500);
			}
	...
	getFileFromClsi(req, res, next) {
		...
		return CompileController._downloadAsUser(req, function(error, user_id) {
			if (error != null) { return next(error); }
			const url = CompileController._getFileUrl(project_id, user_id, req.params.build_id, req.params.file);
			return CompileController.proxyToClsi(project_id, url, req, res, next);
		});
	},
	...
	getFileFromClsiWithoutUser(req, res, next) {
		...
		const url = CompileController._getFileUrl(submission_id, null, req.params.build_id, req.params.file);
		const limits = { compileGroup: (req.body != null ? req.body.compileGroup : undefined) || Settings.defaultFeatures.compileGroup };
		return CompileController.proxyToClsiWithLimits(submission_id, url, limits, req, res, next);
	},
	...
	proxySyncPdf(req, res, next) {
		...
		return CompileController._compileAsUser(req, function(error, user_id) {
			...
			return CompileController.proxyToClsi(project_id, destination, req, res, next);
		});
	},
	...
	proxyToClsi(project_id, url, req, res, next) {
		...
		if ((req.query != null ? req.query.compileGroup : undefined)) {
			return CompileController.proxyToClsiWithLimits(project_id, url, {compileGroup: req.query.compileGroup}, req, res, next);
		} else {
			return CompileManager.getProjectCompileLimits(project_id, function(error, limits) {
				if (error != null) { return next(error); }
				return CompileController.proxyToClsiWithLimits(project_id, url, limits, req, res, next);
			});
		}
	},
	...
	proxyToClsiWithLimits(project_id, url, limits, req, res, next) {
		...
		return ClsiCookieManager.getCookieJar(project_id, function(err, jar){
			...
			if (qs != null) { options.qs = qs; }
			// if we have a build parameter, pass it through to the clsi
			if ((req.query != null ? req.query.pdfng : undefined) && ((req.query != null ? req.query.build : undefined) != null)) { // only for new pdf viewer
			...
			if (req.query != null ? req.query.pdfng : undefined) {
				...
				for (let h in req.headers) {
					...
					if (/^(If-|Range)/i.test(h)) { newHeaders[h] = req.headers[h]; }
				}
				options.headers = newHeaders;
			}
			const proxy = request(options);
			proxy.pipe(res);
			return proxy.on("error", error => logger.warn({err: error, url}, "CLSI proxy error"));
		});
	},
	...
	wordCount(req, res, next) {
		...
		return CompileController._compileAsUser(req, function(error, user_id) {
			if (error != null) { return next(error); }
			return CompileManager.wordCount(project_id, user_id, file, function(error, body) {
				if (error != null) { return next(error); }
				res.contentType("application/json");
