...
module.exports = (CollaboratorsInviteController = {
	...
	getAllInvites(req, res, next) {
		...
		return CollaboratorsInviteHandler.getAllInvites(projectId, function(err, invites) {
			if (err != null) {
	...
	_checkShouldInviteEmail(email, callback) {
		if (callback == null) { callback = function(err, shouldAllowInvite){}; }
		if (Settings.restrictInvitesToExistingAccounts === true) {
			logger.log({email}, "checking if user exists with this email");
			return UserGetter.getUserByAnyEmail(email, {_id: 1}, function(err, user) {
				if (err != null) { return callback(err); }
				const userExists = (user != null) && ((user != null ? user._id : undefined) != null);
	...
	_checkRateLimit(user_id, callback) {
		...
		return LimitationsManager.allowedNumberOfCollaboratorsForUser(user_id, function(err, collabLimit){
			if (collabLimit == null) { collabLimit = 1; }
			if (err != null) { return callback(err); }
			if (collabLimit === -1) { 
				collabLimit = 20;
			}
	...
	inviteToProject(req, res, next) {
		...
		return LimitationsManager.canAddXCollaborators(projectId, 1, (error, allowed) => {
			let privileges;
			...
			return CollaboratorsInviteController._checkRateLimit(sendingUserId, function(error, underRateLimit) {
				if (error != null) { return next(error); }
				if (!underRateLimit) {
					return res.sendStatus(429);
				}
				return CollaboratorsInviteController._checkShouldInviteEmail(email, function(err, shouldAllowInvite){
					if (err != null) {
					...
					return CollaboratorsInviteHandler.inviteToProject(projectId, sendingUser, email, privileges, function(err, invite) {
						if (err != null) {
	...
	revokeInvite(req, res, next) {
		...
		return CollaboratorsInviteHandler.revokeInvite(projectId, inviteId, function(err) {
			if (err != null) {
	...
	resendInvite(req, res, next) {
		...
		const sendingUser = AuthenticationController.getSessionUser(req);
		return CollaboratorsInviteController._checkRateLimit(sendingUser._id, function(error, underRateLimit) {
			if (error != null) { return next(error); }
			if (!underRateLimit) {
				return res.sendStatus(429);
			}
			return CollaboratorsInviteHandler.resendInvite(projectId, sendingUser, inviteId, function(err) {
				if (err != null) {
	...
	viewInvite(req, res, next) {
		...
		return CollaboratorsHandler.isUserInvitedMemberOfProject(currentUser._id, projectId, function(err, isMember, _privilegeLevel) {
			if (err != null) {
			...
			// get the invite
			return CollaboratorsInviteHandler.getInviteByToken(projectId, token, function(err, invite) {
				if (err != null) {
				...
				return UserGetter.getUser({_id: invite.sendingUserId}, {email: 1, first_name: 1, last_name: 1}, function(err, owner) {
					if (err != null) {
					...
					return ProjectGetter.getProject(projectId, {}, function(err, project) {
						if (err != null) {
						...
						return res.render("project/invite/show", {invite, project, owner, title: "Project Invite"});
				});
			});
		});
	});
	...
	acceptInvite(req, res, next) {
		...
		return CollaboratorsInviteHandler.acceptInvite(projectId, token, currentUser, function(err) {
			if (err != null) {
			...
			AnalyticsManger.recordEvent(currentUser._id, "project-invite-accept", {projectId, userId:currentUser._id});
			if (req.xhr) {
