...
module.exports = (CollaboratorsInviteHandler = {
	...
	getAllInvites(projectId, callback) {
		if (callback == null) { callback = function(err, invites){}; }
	...
	_trySendInviteNotification(projectId, sendingUser, invite, callback) {
		...
		return UserGetter.getUserByAnyEmail(email, {_id: 1}, function(err, existingUser) {
			if (err != null) {
			...
			return ProjectGetter.getProject(projectId, {_id: 1, name: 1}, function(err, project) {
				if (err != null) {
				...
				return NotificationsBuilder.projectInvite(invite, project, sendingUser, existingUser).create(callback);
			});
		});
	},
	...
	_tryCancelInviteNotification(inviteId, callback) {
			...
			return NotificationsBuilder.projectInvite({_id: inviteId}, null, null, null).read(callback);
		},
	...
	_sendMessages(projectId, sendingUser, invite, callback) {
		...
		return CollaboratorsEmailHandler.notifyUserOfProjectInvite(projectId, invite.email, invite, sendingUser, function(err){
			if (err != null) { return callback(err); }
			return CollaboratorsInviteHandler._trySendInviteNotification(projectId, sendingUser, invite, function(err){
				if (err != null) { return callback(err); }
				return callback();
			});
		});
	...
	inviteToProject(projectId, sendingUser, email, privileges, callback) {
		...
		return Crypto.randomBytes(24, function(err, buffer) {
			...
			return invite.save(function(err, invite) {
				...
				CollaboratorsInviteHandler._sendMessages(projectId, sendingUser, invite, function(err) {
					if (err != null) {
						return logger.err({err, projectId, email}, "error sending messages for invite");
					}
				});
	...
	revokeInvite(projectId, inviteId, callback) {
		...
		return ProjectInvite.remove({projectId, _id: inviteId}, function(err) {
			...
			CollaboratorsInviteHandler._tryCancelInviteNotification(inviteId, function(){});
			return callback(null);
		});
	},
	...
	resendInvite(projectId, sendingUser, inviteId, callback) {
		...
		return ProjectInvite.findOne({_id: inviteId, projectId}, function(err, invite) {
			if (err != null) {
			...
			return CollaboratorsInviteHandler._sendMessages(projectId, sendingUser, invite, function(err) {
				if (err != null) {
	...
	getInviteByToken(projectId, tokenString, callback) {
		...
		return ProjectInvite.findOne({projectId, token: tokenString}, function(err, invite) {
			if (err != null) {
	...
	acceptInvite(projectId, tokenString, user, callback) {
		...
		return CollaboratorsInviteHandler.getInviteByToken(projectId, tokenString, function(err, invite) {
			if (err != null) {
			...
			return CollaboratorsHandler.addUserIdToProject(projectId, invite.sendingUserId, user._id, invite.privileges, function(err) {
				if (err != null) {
