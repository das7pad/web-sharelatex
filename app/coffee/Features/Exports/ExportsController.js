...
module.exports = {
	...
	exportProject(req, res, next) {
		...
		if (req.body) {
			if (req.body.firstName) { export_params.first_name = req.body.firstName.trim(); }
			if (req.body.lastName) { export_params.last_name = req.body.lastName.trim(); }
			// additional parameters for gallery exports
			if (req.body.title) { export_params.title = req.body.title.trim(); }
			if (req.body.description) { export_params.description = req.body.description.trim(); }
			if (req.body.author) { export_params.author = req.body.author.trim(); }
			if (req.body.license) { export_params.license = req.body.license.trim(); }
			if (req.body.showSource != null) { export_params.show_source = req.body.showSource; }
		}

		return ExportsHandler.exportProject(export_params, function(err, export_data) {
			if (err != null) {
	...
	exportDownload(req, res, next) {
		...
		return ExportsHandler.fetchDownload(export_id, type, function(err, export_file_url) {
