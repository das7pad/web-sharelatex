...
module.exports = (ExportsHandler = (self = {
	...
	exportProject(export_params, callback) {
		if (callback == null) { callback = function(error, export_data) {}; }
	...
	_buildExport(export_params, callback) {
		if (callback == null) { callback = function(err, export_data) {}; }
		const {project_id, user_id, brand_variation_id, title, description, author,
		...
		const jobs = {
			...
			rootDoc: [ 'project', (cb, results) => ProjectRootDocManager.ensureRootDocumentIsValid(project_id, function(error) {
                if (error != null) { return callback(error); }
                return ProjectLocator.findRootDoc({project: results.project, project_id}, cb);
            })
			],
			user(cb) {
				return UserGetter.getUser(user_id, {first_name: 1, last_name: 1, email: 1, overleaf: 1}, cb);
			},
			historyVersion(cb) {
				return ProjectHistoryHandler.ensureHistoryExistsForProject(project_id, function(error) {
					if (error != null) { return callback(error); }
		...
		return async.auto(jobs, function(err, results) {
			if (err != null) {
				logger.err({err, project_id, user_id, brand_variation_id}, "error building project export");
				return callback(err);
			}
			...
			const export_data = {
				project: {
					...
					historyId: __guard__(project.overleaf != null ? project.overleaf.history : undefined, x => x.id),
					historyVersion,
	...
	_requestExport(export_data, callback) {
		if (callback == null) { callback = function(err, export_v1_id) {}; }
		return request.post({
	...
	_requestVersion(project_id, callback) {
		...
		return request.get({
			...
			if (err != null) {
				logger.err({err, project_id}, "error making request to project history");
				return callback(err);
			} else if ((res.statusCode >= 200) && (res.statusCode < 300)) {
				return callback(null, body.version);
			} else {
				err = new Error(`project history version returned a failure status code: ${res.statusCode}`);
				logger.err({err, project_id}, `project history version returned failure status code: ${res.statusCode}`);
				return callback(err);
			}
		});
	},
	...
	fetchExport(export_id, callback) {
		if (callback == null) { callback = function(err, export_json) {}; }
		return request.get({
			...
			if (err != null) {
				...
				logger.err({err, export:export_id}, `v1 export returned failure status code: ${res.statusCode}`);
				return callback(err);
			}
		});
	},
	...
	fetchDownload(export_id, type, callback) {
		if (callback == null) { callback = function(err, file_url) {}; }
		return request.get({
			...
			if (err != null) {
				...
				err = new Error(`v1 export returned a failure status code: ${res.statusCode}`);
				logger.err({err, export:export_id}, `v1 export zip fetch returned failure status code: ${res.statusCode}`);
				return callback(err);
			}
		});
	}
